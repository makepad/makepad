...
pub fn base64_encode(inp: &[u8], table: &[u8; 64]) -> Vec<u8> {
    ...
    while i + 3 < inp.len() { // hop over in chunks of 3 bytes outputting 4 chars
        out.push(table[inp[i + 0] as usize >> 2]);
        out.push(table[(inp[i + 0] as usize & 0x3) << 4 | inp[i + 1] as usize >> 4]);
        out.push(table[(inp[i + 1] as usize & 0xf) << 2 | inp[i + 2] as usize >> 6]);
        out.push(table[(inp[i + 2] as usize & 0x3f)]);
        i += 3;
    }
    
    if inp.len() - i == 1 { // one byte left
        out.push(table[inp[i + 0] as usize >> 2]);
        out.push(table[(inp[i + 0] as usize & 0x3) << 4]);
    }
    else if inp.len() - i == 2 { // two bytes left
        out.push(table[inp[i + 0] as usize >> 2]);
        out.push(table[(inp[i + 0] as usize & 0x3) << 4 | inp[i + 1] as usize >> 4]);
        out.push(table[(inp[i + 1] as usize & 0xf) << 2]);
    }
