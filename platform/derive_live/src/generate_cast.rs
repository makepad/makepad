...
pub fn generate_any_send_trait_api_impl(input:TokenStream)->TokenStream{
    ...
    if let Some(ident) = parser.eat_any_ident(){
        ...
        tb.add("impl dyn ").ident(&ident).add(" + Send{");
        tb.add("    pub fn is<T: ").ident(&ident).add(" + 'static + Send >(&self) -> bool {");
        tb.add("        let t = std::any::TypeId::of::<T>();");
        tb.add("        let concrete = self.ref_cast_type_id();");
        tb.add("        t == concrete");
        tb.add("    }");
        tb.add("    pub fn downcast_ref<T: ").ident(&ident).add(" + 'static + Send>(&self) -> Option<&T> {");
        tb.add("        if self.is::<T>() {");
        ...
        tb.add("        }");
        tb.add("    }");
        tb.add("    pub fn downcast_mut<T: ").ident(&ident).add(" + 'static + Send>(&mut self) -> Option<&mut T> {");
        tb.add("        if self.is::<T>() {");
