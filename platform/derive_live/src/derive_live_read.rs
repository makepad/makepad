...
fn derive_live_read_impl_inner(
        ...
        for field in &mut fields {
                ...
                && field.attrs[0].name != "rust"
            {
                return error_result(&format!(
                    "Field {} does not have a live, calc into or rust attribute",
                    field.name
                ));
            }
        ...
        for item in &items {
            match &item.kind {
                ...
                EnumKind::Tuple(args) => {
                    ...
                    tb.add("        )));");
                    for i in 0..args.len() {
                        tb.ident(&format!("var{}", i))
                            .add(".live_read_to(LiveId(0), out);");
                    }
                    tb.add("        out.push(LiveNode::from_id_value(id, LiveValue::Close));");
                    tb.add("    },");
                }
