...
impl CoreMidiPort {
    unsafe fn new(port_type: MidiPortType, endpoint: MIDIEndpointRef) -> Result<Self, OSError> {
        let mut manufacturer = 0 as CFStringRef;
        let mut name = 0 as CFStringRef;
        let mut uid = 0i32;
        OSError::from(MIDIObjectGetStringProperty(
            endpoint,
        ...
        OSError::from(MIDIObjectGetIntegerProperty(
            endpoint,
        ...
        let name = format!(
            "{} {}",
            cfstring_ref_to_string(manufacturer),
            cfstring_ref_to_string(name)
        );
        ...
        let mut midi_receive = objc_block!(move |event_list: &MIDIEventList, user_data: u64| {
            ...
            let packets = unsafe {
                std::slice::from_raw_parts(
                    event_list.packet.as_ptr(),
                    event_list.numPackets as usize,
                )
            };
            for packet in packets {
                for i in 0..packet.wordCount.min(64) {
                    ...
                    if ty == 0x02 {
                        ...
                        senders.retain(|s| s.send((midi_port_id, MidiData { data })).is_ok());
                    }
                }
            }
            if senders.len() > 0 {
                ...
                .find(|p| p.desc.port_id == *port_id && p.desc.port_type.is_input())
            {
                unsafe {
                    MIDIPortConnectSource(
                        self.midi_in_port,
                        port.endpoint,
                        port.desc.port_id.0 .0 as *mut _,
                    );
                }
            }
        }
