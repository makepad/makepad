...
impl Cx {
    ...
    fn recover_lost_d3d11_device(
        ...
        if self.os.d3d11_force_recreate || !d3d11_cx.device_is_alive() {
            ...
            // The composition device is bound to the DXGI device that just died.
            // Drop every window's tree so `create_swap_chain` can bind a fresh one.
            for window in d3d11_windows.iter_mut() {
                window.dcomp = None;
            }
        ...
        if d3d11_windows
            .iter()
            .any(|window| window.win32_window.is_direct_composition)
        {
            let _ = d3d11_cx.dcomp_device();
        }
