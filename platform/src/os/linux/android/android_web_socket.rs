...
impl Drop for OsWebSocket {
    fn drop(&mut self) {
        unsafe {
            android_jni::to_java_websocket_close(self.request_id);
        }
    }
}
...
impl OsWebSocket {
    pub fn send_message(&mut self, message: WebSocketMessage) -> Result<(), ()> {
        ...
        unsafe {
            android_jni::to_java_websocket_send_message(self.request_id, frame);
        }
    ...
    pub fn open(
        ...
        unsafe {
            android_jni::to_java_websocket_open(request_id, request, pointer);
        }
