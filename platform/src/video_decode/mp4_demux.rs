//! Minimal MP4/ISOBMFF demuxer for extracting AV1 video samples.
//!
//! Reads the box structure, finds the AV1 video track, and extracts
//! sample data with timing information. Handles `mdat`-based layouts.
...
fn parse_moov<R: Read + Seek>(moov: &[u8], _reader: &mut R) -> io::Result<Mp4Track> {
    for (bt, body) in BoxIter::new(moov) {
        if &bt == b"trak" {
            if let Ok(Some(track)) = parse_trak(body) {
                return Ok(track);
            }
        }
    }
...
fn parse_trak(trak: &[u8]) -> io::Result<Option<Mp4Track>> {
        ...
        match &bt {
            b"stsd" => {
                ...
                if body.len() > 8 {
                    ...
                    if entries.len() >= 16 {
                        let codec = box_type_at(entries, 4);
                        if &codec == b"av01" {
                            is_av1 = true;
