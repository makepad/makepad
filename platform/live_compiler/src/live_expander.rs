use {
    crate::{
        ...
        live_node::{LiveValue, LiveNode},
        live_node_vec::{LiveNodeSlice, LiveNodeVec, LivePath},
...
impl<'a> LiveExpander<'a> {
    ...
    pub fn expand(&mut self, in_doc: &LiveOriginal, out_doc: &mut LiveExpanded, generation: LiveFileGeneration) {
        ...
        loop {
            ...
            let out_index = match out_doc.nodes.child_or_append_index_by_name(current_parent.last().unwrap().1, LivePath(in_node.id, in_node.origin.assign_type())) {
                Ok(overwrite) => {
                ...
                LiveValue::Class {live_type, ..} => {
                    ...
                    if has_deref_hop {
                        ...
                        if let Some(file_id) = self.live_registry.module_id_to_file_id.get(&live_type_info.module_id) {
                            ...
                            if let Some(index) = doc.nodes.child_by_name(0, LivePath::prop(live_type_info.type_name)) {
                                let old_len = out_doc.nodes.len();
                        ...
                        for field in &live_type_info.fields {
                            ...
                            if let Some(file_id) = self.live_registry.module_id_to_file_id.get(&lti.module_id) {
                                ...
                                if *file_id == self.in_file_id { // clone on self
                                    if let Some(index) = out_doc.nodes.child_by_name(0, LivePath::prop(lti.type_name)) {
                                    ...
                                    if let Some(index) = other_nodes.child_by_name(0, LivePath::prop(lti.type_name)) {
                                        let node_insert_point = insert_point;
