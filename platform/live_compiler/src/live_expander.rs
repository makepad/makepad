...
use {
    crate::{
        ...
        live_node::{LiveFieldKind, LiveFont, LiveNode, LiveNodeRoot, LivePropType, LiveValue},
    ...
    fn resolve_path(&self, file_id: LiveFileId, path: &str) -> Option<Arc<String>> {
        if let Some(path) = path.strip_prefix("crate://self/") {
            ...
            if let Some(crate_name) = split.next() {
                if let Some(mut final_path) = self
                    .live_registry
                    .crate_name_to_cargo_manifest_path(crate_name)
                {
                    for next in split {
                        final_path.push('/');
        ...
        loop {
                ...
                .child_or_append_index_by_name(current_parent.last().unwrap().1, in_node.prop())
            {
                Ok(overwrite) => {
                    if current_parent.len() == 1 {
                        lazy_define_value =
                            Some((in_node.id, LiveScopeTarget::LocalPtr(overwrite)));
                    }
                    ...
                    { // lets set the target ptr
                        ...
                        if !out_value.is_single_node() {
                            panic!(
                                "overriding is_expr on not is_single_node in file {:?}",
                ...
                Err(insert_point) => {
                    if current_parent.len() == 1 {
                        lazy_define_value =
                            Some((in_node.id, LiveScopeTarget::LocalPtr(insert_point)));
                    }
            ...
            match in_value {
                ...
                LiveValue::Clone { clone, .. } | LiveValue::Deref { clone, .. } => {
                    if let Some(target) =
                        self.live_registry.find_scope_target(*clone, &out_doc.nodes)
                    {
                        match target {
                            ...
                            LiveScopeTarget::LivePtr(live_ptr) => {
                                let doc = &self.live_registry.live_files
                                    [live_ptr.file_id.to_index()]
                                .expanded;
                ...
                LiveValue::Class { live_type, .. } => {
                    ...
                    if let Some(field) = live_type_info
                        .fields
                        .iter()
                        .find(|f| f.live_field_kind == LiveFieldKind::Deref)
                    {
                        if !field.live_type_info.live_ignore {
                            ...
                            if let Some(file_id) = self
                                ...
                                .get(&live_type_info.module_id)
                            {
                                let doc =
                                    &self.live_registry.live_files[file_id.to_index()].expanded;
                    ...
                    for field in &live_type_info.fields {
                        ...
                        if let Some(file_id) =
                            self.live_registry.module_id_to_file_id.get(&lti.module_id)
                        {
                            if *file_id == self.in_file_id {
                                // clone on self
                                let mut index = 1;
                                let mut found = None;
                                while index < out_doc.nodes.len() - 1 {
                                    if let LiveValue::Class { live_type, .. } =
                                        &out_doc.nodes[index].value
                                    {
                                ...
                                if let Some(index) = found {
                                    ...
                                    let old_len = out_doc.nodes.len();
                                    insert_point =
                                        out_doc.nodes.insert_node_from_self(index, insert_point);
                                    self.shift_parent_stack(
                                        &mut current_parent,
                                        &out_doc.nodes,
                                        node_insert_point - 1,
                                    ...
                                    out_doc.nodes[node_insert_point]
                                        .origin
                                        .set_prop_type(LivePropType::Field);
                                ...
                                let other_nodes = &self.live_registry.live_files
                                    [file_id.to_index()]
                                .expanded
                                .nodes;
                                ...
                                if let Some(index) = found {
                                    ...
                                    let old_len = out_doc.nodes.len();
                                    insert_point = out_doc.nodes.insert_node_from_other(
                                        index,
                                        insert_point,
                                        other_nodes,
                                    );
