...
pub struct XrHands {
    ...
    peers: Vec<XrPeer>,
    #[rust]
    align_last_click: f64,
    #[rust]
    align_mode: Option<AlignMode>,
}
        ...
        if e.menu_pressed() || e.clicked_menu() {
            if e.state.time - self.align_last_click < ALIGN_MODE {
                ...
                if let Some(align_mode) = &mut self.align_mode {
                    ...
                    self.align_mode = Some(AlignMode { anchor });
                }
            }
...
impl Widget for XrHands {
    ...
    fn draw_3d(&mut self, cx: &mut Cx3d, _scope: &mut Scope) -> DrawStep {
        ...
        fn draw_head(cx: &mut Cx3d, cube: &mut DrawCube, transform: &Mat4f, xr_state: &XrState) {
        ...
        fn _draw_democube(cx: &mut Cx3d, cube: &mut DrawCube, anchor: &XrAnchor) {
            let _speed = 32.0;
