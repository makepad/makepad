...
struct SnakeGame {
    ...
    field: Vec<Field>,
    #[rust]
    snake_body: VecDeque<(usize, usize)>,
    #[rust]
    snake_head: (usize, usize),
    #[rust]
    snake_direction: (isize, isize),
    #[rust((32,32))]
...
impl SnakeGame {
    ...
    fn find_food_pos(&self) -> Option<(usize, usize)> {
        ...
        self.field
            .iter()
            .position(|f| *f == Field::Food)
            .map(|idx| (idx % grid_w, idx / grid_w))
    }
    ...
    fn restart_game(&mut self) {
        self.field.clear();
        self.field
            .resize(self.grid_size.0 * self.grid_size.1, Field::Empty);
                    ...
                    Field::Snake => {
                        ...
                        if let Some(index) = self.snake_body.iter().position(|&pos| pos == (x, y)) {
                            if snake_len > 1 && index > 0 {
                                fade_factor =
                                    (index - 1) as f32 / ((snake_len - 1) as f32).max(1.0);
                            }
