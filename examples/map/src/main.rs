...
impl App {
    ...
    fn send_search(&mut self, cx: &mut Cx) {
        ...
        // Headless A/B: "@abtest" builds one tile through BOTH road
        // generators with the LIVE theme, rasterizes flat (pure fills),
        // writes /tmp/mp_ab/{ref,union,diff}.ppm + a diff percentage.
        if self.pending_query.trim() == "@abtest" {
            use makepad_widgets::map::geometry::TileKey;
            use makepad_widgets::map::tile::{
                load_local_tile_batch, raster_buffers, write_ppm,
            };
            if let Some(style) = self.map(cx).compiled_style() {
                std::fs::create_dir_all("/tmp/mp_ab").ok();
                let keys = vec![TileKey { z: 14, x: 8413, y: 5385 }];
                let mut images: Vec<(usize, Vec<u8>)> = Vec::new();
                for union in [false, true] {
                    if let Ok((loaded, _)) = load_local_tile_batch(
                        ...
                        if let Some(tile) = loaded.first() {
                            if !union {
                                // Legacy width census: max |offset| per color
                                // from GPU-expandable stroke vertices.
                                use std::collections::HashMap as Map;
                                let mut widths: Map<u32, f32> = Map::new();
                                let verts = &tile.buffers.stroke_vertices;
                                for v in verts.chunks_exact(19) {
                                    if v[10] < 100.0 {
                                        continue;
                                    }
                                    let color = ((v[4].min(1.0) * 255.0) as u32) << 16
                                        | ((v[5].min(1.0) * 255.0) as u32) << 8
                                        | (v[6].min(1.0) * 255.0) as u32;
                                    let off = (v[12] * v[12] + v[13] * v[13]).sqrt();
                                    let entry = widths.entry(color).or_insert(0.0);
                                    if off > *entry {
                                        *entry = off;
                                    }
                                }
                                let mut report = String::new();
                                let mut entries: Vec<_> = widths.into_iter().collect();
                                entries.sort_by_key(|entry| entry.0);
                                for (color, off) in entries {
                                    report.push_str(&format!(
                                        "legacy color={color:06x} max_half_off={off:.2}\n"
                                    ));
                                }
                                std::fs::write("/tmp/mp_ab/legacy_widths.txt", report).ok();
                            }
                            let (size, image) = raster_buffers(&tile.buffers, 8.0);
                            write_ppm(
                                if union { "/tmp/mp_ab/union.ppm" } else { "/tmp/mp_ab/ref.ppm" },
                                size,
                                &image,
                            );
                ...
                if images.len() == 2 {
                    ...
                    write_ppm("/tmp/mp_ab/diff.ppm", *size, &diff);
                    let pct = differing as f64 / (size * size) as f64 * 100.0;
                    std::fs::write(
                        "/tmp/mp_ab/result.txt",
                        format!("diff {differing} px ({pct:.2}%)\n"),
                    )
                    .ok();
                    log!("ABTEST diff {} px ({:.2}%)", differing, pct);
                }
            }
            self.ui.text_input(cx, ids!(search_input)).set_text(cx, "");
            self.pending_query.clear();
        ...
        // Road generator A/B: "@roads 0" = legacy per-way strokes (the 2D
        // pixel reference), "@roads 1" = union meshes.
        if let Some(rest) = self.pending_query.trim().strip_prefix("@roads ") {
            let enabled = rest.trim() != "0";
            self.map(cx).set_union_roads(cx, enabled);
            self.ui.text_input(cx, ids!(search_input)).set_text(cx, "");
            self.pending_query.clear();
            self.hide_results(cx);
            return;
