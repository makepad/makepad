...
pub const HUB_MARKER_MAGIC_1: u32 = 0x89575214;
pub const HUB_MARKER_MAGIC_2: u32 = 0x54027861;
...
pub fn read_block_from_tcp_stream(tcp_stream: &mut TcpStream) -> Vec<u8> {
    ...
    let (magic_r1, magic_r2, bytes_total_u32) = unsafe {std::mem::transmute::<[u8; 12], (u32, u32, u32)>(head_buf)};
    let bytes_total = bytes_total_u32 as usize;
    if magic_r1 != HUB_MARKER_MAGIC_1 || magic_r2 != HUB_MARKER_MAGIC_2 {
...
pub fn write_block_to_tcp_stream(tcp_stream: &mut TcpStream, msg_buf: Vec<u8>) {
    ...
    let mut head_buf = unsafe {std::mem::transmute::<(u32, u32, u32), [u8; 12]>((
        HUB_MARKER_MAGIC_1,
        HUB_MARKER_MAGIC_2,
        bytes_total as u32
    ))};
...
impl HubClient {
    ...
    pub fn wait_for_announce(announce_address: SocketAddr) -> Result<SocketAddr, std::io::Error> {
        ...
        let mut port_buf = [0u8; 12];
        let (bytes, from) = socket.recv_from(&mut port_buf) ?;
        if bytes != 12 {
            panic!("Announce port wrong bytecount");
        }
        let (magic_r1, magic_r2, listen_port) = unsafe {std::mem::transmute::<[u8; 12], (u32, u32, u32)>(port_buf)};
        if magic_r1 != HUB_MARKER_MAGIC_1 || magic_r2 != HUB_MARKER_MAGIC_2 {
            panic!("Announce port wrong marker!");
        }
