...
/// Makepad uses a turtle system for drawing. Drawing on the screen amounts to walking a turtle.
/// Every time a turtle is walked, it generates a rectangle on the screen.
///
/// Turtles can be nested to create structured layouts. This is done by:
/// - Starting a child turtle.
/// - Walking the child turtle to generate one or more rectangles.
/// - Finishing the child turtle.
/// 
/// Once the child turtle is finished, the parent turtle walks itself to generate a bounding
/// rectangle for the rectangles generated by the child turtle.

/// A `Walk` specifies how a turtle is to be walked, by specifying the position, margin, and the
/// dimensions of the rectangle to be generated by the walk.
#[derive(Clone, Copy, Default, Debug, Live, LiveHook, LiveRegister)]
#[live_ignore]
pub struct Walk {
    /// The position of the rectangle.
    /// 
    /// If `Some`, the rectangle will be positioned at the given position.
    /// If `None`, the rectangle will be positioned at the current turtle position.
    #[live] pub abs_pos: Option<DVec2>,

    /// The amount of spacing around the rectangle.
...
/// A `Size` specifies the size of one dimension of the rectangle to be generated by a walk.
#[derive(Clone, Copy, Debug, Live, PartialEq, PartialOrd)]
#[live_ignore]
pub enum Size {
    // However much space is left in the bounding rectangle of the parent turtle.
    //
    // This only makes sense when there is a parent, and the size of its bounding rectangle is
    // known ahead of time (that is, the `Walk` we used when creating the child turtle had its
    // corresponding dimension set to either `Size::Fill` or `Size::Fixed`).
    //
    // In contrast, if the `Walk` we used when creating the child turtle had its width/height
    // set to `Size::Fit`, then walking the child turtle with a `Walk` with the corresponding
    // dimension set to `Size::Fill` will cause nothing to be drawn, since the size of the
    // bounding rectangle of the parent cannot be determined.
    #[pick]
    Fill,
    ...
    // However much space is used by a child turtle in this dimension.
    // 
    // This only makes sense when creating a child turtle. Using a `Walk` with width/height set to
    // `Size::Fit` when creating a child turtle will cause the size of the bounding rectangle
    // generated by the parent to fit the rectangles generated by the child in this dimension.
    //
    // In contrast, when walking a turtle directly, using a `Walk` with width/height set to
    // `Size::Fit` will cause nothing to be drawn, since there is no content to fit.
    Fit,
}
 
#[derive(Clone, Copy, Debug, Live, LiveHook, LiveRegister)]
#[live_ignore]
pub struct Layout {
