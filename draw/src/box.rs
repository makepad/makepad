use {
    crate::{
        makepad_platform::*,
        cx_2d::{Cx2d},
    }
};

#[derive(Copy, Clone, Debug, Live, LiveHook)]
#[live_ignore]
pub struct Layout {
    #[live] pub scroll: DVec2,
    #[live(true)] pub clip_x: bool,
    #[live(true)] pub clip_y: bool,
    #[live] pub padding: Padding,
    #[live] pub align: Align,
    #[live] pub flow: Flow,
    #[live(1.0)] pub scale: f64,
    #[live] pub spacing: f64
}

impl Default for Layout{
    fn default()->Self{
        Self{
            scroll: dvec2(0.0,0.0),
            clip_x: true,
            clip_y: true,
            scale: 1.0,
            padding: Padding::default(),
            align: Align{x:0.0,y:0.0},
            flow: Flow::Right,
            spacing: 0.0
        }
    }
}

#[derive(Copy, Clone, Default, Debug, Live, LiveHook)]
#[live_ignore]
pub struct Walk {
    #[live] pub abs_pos: Option<DVec2>,
    #[live] pub margin: Margin,
    #[live] pub width: Length,
    #[live] pub height: Length,
}

#[derive(Clone, Copy, Default, Debug, Live, LiveHook)]
#[live_ignore]
pub struct Align {
    #[live] pub x: f64,
    #[live] pub y: f64
}

#[derive(Clone, Copy, Default, Debug, Live)]
#[live_ignore]
pub struct Padding {
    #[live] pub left: f64,
    #[live] pub top: f64,
    #[live] pub right: f64,
    #[live] pub bottom: f64
}

#[derive(Copy, Clone, Debug, Live, LiveHook)]
#[live_ignore]
pub enum Axis {
    #[pick] Horizontal,
    Vertical
}

impl Default for Axis {
    fn default() -> Self {
        Axis::Horizontal
    }
}

#[derive(Copy, Clone, Debug, Live, LiveHook)]
#[live_ignore]
pub enum Flow {
    #[pick] Right,
    Down,
    Overlay
}

#[derive(Copy, Clone, Debug, Live)]
#[live_ignore]
pub enum Length {
    #[pick] Fill,
    #[live(200.0)] Fixed(f64),
    Fit,
}

#[derive(Clone, Debug)]
pub enum DeferWalk{
    Unresolved{
        defer_index: usize,
        margin: Margin,
        other_axis: Length,
        pos: DVec2
    },
    Resolved(Walk)
}

#[derive(Debug)]
pub enum AlignEntry{
    Unset,
    Area(Area),
    ShiftBox{area:Area, shift:DVec2, skip:usize},
    SkipBox{skip:usize},
    BeginBox(DVec2,DVec2),
    EndBox
}

#[derive(Clone, Default, Debug)]
pub struct BoxWalk {
    align_start: usize,
    defer_index: usize,
    rect: Rect,
}

#[derive(Clone, Default, Debug)]
pub struct Box {
    walk: Walk,
    layout: Layout,
    align_start: usize,
    box_walks_start: usize,
    defer_count: usize,
    shift: DVec2,
    pos: DVec2,
    origin: DVec2,
    width: f64,
    height: f64,
    width_used: f64,
    height_used: f64,
    guard_area: Area
}

impl<'a> Cx2d<'a> {
    pub fn r#box(&self) -> &Box {
        self.boxes.last().unwrap()
    }
    
    pub fn box_mut(&mut self) -> &mut Box {
        self.boxes.last_mut().unwrap()
    }
    
    pub fn begin_box(&mut self, walk: Walk, layout: Layout) {
        self.begin_box_with_guard(walk, layout, Area::Empty)
    }
    
    pub fn defer_walk(&mut self, walk: Walk) -> Option<DeferWalk> {
        if walk.abs_pos.is_some(){
            return None
        }
        let bx = self.boxes.last_mut().unwrap();
        let defer_index = bx.defer_count;
        let pos = bx.pos;
        let size = dvec2(
            bx.eval_width(walk.width, walk.margin, bx.layout.flow),
            bx.eval_height(walk.height, walk.margin, bx.layout.flow)
        );
        let margin_size = walk.margin.size();
        match bx.layout.flow {
            Flow::Right if walk.width.is_fill() => {
                let spacing = bx.child_spacing(self.box_walks.len());
                bx.pos.x += margin_size.x + spacing.x;
                bx.update_width_max(bx.pos.x, 0.0);
                bx.update_height_max(bx.pos.y, size.y + margin_size.y);
                bx.defer_count += 1;
                Some(DeferWalk::Unresolved{
                    defer_index,
                    margin: walk.margin,
                    other_axis: walk.height,
                    pos: pos + spacing
                })
            },
            Flow::Down if walk.height.is_fill() => {
                let spacing = bx.child_spacing(self.box_walks.len());
                bx.pos.y += margin_size.y + spacing.y;
                bx.update_width_max(bx.pos.x, size.x + margin_size.x);
                bx.update_height_max(bx.pos.y, 0.0);
                bx.defer_count += 1;
                Some(DeferWalk::Unresolved {
                    defer_index,
                    margin: walk.margin,
                    other_axis: walk.width,
                    pos: pos + spacing
                })
            },
            _ => {
                None
            }
        }
    }
    
    pub fn begin_pass_sized_box(&mut self, layout: Layout) {
        let pass_size = self.current_pass_size();
        self.align_list.push(AlignEntry::BeginBox(dvec2(0.0,0.0),pass_size));
        let boxx = Box {
            walk: Walk::fill(),
            layout,
            align_start: self.align_list.len() - 1,
            box_walks_start: self.box_walks.len(),
            defer_count: 0,
            pos: DVec2 {
                x: layout.padding.left,
                y: layout.padding.top
            },
            origin: dvec2(0.0,0.0),
            width: pass_size.x,
            height: pass_size.y,
            shift: dvec2(0.0,0.0),
            width_used: layout.padding.left,
            height_used: layout.padding.top,
            guard_area: Area::Empty,
        };
        self.boxes.push(boxx);
    }
    
    pub fn end_pass_sized_box(&mut self){
        let boxx = self.boxes.pop().unwrap();
        // lets perform clipping on our alignlist.
        self.align_list.push(AlignEntry::EndBox);
        self.perform_nested_clipping_on_align_list_and_shift(boxx.align_start, self.align_list.len());
        //log!("{:?}", self.align_list[boxx.align_start]);
        self.align_list[boxx.align_start] = AlignEntry::SkipBox{skip:self.align_list.len()};
        self.box_walks.truncate(boxx.box_walks_start);
    }

    pub fn end_pass_sized_box_with_shift(&mut self, area:Area, shift:DVec2){
        let boxx = self.boxes.pop().unwrap();
        // lets perform clipping on our alignlist.
        self.align_list.push(AlignEntry::EndBox);
        self.perform_nested_clipping_on_align_list_and_shift(boxx.align_start, self.align_list.len());
        //log!("{:?}", self.align_list[boxx.align_start]);
        self.align_list[boxx.align_start] = AlignEntry::ShiftBox{
            area,
            shift, 
            skip: self.align_list.len()
        };
        self.box_walks.truncate(boxx.box_walks_start);
    }
    
    pub fn begin_box_with_guard(&mut self, walk: Walk, layout: Layout, guard_area: Area) {
        let (origin, width, height, draw_clip) = if let Some(parent) = self.boxes.last() {
            
            let o = walk.margin.left_top() + if let Some(pos) = walk.abs_pos {pos} else {
                parent.pos + parent.child_spacing(self.box_walks.len()) 
            };
            
            let w = parent.eval_width(walk.width, walk.margin, parent.layout.flow) * layout.scale;
            let h = parent.eval_height(walk.height, walk.margin, parent.layout.flow) * layout.scale;
            
            // figure out new clipping rect
            let (x0, x1) = if layout.clip_x {
                (/*parent.draw_clip.0.x.max(*/o.x/*)*/, if w.is_nan() {
                    f64::NAN
                    //parent.draw_clip.1.x
                } else {
                    /*parent.draw_clip.1.x.min*/o.x + w/*)*/
                })
            } else {
                (f64::NAN, f64::NAN)//parent.draw_clip.0.x, parent.draw_clip.1.x)
            };
            
            let (y0, y1) = if layout.clip_y {
                (/*parent.draw_clip.0.y.max(*/o.y/*)*/, if h.is_nan() {
                    f64::NAN
                } else {
                    /*parent.draw_clip.1.y.min(*/o.y + h/*)*/
                })
            }else {(f64::NAN,f64::NAN)};//parent.draw_clip.0.y, parent.draw_clip.1.y)};
            
            (o - layout.scroll, w, h, (dvec2(x0, y0), (dvec2(x1, y1))* layout.scale))
        }
        else {
            let o = DVec2 {x: walk.margin.left, y: walk.margin.top};
            let w = walk.width.fixed_or_nan() * layout.scale;
            let h = walk.height.fixed_or_nan() * layout.scale;
            
            (o, w, h, (dvec2(o.x, o.y), dvec2(o.x + w, o.y + h)))
        };
        self.align_list.push(AlignEntry::BeginBox(draw_clip.0,draw_clip.1));
        let boxx = Box {
            walk,
            layout,
            align_start: self.align_list.len()-1,
            box_walks_start: self.box_walks.len(),
            defer_count: 0,
            pos: DVec2 {
                x: origin.x + layout.padding.left,
                y: origin.y + layout.padding.top
            },
            origin,
            width,
            height,
            shift: dvec2(0.0,0.0),
            width_used: layout.padding.left,
            height_used: layout.padding.top,
            guard_area,
        };
        
        self.boxes.push(boxx);
    }
    
    pub fn box_has_align_items(&mut self)->bool{
        self.align_list.len() != self.r#box().align_start + 1
    }
    
    pub fn end_box(&mut self) -> Rect {
        self.end_box_with_guard(Area::Empty)
    }
    
    pub fn end_box_with_area(&mut self, area: &mut Area) {
        let rect = self.end_box_with_guard(Area::Empty);
        self.add_aligned_rect_area(area, rect)
    }
    
    pub fn end_box_with_guard(&mut self, guard_area: Area) -> Rect {
        let boxx = self.boxes.pop().unwrap();
        if guard_area != boxx.guard_area {
            panic!("End box guard area misaligned!, begin/end pair not matched begin {:?} end {:?}", boxx.guard_area, guard_area)
        }
        
        // computed height
        let w = if boxx.width.is_nan() {
            Length::Fixed(boxx.width_used + boxx.layout.padding.right - boxx.layout.scroll.x)
        }
        else {
            Length::Fixed(boxx.width)
        };
        
        let h = if boxx.height.is_nan() {
            Length::Fixed(boxx.height_used + boxx.layout.padding.bottom - boxx.layout.scroll.y)
        }
        else {
            Length::Fixed(boxx.height)
        };
        
        match boxx.layout.flow {
            Flow::Right => {
                if boxx.defer_count > 0 {
                    let left = boxx.width_left();
                    let part = left / boxx.defer_count as f64;
                    for i in boxx.box_walks_start..self.box_walks.len() {
                        let walk = &self.box_walks[i];
                        let shift_x = walk.defer_index as f64 * part;
                        let shift_y = boxx.layout.align.y * (boxx.padded_height_or_used() - walk.rect.size.y);
                        let align_start = walk.align_start;
                        let align_end = self.get_box_walk_align_end(i);
                        self.move_align_list(shift_x, shift_y, align_start, align_end, false, boxx.shift);
                    }
                }
                else {
                    for i in boxx.box_walks_start..self.box_walks.len() {
                        let walk = &self.box_walks[i];
                        let shift_x = boxx.layout.align.x * boxx.width_left();
                        let shift_y = boxx.layout.align.y * (boxx.padded_height_or_used() - walk.rect.size.y);
                        let align_start = walk.align_start;
                        let align_end = self.get_box_walk_align_end(i);
                        self.move_align_list(shift_x, shift_y, align_start, align_end, false, boxx.shift);
                    }
                }
            },
            Flow::Down => {
                if boxx.defer_count > 0 {
                    let left = boxx.height_left();
                    let part = left / boxx.defer_count as f64;
                    for i in boxx.box_walks_start..self.box_walks.len() {
                        let walk = &self.box_walks[i];
                        let shift_x = boxx.layout.align.x * (boxx.padded_width_or_used() - walk.rect.size.x);
                        let shift_y = walk.defer_index as f64 * part;
                        let align_start = walk.align_start;
                        let align_end = self.get_box_walk_align_end(i);
                        self.move_align_list(shift_x, shift_y, align_start, align_end, false, boxx.shift);
                    }
                }
                else {
                    for i in boxx.box_walks_start..self.box_walks.len() {
                        let walk = &self.box_walks[i];
                        let shift_x = boxx.layout.align.x * (boxx.padded_width_or_used() - walk.rect.size.x);
                        let shift_y = boxx.layout.align.y * boxx.height_left();
                        let align_start = walk.align_start;
                        let align_end = self.get_box_walk_align_end(i);
                        self.move_align_list(shift_x, shift_y, align_start, align_end, false, boxx.shift);
                    }
                }
            },
            Flow::Overlay => {
                for i in boxx.box_walks_start..self.box_walks.len() {
                    let walk = &self.box_walks[i];
                    let shift_x = boxx.layout.align.x * (boxx.padded_width_or_used() - walk.rect.size.x);
                    let shift_y = boxx.layout.align.y * (boxx.padded_height_or_used() - walk.rect.size.y);
                    let align_start = walk.align_start;
                    let align_end = self.get_box_walk_align_end(i);
                    self.move_align_list(shift_x, shift_y, align_start, align_end, false, boxx.shift);
                }
            }
        }

        self.box_walks.truncate(boxx.box_walks_start);
        self.align_list.push(AlignEntry::EndBox);
        if self.boxes.len() == 0 {
            return Rect {
                pos: dvec2(0.0, 0.0),
                size: dvec2(w.fixed_or_zero(), h.fixed_or_zero())
            }
        }
        let rect = self.walk_box_move(Walk {width: w, height: h, ..boxx.walk}, boxx.align_start);
        rect
    }
    
    pub fn walk_box(&mut self, walk: Walk) -> Rect {
        self.walk_box_move(walk, self.align_list.len())
    }
    
    pub fn walk_box_with_area(&mut self, area: &mut Area, walk: Walk) -> Rect {
        let rect = self.walk_box_move(walk, self.align_list.len());
        self.add_aligned_rect_area(area, rect);
        rect
    }
    
    pub fn walk_box_with_align(&mut self, walk: Walk, align_start: usize) -> Rect {
        self.walk_box_move(walk, align_start)
    }
    
    pub fn peek_walk_box(&self, walk: Walk) -> Rect {
        self.walk_box_peek(walk)
    }
    
    pub fn walk_box_would_be_visible(&mut self, walk: Walk) -> bool {
        let rect = self.walk_box_peek(walk);
        self.r#box().rect_is_visible(rect)
    }
    
    pub fn peek_walk_pos(&self, walk: Walk) -> DVec2 {
        if let Some(pos) = walk.abs_pos {
            pos + walk.margin.left_top()
        }
        else {
            let boxx = self.boxes.last().unwrap();
            boxx.pos + walk.margin.left_top()
        }
    }
    
     fn walk_box_move(&mut self, walk: Walk, align_start: usize) -> Rect {
        
        let boxx = self.boxes.last_mut().unwrap();
        let size = dvec2(
            boxx.eval_width(walk.width, walk.margin, boxx.layout.flow),
            boxx.eval_height(walk.height, walk.margin, boxx.layout.flow)
        );
        
        if let Some(pos) = walk.abs_pos {
            self.box_walks.push(BoxWalk {
                align_start,
                defer_index: 0,
                rect: Rect {pos, size: size + walk.margin.size()}
            });
            
            match boxx.layout.flow {
                Flow::Right=>boxx.update_height_max(pos.y, size.y + walk.margin.size().y),
                Flow::Down=>boxx.update_width_max(pos.x, size.x + walk.margin.size().x),
                _=>()
            }
            Rect {pos: pos + walk.margin.left_top(), size}
        }
        else {
            let spacing = boxx.child_spacing(self.box_walks.len());
            let pos = boxx.pos;
        
            let margin_size = walk.margin.size();
            match boxx.layout.flow {
                Flow::Right => {
                    boxx.pos.x = pos.x + size.x + margin_size.x + spacing.x;
                    if size.x < 0.0 {
                        boxx.update_width_min(boxx.pos.x, 0.0);
                        boxx.update_height_max(boxx.pos.y,size.y + margin_size.y);
                    }
                    else {
                        boxx.update_width_max(boxx.pos.x, 0.0);
                        boxx.update_height_max(boxx.pos.y,size.y + margin_size.y);
                    }
                },
                Flow::Down => {
                    boxx.pos.y = pos.y + size.y + margin_size.y + spacing.y;
                    if size.y < 0.0 {
                        boxx.update_width_max(boxx.pos.x, size.x + margin_size.x);
                        boxx.update_height_min(boxx.pos.y,0.0);
                    }
                    else {
                        boxx.update_width_max(boxx.pos.x, size.x + margin_size.x);
                        boxx.update_height_max(boxx.pos.y,0.0);
                    }
                },
                Flow::Overlay => { // do not walk
                    boxx.update_width_max(boxx.pos.x, size.x);
                    boxx.update_height_max(boxx.pos.y,size.y);
                }
            };
            
            self.box_walks.push(BoxWalk {
                align_start,
                defer_index: boxx.defer_count,
                rect: Rect {pos, size: size + margin_size}
            });
            Rect {pos: pos + walk.margin.left_top() + spacing, size}
        }
    }
    
    fn walk_box_peek(&self, walk: Walk) -> Rect {
        if self.boxes.len() == 0{
            return Rect::default()
        }
        let boxx = self.boxes.last().unwrap();
        let size = dvec2(
            boxx.eval_width(walk.width, walk.margin, boxx.layout.flow),
            boxx.eval_height(walk.height, walk.margin, boxx.layout.flow)
        );
        
        if let Some(pos) = walk.abs_pos {
            Rect {pos: pos + walk.margin.left_top(), size}
        }
        else {
            let spacing = boxx.child_spacing(self.box_walks.len());
            let pos = boxx.pos;
            Rect {pos: pos + walk.margin.left_top() + spacing, size}
        }
    }
    
    fn move_align_list(&mut self, dx: f64, dy: f64, align_start: usize, align_end: usize, shift_clip: bool, box_shift:DVec2) {
        let current_dpi_factor = self.current_dpi_factor();
        let dx = if dx.is_nan() {0.0}else {dx} + box_shift.x;
        let dy = if dy.is_nan() {0.0}else {dy} + box_shift.y;
        if dx == 0.0 && dy == 0.0 {
            return 
        }
        let dx = (dx * current_dpi_factor).floor() / current_dpi_factor;
        let dy = (dy * current_dpi_factor).floor() / current_dpi_factor;
        let d = dvec2(dx, dy);
        let mut c = align_start;
        while c < align_end {
            let align_item = &mut self.align_list[c];
            match align_item {
                AlignEntry::Area(Area::Instance(inst)) => {
                    let draw_list = &mut self.cx.draw_lists[inst.draw_list_id];
                    let draw_item = &mut draw_list.draw_items[inst.draw_item_id];
                    let draw_call = draw_item.draw_call().unwrap();
                    let sh = &self.cx.draw_shaders[draw_call.draw_shader.draw_shader_id];
                    let inst_buf = draw_item.instances.as_mut().unwrap();
                    for i in 0..inst.instance_count {
                        if let Some(rect_pos) = sh.mapping.rect_pos {
                            inst_buf[inst.instance_offset + rect_pos + 0 + i * sh.mapping.instances.total_slots] += dx as f32;
                            inst_buf[inst.instance_offset + rect_pos + 1 + i * sh.mapping.instances.total_slots] += dy as f32;
                            if shift_clip{
                                if let Some(draw_clip) = sh.mapping.draw_clip {
                                    inst_buf[inst.instance_offset + draw_clip + 0 + i * sh.mapping.instances.total_slots] += dx as f32;
                                    inst_buf[inst.instance_offset + draw_clip + 1 + i * sh.mapping.instances.total_slots] += dy as f32;
                                    inst_buf[inst.instance_offset + draw_clip + 2 + i * sh.mapping.instances.total_slots] += dx as f32;
                                    inst_buf[inst.instance_offset + draw_clip + 3 + i * sh.mapping.instances.total_slots] += dy as f32;
                                }
                            }
                        }
                    }
                },
                AlignEntry::Area(Area::Rect(ra)) => {
                    let draw_list = &mut self.cx.draw_lists[ra.draw_list_id];
                    let rect_area = &mut draw_list.rect_areas[ra.rect_id];
                    rect_area.rect.pos += d;
                    if shift_clip{
                        rect_area.draw_clip.0 += d;
                        rect_area.draw_clip.1 += d;
                    }
                }
                AlignEntry::BeginBox(clip0, clip1)=>{
                    *clip0 += d;
                    *clip1 += d;
                }
                AlignEntry::SkipBox{skip} | AlignEntry::ShiftBox{skip,..} =>{
                    c = *skip;
                    continue;
                }
                _ => (),
            }
            c += 1;
        }
    }
    
    fn perform_nested_clipping_on_align_list_and_shift(&mut self, align_start:usize, align_end:usize){
        self.box_clips.clear();
        let mut i = align_start;
        while i < align_end{
            let align_item = &self.align_list[i];
            match align_item {
                AlignEntry::SkipBox{skip} =>{
                    i = *skip;
                    continue;
                }
                AlignEntry::ShiftBox{area, shift, skip} =>{
                    let rect = area.get_rect(self);
                    let skip = *skip;
                    self.move_align_list(rect.pos.x+shift.x, rect.pos.y+shift.y, i + 1, skip, true, dvec2(0.0,0.0));
                    i = skip;
                    continue;
                }
                AlignEntry::BeginBox(clip0, clip1)=>{
                    if let Some((tclip0, tclip1)) = self.box_clips.last(){
                        self.box_clips.push((
                            dvec2(clip0.x.max(tclip0.x),clip0.y.max(tclip0.y)),
                            dvec2(clip1.x.min(tclip1.x),clip1.y.min(tclip1.y)),
                        ));
                    }
                    else{
                        self.box_clips.push((*clip0, *clip1));
                    }
                }
                AlignEntry::EndBox=>{
                    self.box_clips.pop().unwrap();
                }
                AlignEntry::Area(Area::Instance(inst)) => if let Some((clip0, clip1)) = self.box_clips.last(){
                    let draw_list = &mut self.cx.draw_lists[inst.draw_list_id];
                    let draw_item = &mut draw_list.draw_items[inst.draw_item_id];
                    let draw_call = draw_item.draw_call().unwrap();
                    let sh = &self.cx.draw_shaders[draw_call.draw_shader.draw_shader_id];
                    let inst_buf = draw_item.instances.as_mut().unwrap();
                    for i in 0..inst.instance_count {
                        if let Some(draw_clip) = sh.mapping.draw_clip {
                            inst_buf[inst.instance_offset + draw_clip + 0 + i * sh.mapping.instances.total_slots] = clip0.x as f32;
                            inst_buf[inst.instance_offset + draw_clip + 1 + i * sh.mapping.instances.total_slots] = clip0.y as f32;
                            inst_buf[inst.instance_offset + draw_clip + 2 + i * sh.mapping.instances.total_slots] = clip1.x as f32;
                            inst_buf[inst.instance_offset + draw_clip + 3 + i * sh.mapping.instances.total_slots] = clip1.y as f32;
                        }
                    }
                },
                AlignEntry::Area(Area::Rect(ra)) => if let Some((clip0, clip1)) = self.box_clips.last(){
                    let draw_list = &mut self.cx.draw_lists[ra.draw_list_id];
                    let rect_area = &mut draw_list.rect_areas[ra.rect_id];
                    rect_area.draw_clip.0 = *clip0;
                    rect_area.draw_clip.1 = *clip1;
                }
                _ => (),
            }
            i += 1;
        }
    }
    
    fn get_box_walk_align_end(&self, i: usize) -> usize {
        if i < self.box_walks.len() - 1 {
            self.box_walks[i + 1].align_start
        }
        else {
            self.align_list.len()
        }
    }
    
    pub fn add_rect_area(&mut self, area: &mut Area, rect: Rect) {
        //let boxx = self.r#box();
        self.add_aligned_rect_area(area, rect)
    }
}

impl Box {
    pub fn update_width_max(&mut self, pos:f64, dx: f64) {
        self.width_used = self.width_used.max((pos + dx) - self.origin.x);
    }
    
    pub fn update_height_max(&mut self, pos:f64, dy: f64) {
        self.height_used = self.height_used.max((pos + dy) - self.origin.y);
    }
    
    pub fn update_width_min(&mut self, pos:f64, dx: f64) {
        self.width_used = self.width_used.min((pos + dx) - self.origin.x);
    }
    
    pub fn update_height_min(&mut self, pos:f64, dy: f64) {
        self.height_used = self.height_used.min((pos + dy) - self.origin.y);
    }
    
    pub fn set_shift(&mut self, shift: DVec2) {
        self.shift = shift;
    }
    
    pub fn layout(&self)->&Layout{
        &self.layout
    }
    
    pub fn used(&self) -> DVec2 {
        dvec2(self.width_used, self.height_used)
    }
    
    pub fn set_used(&mut self, width_used: f64, height_used: f64) {
        self.width_used = width_used;
        self.height_used = height_used;
    }
    /*
    pub fn move_pos(&mut self, dx: f64, dy: f64) {
        self.pos.x += dx;
        self.pos.y += dy;
        self.update_width_max(0.0);
        self.update_height_max(0.0);
    }
    */
    pub fn set_pos(&mut self, pos: DVec2) {
        self.pos = pos
    }
    
    fn child_spacing(&self, walks_len: usize) -> DVec2 {
        if self.box_walks_start < walks_len || self.defer_count > 0 {
            match self.layout.flow {
                Flow::Right => {
                    dvec2(self.layout.spacing, 0.0)
                }
                Flow::Down => {
                    dvec2(0.0, self.layout.spacing)
                }
                Flow::Overlay => {
                    dvec2(0.0, 0.0)
                }
            }
        }
        else {
            dvec2(0.0, 0.0)
        }
    }
    
    pub fn rect_is_visible(&self, geom: Rect) -> bool {
        let view = Rect {pos: self.origin + self.layout.scroll, size: dvec2(self.width, self.height)};
        return view.intersects(geom)
    }
    
    pub fn origin(&self) -> DVec2 {
        self.origin
    }
    
    pub fn rel_pos(&self) -> DVec2 {
        DVec2 {
            x: self.pos.x - self.origin.x,
            y: self.pos.y - self.origin.y
        }
    }
    
    pub fn pos(&self) -> DVec2 {
        self.pos
    }
    
    pub fn scroll(&self) -> DVec2 {
        self.layout.scroll
    }
    
    pub fn eval_width(&self, width: Length, margin: Margin, flow: Flow) -> f64 {
        return match width {
            Length::Fit => std::f64::NAN,
            Length::Fixed(v) => max_zero_keep_nan(v),
            Length::Fill => {
                match flow {
                    Flow::Right => {
                        max_zero_keep_nan(self.width_left() - margin.width())
                    },
                    Flow::Down | Flow::Overlay => {
                        let r = max_zero_keep_nan(self.width - self.layout.padding.width() - margin.width());
                        if r.is_nan() {
                            return self.width_used - margin.width() - self.layout.padding.right
                        }
                        return r
                    }
                }
            },
        }
    }
    
    pub fn eval_height(&self, height: Length, margin: Margin, flow: Flow) -> f64 {
        return match height {
            Length::Fit => std::f64::NAN,
            Length::Fixed(v) => max_zero_keep_nan(v),
            Length::Fill => {
                match flow {
                    Flow::Right | Flow::Overlay => {
                        let r = max_zero_keep_nan(self.height - self.layout.padding.height() - margin.height());
                        if r.is_nan() {
                            return self.height_used - margin.height() - self.layout.padding.bottom
                        }
                        return r
                    }
                    Flow::Down => {
                        max_zero_keep_nan(self.height_left() - margin.height())
                    }
                }
            }
        }
    }
    
    pub fn rect(&self) -> Rect {
        Rect {
            pos: self.origin,
            size: dvec2(self.width, self.height)
        }
    }
    
   pub fn unscrolled_rect(&self) -> Rect {
        Rect {
            pos: self.origin + self.layout.scroll,
            size: dvec2(self.width, self.height)
        }
    }
    
    pub fn padded_rect_used(&self) -> Rect {
        Rect {
            pos: self.origin + self.layout.padding.left_top(),
            size: self.used() - self.layout.padding.left_top()
        }
    }
    pub fn rect_left(&self) -> Rect {
        Rect {
            pos: self.pos,
            size: dvec2(self.width_left(), self.height_left())
        }
    }
    
    pub fn padded_rect(&self) -> Rect {
        Rect {
            pos: self.origin + self.layout.padding.left_top(),
            size: dvec2(self.width, self.height) - self.layout.padding.size()
        }
    }
    
    pub fn size(&self) -> DVec2 {
        dvec2(self.width, self.height)
    }
    
    pub fn width_left(&self) -> f64 {
        return max_zero_keep_nan(self.width - self.width_used - self.layout.padding.right);
    }
    
    pub fn height_left(&self) -> f64 {
        return max_zero_keep_nan(self.height - self.height_used - self.layout.padding.bottom);
    }
    
    pub fn padded_height_or_used(&self) -> f64 {
        let r = max_zero_keep_nan(self.height - self.layout.padding.height());
        if r.is_nan() {
            self.height_used - self.layout.padding.bottom
        }
        else {
            r
        }
    }
    
    pub fn padded_width_or_used(&self) -> f64 {
        let r = max_zero_keep_nan(self.width - self.layout.padding.width());
        if r.is_nan() {
            self.width_used - self.layout.padding.right
        }
        else {
            r
        }
    }
}

impl DeferWalk {
    
    pub fn resolve(&mut self, cx: &Cx2d) -> Walk {
        match self{
            Self::Resolved(walk)=>{*walk},
            Self::Unresolved{pos, defer_index, margin, other_axis}=>{
                let boxx = cx.boxes.last().unwrap();
                let walk = match boxx.layout.flow {
                    Flow::Right => {
                        let left = boxx.width_left();
                        let part = left / boxx.defer_count as f64;
                        Walk {
                            abs_pos: Some(*pos + dvec2(part * *defer_index as f64, 0.)),
                            margin: *margin,
                            width: Length::Fixed(part),
                            height: *other_axis
                        }
                    },
                    Flow::Down => {
                        let left = boxx.height_left();
                        let part = left / boxx.defer_count as f64;
                        Walk {
                            abs_pos: Some(*pos + dvec2(0., part * *defer_index as f64)),
                            margin: *margin,
                            height: Length::Fixed(part),
                            width: *other_axis
                        }
                    }
                    Flow::Overlay => panic!()
                };
                *self = DeferWalk::Resolved(walk);
                walk
            }
        }
    }
    
}

impl Layout {
    pub fn flow_right() -> Self {
        Self {
            flow: Flow::Right,
            ..Self::default()
        }
    }
    
    pub fn flow_down() -> Self {
        Self {
            flow: Flow::Down,
            ..Self::default()
        }
    }
    
    pub fn with_scroll(mut self, v: DVec2) -> Self {
        self.scroll = v;
        self
    }
    
    pub fn with_scale(mut self, s: f64) -> Self {
        self.scale = s;
        self
    }
    
    pub fn with_align_x(mut self, v: f64) -> Self {
        self.align.x = v;
        self
    }
    
    pub fn with_align_y(mut self, v: f64) -> Self {
        self.align.y = v;
        self
    }
    
    pub fn with_clip(mut self, clip_x:bool, clip_y:bool) -> Self {
        self.clip_x = clip_x;
        self.clip_y = clip_y;
        self
    }
    
    pub fn with_padding_all(mut self, v: f64) -> Self {
        self.padding = Padding {left: v, right: v, top: v, bottom: v};
        self
    }
    
    pub fn with_padding_top(mut self, v: f64) -> Self {
        self.padding.top = v;
        self
    }
    
    pub fn with_padding_right(mut self, v: f64) -> Self {
        self.padding.right = v;
        self
    }
    
    pub fn with_padding_bottom(mut self, v: f64) -> Self {
        self.padding.bottom = v;
        self
    }
    
    pub fn with_padding_left(mut self, v: f64) -> Self {
        self.padding.left = v;
        self
    }
    
    pub fn with_padding(mut self, v: Padding) -> Self {
        self.padding = v;
        self
    }
}

impl Walk {
    pub fn empty() -> Self {
        Self {
            abs_pos: None,
            margin: Margin::default(),
            width: Length::Fixed(0.0),
            height: Length::Fixed(0.0),
        }
    }
    
    pub fn size(w: Length, h: Length) -> Self {
        Self {
            abs_pos: None,
            margin: Margin::default(),
            width: w,
            height: h,
        }
    }


    pub fn fixed(w:f64, h:f64) -> Self {
        Self {
            abs_pos: None,
            margin: Margin::default(),
            width: Length::Fixed(w),
            height: Length::Fixed(h),
        }
    }
        
    pub fn fixed_size(size: DVec2) -> Self {
        Self {
            abs_pos: None,
            margin: Margin::default(),
            width: Length::Fixed(size.x),
            height: Length::Fixed(size.y),
        }
    }
    
    pub fn fit() -> Self {
        Self {
            abs_pos: None,
            margin: Margin::default(),
            width: Length::Fit,
            height: Length::Fit,
        }
    }
    
    pub fn fill() -> Self {
        Self {
            abs_pos: None,
            margin: Margin::default(),
            width: Length::Fill,
            height: Length::Fill,
        }
    }
    
    pub fn fill_fit() -> Self {
        Self {
            abs_pos: None,
            margin: Margin::default(),
            width: Length::Fill,
            height: Length::Fit,
        }
    }
    
    pub fn with_abs_pos(mut self, v: DVec2) -> Self {
        self.abs_pos = Some(v);
        self
    }
    pub fn with_margin_all(mut self, v: f64) -> Self {
        self.margin = Margin {left: v, right: v, top: v, bottom: v};
        self
    }
    
    pub fn with_margin_top(mut self, v: f64) -> Self {
        self.margin.top = v;
        self
    }
    
    pub fn with_margin_right(mut self, v: f64) -> Self {
        self.margin.right = v;
        self
    }
    
    pub fn with_margin_bottom(mut self, v: f64) -> Self {
        self.margin.bottom = v;
        self
    }
    
    pub fn with_margin_left(mut self, v: f64) -> Self {
        self.margin.left = v;
        self
    }
    
    pub fn with_margin(mut self, v: Margin) -> Self {
        self.margin = v;
        self
    }
}

impl Padding {
    pub fn left_top(&self) -> DVec2 {
        dvec2(self.left, self.top)
    }
    pub fn right_bottom(&self) -> DVec2 {
        dvec2(self.right, self.bottom)
    }
    pub fn size(&self) -> DVec2 {
        dvec2(self.left + self.right, self.top + self.bottom)
    }
    pub fn width(&self) -> f64 {
        self.left + self.right
    }
    pub fn height(&self) -> f64 {
        self.top + self.bottom
    }
}

impl LiveHook for Padding {
    fn skip_apply(&mut self, _cx: &mut Cx, _apply_from: ApplyFrom, index: usize, nodes: &[LiveNode]) -> Option<usize> {
        if let Some(v) = nodes[index].value.as_float(){
            *self = Self {left: v, top: v, right: v, bottom: v};
            Some(index + 1)
        }
        else{
            None
        }
    }
}

impl Default for Flow {
    fn default() -> Self {Self::Down}
}


impl LiveHook for Length {
    fn skip_apply(&mut self, cx: &mut Cx, _apply_from: ApplyFrom, index: usize, nodes: &[LiveNode]) -> Option<usize> {
        match &nodes[index].value {
            LiveValue::Expr {..} => {
                match live_eval(&cx.live_registry.clone().borrow(), index, &mut (index + 1), nodes) {
                    Ok(ret) => match ret {
                        LiveEval::Float64(v) => {
                            *self = Self::Fixed(v);
                        }
                        LiveEval::Int64(v) => {
                            *self = Self::Fixed(v as f64);
                        }
                        _ => {
                            cx.apply_error_wrong_expression_type_for_primitive(live_error_origin!(), index, nodes, "bool", ret);
                        }
                    }
                    Err(err) => cx.apply_error_eval(err)
                }
                Some(nodes.skip_node(index))
            }
            LiveValue::Float32(v) => {
                *self = Self::Fixed(*v as f64);
                Some(index + 1)
            }
            LiveValue::Float64(v) => {
                *self = Self::Fixed(*v);
                Some(index + 1)
            }
            LiveValue::Int64(v) => {
                *self = Self::Fixed(*v as f64);
                Some(index + 1)
            }
            _ => None
        }
    }
}

impl Default for Length {
    fn default() -> Self {
        Length::Fill
    }
}

impl Length {
    pub fn fixed_or_zero(&self) -> f64 {
        match self {
            Self::Fixed(v) => *v,
            _ => 0.
        }
    }
    
    pub fn fixed_or_nan(&self) -> f64 {
        match self {
            Self::Fixed(v) => max_zero_keep_nan(*v),
            _ => std::f64::NAN,
        }
    }

    pub fn is_fixed(&self) -> bool {
        match self {
            Self::Fixed(_) => true,
            _ => false
        }
    }

    pub fn is_fit(&self) -> bool {
        match self {
            Self::Fit => true,
            _ => false
        }
    }
    
    pub fn is_fill(&self) -> bool {
        match self {
            Self::Fill => true,
            _ => false
        }
    }
}

fn max_zero_keep_nan(v: f64) -> f64 {
    if v.is_nan() {
        v
    }
    else {
        f64::max(v, 0.0)
    }
}


