...
impl DrawText2 {
    ...
    fn draw_allocated_glyph(
        ...
        let transform = Transform::scale_uniform(
            font_size_in_pxs / glyph.pxs_per_em,
        );
        self.p_min = point_to_dvec2(glyph.bounds_in_pxs.min().apply_transform(transform));
        self.p_max = point_to_dvec2(glyph.bounds_in_pxs.max().apply_transform(transform));
