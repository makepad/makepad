...
struct ReliefWindow {
    ...
    /// The key light swung toward a direction (xyz) by an amount (w).
    swing: Option<Vec4f>,
}
...
/// Swing a window's key light: `Some((dir, amount))` turns every surface's
/// own key light (its style's `light`) toward `dir` (x right, y down, z out
/// of the screen) by `amount` (0..1), keeping its intensity; `None` gives
/// each surface its own light back. Surfaces take it as they draw, so the
/// caller redraws them; a window never swung costs nothing.
pub fn set_relief_light_swing(cx: &mut Cx, window: WindowId, swing: Option<(Vec3f, f32)>) {
    let swing = swing.filter(|(_, a)| *a > 0.0005).map(|(d, a)| vec4(d.x, d.y, d.z, a.min(1.0)));
    cx.global::<ReliefGlobal>().window(window).swing = swing;
}

/// `base` (direction, intensity) turned toward `swing` (direction, amount).
fn swung_light(base: [f32; 4], swing: Vec4f) -> [f32; 4] {
    let len = (base[0] * base[0] + base[1] * base[1] + base[2] * base[2]).sqrt().max(1e-6);
    let b = [base[0] / len, base[1] / len, base[2] / len];
    let sl = (swing.x * swing.x + swing.y * swing.y + swing.z * swing.z).sqrt().max(1e-6);
    let t = [swing.x / sl, swing.y / sl, swing.z / sl];
    let a = swing.w;
    let m = [b[0] + (t[0] - b[0]) * a, b[1] + (t[1] - b[1]) * a, b[2] + (t[2] - b[2]) * a];
    let ml = (m[0] * m[0] + m[1] * m[1] + m[2] * m[2]).sqrt().max(1e-6);
    [m[0] / ml * len, m[1] / ml * len, m[2] / ml * len, base[3]]
}
...
pub struct ReliefView {
    ...
    #[rust]
    drawing: bool,
    /// The style's own key light while the window's is swung, and the value
    /// last set over it (a restyle's new light is told apart from ours).
    #[rust]
    light_base: Option<[f32; 4]>,
    #[rust]
    light_set: Option<[f32; 4]>,
}
...
impl ReliefView {
    ...
    /// The window's key light swing (`set_relief_light_swing`) over the
    /// style's own light.
    fn swing_light(&mut self, cx: &mut Cx2d) {
        let swing = cx.get_current_window_id().and_then(|w| cx.global::<ReliefGlobal>().find(w).and_then(|rw| rw.swing));
        if swing.is_none() && self.light_set.is_none() {
            return;
        }
        let vars = &mut self.view.draw_bg.draw_vars;
        let mut now = [0.0f32; 4];
        vars.get_uniform(cx, live_id!(light), &mut now);
        if self.light_set != Some(now) {
            self.light_base = Some(now);
        }
        let base = self.light_base.unwrap_or(now);
        let target = match swing {
            Some(swing) => swung_light(base, swing),
            None => base,
        };
        vars.set_uniform(cx, live_id!(light), &target);
        self.light_set = swing.map(|_| target);
    }
