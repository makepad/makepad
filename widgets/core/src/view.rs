...
mod pointer_capture_tests {
    ...
    fn a_view_does_not_stand_down_against_its_own_scroll_bar() {
        ...
        // The mouse arrives first, as a real one does: an idle bar fades
        // out and lets a press through, and the pointer entering its strip
        // is what shows it.
        let arrive = Event::MouseMove(MouseMoveEvent {
            abs: at,
        ...
        view.handle_event(&mut cx, &arrive, &mut Scope::empty());
        cx.fingers.first_mouse_button = Some((MouseButton::PRIMARY, WindowId(1, 1)));
        let event = mouse_down(at);
        view.handle_event(&mut cx, &event, &mut Scope::empty());
        assert!(
