...
script_mod! {
    ...
    mod.widgets.Window = set_type_default() do mod.widgets.WindowBase{
        ...
        // sound (widgets/core/src/screen_cap.rs). Hardcoded like the caption bar:
        ...
        // The AI chat overlay (the `ai` family, ai_slot.rs): F10 in an app
        // that links the aichat crate, filled by name from it, inert under
        // the window manager, zero cost while off. Without the family it is
        // an empty hidden view and F10 reaches the app.
        ai_chat := AiChatSlot {}
        // The design-feedback overlay (the `tweaker` family, tweaker.rs):
        // hardcoded like the caption bar, inert unless --remote, zero cost
        // while off. Without the family it is an empty hidden view.
        tweaker := Tweaker {}
...
pub struct Window {
    ...
    /// A line over the frozen picture while a crossfade waits on its change
    /// (`set_crossfade_note`, e.g. "Compiling shaders…"), centred in the
    /// caption bar; nothing unless an app sets one.
    #[live]
    draw_note: DrawText,
    #[rust]
    crossfade_note: String,
    /// An app-drawn caption (`set_drag_region`): the area that drags the
    /// window like the caption bar does, and the controls on it that are
    /// the app's. Used while the stock caption bar is hidden.
    #[rust]
    drag_region: Option<(WidgetRef, Vec<WidgetRef>)>,
...
struct Crossfade {
    ...
    start_asked: bool,
    settle: u32,
    /// The draws left out of frames so far (not ready: a pipeline still
    /// compiling), as last seen: the fade waits for frames that add none.
    skips_seen: u64,
    /// When the change was said to be drawn (`start_crossfade`).
    start_asked_at: Option<f64>,
    /// When the fade began.
    started: Option<f64>,
}
...
/// The longest a held crossfade waits for its change to draw whole: a new
/// style's shaders can take seconds to compile the first time.
const CROSSFADE_HOLD: f64 = 8.0;
/// Complete frames in a row (none of their draws waiting on the GPU) a
/// started crossfade needs before it fades.
const CROSSFADE_SETTLE: u32 = 3;
...
impl Window {
    ...
    /// The stock caption bar is wanted: asked for, and no app-drawn caption
    /// (`set_drag_region`) replaces it. Checked on every sync, so a re-apply
    /// of the window's template (a restyle, a live edit) cannot bring the
    /// stock bar back over an app's own caption.
    fn stock_caption(&self) -> bool {
        self.show_caption_bar && self.drag_region.is_none()
    }
    ...
    fn sync_caption_bar_state(&mut self, cx: &mut Cx) {
        ...
        if cx.in_makepad_studio() {
            ...
            self.view(cx, ids!(caption_bar)).set_visible(cx, self.stock_caption() && enabled && has_content);
        ...
        match cx.os_type() {
            OsType::Windows => {
                self.view(cx, ids!(caption_bar))
                    .set_visible(cx, self.stock_caption());
            ...
            OsType::Macos => {
                ...
                self.view(cx, ids!(caption_bar))
                    .set_visible(cx, self.stock_caption() && (!is_fullscreen || has_content));
            }
            OsType::LinuxWindow(params) => {
                ...
                self.view(cx, ids!(caption_bar)).set_visible(
                    cx,
                    self.stock_caption() && (custom_chrome || has_content),
                );
    ...
    fn caption_app_rects(&mut self, cx: &mut Cx) -> Vec<Rect> {
        fn collect(cx: &mut Cx, children: Vec<WidgetRef>, rects: &mut Vec<Rect>) {
            for child in children {
                ...
                let inner = child.borrow::<View>().map(|view| view.children.iter().map(|(_, c)| c.clone()).collect::<Vec<_>>());
...
impl Window {
    ...
    pub fn begin_crossfade(&mut self, cx: &mut Cx, secs: f64, hold: bool) {
        ...
        self.crossfade = Some(Crossfade { texture, secs: secs.max(0.01), asked_at: now, hold, ready_at: None, start_asked: false, settle: 0, skips_seen: 0, start_asked_at: None, started: None });
    ...
    pub fn start_crossfade(&mut self, cx: &mut Cx) {
        if let Some(fade) = &mut self.crossfade {
            if fade.ready_at.is_some() && fade.started.is_none() {
                ...
                fade.start_asked_at.get_or_insert(cx.seconds_since_app_start());
    ...
    fn draw_crossfade(&mut self, cx: &mut Cx2d) {
        ...
        if let Some(fade) = &mut self.crossfade {
            if fade.start_asked && fade.started.is_none() {
                // Count complete frames only: one that left a draw out (a
                // new style's pipeline still compiling) starts the count over,
                // so the old picture stays until the new one is whole.
                let skips = cx.pipeline_skips();
                if skips != fade.skips_seen || cx.pipelines_pending() > 0 {
                    fade.skips_seen = skips;
                    fade.settle = 0;
                } else {
                    fade.settle += 1;
                }
                let waited = fade.ready_at.map(|t| now - t).unwrap_or(0.0);
                if fade.settle > CROSSFADE_SETTLE || waited >= CROSSFADE_HOLD {
                    fade.started = Some(now);
                }
            }
        }
        ...
        // The note, while the frozen picture is held past a moment.
        let note = match &self.crossfade {
            Some(Crossfade { started: None, start_asked_at: Some(t), .. }) if !self.crossfade_note.is_empty() && now - t > 0.15 => true,
            _ => false,
        };
        ...
        if let Some(alpha) = alpha {
            ...
            if note {
                ...
                let h = if bar.size.y > 1.0 { bar.size.y } else { 28.0 };
                let text = self.crossfade_note.clone();
                let w = self.draw_note.layout(cx, 0.0, 0.0, None, false, Align::default(), &text).size_in_lpxs.width as f64;
                let th = self.draw_note.layout(cx, 0.0, 0.0, None, false, Align::default(), &text).size_in_lpxs.height as f64;
                // A slow breath, so it reads as working, not stuck.
                let pulse = 0.65 + 0.35 * (now * 3.0).sin();
                let keep = self.draw_note.color;
                self.draw_note.color.w = keep.w * pulse as f32;
                self.draw_note.draw_depth = 41.0;
                self.draw_note.draw_abs(cx, dvec2((size.x - w) * 0.5, (h - th) * 0.5), &text);
                self.draw_note.color = keep;
            }
        }
...
impl WindowRef {
    ...
    /// How long the frozen picture has been held after the change was made
    /// (the new one not yet drawn whole: its shaders compiling), if it is.
    /// Hide the stock caption bar and let `region` (an app-drawn caption,
    /// such as a device's top rail) drag the window instead, with `clients`
    /// the controls on it; `None` gives the stock caption bar back.
    pub fn set_drag_region(&self, cx: &mut Cx, region: Option<(WidgetRef, Vec<WidgetRef>)>) {
        if let Some(mut inner) = self.borrow_mut() {
            let changed = inner.drag_region.is_some() != region.is_some();
            inner.drag_region = region;
            if changed {
                inner.caption_has_content = None;
                inner.drag_query_cache = None;
                inner.sync_caption_bar_state(cx);
                inner.redraw(cx);
            }
        }
    }

    /// The note shown over a held crossfade (empty: none).
    pub fn set_crossfade_note(&self, note: &str) {
        if let Some(mut inner) = self.borrow_mut() {
            inner.crossfade_note = note.to_string();
        }
    }

    pub fn crossfade_waiting(&self, cx: &Cx) -> Option<f64> {
        let inner = self.borrow()?;
        let fade = inner.crossfade.as_ref()?;
        if fade.started.is_some() {
            return None;
        }
        fade.start_asked_at.map(|t| cx.seconds_since_app_start() - t)
    }
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        let is_for_other_window = match event {
            ...
            Event::WindowDragQuery(dq) => {
                if dq.window_id == self.window.window_id() { 'dq: {
                    ...
                    // The app draws its own caption (its chassis's top rail):
                    // that rail drags the window, its controls are clicks.
                    let _ = visible;
                    {
                        if let Some((region, clients)) = &self.drag_region {
                            let area = region.area();
                            let rect = if area.is_valid(cx) { area.rect(cx) } else { Rect::default() };
                            if rect.contains(dq.abs) {
                                let client = clients.iter().any(|w| w.visible() && w.area().is_valid(cx) && w.area().rect(cx).contains(dq.abs));
                                let held = cx.fingers.is_mouse_held_outside(&[self.view.area()]);
                                dq.response.set(if client || held { WindowDragQueryResponse::Client } else { WindowDragQueryResponse::Caption });
                                cx.set_cursor(MouseCursor::Default);
                            } else if crate::makepad_platform::makepad_error_log::trace_enabled("dragregion") {
                                log!("drag region {:?} does not hold {:?}", rect, dq.abs);
                            }
                            break 'dq;
                        }
                    }
                    ...
                    let over_content = content_toolbar
                        || WidgetHooks::tweaker(cx)
                            .is_some_and(|tweaker| (tweaker.panel_owns_pointer)(dq.abs));
        ...
        if is_for_other_window {
            ...
            // logic lives in the tweaker family (tweaker.rs).
            // The AI overlay does the same for F10 and for the pointer over
            // its open pane (the ai family, ai_slot.rs). Each is a hook the
            // family installs; without it the event goes straight on.
            let window_id = self.window.window_id();
            let tweaked = WidgetHooks::tweaker(cx).is_some_and(|tweaker| {
                (tweaker.window_intercept)(cx, event, &mut self.view, window_id)
            });
            let ai_took = !tweaked
                && WidgetHooks::ai(cx)
                    .is_some_and(|ai| (ai.window_intercept)(cx, event, &mut self.view, window_id));
            if !tweaked && !ai_took {
        ...
        if let Event::Actions(actions) = event {
            if let Some(voice) = WidgetHooks::voice(cx) {
                (voice.window_actions)(cx, actions, &mut self.view, scope);
            }
            if self
