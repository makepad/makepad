...
pub fn script_mod(vm: &mut ScriptVm) {
    ...
    if panel_wears_theme(vm.cx_mut()) {
        ...
        // the fab ones. Worn, they come off the same pairs. The table is
        // the tweaker's, so only when the tweaker family registered it.
        if crate::widget_hooks::WidgetHooks::tweaker(vm.cx_mut()).is_some() {
            let block = script! {
                use mod.prelude.widgets_internal.*
