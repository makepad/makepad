...
mod tests {
    ...
    fn press_at(cx: &mut Cx, grid: &mut ItemGrid, abs: DVec2) -> (Area, ActionsBuf) {
        // The mouse arrives before it presses, as a real one does: an idle
        // scroll bar is faded out and shows when the pointer enters it.
        ...
        grid.handle_event(cx, &arrive, &mut Scope::empty());
