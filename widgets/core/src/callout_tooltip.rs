...
// Where the callout sits moved to the base tooltip; its old path still works.
pub use crate::tooltip::TooltipPosition;
...
pub struct CalloutTooltip {
    ...
    /// Where the mouse pointer last was: a tip below or beside the control
    /// it hovers is placed clear of the cursor drawn there.
    #[rust]
...
impl CalloutTooltip {
    ...
    pub fn show_with_options(
        ...
        // Placed clear of the cursor; the callout still points at the control.
        let rect = crate::tip::rect_clear_of_cursor(
            widget_rect,
            self.pointer,
            matches!(options.position, TooltipPosition::Bottom),
            matches!(options.position, TooltipPosition::Right),
        );
