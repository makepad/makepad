...
use {
    ...
    crate::widget_tree::{set_ui_root, CxWidgetExt, WidgetTree},
    std::any::{Any, TypeId},
    std::cell::RefCell,
...
pub trait WidgetNode: ScriptApply {
    ...
    fn child_by_path(&self, path: &[LiveId]) -> WidgetRef {
        child_by_path_in(path, &mut |visit| self.children(visit))
    }

    fn child(&self, id: LiveId) -> WidgetRef {
        child_in(id, &mut |visit| self.children(visit))
    }
    ...
    fn find_interactive_widget_from_point(&self, cx: &Cx, point: DVec2) -> Option<WidgetRef> {
        first_interactive_widget_in(&mut |found| self.find_widgets_from_point(cx, point, found))
    }
...
/// The body of [`WidgetNode::child_by_path`], shared by every widget type: the
/// provided method only hands over its `children`, so this is compiled once
/// instead of once per type.
#[inline(never)]
fn child_by_path_in(
    path: &[LiveId],
    children: &mut dyn FnMut(&mut dyn FnMut(LiveId, WidgetRef)),
) -> WidgetRef {
...
/// The body of [`WidgetNode::child`], shared by every widget type.
#[inline(never)]
fn child_in(id: LiveId, children: &mut dyn FnMut(&mut dyn FnMut(LiveId, WidgetRef))) -> WidgetRef {
    let mut found = WidgetRef::empty();
    children(&mut |name, child| {
...
/// The body of [`WidgetNode::find_interactive_widget_from_point`], shared by
/// every widget type.
#[inline(never)]
fn first_interactive_widget_in(
    find_widgets: &mut dyn FnMut(&mut dyn FnMut(&WidgetRef)),
) -> Option<WidgetRef> {
    let mut result = None;
    find_widgets(&mut |widget| {
...
pub trait Widget: WidgetNode {
    ...
    fn widget(&self, cx: &Cx, path: &[LiveId]) -> WidgetRef {
        ...
        widget_in(tree, uid, path, &mut |visit| self.children(visit))
    }

    fn widgets(&self, cx: &Cx, paths: &[&[LiveId]]) -> WidgetSet {
        let tree = cx.widget_tree();
        let uid = self.widget_uid();
        widgets_in(tree, uid, paths, &mut |visit| self.children(visit))
    }
    ...
    fn widget_flood(&self, cx: &Cx, path: &[LiveId]) -> WidgetRef {
        let tree = cx.widget_tree();
        let uid = self.widget_uid();
        widget_flood_in(tree, uid, path, &mut |visit| self.children(visit))
    }
    ...
    fn widgets_flood(&self, cx: &Cx, paths: &[&[LiveId]]) -> WidgetSet {
        let tree = cx.widget_tree();
        let uid = self.widget_uid();
        widgets_flood_in(tree, uid, paths, &mut |visit| self.children(visit))
    }
...
// The bodies of `Widget`'s tree lookups, shared by every widget type: the
// provided methods only hand over their uid and `children`, so these are
// compiled once instead of once per type.
...
fn widget_in(
    ...
    tree.refresh_from_borrowed_dyn(uid, children);
    tree.find_within(uid, path)
}
