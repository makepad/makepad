...
#[derive(Clone)]
pub struct ImGUIActions(pub Rc<Vec<WidgetActionItem >>);
...
impl<'a> ImGUIRun<'a> {
    pub fn safe_ref<T: 'static + Widget>(&self, what: Option<&mut Box<dyn Widget >>) -> ImGUIRef {
        let uid = if let Some(what) = what {
            if what.cast::<T>().is_none() {
                WidgetUid::empty()
            }
            else {
                WidgetUid::from_frame_component(what)
            }
        }
        else {
            WidgetUid::empty()
...
impl ImGUIRef {
    pub fn find_single_action(&self) -> Option<&WidgetActionItem> {
        self.actions.0.iter().find( | v | v.uid() == self.uid)
    }
    
    pub fn inner<T: 'static + Widget>(&self) -> Option<std::cell::RefMut<'_, T >> {
...
impl ImGUI {
    ...
    pub fn by_type<T: 'static + Widget>(&mut self) -> Option<std::cell::RefMut<'_, T >> {
...
impl ImGuiEventExt for Event {
    ...
    //Destruct,
    fn on_destruct(&self) -> bool {
        if let Self::Destruct = self {true}else {false}
    }
    //Draw(DrawEvent),
    fn on_draw(&self) -> Option<DrawEvent> {
