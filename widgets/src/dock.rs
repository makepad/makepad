    ...
    pub fn compact_dump(&self, cx: &Cx) -> DockCompactDump {
        ...
        for tabs_id in tabs_ids {
            ...
            for (index, tab_id) in tab_ids.iter().enumerate() {
                ...
                // A tab bar with more tabs than room scrolls them, and a tab
                // past its edge is drawn clipped (or not at all). What is
                // reported is the part a person could click: a tab half out
                // of view has a rect whose centre is still ON it, not on
                // whatever the neighbouring panel shows there, and a tab
                // wholly out of view is not reported as present.
                let tab_rect = tab_rect.clip((bar_rect.pos, bar_rect.pos + bar_rect.size));
                if tab_rect.size.x < 1.0 || tab_rect.size.y < 1.0 {
                    continue;
                }
