...
use super::{
    scene_3d::{apply_scene_to_draw_pbr, chart_data_from_scope, scene_state_from_scope},
};
...
impl BarChart3D {
    ...
    fn draw_axes(&mut self, cx: &mut Cx2d, x_span: f32, z_span: f32, y_top: f32) {
        self.draw_pbr.material_rgba(0.22, 0.22, 0.24, 1.0, 0.02, 0.88);

        self.draw_pbr.push_matrix();
        self.draw_pbr
            .translate(0.0, self.base_y + y_top * 0.5, -z_span * 0.5 - self.spacing * 0.5);
        let _ = self
            .draw_pbr
            .draw_cube(cx, vec3(0.03, y_top, 0.03), 1);
...
impl Widget for BarChart3D {
    fn draw_3d(&mut self, cx: &mut Cx3d, scope: &mut Scope) -> DrawStep {
        ...
        let node_transform = self.draw_pbr.cur_transform;

        let spacing = self.spacing.max(0.001);
        let bar_width = self.bar_width.clamp(0.001, spacing * 0.72);
        let bar_depth = self.bar_depth.clamp(0.001, spacing * 0.72);
        ...
        for bar in &data.bars {
            ...
            self.draw_pbr.push_matrix();
            self.draw_pbr.scale_xyz(bar_width, h, bar_depth);
            self.draw_pbr.translate(x, self.base_y+0.5*h, z);
            //self.draw_pbr.translate(0.0, 0.5, 0.0);
            self.draw_pbr.material(bar.color, self.metallic, self.roughness);
            let _ = self.draw_pbr.draw_cube(cx, vec3(1.0, 1.0, 1.0), 1);
            self.draw_pbr.pop_matrix();
        }
