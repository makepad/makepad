...
pub fn theme_mod(vm: &mut ScriptVm) {
    ...
    // Note: on wasm there is no runtime system-font access and no fallback. The
    // default themes use system_font(...) which resolves to nothing on wasm, so a
    // wasm app MUST preset its own fonts by overriding these TextStyles with
    // crate_resource(...)-bundled fonts (and bundle emoji/CJK fonts if it needs
    // those scripts). Building for wasm with no bundled font is a hard error in
    // the packaging tool. The framework itself bundles no text fonts.
    script_eval!(vm, {
