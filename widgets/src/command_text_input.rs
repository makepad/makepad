...
impl CommandTextInput {
    ...
    /// 
    /// Warning: `CommandTextInput` will not be aware if you call `set_text` on the
    /// raw text input, causing problems with trigger detection. Use `set_text` on
    /// `CommandTextInput` instead.
