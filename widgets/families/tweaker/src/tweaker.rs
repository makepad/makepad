...
// The diff entry and the feedback snapshot are plain data in the core
// (`widget_hooks`), so the chat reads the design feedback through the
// tweaker's hook without linking the tweaker.
pub use crate::widget_hooks::{TweakDiffEntry, TweakFeedbackSnapshot};
...
/// Hand the core's Window and widget tree the overlay: its intercept, its
/// panel's pointer claim, the remote `/tweak` callback and the inspector's
/// type. Run by the family's registration.
pub fn install_tweaker_hooks(cx: &mut Cx) {
    crate::widget_hooks::WidgetHooks::of(cx).tweaker = Some(crate::widget_hooks::TweakerHooks {
        window_intercept,
        panel_owns_pointer,
        tweak_callback,
...
pub fn window_intercept(
    ...
    // (widgets/core/src/screen_cap.rs), and it must not drag the design surface
...
fn cascade_icon_kind(file: &str) -> usize {
    if file.is_empty() {
        ...
        2
    } else if file.starts_with("widgets/") || file.contains("/widgets/") {
        1
    } else {
        0
    }
                ...
                let CascadeRowT = View {
                    ...
                    head := View {
                        ...
                        ic_app := View { width: Fit height: Fit visible: false
                            i := Icon { icon_walk: Walk{width: 12 height: Fit} draw_icon +: { color: #xbbbbbb svg: crate_resource("makepad_widgets:resources/icons/icon_file.svg") } }
                        }
                        ic_lib := View { width: Fit height: Fit visible: false
                            i := Icon { icon_walk: Walk{width: 12 height: Fit} draw_icon +: { color: #xbbbbbb svg: crate_resource("makepad_widgets:resources/icons/icon_widget.svg") } }
                        }
                        ic_theme := View { width: Fit height: Fit visible: false
                            i := Icon { icon_walk: Walk{width: 12 height: Fit} draw_icon +: { color: #xbbbbbb svg: crate_resource("makepad_widgets:resources/icons/icon_draw.svg") } }
                        }
                ...
                View {
                    ...
                    filter_row := View {
                        ...
                        find := PanelButton {
                            ...
                            draw_icon +: {
                                color: #xd8d8d8
                                svg: crate_resource("makepad_widgets:resources/icons/icon_search.svg")
                            }
                        }
                        select := PanelButton {
                            width: 28
                            ...
                            draw_icon +: {
                                color: #xd8d8d8
                                svg: crate_resource("makepad_widgets:resources/icons/icon_select.svg")
                            }
                        }
                        sploded := PanelButton {
                            width: 28
                            ...
                            draw_icon +: {
                                color: #xd8d8d8
                                svg: crate_resource("makepad_widgets:resources/icons/sploded.svg")
                            }
                        }
                    }
                    tab_row := View {
                    ...
                    prompt_row := View {
                        ...
                        prompt_bar := View {
                            ...
                            queue := PanelButton {
                                ...
                                draw_icon +: {
                                    color: #xc8c8d4
                                    svg: crate_resource("makepad_widgets:resources/icons/note_queue.svg")
                                }
                            }
                            send := PanelButton {
                                width: Fit
                                ...
                                draw_icon +: {
                                    color: #x8fd8ff
                                    svg: crate_resource("makepad_widgets:resources/icons/note_send.svg")
                                }
                            }
                        }
                    }
...
/// The widget tree reads an inspector from the type the tweaker's hooks
/// name (moved here from widget_tree.rs with the tweaker).
#[cfg(test)]
mod flat_tree_tests {
    use crate::widget_tree::WidgetTree;
    use crate::{makepad_draw::*, widget::*};
