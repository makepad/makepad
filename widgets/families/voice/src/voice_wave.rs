...
/// Let the window hand its actions to the wave in its caption bar. Run by
/// the family's registration.
pub fn install_voice_hooks(cx: &mut Cx) {
    crate::widget_hooks::WidgetHooks::of(cx).voice = Some(crate::widget_hooks::VoiceHooks {
        window_actions,
    });
}

fn window_actions(cx: &mut Cx, actions: &Actions, view: &mut View, scope: &mut Scope) {
    let voice_wave = view.voice_wave(cx, ids!(voice_wave));
    voice_wave.handle_actions(cx, actions, view, scope);
}
