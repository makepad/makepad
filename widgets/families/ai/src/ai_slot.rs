...
/// The requests global lives in the core ([`crate::widget_hooks`]), so the
/// screen recorder, the tweaker and the window manager read it whether or
/// not the slot is linked.
pub use crate::widget_hooks::{AiSelectedRegion, AiSlotRequests};
...
impl AiChatSlot {
    ...
    fn take_requests(&mut self, cx: &mut Cx) {
        ...
        // person has never opened F10, and an app started for Claude Desktop
        // serves its tools: the hidden overlay owns that work.
        let requests = cx.global::<AiSlotRequests>();
        if requests.feedback_enabled || requests.engine_at_start {
...
/// Hand the core's Window the F10 intercept. Run by the family's
/// registration; an app without it has no slot and F10 reaches the app.
pub fn install_ai_hooks(cx: &mut Cx) {
    crate::widget_hooks::WidgetHooks::of(cx).ai = Some(crate::widget_hooks::AiHooks {
        window_intercept,
    });
}
