use crate::{makepad_platform::*, widget::*};
...
impl DataBindingStore {
    ...
    pub fn data_bind<F>(&mut self, cx: &mut Cx, actions: &Actions, ui: &WidgetRef, f: F)
    where
        F: Fn(DataBindingMap),
    {
...
impl<'a> DataBindingMap<'a> {
    ...
    pub fn is_data_to_widgets(&self) -> bool {
        if let Direction::DataToWidgets(_) = self.direction {
            true
        } else {
            false
        }
    ...
    pub fn is_widgets_to_data(&self) -> bool {
        if let Direction::WidgetsToData(_, _) = self.direction {
            true
        } else {
            false
        }
