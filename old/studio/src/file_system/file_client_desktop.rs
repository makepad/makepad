use {
    crate::{
        makepad_file_protocol::{FileClientMessage, FileRequest},
        makepad_file_server::{FileServer, FileServerConnection, FileSystemRoots},
        makepad_micro_serde::*,
        makepad_platform::*,
    },
    std::{
...
impl FileClient {
    ...
    pub fn load_snapshot_image(&mut self, root: &str, hash: &str) {
        self.send_request(FileRequest::LoadSnapshotImage {
            root: root.to_string(),
            hash: hash.to_string(),
        });
    }
    ...
    pub fn new_with_local_server(roots: FileSystemRoots) -> Self {
        ...
        spawn_local_request_handler(
            ...
            server.connect(Box::new({
                ...
                move |notification| {
                    message_sender
                        .send(FileClientMessage::Notification(notification))
                        .unwrap();
                    message_signal.set();
                }
            })),
...
fn _spawn_remote_request_handler(
    ...
    thread::spawn(move || loop {
        ...
        message_sender
            .send(FileClientMessage::Response(response))
            .unwrap();
    });
}
