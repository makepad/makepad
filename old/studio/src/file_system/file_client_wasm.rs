use {
    crate::{
        makepad_collab_protocol::{CollabClientAction, CollabRequest},
        makepad_micro_serde::*,
        makepad_platform::*,
    },
    //std::{
...
pub struct CollabClient {
    ...
    #[rust]
    requests: Rc<RefCell<Vec<CollabRequest>>>,
    #[rust(LiveId::unique())]
    signal: SignalToUI,
}
...
impl LiveHook for CollabClient {
    fn after_apply(&mut self, cx: &mut Cx, _apply: &mut Apply, _index: usize, _nodes: &[LiveNode]) {
        if self.web_socket.is_none() {
            // connect websocket
            let (host, protocol) =
                if let OsType::WebBrowser { host, protocol, .. } = &cx.platform_type() {
                    (host, protocol)
                } else {
                    panic!()
                };

            self.web_socket = Some(cx.web_socket_open(
                format!(
                    "{}://{}",
...
impl CollabClient {
    ...
    pub fn handle_event_with_fn(
        ...
        cx: &mut Cx,
        event: &Event,
        dispatch_action: &mut dyn FnMut(&mut Cx, CollabClientAction),
    ) {
        match event {
