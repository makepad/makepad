...
impl FileServer {
    ...
    pub fn new(roots: FileSystemRoots) -> FileServer {
        FileServer {
            ...
            shared: Arc::new(RwLock::new(Shared { roots })),
        }
    }
...
impl FileServerConnection {
    ...
    pub fn handle_request(&self, request: FileRequest) -> FileResponse {
        match request {
            ...
            FileRequest::LoadFileTree { with_data } => {
                FileResponse::LoadFileTree(self.load_file_tree(with_data))
            }
            FileRequest::OpenFile { path, id } => FileResponse::OpenFile(self.open_file(path, id)),
            FileRequest::SaveFile {
                path,
                data,
                id,
                patch,
            } => FileResponse::SaveFile(self.save_file(path, data, id, patch)),
            FileRequest::LoadSnapshotImage { root, hash } => {
                FileResponse::LoadSnapshotImage(self.load_snapshot_image(root, hash))
            }
            FileRequest::SaveSnapshotImage { root, hash, data } => {
                FileResponse::SaveSnapshotImage(self.save_snapshot_image(root, hash, data))
            }
            FileRequest::CreateSnapshot { root, message } => {
                FileResponse::CreateSnapshot(self.create_snapshot(root, message))
            }
            FileRequest::LoadSnapshot { root, hash } => {
                FileResponse::LoadSnapshot(self.load_snapshot(root, hash))
            }
        }
    }
    ...
    fn search_start(&self, what: Vec<SearchItem>, id: u64) {
        ...
        thread::spawn(move || {
            ...
            fn search_files(
                ...
                path: &Path,
                string_path: &str,
                sender: &mut Box<dyn NotificationSender>,
                    ...
                    for entry in entries {
                        if let Ok(entry) = entry {
                            ...
                            if let Ok(name) = name.into_string() {
                                if entry_path.is_file() && !name.ends_with(".rs")
                                    || entry_path.is_dir() && name == "target"
                                    || name.starts_with('.')
                                {
                                    continue;
                                }
                            ...
                            if entry_path.is_dir() {
                                ...
                                if let Ok(bytes) = fs::read(&entry_path) {
                                    ...
                                    for item in set {
                                        ...
                                        makepad_rabin_karp::search(
                                            &bytes,
                                            &needle_bytes,
                                            &mut rk_results,
                                        );
                                        for result in &rk_results {
                                            if item.pre_word_boundary
                                                && result.byte > 0
                                                && is_word_char(bytes[result.byte - 1])
                                            {
                                                continue;
                                            }
                                            if item.post_word_boundary
                                                && result.byte + needle_bytes.len() < bytes.len()
                                                && is_word_char(
                                                    bytes[result.byte + needle_bytes.len()],
                                                )
                                            {
                                                continue;
                                            }
                                            if let Some(prefixes) = &item.prefixes {
                                                ...
                                                if !prefixes.iter().any(|prefix| {
                                                    ...
                                                    if result.byte > pb.len() {
                                                        if &bytes
                                                            [result.byte - pb.len()..result.byte]
                                                            == pb
                                                        {
                                                            return true;
                                                        }
                                            ...
                                            for i in result.new_line_byte..bytes.len() + 1 {
                                                ...
                                                if i == bytes.len()
                                                    || bytes[i] == b'\n' && line_count == 4
                                                {
                                                    if let Ok(result_line) = str::from_utf8(
                                                        &bytes[result.new_line_byte..i],
                                                    ) {
            ...
            if results.len() > 0 {
                sender.send_notification(FileNotification::SearchResults { id, results });
            }
        });
    }
    ...
    fn create_snapshot(
        ...
        root: String,
        message: String,
    ) -> Result<CreateSnapshotResponse, CreateSnapshotError> {
        let root_path = self
            .shared
            .read()
            .unwrap()
            .roots
            .find_root(&root)
            .map_err(|error| CreateSnapshotError {
                error: format!("{:?}", error),
                root: root.clone(),
            })?;
        ...
        match shell_env_cap(
            ...
            Ok(_) => {
                match shell_env_cap(
                    ...
                    &root_path,
                    "git",
                    &["log", "--pretty=format:%H", "--max-count=1"],
                ) {
                    Ok(stdout) => {
                    ...
                    Err(e) => Err(CreateSnapshotError { root, error: e }),
                }
            }
            // we expect it on stderr
            Err(e) => Err(CreateSnapshotError { root, error: e }),
    ...
    fn load_snapshot(
        ...
        root: String,
        hash: String,
    ) -> Result<LoadSnapshotResponse, LoadSnapshotError> {
        let root_path = self
            .shared
            .read()
            .unwrap()
            .roots
            .find_root(&root)
            .map_err(|error| LoadSnapshotError {
                error: format!("{:?}", error),
                root: root.clone(),
            })?;

        match shell_env_cap(&[], &root_path, "git", &["checkout", &hash]) {
            Ok(_) => Ok(LoadSnapshotResponse { root, hash }),
            // we expect it on stderr
            Err(e) => Err(LoadSnapshotError { root, error: e }),
        }
    }
            ...
            while !*stop_observation.lock().unwrap() {
                if let Ok(mut files) = open_files.lock() {
                    for (path, file_id, last_content) in files.iter_mut() {
                        let full_path =
                            { shared.read().unwrap().roots.make_full_path(&path) }.unwrap();
                        if let Ok(bytes) = fs::read(&full_path) {
                            if bytes.len() > 0 && bytes != *last_content {
                                ...
                                notification_sender.send_notification(
                                    FileNotification::FileChangedOnDisk(SaveFileResponse {
                                        path: path.to_string(),
    ...
    fn load_snapshot_image(
        &self,
        root: String,
        hash: String,
    ) -> Result<LoadSnapshotImageResponse, LoadSnapshotImageError> {
        ...
        let root_path = self
            ...
            .roots
            .find_root(&root)
            .map_err(|error| LoadSnapshotImageError {
                error,
                root: root.clone(),
                hash: hash.clone(),
            })?;
        let path = root_path
            ...
            .join(&hash)
            .with_extension("jpg");
        let bytes = fs::read(&path).map_err(|error| LoadSnapshotImageError {
    ...
    fn save_snapshot_image(
        &self,
        root: String,
        hash: String,
        data: Vec<u8>,
    ) -> Result<SaveSnapshotImageResponse, FileError> {
        // alright letrs find the root
        ...
        fs::write(&path, data).map_err(|error| FileError::Unknown(error.to_string()))?;

        return Ok(SaveSnapshotImageResponse { root, hash });
    }
    ...
    fn open_file(&self, child_path: String, id: u64) -> Result<OpenFileResponse, FileError> {
        let path = self
            ...
            .unwrap()
            .roots
            .make_full_path(&child_path)?;

        let bytes = fs::read(&path).map_err(|error| FileError::Unknown(error.to_string()))?;

        let mut open_files = self.open_files.lock().unwrap();

        if open_files
            .iter()
            .find(|(cp, _, _)| *cp == child_path)
            .is_none()
        {
    ...
    fn save_file(
        ...
        let old_data = String::from_utf8_lossy(
            &fs::read(&path).map_err(|error| FileError::Unknown(error.to_string()))?,
        )
        .to_string();

        fs::write(&path, &new_data).map_err(|error| FileError::Unknown(error.to_string()))?;
