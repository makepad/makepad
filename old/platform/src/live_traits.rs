use crate::{cx::Cx, makepad_live_compiler::*, scope::Scope};
    ...
    fn after_apply_from(&mut self, cx: &mut Cx, apply: &mut Apply) {
        match &apply.from {
            ApplyFrom::NewFromDoc { .. } => {
                self.after_new_from_doc(cx);
                self.after_apply_from_doc(cx);
            }
            ApplyFrom::UpdateFromDoc { .. } => {
                self.after_update_from_doc(cx);
                self.after_apply_from_doc(cx);
            }
            _ => (),
        }
...
pub trait LiveNew: LiveApply {
        ...
        Self: Sized,
    {
        ...
        if let Some(live_ptr) = live_ptr {
            cx.get_nodes_from_live_ptr(live_ptr, |cx, file_id, index, nodes| {
                ret.apply(
                    cx,
                    &mut ApplyFrom::NewFromDoc { file_id }.into(),
                    index,
                    nodes,
                )
            });
    ...
    fn update_from_ptr(&mut self, cx: &mut Cx, live_ptr: Option<LivePtr>) {
        if let Some(live_ptr) = live_ptr {
            cx.get_nodes_from_live_ptr(live_ptr, |cx, file_id, index, nodes| {
                self.apply(
                    cx,
                    &mut ApplyFrom::UpdateFromDoc { file_id }.into(),
                    index,
                    nodes,
                )
            });
    ...
    fn update_from_ptr_with_scope(
        ...
        if let Some(live_ptr) = live_ptr {
            cx.get_nodes_from_live_ptr(live_ptr, |cx, file_id, index, nodes| {
                self.apply(
                    cx,
                    &mut ApplyFrom::UpdateFromDoc { file_id }.with_scope(scope),
                    index,
                    nodes,
                )
            });
    ...
    fn new_from_ptr_with_scope(cx: &mut Cx, live_ptr: Option<LivePtr>, scope: &mut Scope) -> Self
    where
        Self: Sized,
    {
        let mut ret = Self::new(cx);
        if let Some(live_ptr) = live_ptr {
            cx.get_nodes_from_live_ptr(live_ptr, |cx, file_id, index, nodes| {
                ret.apply(
                    cx,
                    &mut ApplyFrom::NewFromDoc { file_id }.with_scope(scope),
                    index,
                    nodes,
                )
            });
        ...
        Self: Sized,
    {
        ...
        if let Some(file_id) = live_registry.module_id_to_file_id.get(&module_id) {
            ...
            if let Some(index) = file.expanded.nodes.child_by_name(0, id.as_instance()) {
                ...
                ret.apply(
                    cx,
                    &mut ApplyFrom::NewFromDoc { file_id: *file_id }.into(),
                    index,
                    &file.expanded.nodes,
                );
                return Some(ret);
    ...
    fn update_from_module(&mut self, cx: &mut Cx, module_id: LiveModuleId, id: LiveId) {
        ...
        if let Some(file_id) = live_registry.module_id_to_file_id.get(&module_id) {
            ...
            if let Some(index) = file.expanded.nodes.child_by_name(0, id.as_instance()) {
                self.apply(
                    cx,
                    &mut ApplyFrom::UpdateFromDoc { file_id: *file_id }.into(),
                    index,
                    &file.expanded.nodes,
                );
            }
...
impl<T, const N: usize> LiveRead for [T; N]
where
    T: LiveRead,
{
    fn live_read_to(&self, id: LiveId, out: &mut Vec<LiveNode>) {
...
impl<'a, 'b, 'c> Apply<'a, 'b, 'c> {
    pub fn override_from<F, R>(&mut self, from: ApplyFrom, f: F) -> R
    where
        F: FnOnce(&mut Apply) -> R,
    {
