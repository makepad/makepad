use {
    self::super::{
        ...
        to_wasm::{ToWasmMidiInputData, ToWasmMidiPortList},
    },
    crate::{
...
impl WebMidiAccess {
    ...
    pub fn create_midi_output(&mut self) -> MidiOutput {
        ...
        MidiOutput(Some(OsMidiOutput { sender: send }))
    }
    ...
    pub fn use_midi_inputs(&mut self, os: &mut CxOs, port_ids: &[MidiPortId]) {
        ...
        os.from_wasm(FromWasmUseMidiInputs { input_uids })
    }
    ...
    pub fn to_wasm_midi_input_data(&mut self, tw: ToWasmMidiInputData) {
        if let Some(port) = self.ports.iter().find(|v| v.uid == tw.uid) {
            let data = MidiData {
                data: [
                    ((tw.data >> 16) & 0xff) as u8,
                    ((tw.data >> 8) & 0xff) as u8,
                    ((tw.data >> 0) & 0xff) as u8,
                ],
            };
            self.input_senders
                .retain(|send| send.send((port.desc.port_id, data)).is_ok())
        }
    }
    ...
    pub fn send_midi_output_data(&mut self, from_wasm: &mut FromWasmMsg) {
        ...
        self.output_receivers.retain(|recv| loop {
            match recv.try_recv() {
                Ok((port_id, d)) => {
                    for port in ports {
    ...
    pub fn handle_web_midi_signals(&mut self) {
        ...
        if let Some(web_midi) = &self.media.web_midi {
            web_midi
                .lock()
                .unwrap()
                .send_midi_output_data(self.from_wasm.as_mut().unwrap());
        }
    }
}
