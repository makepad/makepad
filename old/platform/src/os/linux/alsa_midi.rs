...
const kRequiredInputPortCaps: c_uint = SND_SEQ_PORT_CAP_READ | SND_SEQ_PORT_CAP_SUBS_READ;
const kRequiredOutputPortCaps: c_uint = SND_SEQ_PORT_CAP_WRITE | SND_SEQ_PORT_CAP_SUBS_WRITE;
const kCreateOutputPortCaps: c_uint = SND_SEQ_PORT_CAP_READ | SND_SEQ_PORT_CAP_NO_EXPORT;
const kCreateInputPortCaps: c_uint = SND_SEQ_PORT_CAP_WRITE | SND_SEQ_PORT_CAP_NO_EXPORT;
const kCreatePortType: c_uint = SND_SEQ_PORT_TYPE_MIDI_GENERIC | SND_SEQ_PORT_TYPE_APPLICATION;
...
impl AlsaClient {
    unsafe fn new() -> Result<AlsaClient, AlsaError> {
        ...
        alsa_error!(snd_seq_open(
            &mut in_client,
            "default\0".as_ptr(),
            SND_SEQ_OPEN_INPUT,
            0
        ))?;
        alsa_error!(snd_seq_set_client_name(
            in_client,
    ...
    unsafe fn enumerate_ports(&self) -> Result<Vec<AlsaMidiPort>, AlsaError> {
        ...
        while snd_seq_query_next_client(self.in_client.0, client_info) == 0 {
            ...
            let client_name = CStr::from_ptr(snd_seq_client_info_get_name(client_info))
                .to_str()
                .unwrap()
                .to_string();
            ...
            while snd_seq_query_next_port(self.in_client.0, port_info) == 0 {
                ...
                let port_name = CStr::from_ptr(snd_seq_port_info_get_name(port_info))
                    .to_str()
                    .unwrap()
                    .to_string();
...
impl AlsaMidiAccess {
    pub fn new(change_signal: SignalToUI) -> Arc<Mutex<Self>> {
        ...
        std::thread::spawn(move || unsafe {
            let in_client = midi_access_clone
                .lock()
                .unwrap()
                .client
                .as_ref()
            ...
            loop {
                ...
                let msg = match (*ev).type_ {
                    ...
                    SND_SEQ_EVENT_NOTEON | SND_SEQ_EVENT_NOTEOFF => Some(
                        MidiNote {
                ...
                if let Some(msg) = msg {
                    if let Some(port_id) = midi_access_clone
                        .lock()
                        .unwrap()
                        .find_port((*ev).source.client as i32, (*ev).source.port as i32)
                    {
                        let mut senders = input_senders.lock().unwrap();
                        senders.retain(|s| s.send((port_id, msg)).is_ok());
                        if senders.len() > 0 {
    ...
    pub fn use_midi_outputs(&mut self, ports: &[MidiPortId]) {
        ...
        for port_id in ports {
            if let Some(port) = self
                .ports
                .iter_mut()
                .find(|p| p.desc.port_id == *port_id && p.desc.port_type.is_output())
            {
                unsafe { port.subscribe(self.client.as_ref().unwrap()) }
            }
        }
