...
impl AndroidMidiAccess {
            ...
            for input in &mut self.inputs {
                ...
                if messages == 1 {
                    ...
                    for i in (0..bytes_recv as usize).step_by(3) {
                        ...
                        self.senders
                            .retain(|s| s.send((input.port_id, data)).is_ok());
                    }
                } else if messages < 0 {
    ...
    pub fn send_midi(&mut self, port_id: Option<MidiPortId>, data: MidiData) {
        for output in &self.outputs {
            if port_id.is_none() || port_id == Some(output.port_id) {
                unsafe {
                    AMidiInputPort_send(output.amidi_port, data.data.as_ptr() as *const _, 3)
                };
            }
        }
    }
    ...
    pub fn create_midi_input(&mut self, amidi: Arc<Mutex<Self>>) -> MidiInput {
        ...
        MidiInput(Some(OsMidiInput { amidi, recv }))
    }
    ...
    fn find_device_for_port_id(
        devices: &[AndroidMidiDevicePtr],
        port_id: MidiPortId,
    ) -> Option<(usize, usize)> {
    ...
    pub fn use_midi_outputs(&mut self, ports: &[MidiPortId]) {
        ...
        for port_id in ports {
            ...
            {
                // new this one
                if let Some((device_index, port_index)) =
                    Self::find_device_for_port_id(&self.devices, *port_id)
                {
                    if let Some(output_port) =
                        AndroidMidiOutput::new(*port_id, &self.devices[device_index], port_index)
                    {
                ...
                .is_none()
            {
                    ...
                    Self::find_device_for_port_id(&self.devices, *port_id)
                {
                    if let Some(input_port) =
                        AndroidMidiInput::new(*port_id, &self.devices[device_index], port_index)
                    {
