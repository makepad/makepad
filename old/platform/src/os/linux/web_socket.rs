        ...
        if stream.is_err() {
            rx_sender
                .send(WebSocketMessage::Error(
                    "Error connecting websocket tcpstream".into(),
                ))
                .unwrap();
            return OsWebSocket {
                sender: None,
            ...
            while !done {
                ...
                match input_stream.read(&mut buffer) {
                    Ok(bytes_read) => {
                        ...
                        web_socket.parse(&buffer[0..bytes_read], |result| match result {
                            Ok(ServerWebSocketMessage::Ping(_)) => {
                                if write_bytes_to_tcp_stream_no_error(
                                    &mut input_stream,
                                    &SERVER_WEB_SOCKET_PONG_MESSAGE,
                                ) {
                                    done = true;
                                    let _ = rx_sender.send(WebSocketMessage::Error(
                                        "Pong message send failed".into(),
                                    ));
                                }
                            }
