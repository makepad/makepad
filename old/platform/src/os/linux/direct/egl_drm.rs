...
impl Drm {
    pub unsafe fn new(mode_want: &str) -> Option<Self> {
        ...
        let num_devices =
            drmGetDevices2(0, drm_devices.as_mut_ptr(), MAX_DRM_DEVICES as _) as usize;
        ...
        'outer: for i in 0..num_devices {
            ...
            let drm_fd = libc_sys::open(
                *drm_device.nodes.offset(DRM_NODE_PRIMARY as _),
                libc_sys::O_RDWR,
            );
...
impl Egl {
    pub unsafe fn new(drm: &Drm) -> Option<Self> {
        ...
        let mut cfg_count = 0;
        if (libegl.eglGetConfigs.unwrap())(egl_display, std::ptr::null_mut(), 0, &mut cfg_count)
            == 0
            || cfg_count == 0
        {
        ...
        for i in 0..cfg_count as usize {
            let mut native_id = 0;
            if (libegl.eglGetConfigAttrib.unwrap())(
                egl_display,
                configs[i],
