...
pub fn derive_live_component_registry_impl(input: TokenStream) -> TokenStream {
    ...
    if parser.eat_ident("struct") {
        if let Some(struct_name) = parser.eat_any_ident() {
            ...
            tb.add("impl LiveComponentRegistry for")
                .ident(&struct_name)
                .stream(generic.clone())
                .stream(where_clause.clone())
                .add("{");
            tb.add("    fn ref_cast_type_id(&self) -> LiveType {LiveType::of::<")
                .ident(&struct_name)
                .add(">()}");

            tb.add("    fn component_type(&self) -> LiveId {live_id!(")
                .ident(&trait_name)
                .add(")}");
            tb.add(
                "    fn get_module_set(&self, set: &mut std::collections::BTreeSet<LiveModuleId>){",
            );
            tb.add(
                "        self.map.values().for_each( | (info, _) | {set.insert(info.module_id);});",
            );
            tb.add("    }");
            ...
            tb.add("impl ")
                ...
                .stream(where_clause)
                .add("{");
            tb.add("    pub fn new(&self, cx: &mut Cx, ty: LiveType) -> Option<Box<dyn ")
                .ident(&trait_name)
                .add(" >> {");
