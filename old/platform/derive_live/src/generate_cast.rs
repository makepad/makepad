...
pub fn generate_any_send_trait_api_impl(input: TokenStream) -> TokenStream {
    ...
    if let Some(ident) = parser.eat_any_ident() {
        ...
        tb.add("    pub fn is<T: ")
            .ident(&ident)
            .add(" + 'static + Send + Sync >(&self) -> bool {");
        tb.add("        let t = std::any::TypeId::of::<T>();");
        tb.add("        let concrete = self.ref_cast_type_id();");
        tb.add("        t == concrete");
        tb.add("    }");
        tb.add("    pub fn downcast_ref<T: ")
            .ident(&ident)
            .add(" + 'static + Send + Sync>(&self) -> Option<&T> {");
        tb.add("        if self.is::<T>() {");
        tb.add("            Some(unsafe {&*(self as *const dyn ")
            .ident(&ident)
            .add(" as *const T)})");
        tb.add("        } else {");
        tb.add("            None");
        tb.add("        }");
        tb.add("    }");
        tb.add("    pub fn downcast_mut<T: ")
            .ident(&ident)
            .add(" + 'static + Send + Sync>(&mut self) -> Option<&mut T> {");
        tb.add("        if self.is::<T>() {");
        tb.add("            Some(unsafe {&mut *(self as *const dyn ")
            .ident(&ident)
            .add(" as *mut T)})");
        tb.add("        } else {");
        tb.add("            None");
...
pub fn generate_any_trait_api_impl(input: TokenStream) -> TokenStream {
    ...
    if let Some(ident) = parser.eat_any_ident() {
        ...
        tb.add("    pub fn downcast_mut<T: ")
            .ident(&ident)
            .add(" + 'static >(&mut self) -> Option<&mut T> {");
        tb.add("        if self.is::<T>() {");
        tb.add("            Some(unsafe {&mut *(self as *const dyn ")
            .ident(&ident)
            .add(" as *mut T)})");
...
pub fn derive_default_none_impl(input: TokenStream) -> TokenStream {
    ...
    if parser.eat_ident("enum") {
        if let Some(enum_name) = parser.eat_any_ident() {
            ...
            tb.add("impl ActionDefaultRef for ")
                .ident(&enum_name)
                .stream(generic.clone())
                .stream(where_clause.clone());
            tb.add("{");
