...
use makepad_live_id::*;
use makepad_micro_proc_macro::{error_result, Attribute, TokenBuilder, TokenParser};
...
fn derive_live_atomic_impl_inner(
        ...
        for field in &fields {
            if field.attrs[0].name == "live" {
                tb.add("    LiveId(")
                    ...
                    .add(")=>self.")
                    .ident(&field.name)
                    .add(".apply_atomic(cx, apply, index, nodes),");
            }
        }
        tb.add("            _=> {");
        tb.add(
        ...
        */
        ...
        while !parser.eat_eot() {
            ...
            if let Some(name) = parser.eat_any_ident() {
                ...
                if parser.eat_all_types().is_some() || parser.eat_all_struct_fields().is_some() {
                    // named variant
        ...
        tb.add("        _=>panic!(")
            .string("Invalid u32 for enum, should be impossible")
            .add(")");
        tb.add("        }");
        tb.add("    }");
        tb.add("}");
