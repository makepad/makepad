...
use {
    crate::{
        ...
        live_ptr::{LiveFileGeneration, LiveFileId, LiveModuleId, LivePtr},
        live_token::{LiveToken, LiveTokenId, TokenWithSpan},
        makepad_live_id::*,
        // makepad_error_log::*,
        makepad_live_tokenizer::{
            live_error_origin, Cursor, Delim, FullToken, LiveErrorOrigin, State, TokenWithLen,
        },
...
impl LiveRegistry {
    ...
    pub fn ptr_to_node(&self, live_ptr: LivePtr) -> &LiveNode {
        ...
        if doc.generation != live_ptr.generation {
            panic!(
                "ptr_to_node generation invalid for file {} gen:{} ptr:{}",
                doc.file_name, doc.generation, live_ptr.generation
            );
        }
    ...
    pub fn ptr_to_doc_node(&self, live_ptr: LivePtr) -> (&LiveExpanded, &LiveNode) {
        ...
        if doc.generation != live_ptr.generation {
            panic!(
                "ptr_to_doc_node generation invalid for file {} gen:{} ptr:{}",
                doc.file_name, doc.generation, live_ptr.generation
            );
        }
        (
    ...
    pub fn ptr_to_file_name_and_object_span(&self, ptr: LivePtr) -> (String, TextSpan) {
        ...
        let start_token = self
            .token_id_to_token(start_node.origin.token_id().unwrap())
            .clone();
        let end_token = self
            .token_id_to_token(end_node.origin.token_id().unwrap())
            .clone();
    ...
    pub fn ptr_to_doc(&self, live_ptr: LivePtr) -> &LiveExpanded {
        ...
        if doc.generation != live_ptr.generation {
            panic!(
                "ptr_to_doc generation invalid for file {} gen:{} ptr:{}",
                doc.file_name, doc.generation, live_ptr.generation
            );
        }
        &doc.expanded
    ...
    pub fn ptr_to_nodes_index(&self, live_ptr: LivePtr) -> (&[LiveNode], usize) {
        ...
        if doc.generation != live_ptr.generation {
            panic!(
                "ptr_to_nodes_index generation invalid for file {} gen:{} ptr:{}",
                doc.file_name, doc.generation, live_ptr.generation
            );
        }
    ...
    pub fn token_id_to_token(&self, token_id: LiveTokenId) -> &TokenWithSpan {
        &self.live_files[token_id.file_id().unwrap().to_index()]
            .original
            .tokens[token_id.token_index()]
    }
    ...
    pub fn live_node_as_string(&self, node: &LiveNode) -> Option<String> {
        match &node.value {
            ...
            LiveValue::String(v) => Some(v.as_str().to_string()),
            LiveValue::InlineString(v) => Some(v.as_str().to_string()),
            LiveValue::Dependency(v) => Some(v.as_str().to_string()),
            LiveValue::Font(v) => Some(v.paths[0].as_str().to_string()),
            _ => None,
        }
    }
        ...
        if let Some(file_id) = self.module_id_to_file_id.get(&module_id) {
            ...
            if name != LiveId::empty() {
                if doc.nodes.is_empty() {
                    eprintln!(
                        "module_path_id_to_doc zero nodelen {}",
                        self.file_id_to_file_name(*file_id)
                    );
                    return None;
                }
    ...
    pub fn find_module_id_name(
        ...
        if let Some(file_id) = self.module_id_to_file_id(module_id) {
            ...
            if file.expanded.nodes.is_empty() {
                println!(
                    "Looking for {} but its not expanded yet, dependency order bug",
                    file.file_name
                );
                return None;
            }
            if let Some(index) = file.expanded.nodes.child_by_name(0, item.as_instance()) {
                return Some(LiveScopeTarget::LivePtr(LivePtr {
                    file_id,
                    index: index as u32,
    ...
    pub fn find_scope_target_one_level_or_global(
        &self,
        item: LiveId,
        index: usize,
        nodes: &[LiveNode],
        ...
        match live_error.span {
            ...
            LiveErrorSpan::Token(token_span) => {
                let (file_name, span) = if let Some(file_id) = token_span.token_id.file_id() {
                    ...
                    (
                        live_file.file_name.as_str(),
                        live_file.original.tokens[token_span.token_id.token_index()].span,
                    )
                } else {
    ...
    pub fn tokenize_from_str(
        ...
        start_pos: TextPos,
        file_id: LiveFileId,
    ) -> Result<Vec<TokenWithSpan>, LiveError> {
        ...
        loop {
            ...
            if let Some(full_token) = full_token {
                ...
                let span = TextSpan {
                    file_id,
                    start: TextPos {
                        column: (last_index - last_new_line) as u32,
                        line: line_start,
                    },
                    end: TextPos {
                        column: (last_index - last_new_line) as u32 + full_token.len as u32,
                        line: line_end,
                    },
                };
    ...
    pub fn tokenize_from_str_live_design(
        source: &str,
        start_pos: TextPos,
        file_id: LiveFileId,
        mut negative: Option<&mut Vec<TokenWithLen>>,
    ) -> Result<Vec<TokenWithSpan>, LiveError> {
        let mut chars = Vec::new();
        chars.extend(source.chars());
        let mut state = State::default();
        ...
        loop {
            ...
            if let Some(full_token) = full_token {
                ...
                match parse {
                    ...
                    Parse::Bang => {
                        if let FullToken::Punct(live_id!(!)) = &full_token.token {
                            parse = Parse::Brace;
    ...
    pub fn process_file_changes(
        ...
        for change in changes {
            if let Some(file_id) = self.file_name_to_file_id(&change.file_name) {
                ...
                match Self::tokenize_from_str_live_design(
                    &change.content,
                    TextPos::default(),
                    file_id,
                    None,
                    ...
                    Ok(new_tokens) => {
                        ...
                        match LiveParser::parse_live_document(
                            &new_tokens,
                            &live_file.live_type_infos,
                            file_id,
                        ) {
                            Err(msg) => {
                                errors.push(msg);
                            ...
                            Ok(mut ld) => {
                                // only swap it out when it parses
                                any_changes = true;
                                ld.tokens = new_tokens;
                                self.build_imports_and_exports(
                                    &mut ld,
                                    module_id,
                                    &change.file_name,
                                );
    ...
    pub fn register_live_file(
        ...
        for live_type_info in &live_type_infos {
            ...
            self.live_type_infos
                .insert(live_type_info.live_type, live_type_info.clone());
        }
    ...
    fn doc_original_raw_imports_to_resolved_recur(
        &mut self,
        file_id: LiveFileId,
        errors: &mut Vec<LiveError>,
        dep_order: &mut Vec<LiveFileId>,
    ) {
        ...
        if dep_order.iter().find(|v| **v == file_id).is_some() {
            fn recur_dep_order(
                ...
                dep_order: &mut Vec<LiveFileId>,
                docs: &[LiveFile],
                recur_block: &mut Vec<LiveFileId>,
            ) {
                if recur_block.contains(&file_id) {
                    for recur in recur_block {
                        println!(
                            "Dependency recursion in: {:?}",
                            docs[recur.to_index()].file_name
                        );
                    }
                    return;
                }
                ...
                let pos = if let Some(pos) = dep_order.iter().position(|v| *v == file_id) {
                    ...
                    println!(
                        "Invalid state in dependency list for {}",
                        docs[file_id.to_index()].file_name
                    );
                    return;
                };
                    ...
                    if import.import_id == LiveId(0) {
                        ...
                        if let Some(file_id) = combined_exports.get(&import.import_id) {
                            ...
                            errors.push(LiveError {
                                ...
                                span: doc
                                    .original
                                    .token_id_to_span(origin.token_id().unwrap())
                                    .into(),
        ...
        self.live_files[file_id.to_index()]
            .original
            .resolved_imports = Some(ident_to_file.clone());
    ...
    fn build_imports_and_exports(
        &self,
        doc: &mut LiveOriginal,
        own_module_id: LiveModuleId,
        _name: &str,
    ) {
        ...
        while let Some(node_index) = node_iter {
            ...
            match &mut node.value {
                LiveValue::Import(live_import) => {
                    if live_import.module_id.0 == live_id!(crate) {
                        // patch up crate refs
    ...
    pub fn expand_all_documents(&mut self, errors: &mut Vec<LiveError>) {
        ...
        for file_id in dep_order.iter().rev() {
            ...
            std::mem::swap(
                &mut out_doc,
                &mut self.live_files[file_id.to_index()].expanded,
            );
            ...
            live_document_expander.expand(
                in_doc,
                &mut out_doc,
                self.live_files[file_id.to_index()].generation,
            );
