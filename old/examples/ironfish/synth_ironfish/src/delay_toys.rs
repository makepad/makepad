...
impl Default for DelayToy {
    fn default() -> Self {
        Self {
            buffer: {
                let mut v = Vec::new();
                v.resize(MAX_DELAYTOY_DELAY, 0.0);
                v
            },
            writeidx: 0,
            localidx: 0,
...
impl DelayToy {
    ...
    pub fn linear_interpolate(&mut self, index: usize, offset: f32) -> f32 {
        let adjustedindex =
            (index + MAX_DELAYTOY_DELAY - (offset.floor() as usize)) & DELAYTOY_BUFFERMASK;
        let frac = offset.fract();
        let ifrac = 1.0 - frac;
        return self.buffer[adjustedindex] * ifrac
            + self.buffer[(adjustedindex + 1) & DELAYTOY_BUFFERMASK] * frac;
    }
