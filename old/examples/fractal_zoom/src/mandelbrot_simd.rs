...
fn f32x4v(a: f32, b: f32, c: f32, d: f32) -> f32x4 {
    f32x4::from_array([a, b, c, d])
}
fn f32x4s(a: f32) -> f32x4 {
    f32x4::from_array([a; 4])
}
fn m32x4s(a: bool) -> Mask<i32, 4> {
    Mask::<i32, 4>::from_array([a; 4])
}
fn _u32x4v(a: u32, b: u32, c: u32, d: u32) -> u32x4 {
    u32x4::from_array([a, b, c, d])
}
fn u32x4s(a: u32) -> u32x4 {
    u32x4::from_array([a; 4])
}

fn f64x2v(a: f64, b: f64) -> f64x2 {
    f64x2::from_array([a, b])
}
fn f64x2s(a: f64) -> f64x2 {
    f64x2::from_array([a; 2])
}
fn m64x2s(a: bool) -> Mask<i64, 2> {
    Mask::<i64, 2>::from_array([a; 2])
}
fn u64x2s(a: u64) -> u64x2 {
    u64x2::from_array([a; 2])
}
fn _u64x2v(a: u64, b: u64) -> u64x2 {
...
pub fn mandelbrot_f32x4(tile: &mut Tile, max_iter: usize) {
    ...
    let fractal_pos = (
        f32x4s(tile.fractal.pos.x as f32),
        f32x4s(tile.fractal.pos.y as f32),
    );
    let fractal_size = (
        f32x4s(tile.fractal.size.x as f32),
        f32x4s(tile.fractal.size.y as f32),
    );
