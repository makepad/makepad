...
impl<'a> ShaderParser<'a> {
    ...
    fn expect_fn_decl(&mut self, prefix: Option<Ident>) -> Result<FnDecl, LiveError> {
        let span = self.begin_span();
        ...
        match self.peek_token() {
            Token::Ident(id) => {
                ...
                if let Some(ty_lit) = TyLit::from_id(id){
                    ...
                    Ok(span.end(self, | span | TyExpr {
                        ty: RefCell::new(None),
                        kind: TyExprKind::Var {span, ident:Ident(id)},
                    }))
                }
            }
    ...
    fn expect_prim_expr(&mut self) -> Result<Expr, LiveError> {
        ...
        match self.peek_token() {
            ...
            Token::Bool(_)  | Token::Int(_) | Token::Float(_)| Token::Color(_)=>{
                let token = self.peek_token();
                self.skip_token();
                Ok(span.end(self, | span | Expr {
                    ...
                    const_val: RefCell::new(None),
                    const_index: Cell::new(None),
                    kind: ExprKind::Lit {span, lit:Lit::from_token(token).unwrap()},
                }))                
            }
            Token::OpenParen => {
                self.skip_token();
