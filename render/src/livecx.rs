...
impl Cx {
    ...
    pub fn apply_error_wrong_value_type_for_primitive(&mut self, apply_from: ApplyFrom, index: usize, nodes: &[LiveNode], prim: &str) {
        self.apply_error(apply_from, index, nodes, format!("wrong value type. Prop: {} primitive: {} value: {:?}", nodes[index].id, prim, nodes[index].value))
    }

    pub fn apply_error_wrong_expression_type_for_primitive(&mut self, apply_from: ApplyFrom, index: usize, nodes: &[LiveNode], prim: &str, b:LiveEval) {
        self.apply_error(apply_from, index, nodes, format!("wrong expression return. Prop: {} primitive: {} value: {:?}", nodes[index].id, prim, b))
    }

    pub fn apply_error_wrong_value_in_expression(&mut self, apply_from: ApplyFrom, index: usize, nodes: &[LiveNode], ty: &str) {
        self.apply_error(apply_from, index, nodes, format!("wrong value in expression of type {} value: {:?}", ty, nodes[index].value))
    }
    
    pub fn apply_error_binop_undefined_in_expression(&mut self, apply_from: ApplyFrom, index: usize, nodes: &[LiveNode], op: LiveBinOp, a: LiveEval, b: LiveEval) {
        self.apply_error(apply_from, index, nodes, format!("Operation {:?} undefined between {:?} and {:?}", op, a, b))
    }
    
    pub fn apply_error_unop_undefined_in_expression(&mut self, apply_from: ApplyFrom, index: usize, nodes: &[LiveNode], op: LiveUnOp, a: LiveEval) {
        self.apply_error(apply_from, index, nodes, format!("Operation {:?} undefined for {:?}", op, a))
    }

    pub fn apply_error_expression_call_not_implemented(&mut self, apply_from: ApplyFrom, index: usize, nodes: &[LiveNode], ident:LiveId, args:usize) {
        self.apply_error(apply_from, index, nodes, format!("Expression call not implemented ident:{} with number of args: {}", ident, args))
    }
