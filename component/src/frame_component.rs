...
pub trait FrameComponent: LiveApply {
    ...
    fn query(&mut self, _query:&ChildQuery, _result:&mut dyn FnMut(ChildResult)){
    }
...
impl FrameRef {
    ...
    pub fn query(&mut self, query:&ChildQuery, result:&mut dyn FnMut(ChildResult)){
        if let Some(inner) = &mut self.0 {
            return inner.query(query, result)
        }
    }
