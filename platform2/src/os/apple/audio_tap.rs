...
use {
    ...
    crate::{
        ...
        audio::*,
        os::apple::apple_sys::*,
        os::apple::apple_classes::get_apple_class_global,
        makepad_objc_sys::objc_block,
        makepad_objc_sys::runtime::Sel,
        makepad_objc_sys::declare::ClassDecl,
    },
};
        ...
        unsafe {
            ...
            let completion_handler = objc_block!(move |content: ObjcId, error: ObjcId| {
                if error != nil {
                    ...
                    println!("ScreenCaptureKit: Failed to get shareable content: {}", desc_str);
                    return;
                }
                if content == nil {
                ...
                let filter: ObjcId = msg_send![filter, initWithDisplay: display excludingWindows: empty_array];
                ...
                let stream: ObjcId = msg_send![stream, initWithFilter: filter configuration: config delegate: nil];
                ...
                if !success || add_error != nil {
                    if add_error != nil {
                        ...
                        let desc_str = nsstring_to_string(desc);
                        println!("ScreenCaptureKit: Failed to add stream output: {}", desc_str);
                    } else {
...
unsafe fn handle_audio_sample(sample_buffer: ObjcId, audio_cb: &Arc<Mutex<Option<AudioInputFn>>>) {
    ...
    if is_non_interleaved {
        ...
        for ch in 0..channel_count.min(abl.mNumberBuffers as usize) {
            ...
            if is_float {
                let float_data = std::slice::from_raw_parts(buf.mData as *const f32, samples_to_copy);
        ...
        if abl.mNumberBuffers >= 1 {
            ...
            if !buf.mData.is_null() {
                ...
                if is_float {
                    let float_data = std::slice::from_raw_parts(buf.mData as *const f32, samples_to_use);
                    // De-interleave
                    for frame in 0..(samples_to_use / channel_count) {
                        for ch in 0..channel_count {
                            audio_buffer.channel_mut(ch)[frame] = float_data[frame * channel_count + ch];
                        }
                    }
                } else {
                    // 16-bit integer interleaved
                    let i16_data = std::slice::from_raw_parts(buf.mData as *const i16, samples_to_use);
                    for frame in 0..(samples_to_use / channel_count) {
                        for ch in 0..channel_count {
...
extern "C" {
    ...
    fn CMAudioFormatDescriptionGetStreamBasicDescription(desc: ObjcId) -> *const AudioStreamBasicDescription;
