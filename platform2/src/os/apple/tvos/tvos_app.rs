...
pub fn init_tvos_app_global(
    metal_device: ObjcId,
    event_callback: Box<dyn FnMut(TvosEvent) -> EventFlow>,
) {
    unsafe {
...
impl TvosApp {
    ...
    pub fn did_finish_launching_with_options(&mut self) {
        unsafe {
            ...
            let mtk_view_dlg_obj: ObjcId =
                msg_send![get_tvos_class_global().mtk_view_delegate, alloc];
    ...
    pub fn check_window_geom() {
        ...
        if get_tvos_app_global().first_draw {
            ...
            TvosApp::do_callback(TvosEvent::Init);
        }
        let old_geom = get_tvos_app_global().update_geom(new_geom.clone());
        if let Some(old_geom) = old_geom {
            TvosApp::do_callback(TvosEvent::WindowGeomChange(WindowGeomChangeEvent {
    ...
    pub fn send_timer_received(nstimer: ObjcId) {
        ...
        for i in 0..len {
            if get_tvos_app_global().timers[i].nstimer == nstimer {
                ...
                TvosApp::do_callback(TvosEvent::Timer(TimerEvent {
                    timer_id: timer_id,
                    time: Some(time),
                }));
            }
