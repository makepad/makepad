...
impl CxOpenXrSession {
    pub(crate) fn handle_anchor_events(
        &mut self,
        xr: &LibOpenXr,
        event_buffer: &XrEventDataBuffer,
        os_type: &OsType,
    ) {
        match event_buffer.ty {
            XrStructureType::EVENT_DATA_SPATIAL_ANCHOR_CREATE_COMPLETE_FB => {
                let response = &unsafe {
                    *(event_buffer as *const _ as *const XrEventDataSpatialAnchorCreateCompleteFB)
                };
                self.create_anchor_response(xr, response, os_type).ok();
            }
            XrStructureType::EVENT_DATA_SPACE_SET_STATUS_COMPLETE_FB => {
                let response = &unsafe {
                    *(event_buffer as *const _ as *const XrEventDataSpaceSetStatusCompleteFB)
                };
                if response.result != XrResult::SUCCESS {
                    crate::log!(
                        "EVENT_DATA_SPACE_SET_STATUS_COMPLETE_FB: {:?}",
                        response.result
                    );
                }
            }
            XrStructureType::EVENT_DATA_SHARE_SPACES_COMPLETE_META => {
                let response = &unsafe {
                    *(event_buffer as *const _ as *const XrEventDataShareSpacesCompleteMETA)
                };
            ...
            XrStructureType::EVENT_DATA_START_COLOCATION_DISCOVERY_COMPLETE_META => {
                let response = &unsafe {
                    *(event_buffer as *const _
                        as *const XrEventDataStartColocationDiscoveryCompleteMETA)
                };
                if response.result != XrResult::SUCCESS {
                    crate::log!(
                        "START_COLOCATION_DISCOVERY_COMPLETE_META: {:?}",
                        response.result
                    );
                }
            }
            XrStructureType::EVENT_DATA_COLOCATION_DISCOVERY_RESULT_META => {
                let _response = &unsafe {
                    *(event_buffer as *const _ as *const XrEventDataColocationDiscoveryResultMETA)
                };
            ...
            XrStructureType::EVENT_DATA_COLOCATION_DISCOVERY_COMPLETE_META => {
                let response = &unsafe {
                    *(event_buffer as *const _ as *const XrEventDataColocationDiscoveryCompleteMETA)
                };
                if response.result != XrResult::SUCCESS {
            ...
            XrStructureType::EVENT_DATA_STOP_COLOCATION_DISCOVERY_COMPLETE_META => {
                let response = &unsafe {
                    *(event_buffer as *const _
                        as *const XrEventDataStopColocationDiscoveryCompleteMETA)
                };
                if response.result != XrResult::SUCCESS {
                    crate::log!(
                        "STOP_COLOCATION_DISCOVERY_COMPLETE_META: {:?}",
                        response.result
                    );
                }
            }
            XrStructureType::EVENT_DATA_START_COLOCATION_ADVERTISEMENT_COMPLETE_META => {
                let _response = &unsafe {
                    *(event_buffer as *const _
                        as *const XrEventDataStartColocationAdvertisementCompleteMETA)
                };
                /*if response.result != XrResult::SUCCESS{
            ...
            XrStructureType::EVENT_DATA_COLOCATION_ADVERTISEMENT_COMPLETE_META => {
                let response = &unsafe {
                    *(event_buffer as *const _
                        as *const XrEventDataColocationAdvertisementCompleteMETA)
                };
                if response.result != XrResult::SUCCESS {
                    crate::log!(
                        "COLOCATION_ADVERTISEMENT_COMPLETE_META: {:?}",
                        response.result
                    );
                }
            }
            XrStructureType::EVENT_DATA_STOP_COLOCATION_ADVERTISEMENT_COMPLETE_META => {
                let response = &unsafe {
                    *(event_buffer as *const _
                        as *const XrEventDataStopColocationAdvertisementCompleteMETA)
                };
                if response.result != XrResult::SUCCESS {
                    crate::log!(
                        "STOP_COLOCATION_ADVERTISEMENT_COMPLETE_META: {:?}",
                        response.result
                    );
                }
            }
            XrStructureType::EVENT_DATA_SPACE_QUERY_RESULTS_AVAILABLE_FB => {
                let response = &unsafe {
                    *(event_buffer as *const _ as *const XrEventDataSpaceQueryResultsAvailableFB)
                };
            ...
            XrStructureType::EVENT_DATA_SPACES_SAVE_RESULT_META => {
                let response = &unsafe {
                    *(event_buffer as *const _ as *const XrEventDataSpacesSaveResultMETA)
                };
            ...
            XrStructureType::EVENT_DATA_SPACES_ERASE_RESULT_META => {
                let response = &unsafe {
                    *(event_buffer as *const _ as *const XrEventDataSpacesEraseResultMETA)
                };
                if response.result != XrResult::SUCCESS {
    ...
    pub fn set_local_anchor(&mut self, xr: &LibOpenXr, anchor: XrAnchor) {
        ...
        if !self.anchor.async_state.idle() {
            return crate::log!(
                "set_local_anchor - async state not idle {:?}",
    ...
    fn create_anchor_request(
        &self,
        xr: &LibOpenXr,
        pose: Pose,
    ) -> Result<XrAsyncRequestIdFB, String> {
        ...
        unsafe { (xr.xrCreateSpatialAnchorFB)(self.handle, &anchor_create_info, &mut request_id) }
            .to_result("xrCreateSpatialAnchorFB")?;
        return Ok(request_id);
    }

    fn create_anchor_response(
        &mut self,
        xr: &LibOpenXr,
        response: &XrEventDataSpatialAnchorCreateCompleteFB,
        os_type: &OsType,
    ) -> Result<(), String> {
        match self.anchor.async_state {
            AsyncAnchorState::LocalAnchorLeft {
                ...
                if let Ok(request_id) = self.create_anchor_request(xr, right_pose) {
                    ...
                    self.anchor
                        .async_state
                        .error(true, "create_anchor_request")?;
                }
            }
            AsyncAnchorState::LocalAnchorRight {
                ...
                if let Err(e) = self.set_space_status_request(
                    ...
                    self.anchor
                        .async_state
                        .error(true, &format!("set_space_status_request - {:?}", e))?;
                } else {
                    //let group_uuid = XrUuid::from_live_id(live_id!(makepad_space));
                    ...
                    match self.save_spaces_request(xr, [left_space, right_space]) {
                        ...
                        Err(e) => {
                            self.anchor
                                .async_state
                                .error(true, &format!("set_space_status_response {e}"))
                                .ok();
                        }
                    }
                }
            ...
            _ => {
                self.anchor
                    .async_state
                    .error(true, "create_anchor_response - unexpected")?;
            }
        }
        Ok(())
    }

    fn set_space_status_request(
        &mut self,
        xr: &LibOpenXr,
        spaces: &[XrSpace],
        flags: &[XrSpaceComponentTypeFB],
    ) -> Result<(), String> {
        for space in spaces {
            let components = xr_array_fetch(XrSpaceComponentTypeFB::default(), |cap, len, buf| {
                unsafe { (xr.xrEnumerateSpaceSupportedComponentsFB)(*space, cap, len, buf) }
            ...
            for flag in flags {
                ...
                let mut request_id = XrAsyncRequestIdFB(0);
                unsafe { (xr.xrSetSpaceComponentStatusFB)(*space, &request, &mut request_id) }
    ...
    fn save_spaces_request(
        ...
        unsafe { (xr.xrSaveSpacesMETA)(self.handle, &spaces_info, &mut request) }
            .to_result("xrSaveSpacesMETA")?;
        Ok(request)
    }

    fn save_spaces_response(
        &mut self,
        _xr: &LibOpenXr,
        response: &XrEventDataSpacesSaveResultMETA,
    ) -> Result<(), String> {
        match self.anchor.async_state {
            AsyncAnchorState::LocalAnchorSaveSpaces { request_id } => {
                self.anchor.async_state.result(response.result)?;
            ...
            _ => {
                self.anchor
                    .async_state
                    .error(true, "share_spaces_response - unexpected")?;
            }
        }
        Ok(())
    }

    fn _share_spaces_request(
        &self,
        xr: &LibOpenXr,
        group_uuid: XrUuid,
        spaces: [XrSpace; 2],
        ...
        let mut request = XrAsyncRequestIdFB(0);
        unsafe { (xr.xrShareSpacesMETA)(self.handle, &spaces_info, &mut request) }
            .to_result("xrShareSpacesMETA")?;
        Ok(request)
    }

    fn share_spaces_response(
        &mut self,
        _xr: &LibOpenXr,
        response: &XrEventDataShareSpacesCompleteMETA,
    ) -> Result<(), String> {
        match self.anchor.async_state {
    ...
    fn _start_colocation_advertisement_request(
        &mut self,
        xr: &LibOpenXr,
        ...
        let mut request = XrAsyncRequestIdFB(0);
        unsafe {
            (xr.xrStartColocationAdvertisementMETA)(self.handle, &advertisement_info, &mut request)
        }
    ...
    fn _stop_colocation_advertisement_request(&mut self, xr: &LibOpenXr) {
        let mut request = XrAsyncRequestIdFB(0);
        unsafe { (xr.xrStopColocationAdvertisementMETA)(self.handle, 0 as *const _, &mut request) }
            .log_error("xrStopColocationAdvertisementMETA");
    }

    fn _start_colocation_discovery_request(&mut self, xr: &LibOpenXr) {
        let mut request = XrAsyncRequestIdFB(0);
        unsafe { (xr.xrStartColocationDiscoveryMETA)(self.handle, 0 as *const _, &mut request) }
            .log_error("xrStartColocationDiscoveryMETA");
    }

    fn _start_colocation_discovery_result(
        &mut self,
        _xr: &LibOpenXr,
        response: &XrEventDataColocationDiscoveryResultMETA,
    ) {
    ...
    fn _stop_colocation_discovery_request(&mut self, xr: &LibOpenXr) {
        let mut request = XrAsyncRequestIdFB(0);
        unsafe { (xr.xrStopColocationDiscoveryMETA)(self.handle, 0 as *const _, &mut request) }
            .log_error("xrStopColocationDiscoveryMETA");
    }

    fn query_local_spaces_request(
        &mut self,
        xr: &LibOpenXr,
        uuids: [XrUuid; 2],
    ) -> Result<XrAsyncRequestIdFB, String> {
    ...
    fn _query_cloud_spaces_request(
        &mut self,
        xr: &LibOpenXr,
        group_uuid: XrUuid,
    ) -> Result<XrAsyncRequestIdFB, String> {
    ...
    fn erase_spaces_request(
        ...
        let mut request = XrAsyncRequestIdFB(0);
        unsafe { (xr.xrEraseSpacesMETA)(self.handle, &info as *const _ as *const _, &mut request) }
            .to_result("xrEraseSpacesMETA")?;
        Ok(request)
    }

    fn query_spaces_response(
        &mut self,
        xr: &LibOpenXr,
        response: &XrEventDataSpaceQueryResultsAvailableFB,
    ) -> Result<(), String> {
        let async_state = self.anchor.async_state.clone();
        match self.anchor.async_state {
            AsyncAnchorState::ClearAndSet { .. } | AsyncAnchorState::GetLocalAnchor { .. } => {
                self.anchor.async_state = AsyncAnchorState::Idle;
            }
            _ => {
                self.anchor
                    .async_state
                    .error(true, "query_anchors_response - unexpected")?;
            }
        }
        let mut query = XrSpaceQueryResultsFB::default();
        unsafe { (xr.xrRetrieveSpaceQueryResultsFB)(self.handle, response.request_id, &mut query) }
            .to_result("xrRetrieveSpaceQueryResultsFB")?;

        let mut results_vec = Vec::new();
        results_vec.resize(
            query.result_count_output as usize,
            XrSpaceQueryResultFB::default(),
        );
        ...
        match &async_state {
            ...
            AsyncAnchorState::GetLocalAnchor {
                ...
                if results_vec.len() != 2 {
                    crate::log!(
                        "query_anchors_response unexpected result count {}",
                        results_vec.len()
                    );
                    return Err("query_anchors_response not zero or 2".to_string());
                }
                let left_index = results_vec
                    .iter()
                    .position(|v| v.uuid == *left_uuid)
                    .unwrap_or(0);
                let right_index = results_vec
                    .iter()
                    .position(|v| v.uuid == *right_uuid)
                    .unwrap_or(0);
...
impl CxOpenXrAnchor {
    pub fn locate_anchor(
        ...
        if let Some(anchor) = &self.anchor {
            let left =
                XrSpaceLocation::locate(xr, local_space, predicted_display_time, anchor.left_space)
                    .pose
                    .position;
            let right = XrSpaceLocation::locate(
                xr,
                local_space,
...
impl AsyncAnchorState {
    ...
    fn idle(&self) -> bool {
        if let AsyncAnchorState::Idle = self {
            true
        } else {
            false
        }
