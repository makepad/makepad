...
impl CxOs {
    pub(crate) fn web_audio(&mut self) -> Arc<Mutex<WebAudioAccess>> {
        if self.media.web_audio.is_none() {
            self.media.web_audio = Some(WebAudioAccess::new(
                self,
                self.media.web_audio_change.clone(),
            ));
        }
    ...
    pub(crate) fn web_midi(&mut self) -> Arc<Mutex<WebMidiAccess>> {
        if self.media.web_midi.is_none() {
            self.media.web_midi =
                Some(WebMidiAccess::new(self, self.media.web_midi_change.clone()));
        }
...
impl CxMediaApi for Cx {
    ...
    fn use_midi_inputs(&mut self, ports: &[MidiPortId]) {
        self.os
            ...
            .lock()
            .unwrap()
            .use_midi_inputs(&mut self.os, ports);
    }
    ...
    fn use_midi_outputs(&mut self, ports: &[MidiPortId]) {
        self.os
            ...
            .lock()
            .unwrap()
            .use_midi_outputs(&mut self.os, ports);
    }
