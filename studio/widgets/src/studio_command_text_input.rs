...
script_mod! {
    ...
    mod.widgets.StudioCommandTextInputListBase = #(List::register_widget(vm))

    mod.widgets.StudioCommandTextInputList = mod.widgets.StudioCommandTextInputListBase{
        flow: Down
        width: Fill
        height: Fill
    }

    mod.widgets.StudioCommandTextInputBase = #(StudioCommandTextInput::register_widget(vm))

    mod.widgets.StudioCommandTextInput = mod.widgets.StudioCommandTextInputBase{
        flow: Down
        height: Fit
        ...
        popup := RoundedView{
            ...
            list := mod.widgets.StudioCommandTextInputList{
                height: Fit
            }
        }
...
pub struct StudioCommandTextInput {
    #[source]
    ...
    /// Values associated with `selectable_widgets` at matching indexes.
    #[rust]
    selectable_values: Vec<String>,

    /// To deal with widgets not being `Send`.
    #[rust]
    last_selected_widget: WidgetRef,

    /// Value associated with the last selected widget, if provided by caller.
    #[rust]
    last_selected_value: Option<String>,
...
impl Widget for StudioCommandTextInput {
...
impl StudioCommandTextInput {
    ...
    fn select_item(&mut self, cx: &mut Cx, _scope: &mut Scope, selected: WidgetRef) {
        ...
        self.last_selected_value = self
            .selectable_widgets
            .iter()
            .position(|item| item.widget_uid() == selected.widget_uid())
            .and_then(|idx| self.selectable_values.get(idx).cloned());
    ...
    pub fn add_item(&mut self, cx: &mut Cx, widget: WidgetRef) {
        self.add_item_with_value(cx, widget, String::new());
    }

    /// Add a custom selectable item to the list and associate it with a value.
    ///
    /// The value can be retrieved via `item_selected_value`.
    pub fn add_item_with_value(&mut self, cx: &mut Cx, widget: WidgetRef, value: String) {
        self.list(cx, ids!(list)).add(widget.clone());
        self.selectable_widgets.push(widget);
        self.selectable_values.push(value);
    ...
    ///
    /// Normally called after clearing the previous items.
    pub fn add_unselectable_item(&mut self, cx: &mut Cx, widget: WidgetRef) {
    ...
    /// Checks if any item has been selected in the given `actions`
    /// and returns the value associated with that item.
    pub fn item_selected_value(&self, actions: &Actions) -> Option<String> {
        actions
            .iter()
            .filter_map(|a| a.as_widget_action())
            .filter(|a| a.widget_uid == self.widget_uid())
            .find_map(|a| {
                if let InternalAction::ItemSelected = a.cast() {
                    self.last_selected_value.clone()
                } else {
                    None
                }
            })
...
impl StudioCommandTextInputRef {
    /// Sets the keyboard trigger that opens the popup.
    pub fn set_trigger(&self, trigger: Option<String>) {
        if let Some(mut inner) = self.borrow_mut() {
            inner.trigger = trigger;
        }
    }

    /// See [`StudioCommandTextInput::should_build_items()`].
    pub fn should_build_items(&self, actions: &Actions) -> bool {
        self.borrow()
            .map_or(false, |inner| inner.should_build_items(actions))
    }

    /// See [`StudioCommandTextInput::clear_items()`].
    pub fn clear_items(&mut self, cx: &mut Cx) {
        if let Some(mut inner) = self.borrow_mut() {
    ...
    /// See [`StudioCommandTextInput::add_item()`].
    ...
    /// See [`StudioCommandTextInput::add_item_with_value()`].
    pub fn add_item_with_value(&self, cx: &mut Cx, widget: WidgetRef, value: String) {
        if let Some(mut inner) = self.borrow_mut() {
            inner.add_item_with_value(cx, widget, value);
        }
    }

    /// See [`StudioCommandTextInput::add_unselectable_item()`].
    pub fn add_unselectable_item(&self, cx: &mut Cx, widget: WidgetRef) {
        if let Some(mut inner) = self.borrow_mut() {
            inner.add_unselectable_item(cx, widget);
        }
    }

    /// See [`StudioCommandTextInput::item_selected()`].
    pub fn item_selected(&self, actions: &Actions) -> Option<WidgetRef> {
        self.borrow().and_then(|inner| inner.item_selected(actions))
    }

    /// See [`StudioCommandTextInput::item_selected_value()`].
    pub fn item_selected_value(&self, actions: &Actions) -> Option<String> {
        self.borrow()
            .and_then(|inner| inner.item_selected_value(actions))
    }

    /// See [`StudioCommandTextInput::text_input_ref()`].
    pub fn text_input_ref(&self, cx: &Cx) -> TextInputRef {
        self.borrow()
    ...
    /// See [`StudioCommandTextInput::search_input_ref()`].
    pub fn search_input_ref(&self, cx: &Cx) -> TextInputRef {
        self.borrow()
            .map_or(WidgetRef::empty().as_text_input(), |inner| {
    ...
    /// See [`StudioCommandTextInput::reset()`].
    ...
    /// See [`StudioCommandTextInput::request_text_input_focus()`].
    ...
    /// See [`StudioCommandTextInput::search_text()`].
    pub fn search_text(&self, cx: &Cx) -> String {
        self.borrow()
