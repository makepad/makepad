...
script_mod! {
    ...
    let StudioFlowBlockType = set_type_default() do #(StudioFlowBlockType::script_api(vm))

    mod.widgets.DrawStudioFlowBlock = set_type_default() do #(DrawStudioFlowBlock::script_shader(vm)){
        ..mod.draw.DrawQuad

        block_type: instance(StudioFlowBlockType.Quote)
        line_color: #fff
        sep_color: #888
        code_color: #333
        quote_bg_color: #222
        ...
        pixel: fn() {
            ...
            match self.block_type {
                StudioFlowBlockType.Quote => {
                    sdf.box(0. 0. self.rect_size.x self.rect_size.y 6.)
                    sdf.fill_keep(self.quote_bg_color)
    ...
    mod.widgets.StudioFlowBlockType = StudioFlowBlockType

    mod.widgets.StudioTextFlowBase = #(StudioTextFlow::register_widget(vm)){
        font_size: 8
    ...
    mod.widgets.StudioTextFlowLinkBase = #(StudioTextFlowLink::register_widget(vm)){}

    mod.widgets.StudioTextFlowLink = set_type_default() do mod.widgets.StudioTextFlowLinkBase{
        color: #xa
        color_hover: #xf
        color_down: #x3
    ...
    mod.widgets.StudioTextFlow = set_type_default() do mod.widgets.StudioTextFlowBase{
        ...
        link := mod.widgets.StudioTextFlowLink{}
...
#[derive(Script, ScriptHook)]
#[repr(C)]
pub struct DrawStudioFlowBlock {
    #[deref]
    draw_super: DrawQuad,
    #[live]
    ...
    #[live]
    pub block_type: StudioFlowBlockType,
}
...
/// A segment in the StudioTextFlow selection stream
pub enum SelectionSegment {
    ...
    /// The widget draws its own selection highlights; StudioTextFlow skips it in draw_selection.
    WidgetText {
...
pub struct StudioTextFlow {
    ...
    #[live]
    pub draw_block: DrawStudioFlowBlock,
...
impl ScriptHook for StudioTextFlow {
    fn on_after_apply(
        &mut self,
        vm: &mut ScriptVm,
        apply: &Apply,
...
impl WidgetNode for StudioTextFlow {
...
impl Widget for StudioTextFlow {
...
impl StudioTextFlow {
    ...
    pub fn is_animation_idle(&self) -> bool {
        ...
        // StudioTextFlow would be treated as "mid-animation" because 0.0 < 0.0 + fade_chars,
    ...
    /// Select all text in this StudioTextFlow
    ...
    /// Set selection range (for external control, e.g., cross-StudioTextFlow selection).
    ...
    pub fn begin_code(&mut self, cx: &mut Cx2d) {
        self.draw_block.block_type = StudioFlowBlockType::Code;
    ...
    pub fn begin_list_item(&mut self, cx: &mut Cx2d, dot: &str, pad: f64) {
        ...
        StudioTextFlow::walk_margin(cx, self.list_item_marker_pad);
    ...
    pub fn sep(&mut self, cx: &mut Cx2d) {
        self.draw_block.block_type = StudioFlowBlockType::Sep;
        self.draw_block.draw_walk(cx, self.sep_walk);
    }

    pub fn begin_quote(&mut self, cx: &mut Cx2d) {
        self.draw_block.block_type = StudioFlowBlockType::Quote;
        self.draw_block
    ...
    fn draw_row_cell_borders(&mut self, cx: &mut Cx2d, row_rect: Rect) {
        ...
        self.draw_block.block_type = StudioFlowBlockType::TableCell;
    ...
    where
        F: FnOnce(&mut Cx2d, &WidgetRef, &mut StudioTextFlow) -> R,
    {
    ...
    pub fn draw_text(&mut self, cx: &mut Cx2d, text: &str) {
        if let Some(DrawState::Drawing) = self.draw_state.get() {
            ...
            let (run_rows, run_truncated) = if self.inline_code.value() > 0 {
                ...
                db.block_type = StudioFlowBlockType::InlineCode;
                if !self.first_thing_on_a_line {
                    let rect = StudioTextFlow::walk_margin(cx, self.inline_code_margin.left);
                ...
                let pad_r_rect = StudioTextFlow::walk_margin(cx, pad_r);
                ...
                let rect = StudioTextFlow::walk_margin(cx, self.inline_code_margin.right);
                areas_tracker.track_rect(cx, rect);
                result
            } else if self.strikethrough.value() > 0 {
                let db = &mut self.draw_block;
                db.line_color = *font_color;
                db.block_type = StudioFlowBlockType::Strikethrough;
                ...
                let db = &mut self.draw_block;
                db.line_color = *font_color;
                db.block_type = StudioFlowBlockType::Underline;
                dt.draw_walk_resumable_with(cx, text, |cx, rect, _| {
                    db.draw_abs(cx, rect);
                    areas_tracker.track_rect(cx, rect);
                })
...
/// Actions emitted by StudioTextFlow for cross-boundary selection in PortalList
#[derive(Debug, Clone, Default)]
pub enum StudioTextFlowAction {
    #[default]
    None,
}

#[derive(Debug, Clone, Default)]
pub enum StudioTextFlowLinkAction {
    Clicked {
...
impl WidgetNode for StudioTextFlowLink {
    fn widget_uid(&self) -> WidgetUid {
        self.uid
    }
    fn walk(&mut self, _cx: &mut Cx) -> Walk {
...
impl Widget for StudioTextFlowLink {
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        if self.animator_handle_event(cx, event).must_redraw() {
            if let Some(tf) = scope.data.get_mut::<StudioTextFlow>() {
                tf.redraw(cx);
            } else {
                ...
                Hit::FingerDown(fe) if fe.is_primary_hit() => {
                    ...
                    if self.click_on_down {
                        cx.widget_action_with_data(
                            &self.action_data,
                            self.widget_uid(),
                            StudioTextFlowLinkAction::Clicked {
    ...
    fn draw_walk(&mut self, cx: &mut Cx2d, scope: &mut Scope, _walk: Walk) -> DrawStep {
        let Some(tf) = scope.data.get_mut::<StudioTextFlow>() else {
        ...
        StudioTextFlow::walk_margin(cx, self.margin.left);
        tf.draw_text(cx, self.text.as_ref());
        StudioTextFlow::walk_margin(cx, self.margin.right);
