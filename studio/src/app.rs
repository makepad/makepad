use crate::{
    ...
    makepad_code_editor::{text::Position, CodeSession},
...
impl App {
            ...
            match AppStateRon::deserialize_ron(&contents) {
                Ok(state) => {
                    ...
                    if let Some(mut dock) = dock.borrow_mut() {
                        ...
                        self.data.file_system.tab_id_to_file_node_id =
                            state.tab_id_to_file_node_id.clone();
                        for (tab_id, file_node_id) in state.tab_id_to_file_node_id.iter() {
                            self.data
                                .file_system
                                .request_open_file(*tab_id, *file_node_id);
                        }
    ...
    fn save_state(&self, cx: &Cx, slot: usize) {
        ...
        let mut f =
            File::create(format!("makepad_state{}.ron", slot)).expect("Unable to create file");
...
impl MatchEvent for App {
    ...
    fn handle_action(&mut self, cx: &mut Cx, action: &Action) {
        let dock = self.ui.dock(cx, ids!(dock));
        let file_tree = self.ui.studio_file_tree(cx, ids!(file_tree));
        let log_list = self.ui.log_list(cx, ids!(log_list));
        let run_list = self.ui.view(cx, ids!(run_list_tab));
        let profiler = self.ui.view(cx, ids!(profiler));
        let search = self.ui.view(cx, ids!(search));
        let snapshot = self.ui.snapshot(cx, ids!(snapshot_tab));

        match action.cast() {
            AppAction::SwapSelection(ss) => {
                let s1_start = Position {
                    line_index: ss.s1_line_start as usize,
                    byte_index: ss.s1_column_start as usize,
                };
                let s1_end = Position {
                    line_index: ss.s1_line_end as usize,
                    byte_index: ss.s1_column_end as usize,
                };
                let s2_start = Position {
                    line_index: ss.s2_line_start as usize,
                    byte_index: ss.s2_column_start as usize,
                };
                let s2_end = Position {
                    line_index: ss.s2_line_end as usize,
                    byte_index: ss.s2_column_end as usize,
                };
                if let Some(s1_file_id) =
                    self.data.file_system.path_to_file_node_id(&ss.s1_file_name)
                {
                    if let Some(s2_file_id) =
                        self.data.file_system.path_to_file_node_id(&ss.s1_file_name)
                    {
                        if let Some(OpenDocument::Code(doc1)) =
                            self.data.file_system.open_documents.get(&s1_file_id)
                        {
                            if let Some(OpenDocument::Code(doc2)) =
                                self.data.file_system.open_documents.get(&s2_file_id)
                            {
                                // Create sessions
                                ...
                                session1.set_selection(
                                    session1.clamp_position(s1_start),
                                    Affinity::After,
                                    SelectionMode::Simple,
                                    NewGroup::Yes,
                                );
                                session1.move_to(
                                    session1.clamp_position(s1_end),
                                    Affinity::Before,
                                    NewGroup::Yes,
                                );

                                session2.set_selection(
                                    session2.clamp_position(s2_start),
                                    Affinity::After,
                                    SelectionMode::Simple,
                                    NewGroup::Yes,
                                );
                                session2.move_to(
                                    session2.clamp_position(s2_end),
                                    Affinity::Before,
                                    NewGroup::Yes,
                                );
                                ...
                                self.data
                                    .file_system
                                    .request_save_file_for_file_node_id(s1_file_id, false);
            ...
            AppAction::SelectInFile(sf) => {
                let start = Position {
                    line_index: sf.line_start as usize,
                    byte_index: sf.column_start as usize,
                };
                let end = Position {
                    line_index: sf.line_end as usize,
                    byte_index: sf.column_end as usize,
                };
                if let Some(file_id) = self.data.file_system.path_to_file_node_id(&sf.file_name) {
                    if let Some(tab_id) = self.data.file_system.file_node_id_to_tab_id(file_id) {
                        ...
                        if let Some(mut editor) = dock
                            .item(tab_id)
                            .studio_code_editor(cx, ids!(editor))
                            .borrow_mut()
                        {
                            if let Some(EditSession::Code(session)) =
                                self.data.file_system.get_session_mut(tab_id)
                            {
                                editor
                                    .editor
                                    .set_selection_and_scroll(cx, start, end, session);
            ...
            AppAction::JumpTo(jt) => {
                let pos = Position {
                    line_index: jt.line as usize,
                    byte_index: jt.column as usize,
                };
            ...
            AppAction::PatchFile(_ef) => {
                ...
                if let Some(file_id) = self.data.file_system.path_to_file_node_id(&ef.file_name) {
                    if let Some(tab_id) = self.data.file_system.file_node_id_to_tab_id(file_id){
                        //dock.select_tab(cx, tab_id);
                        // ok lets scroll into view
                        if let Some(mut editor) = dock.item(tab_id).studio_code_editor(cx, ids!(editor)).borrow_mut() {
            ...
            AppAction::EditFile(ef) => {
                let start = Position {
                    line_index: ef.line_start as usize,
                    byte_index: ef.column_start as usize,
                };
                let end = Position {
                    line_index: ef.line_end as usize,
                    byte_index: ef.column_end as usize,
                };
                if let Some(file_id) = self.data.file_system.path_to_file_node_id(&ef.file_name) {
                    if let Some(tab_id) = self.data.file_system.file_node_id_to_tab_id(file_id) {
                        dock.select_tab(cx, tab_id);
                            ...
                            .borrow_mut()
                        {
                            ...
                            self.data
                                .file_system
                                .request_save_file_for_file_node_id(file_id, false)
                        }
                    }
                }
            }
            AppAction::RedrawFile(file_id) => {
                self.data
                    .file_system
                    .redraw_view_by_file_id(cx, file_id, &dock);
            }
            AppAction::ClearLog => {
                self.data
                    .build_manager
            ...
            AppAction::None => (),
            AppAction::SendAiChatToBackend {
                chat_id,
                history_slot,
            } => {
                self.data.ai_chat_manager.send_chat_to_backend(
                    cx,
                    chat_id,
                    history_slot,
                    &mut self.data.file_system,
                );
            }
            AppAction::CancelAiGeneration { chat_id } => {
                self.data.ai_chat_manager.cancel_chat_generation(
                    cx,
                    &self.ui,
                    chat_id,
                    &mut self.data.file_system,
            ...
            AppAction::SaveAiChat { chat_id } => {
                self.data
                    .file_system
                    .request_save_file_for_file_node_id(chat_id, false);
            }
            AppAction::RedrawAiChat { chat_id } => {
                self.data.ai_chat_manager.redraw_ai_chat_by_id(
                    cx,
                    chat_id,
                    &self.ui,
                    &mut self.data.file_system,
            ...
            AppAction::RunAiChat {
                ...
                self.data.ai_chat_manager.run_ai_chat(
                    cx,
                    chat_id,
                    history_slot,
                    item_id,
                    ...
                    StdinToHost::DrawCompleteAndFlip(presentable_draw) => {
                        if let Some(mut dock) = dock.borrow_mut() {
                            for (_, (_, item)) in dock.items().iter() {
                                if let Some(mut run_view) = item.as_run_view().borrow_mut() {
                                    if run_view.build_id == Some(build_id)
                                        && run_view.window_id == presentable_draw.window_id
                                    {
                                        run_view.draw_complete_and_flip(
                                            cx,
                                            &presentable_draw,
                                            &mut self.data.build_manager,
                                        );
                                    }
                                }
                            }
        ...
        if let Some(action) = action.as_widget_action() {
            match action.cast() {
                CodeEditorAction::UnhandledKeyDown(ke)
                    if ke.key_code == KeyCode::F12 && !ke.modifiers.shift =>
                {
                    ...
                    if let Some(word) = self
                        .data
                        .file_system
                        .get_word_under_cursor_for_session(tab_id)
                    {
                ...
                CodeEditorAction::TextDidChange => {
                    ...
                    self.data
                        .file_system
                        .request_save_file_for_tab_id(tab_id, false)
                }
                ...
                DockAction::Drop(drop_event) => {
                    if let DragItem::FilePath { path, internal_id } = &drop_event.items[0] {
                        if let Some(internal_id) = internal_id {
                            ...
                            // external file, we have to create a new tab
                            if let Some(file_id) = self.data.file_system.path_to_file_node_id(&path)
                            {
                                let tab_id = dock.unique_id(file_id.0);
                                self.data.file_system.request_open_file(tab_id, file_id);
    ...
    fn handle_actions(&mut self, cx: &mut Cx, actions: &Actions) {
        ...
        let file_tree_filter = self.ui.text_input(cx, ids!(file_tree_filter));
        let file_tree_view = self.ui.file_tree_view(cx, ids!(file_tree_view));
        ...
        if let Some(file_id) = file_tree_view.filter_file_clicked(cx, &actions) {
        ...
        if self.ui.button(cx, ids!(stop_all)).clicked(&actions) {
            self.data.build_manager.stop_all_active_builds(cx);
            // Redraw the run list to update the play/pause button states
            self.ui.view(cx, ids!(run_list)).redraw(cx);
        }

        // Handle Tail checkbox for log list
        let log_list = self.ui.log_list(cx, ids!(log_list));
        let tail_checkbox = self.ui.check_box(cx, ids!(tail_checkbox));
        ...
        if log_list.scrolled(cx, &actions) && !log_list.is_at_end(cx) {
            tail_checkbox.set_active(cx, false);
        }

        // Handle log filter
        let log_filter = self.ui.text_input(cx, ids!(log_filter));
        if let Some(filter) = log_filter.changed(&actions) {
            log_list.set_filter(cx, filter, &self.data.build_manager.log);
        }
        if self.ui.button(cx, ids!(clear_filter)).clicked(&actions) {
        ...
        .enumerate()
        {
            if let Some(km) = self.ui.button(cx, id).pressed_modifiers(actions) {
                if km.control {
                    self.save_state(cx, i + 1)
                } else {
