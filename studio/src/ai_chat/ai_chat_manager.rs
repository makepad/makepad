...
impl AiChatFile {
    pub fn new() -> Self {
        Self {
            history: vec![AiChatMessages::new()],
        }
    }
    ...
    pub fn fork_chat_at(
        &mut self,
        _cx: &mut Cx,
        history_slot: &mut usize,
        at: usize,
        data: String,
    ) {
        // alriught so first we clamp the history slot
        self.clamp_slot(history_slot);
        if at + 1 != self.history[*history_slot].messages.len() {
            // fork it first
...
impl AiChatManager {
                        ...
                        match &e.response {
                                                        ...
                                                        Ok(chat_response) => {
                                                                ...
                                                                .content
                                                            {
                                                                if let Some(msg) = doc
                                                                    .file
                                                                    .history
                                                                    .get_mut(in_flight.history_slot)
                                                                {
                                                                    if let Some(
                                                                        AiChatMessage::Assistant(s),
                                                                    ) = msg.messages.last_mut()
                                                                    {
                                                                        s.push_str(&content);
                                                                    } else {
                            ...
                            NetworkResponse::HttpStreamComplete(_res) => {
                                ...
                                if let Some(OpenDocument::AiChat(doc)) =
                                    fs.open_documents.get_mut(&chat_id)
                                {
                                    if let Some(in_flight) = doc.in_flight.take() {
                                        ...
                                        if doc.auto_run {
                                            let item_id = doc.file.history[in_flight.history_slot]
                                                .messages
                                                .len()
                                                .saturating_sub(3);
                                            ...
                                            cx.action(AppAction::RunAiChat {
                                                chat_id,
                                                history_slot: in_flight.history_slot,
                                                item_id,
                                            });
                                        }
                        ...
                        if let Some(new_data) = ast.strip_prefix("```rust") {
                            if let Some(new_data) = new_data.strip_suffix("```") {
                                ...
                                if let Some(AiChatMessage::User(usr)) = usr.cloned() {
                                    cx.action(AppAction::SetSnapshotMessage {
                                        message: usr.message.clone(),
                                    });
                                }
                                if let Some(ctx) = self
                                    .contexts
                                    .iter()
                                    .find(|v| v.name == messages.base_context)
                                {
                                    match ctx.apply {
                                        AiApply::PatchDSL => {
                                            fs.replace_live_design(cx, file_id, &new_data);
    ...
    pub fn send_chat_to_backend(
        ...
        // build the request
        let (request, backend) = if let Some(OpenDocument::AiChat(doc)) =
            fs.open_documents.get(&chat_id)
        {
            // alright lets fetch which backend we want
            let ai_model = if let Some(backend) = self
                .models
                .iter()
                .find(|v| v.name == doc.file.history[history_slot].model)
            {
                backend
            } else {
            ...
            match &ai_model.backend {
                ...
                AiBackend::Google { url, key } => {
                    let mut request =
                        HttpRequest::new(format!("{}{}", url.clone(), key), HttpMethod::POST);
                    request.set_is_streaming();
                    request.set_header("Content-Type".to_string(), "application/json".to_string());
                    request.set_metadata_id(chat_id);
                    ...
                    let contents = GoogleAiChatPrompt { contents };
        ...
        if let Some(OpenDocument::AiChat(doc)) = fs.open_documents.get_mut(&chat_id) {
            ...
            doc.file.history[history_slot]
                .messages
                .push(AiChatMessage::Assistant("".to_string()));
