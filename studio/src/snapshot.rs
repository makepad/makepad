...
impl Snapshot {
    fn draw_snapshots(
        ...
        while let Some(item_id) = list.next_visible_item(cx) {
            let item = if let Some(commit) = git_log.commits.get(item_id) {
                ...
                item.label(cx, ids!(message)).set_text(cx, &commit.message);
    ...
    fn load_snapshot(&mut self, cx: &mut Cx, data: &mut AppData, item_id: usize) {
        let root_id = self.drop_down(cx, ids!(roots_dropdown)).selected_item();
    ...
    fn make_snapshot(&mut self, cx: &mut Cx, data: &mut AppData) {
        let root_id = self.drop_down(cx, ids!(roots_dropdown)).selected_item();
        let git_log = data.file_system.git_logs.get(root_id).unwrap();
        ...
        if let Some(item) = iter.next() {
            ...
            let message = self.view(cx, ids!(message_input)).text();
            if message.len() == 0 {
                return;
            }
            data.file_system
...
impl Widget for Snapshot {
    fn draw_walk(&mut self, cx: &mut Cx2d, scope: &mut Scope, walk: Walk) -> DrawStep {
        ...
        let dd = self.drop_down(cx, ids!(roots_dropdown));
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        let snapshots = self.view.portal_list(cx, ids!(list));
        self.view.handle_event(cx, event, scope);
        let data = scope.data.get_mut::<AppData>().unwrap();
        if let Event::Actions(actions) = event {
            if self.view.button(cx, ids!(snapshot_button)).clicked(actions) {
                self.make_snapshot(cx, data);
            }
            if let Some(_search) = self
                .view
                .text_input(cx, ids!(search_input))
                .changed(&actions)
            {}
            for (item_id, item) in snapshots.items_with_actions(&actions) {
...
impl SnapshotRef {
    pub fn set_message(&self, cx: &mut Cx, message: String) {
        if let Some(inner) = self.borrow_mut() {
            inner.view(cx, ids!(message_input)).set_text(cx, &message);
        }
    }
}
