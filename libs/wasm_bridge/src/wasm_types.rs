...
impl ToWasm for WasmDataU8 {
    fn read_to_wasm(inp: &mut ToWasmMsgRef) -> Self {
        ...
        unsafe { Self(Vec::from_raw_parts(ptr as *mut u8, len, len)) }
    }
...
impl FromWasm for bool {
    fn from_wasm_js_body(
        ...
        out.push_ln(
            slot,
            &format!("{} = app.u32[this.u32_offset++]!==0?true:false;", prop),
        );
    }
    ...
    T: FromWasm,
{
    ...
    fn from_wasm_js_body(
        ...
        out.push_ln(
            slot,
            &format!("for(let i{0} = 0; i{0} < {1}; i{0}++){{", temp, N),
        );
    ...
    T: FromWasm,
{
    ...
    fn from_wasm_js_body(
        ...
        out.push_ln(
            slot,
            &format!("t{}.length = app.u32[this.u32_offset++];", temp),
        );
        out.push_ln(
            slot,
            &format!("for(let i{0} = 0; i{0} < t{0}.length; i{0}++){{", temp),
        );
        let new_temp = out.alloc_temp();
    ...
    T: ToWasm,
{
    ...
    fn to_wasm_js_body(
        ...
        out.push_ln(
            slot,
            &format!("app.u32[this.u32_offset ++] = t{}.length;", temp),
        );
        out.push_ln(
            slot,
            &format!("this.reserve_u32({} * t{}.length);", item_size, temp),
        );
        out.push_ln(
            slot,
            &format!("for(let i{0} = 0; i{0} < t{0}.length; i{0}++){{", temp),
