//! Kokoro / system-voice synthesis engine. Text in, PCM out — the caller
//! owns the audio device. Moved here from `libs/tts` (aiarch.md §1).
...
pub enum TtsBackend {
    /// Kokoro-82M on the Metal/CPU path.
    Kokoro,
...
impl Speaker {
    ...
    pub fn synthesize(&mut self, text: &str) -> std::result::Result<SpeechAudio, TtsError> {
        ...
        match self {
            ...
            #[cfg(all(any(target_os = "macos", target_os = "ios"), not(no_apple_tts)))]
            Self::NativeApple => crate::apple::synthesize(text, None, 0.0).ok_or(TtsError::Empty),
