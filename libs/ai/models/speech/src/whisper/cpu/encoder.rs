...
use crate::whisper::tensor::{parallel_for, RawTensor, Tensor};
...
fn multi_head_attention(
    ...
    if crate::whisper::accel::is_requested() {
        if let Some(out) = crate::whisper::accel::try_flash_attn_f32_packed(
            &q.data,
            &k.data,
            &v.data,
            seq_len,
    ...
    let out_ptr = crate::whisper::tensor::SendPtr::new(out.as_mut_ptr());
    let q_data = &q.data;
    let k_data = &k.data;
    let v_data = &v.data;
...
pub fn encode(model: &WhisperModel, mel_data: &[f32], n_ctx: usize) -> Tensor {
    ...
    cur = Tensor::gelu(&cur);
    crate::whisper::PROF_ENC_CONV.fetch_add(
    ...
    if crate::whisper::accel::is_requested() {
        ...
        if let Some(out) = crate::whisper::accel::try_encoder_stack_f32(
            &cur.data,
            cur.shape[0],
            cur.shape[1],
            n_head,
            ...
            crate::whisper::PROF_ENC_ATTN.fetch_add(dt / 2, std::sync::atomic::Ordering::Relaxed);
            crate::whisper::PROF_ENC_ELEM.fetch_add(dt - dt / 2, std::sync::atomic::Ordering::Relaxed);
            return Tensor {
                data: out,
    ...
    for layer in &model.encoder_layers {
        if crate::whisper::accel::is_requested() {
            ...
            if let Some(out) = crate::whisper::accel::try_encoder_layer_f32(
                &cur.data,
                cur.shape[0],
                cur.shape[1],
                n_head,
        ...
        if crate::whisper::accel::is_requested() {
            if let Some(out) = crate::whisper::accel::try_encoder_attn_block_f32(
                &cur.data,
                cur.shape[0],
                cur.shape[1],
                n_head,
        ...
        crate::whisper::PROF_ENC_ATTN.fetch_add(
        ...
        if crate::whisper::accel::is_requested() {
            if let Some(out) = crate::whisper::accel::try_encoder_ffn_block_f32(
                &cur.data,
                cur.shape[0],
                cur.shape[1],
                &layer.mlp_ln_w.data,
        ...
        crate::whisper::PROF_ENC_ELEM.fetch_add(
