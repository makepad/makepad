...
fn quant_gpu_enabled() -> bool {
    crate::whisper::settings::ENABLE_METAL_QUANT
}
...
#[inline]
fn use_act_q8() -> bool {
    crate::whisper::settings::ENABLE_ACT_Q8
}
...
impl Tensor {
    ...
    pub fn add(a: &Tensor, b: &Tensor) -> Tensor {
        if let Some(out) = crate::whisper::accel::try_add_f32(&a.data, &a.shape, &b.data, &b.shape) {
            return Tensor {
                data: out,
                shape: a.shape.clone(),
            };
    ...
    pub fn mul(a: &Tensor, b: &Tensor) -> Tensor {
        if let Some(out) = crate::whisper::accel::try_mul_f32(&a.data, &a.shape, &b.data, &b.shape) {
            return Tensor {
                data: out,
                shape: a.shape.clone(),
            };
    ...
    pub fn gelu(a: &Tensor) -> Tensor {
        if let Some(out) = crate::whisper::accel::try_gelu_f32(&a.data, &a.shape) {
            return Tensor {
                data: out,
                shape: a.shape.clone(),
            };
    ...
    pub fn layer_norm(x: &Tensor, eps: f32) -> Tensor {
        if let Some(out) = crate::whisper::accel::try_layer_norm_f32(&x.data, &x.shape, eps) {
            return Tensor {
                data: out,
                shape: x.shape.clone(),
            };
    ...
    pub fn layer_norm_mul_add(x: &Tensor, mul: &Tensor, add: &Tensor, eps: f32) -> Tensor {
        if let Some(out) = crate::whisper::accel::try_layer_norm_mul_add_f32(
    ...
    pub fn matmul(a: &Tensor, b: &Tensor) -> Tensor {
        ...
        if let Some(out) = crate::whisper::accel::try_matmul_nn_f32(&a.data, &b.data, m, k, n) {
            return Tensor {
                data: out,
                shape: vec![m, n],
            };
    ...
    pub fn matmul_t(x: &Tensor, w: &Tensor) -> Tensor {
        ...
        if let Some(out) = crate::whisper::accel::try_matmul_nt_f32(
            &x.data,
            &w.data,
            batch,
            in_features,
            ...
            crate::whisper::PROF_MATMUL_T.fetch_add(
                _t.elapsed().as_nanos() as u64,
                std::sync::atomic::Ordering::Relaxed,
            );
            crate::whisper::PROF_MATMUL_T_CALLS.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            return Tensor {
                data: out,
    ...
    pub fn linear_raw(x: &Tensor, weight: &RawTensor, bias: &Tensor) -> Tensor {
        ...
        if bias.numel() == out_features {
            if let Some(out) = crate::whisper::accel::try_matmul_nt_ggml_bytes_add_bias(
                &x.data,
                &weight.data,
                weight.ggml_type,
                batch,
    ...
    pub fn matmul_raw_with_prequant(
        ...
        if weight.ggml_type == GGML_TYPE_F32 {
            if let Some(out) = crate::whisper::accel::try_matmul_nt_f32_bytes(
                &x.data,
                &weight.data,
                batch,
                in_features,
                ...
                crate::whisper::PROF_MATMUL_RAW.fetch_add(
                    _t.elapsed().as_nanos() as u64,
                    std::sync::atomic::Ordering::Relaxed,
                );
                crate::whisper::PROF_MATMUL_RAW_CALLS.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                return Tensor {
                    data: out,
                    shape: vec![batch, out_features],
                };
            ...
            if let Some(out) = crate::whisper::accel::try_matmul_nt_f16_bytes(
                &x.data,
                &weight.data,
                batch,
                in_features,
            ...
            match weight.ggml_type {
                ...
                | GGML_TYPE_Q8_0 => {
                    if let Some(out) = crate::whisper::accel::try_matmul_nt_ggml_bytes(
                        &x.data,
                        &weight.data,
                        weight.ggml_type,
                        batch,
                        ...
                        crate::whisper::PROF_MATMUL_RAW_CALLS
    ...
    pub fn conv1d(input: &Tensor, weight: &Tensor, bias: &Tensor, stride: usize) -> Tensor {
        ...
        if let Some(im2col) =
            crate::whisper::accel::try_im2col_1d_f32(&input.data, ch_in, in_len, ksize, stride, pad)
        {
            let k = ch_in * ksize;
            if let Some(mm_out) =
                crate::whisper::accel::try_matmul_nt_f32(&im2col, &weight.data, out_len, k, ch_out)
            {
    ...
    pub fn conv1d_raw(input: &Tensor, weight: &RawTensor, bias: &Tensor, stride: usize) -> Tensor {
        if weight.shape.len() == 3 {
            ...
            if let Some(im2col) = crate::whisper::accel::try_im2col_1d_f32(
                &input.data,
                ch_in,
                in_len,
                ksize,
                ...
                let mm_out = match weight.ggml_type {
                    GGML_TYPE_F32 => crate::whisper::accel::try_matmul_nt_f32_bytes(
                        &im2col,
                        &weight.data,
                        out_len,
                        k,
                    ...
                    GGML_TYPE_F16 => crate::whisper::accel::try_matmul_nt_f16_bytes(
                        &im2col,
                        &weight.data,
                        out_len,
                        k,
                    ...
                    {
                        crate::whisper::accel::try_matmul_nt_ggml_bytes(
                            &im2col,
                            &weight.data,
                            weight.ggml_type,
                            out_len,
