...
fn metal_requested() -> bool {
    crate::whisper::settings::USE_METAL_BACKEND
}
...
pub(crate) fn try_encoder_stack_f32(
    ...
    n_state: usize,
    n_head: usize,
    layers: &[crate::whisper::model::EncoderLayer],
    final_ln_w: &[f32],
    final_ln_b: &[f32],
) -> Option<Vec<f32>> {
    if !metal_requested() {
...
pub(crate) fn try_decoder_cross_ffn_step_f32(
    ...
    v_cross: &[f32],
    n_audio_ctx: usize,
    layer: &crate::whisper::model::DecoderLayer,
) -> Option<Vec<f32>> {
    if !metal_requested() {
        return None;
    }
...
mod imp {
    ...
    pub(super) fn try_encoder_stack_f32(
        ...
        _n_state: usize,
        _n_head: usize,
        _layers: &[crate::whisper::model::EncoderLayer],
        _final_ln_w: &[f32],
        _final_ln_b: &[f32],
    ) -> Option<Vec<f32>> {
        None
    ...
    pub(super) fn try_decoder_cross_ffn_step_f32(
        ...
        _v_cross: &[f32],
        _n_audio_ctx: usize,
        _layer: &crate::whisper::model::DecoderLayer,
    ) -> Option<Vec<f32>> {
        None
    }
...
mod imp {
    ...
    fn metal_compile_feature_macros(device: ObjcId) -> (bool, bool) {
        ...
        if crate::whisper::settings::DISABLE_GGML_METAL_TENSOR {
            has_tensor = false;
        }

        if !crate::whisper::settings::FORCE_ENABLE_GGML_METAL_TENSOR && has_tensor {
    ...
    impl MetalContext {
        ...
        fn get_or_compile_cached_pipeline(
            ...
            if !self.pipeline_cache.contains_key(&cache_name) {
                if crate::whisper::settings::LOG_METAL_PIPELINES {
