...
use makepad_ai_sfx::sa3::{linear, par_rows, rms_norm_rows, silu};
...
fn ace_fwd_rope(
    ...
    ACE_FWD_CONSTS.with(|cell| {
        let mut c = cell.borrow_mut();
        let key = (tokens, batch);
        if c.rope_key == key {
            if let Some((cos, sin)) = c.rope.as_ref() {
                return Ok((cos.clone(), sin.clone()));
            }
        }
        let (cos_h, sin_h) = ace_rope_tables_bf16(tokens);
        let cos = Rc::new(
...
fn ace_rms_two_store(
    ...
    use makepad_ai_sfx::sa3::dev_err;
    use makepad_ai_common::backend::cuda::gpu_rms_norm_mul_round_bf16;
    ...
    gpu_rms_norm_mul_round_bf16(x, group_cols, "acedit", cache_key, weight, eps)
...
fn ace_adaln_stores(
    ...
    use makepad_ai_sfx::sa3::dev_err;
    use makepad_ai_common::backend::cuda::gpu_adaln_mod;
    gpu_adaln_mod(normed, mods, scale_off, shift_off)
...
impl AceDit {
    ...
    pub fn forward_device(
        ...
        self.forward_device_enc(device, x, context, frames, encoder, t, t_r, None)
    }
    ...
    fn forward_device_enc(
        ...
        use makepad_ai_common::backend::cuda::{
            ...
            gpu_concat_cols, gpu_download,
            gpu_gated_residual_round_add_bf16, gpu_linear_nt_cached_bf16_bias_epilogue,
            gpu_linear_nt_cached_bf16_mm, gpu_rms_norm_mod_indexed,
            gpu_rope_half_round_bf16, gpu_silu_round_mul_round_bf16, gpu_upload,
            gpu_upload_u32,
        };
        let lin = |x: &makepad_ai_common::backend::cuda::GpuTensor, w: &Bf16Weight| {
        ...
        let (rope_cos, rope_sin) = ace_fwd_rope(tokens, 1, rows, half)?;
        ...
        for (bi, (block, devb)) in self.blocks.iter().zip(&device.blocks).enumerate() {
            ...
            let normed = ace_adaln_stores(&rms_self, &mods_g, d, 0)?;
            let q = lin(&normed, &devb.q).map_err(|e| dev_err("ace dit q", e))?;
            let k = lin(&normed, &devb.k).map_err(|e| dev_err("ace dit k", e))?;
            let v = lin(&normed, &devb.v).map_err(|e| dev_err("ace dit v", e))?;
            let q = ace_rms_two_store(&q, &block.self_qn, hd, &format!("b{bi}.qn"), ACE_RMS_EPS)?;
            let k = ace_rms_two_store(&k, &block.self_kn, hd, &format!("b{bi}.kn"), ACE_RMS_EPS)?;
            let q = gpu_rope_half_round_bf16(&q, q_heads, half, &rope_cos, &rope_sin)
                .map_err(|e| dev_err("ace dit rq", e))?;
            let k = gpu_rope_half_round_bf16(&k, kv_heads, half, &rope_cos, &rope_sin)
                .map_err(|e| dev_err("ace dit rk", e))?;
            let (k, v) = expand_gqa_dev(k, v, q_heads, kv_heads, hd, group)?;
            let attn = ace_self_attn_dev(&q, &k, &v, q_heads, scale, devb.sliding)?;
            let proj = lin(&attn, &devb.o).map_err(|e| dev_err("ace dit o", e))?;
            h = gpu_gated_residual_round_add_bf16(&h, &proj, &mods_g, 2 * d)
            ...
            let cattn = ace_cross_attn_dev(&cq, ck, cv, q_heads, scale)?;
            let cproj = lin(&cattn, &devb.cross_o).map_err(|e| dev_err("ace dit co", e))?;
            ...
            let fnorm = ace_adaln_stores(&rms_mlp, &mods_g, 4 * d, 3 * d)?;
            let gate = lin(&fnorm, &devb.gate).map_err(|e| dev_err("ace dit gate", e))?;
            let up = lin(&fnorm, &devb.up).map_err(|e| dev_err("ace dit up", e))?;
            let act = gpu_silu_round_mul_round_bf16(&gate, &up)
                .map_err(|e| dev_err("ace dit swiglu fused", e))?;
            let down = lin(&act, &devb.down).map_err(|e| dev_err("ace dit down", e))?;
            h = gpu_gated_residual_round_add_bf16(&h, &down, &mods_g, 5 * d)
        ...
        let mut y = vec![0f32; padded * ACE_LATENT_DIM];
        for i in 0..tokens {
            let src = i * 2 * ACE_LATENT_DIM;
            y[(2 * i) * ACE_LATENT_DIM..(2 * i + 1) * ACE_LATENT_DIM]
                .copy_from_slice(&eo_y[src..src + ACE_LATENT_DIM]);
            y[(2 * i + 1) * ACE_LATENT_DIM..(2 * i + 2) * ACE_LATENT_DIM]
...
fn ace_self_attn_dev(
    ...
    use makepad_ai_sfx::sa3::dev_err;
    use makepad_ai_common::backend::cuda::{
        gpu_attention_packed_composite_bf16, gpu_attention_packed_sliding_bf16,
    };
    if sliding {
...
fn ace_cross_attn_dev(
    ...
    use makepad_ai_sfx::sa3::dev_err;
    use makepad_ai_common::backend::cuda::gpu_attention_packed_cross_composite_bf16;
