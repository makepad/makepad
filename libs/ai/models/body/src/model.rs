...
use crate::decoder::{Decoder, StepFeedback, StepInput};
...
use crate::preprocess::{
    condition_info, crop_geometry, crop_normalized, full_to_crop, patch_rays, CropGeometry,
};
use crate::weights::BodyWeights;
use crate::{DiffusionError, ProgressHook, Result, DINO_DIM, MHR_JOINTS, NUM_KEYPOINTS};
use std::path::Path;
...
impl BodyModel {
    ...
    pub fn infer(
        ...
        let geo = crop_geometry(bbox, w, h, None);
        ...
        self.decoder
            .run(tokens, &context, &self.dense_pe, |step: StepInput| {
...
mod tests {
    ...
    fn oracle_end_to_end() {
        ...
        eprintln!(
            "body infer warm {:.1} ms: crop {:.1}, backbone {:.1}, context {:.1}, decoder+rig {:.1}, packet {:.1}",
            packet.ms,
