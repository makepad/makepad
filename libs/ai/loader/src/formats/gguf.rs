//! GGUF container reader. Moved verbatim from libs/llama/src/gguf.rs (lane
//! T2, /aiarch.md §1) — it was already the only GGUF parser in the tree.
//! llama keeps a `pub use makepad_ai_loader::formats::gguf::*;` shim at the
//! old path so its ~220 existing call sites keep compiling unchanged.
//!
//! The only two adaptations from the original:
//! - `TensorType`/`block_elements`/`block_size` come from `crate::quant`
//!   (this crate's own dtype-metadata module, itself lifted out of
//!   libs/ggml — see quant.rs) instead of `makepad_ggml`, since loader must
//!   stay GPU-free and cannot depend on the ggml crate.
//! - errors use a small local `GgufError` (identical shape to llama's
//!   `LlamaError`) instead of `crate::error::LlamaError`, since loader
//!   cannot depend back on llama. llama's `error.rs` carries a
//!   `From<GgufError> for LlamaError` so the `?` operator at every existing
//!   call site keeps working unchanged.

use crate::quant::{block_elements, block_size, TensorType};
use std::borrow::Cow;
...
impl GgufString {
    ...
    pub fn try_utf8(&self) -> Result<&str> {
        std::str::from_utf8(&self.0)
            .map_err(|err| GgufError::format(format!("invalid utf-8 gguf string: {}", err)))
    }
}
...
impl GgufFile {
    pub fn open(path: impl AsRef<Path>) -> Result<Self> {
        ...
        for _ in 0..n_kv as usize {
            ...
            let value_type = GgufType::from_i32(value_type_raw).ok_or_else(|| {
                GgufError::format(format!("invalid gguf value type {}", value_type_raw))
            })?;
        ...
        if alignment == 0 {
            return Err(GgufError::format("invalid zero gguf alignment"));
        }
    ...
    pub fn require_value(&self, key: &str) -> Result<&GgufValue> {
        self.get_value(key)
            .ok_or_else(|| GgufError::format(format!("missing required gguf key '{}'", key)))
    }
    ...
    pub fn require_tensor(&self, name: &str) -> Result<&GgufTensorInfo> {
        self.get_tensor(name)
            .ok_or_else(|| GgufError::format(format!("missing tensor '{}'", name)))
    }
...
fn align_up(value: u64, alignment: u64) -> Result<u64> {
    ...
    value
        ...
        .ok_or_else(|| GgufError::format("overflow while aligning gguf data offset"))
}
...
fn ggml_tensor_size_bytes(tensor_type: TensorType, dimensions: &[u64]) -> Result<u64> {
    ...
    let row_blocks = dimensions[0]
        ...
        .ok_or_else(|| GgufError::format("overflow computing row blocks"))?
        / block_elems;
    let row_size = row_blocks
        .checked_mul(block_bytes)
        .ok_or_else(|| GgufError::format("overflow computing row size"))?;

    dimensions[1..].iter().try_fold(row_size, |acc, &dim| {
        acc.checked_mul(dim)
            .ok_or_else(|| GgufError::format("overflow computing tensor size"))
    })
}
    ...
    Ok(match value_type {
        ...
        GgufType::Array => {
            ...
            let array_type = GgufType::from_i32(array_type_raw).ok_or_else(|| {
                GgufError::format(format!("invalid gguf array type {}", array_type_raw))
            })?;
...
fn read_key_string(reader: &mut impl Read) -> Result<String> {
    ...
    bytes
        .try_utf8()
        .map(|s| s.to_owned())
        .map_err(|err| GgufError::format(format!("invalid utf-8 gguf key: {}", err)))
}
...
mod tests {
    use super::*;
    use crate::quant::ggml_type_name;
