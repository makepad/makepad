//! Minimal `.npy` reader, staged from libs/tts/src/kokoro/npy.rs (the cleanest
//! of the 4 standalone npy readers in the tree — see /aiarch.md §1/§2).
//! Pure staging: the original stays in place and keeps its tts-crate error
//! type; consumers are re-pointed in a later lane. Here the tts-specific
//! `TtsError` is swapped for a plain `String` error so this module has no
//! dependency on the tts crate (loader must not depend on model crates).
//!
...
fn bad(message: impl Into<String>) -> String {
    message.into()
}
