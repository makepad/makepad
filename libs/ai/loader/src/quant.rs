//! GGUF/ggml dtype metadata and the CPU quant kernels: the `TensorType`
//! enum, its GGML_TYPE_* ids, the per-type size tables (`ggml_type_name`,
//! `block_size`, `block_elements`), and the scalar/SIMD CPU reference code
//! for the block formats (f16/bf16/fp8 conversions, `dequantize_*`,
//! `vec_dot_*`, `quantize_*`, `get_rows_ggml_bytes_cpu`, the H3 NVFP4 pair
//! helpers). Backend-neutral: no GPU store depends on this crate's callers,
//! so every backend (CUDA, Metal, CPU) and the loader's own formats/gguf.rs
//! share one definition. The IQ-class codebook dequant lives in `quant_iq`.
