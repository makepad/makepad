...
static __device__ __forceinline__ void flash_attn_ext_f16_load_tile(
    ...
    if constexpr (use_cp_async) {
        ...
        struct LoadCp {
            ...
            const half2 * KV;
            int D2, stride_KV, chunks_per_row, nwarps, nbatch_fa, warp_size, stride_tile;
            __device__ void operator()(int n) const {
...
#pragma unroll
                    for (int k0 = k0_start; k0 < k0_stop; k0 += stride_k) {
                        ...
                        cp_async_cg_16<64>(tile_KV_32 + i*(stride_tile*sizeof(half2)) + k*16, KV + i*stride_KV + k*(16/sizeof(half2)));
                    }
                }
            }
        };
        ggml_cuda_unroll<6>{}(LoadCp{tile_KV_32, KV, D2, stride_KV, chunks_per_row, nwarps, nbatch_fa, warp_size, stride_tile});
    } else {
        struct LoadSync {
            half2 * tile_KV;
            const half2 * KV;
            int D2, stride_KV, i_sup, nwarps, nbatch_fa, warp_size, stride_tile;
            bool oob_check;
            __device__ void operator()(int n) const {
                const int stride_k = warp_size >> n;
...
#pragma unroll
        ...
        ggml_cuda_unroll<4>{}(LoadSync{tile_KV, KV, D2, stride_KV, i_sup, nwarps, nbatch_fa, warp_size, stride_tile, oob_check});
    }
}
