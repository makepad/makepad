//! MCP streamable-HTTP endpoint on loopback.
//!
//! A host serves `POST http://127.0.0.1:<port>/mcp` (JSON-RPC 2.0) so an MCP
//! client — a Director lane's CLI (Claude, Codex), or Claude Desktop through
//! an app's `--mcp` relay ([`super::host`]) — can discover and call the
//! host's tools natively. The transport implements `initialize`,
...
//! answer an empty 202.
//!
//! Every request that reaches the JSON-RPC layer carries a bearer token; the transport rejects an unknown path (404), a foreign Host or
//! Origin (403) and a non-POST method (405) before looking at credentials, so
//! those replies never reveal whether a token was valid. Tokens are minted
//! ([`TokenStore::mint`]) OS-random. Director persists its lanes' tokens
//! privately under the state directory (mode 0600 inside a 0700 directory)
//! so a PTY that survives a restart keeps its credential, and revokes them
//! when the terminal's lifecycle ends ([`TokenStore::revoke`]); an app's
//! endpoint keeps one token in memory ([`TokenStore::ephemeral`]).
//!
//! Execution is not here. The server calls a [`ToolDispatcher`] which the host
//! binds to its tool registry (`McpServer::start(.., dispatcher)`).
...
pub const MAX_RESULT_BYTES: usize = wire::MAX_RESULT_BYTES;
...
/// Longest tool name the transport forwards: a service id, `__`, a tool
/// name (the form native tool APIs see, `route__plan`).
const MAX_TOOL_NAME: usize = wire::MAX_SERVICE_ID + 2 + wire::MAX_TOOL_NAME;
/// Largest `arguments` object (encoded) the transport forwards.
const MAX_ARGS_BYTES: usize = wire::MAX_ARGS_BYTES;
...
/// The execution seam. A host binds its tool registry here.
pub trait ToolDispatcher: Send + Sync {
    /// The `serverInfo.name` answered to `initialize`.
    fn server_name(&self) -> String {
        "makepad".into()
    }
    /// The `instructions` answered to `initialize`, when the host has any.
...
/// Per-lane bearer tokens, persisted privately so PTYs that outlive
/// Director keep their credential (or held in memory only, [`Self::ephemeral`]). Every access is constant-time over the token
...
impl TokenStore {
    ...
    pub fn open(state_dir: &Path) -> Result<Self, String> {
        ...
        Ok(Self { path: Some(path), records: Mutex::new(records) })
    }

    /// A store that lives only as long as this process: an app's endpoint
    /// mints one token per start and hands it out through its discovery file.
    pub fn ephemeral() -> Self {
        Self { path: None, records: Mutex::new(Vec::new()) }
    }
    ...
    pub fn mint(&self, lane: &str, owner: &str) -> Result<Token, String> {
        ...
        persist(self.path.as_deref(), &records)?;
        Ok(Token(token))
    }
...
fn persist(path: Option<&Path>, records: &[TokenRecord]) -> Result<(), String> {
    ...
    let Some(caller) = caller else {
        ...
        return (
            json_status(401, Value::Null, INVALID_REQUEST, "bearer token required")
                .with_header("WWW-Authenticate", "Bearer realm=\"makepad-mcp\"".into()),
            true,
        );
    };
        ...
        "initialize" => {
            ...
            let mut result = vec![
                ...
                (
                    "serverInfo",
                    json::obj(vec![
                        ("name", json::s(shared.dispatcher.server_name())),
                        ("version", json::s(env!("CARGO_PKG_VERSION"))),
                    ]),
                ),
            ];
            if let Some(instructions) = shared.dispatcher.instructions() {
                result.push(("instructions", json::s(instructions)));
            }
            rpc_result(id, json::obj(result))
        }
        "ping" => rpc_result(id, json::obj(vec![])),
        "tools/list" => rpc_result(id, json::obj(vec![("tools", tools_json(&shared.dispatcher.list(caller)))])),
        "tools/call" => {
...
/// The `tools` array of a `tools/list` result: each definition with its
/// parameters as the `inputSchema` object.
pub fn tools_json(tools: &[ToolDef]) -> Value {
    Value::Arr(
        tools
            .iter()
            .map(|t| {
