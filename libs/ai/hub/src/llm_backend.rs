...
pub struct ExpandJob {
    ...
    pub kind: String,
    /// Conversation identity for lane stickiness (wire `chat_session`).
    /// Empty = legacy client; the lane identity then falls back to `kind`,
    /// which serializes every conversation onto one lane identity.
    pub session: String,
}
    ...
    cancel: &CancelToken,
        ...
        for index in 0..variants {
            ...
            let job = ExpandJob {
                ...
                session: params.chat_session.clone(),
            };
        ...
        loop {
            ...
            'arrival: for (job, cancel, events) in arrivals {
                ...
                let request = LaneRequest {
                    job: id,
                    // The CONVERSATION is the identity (lanes.rs: never a
                    // prompt hash, and never a shared class name — under
                    // sticky affinity a shared name hands two players one
                    // lane and each other's KV). Legacy clients that sent
                    // no chat_session keep the old kind-keyed behavior.
                    session: if job.session.is_empty() {
                        job.kind.clone()
                    } else {
                        format!("{}:{}", job.kind, job.session)
                    },
