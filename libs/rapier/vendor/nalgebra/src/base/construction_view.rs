...
#[inline]
fn reduced_ratio(mut numer: usize, mut denom: usize) -> (usize, usize) {
    let mut a = numer;
    let mut b = denom;
    while b != 0 {
        let next = a % b;
                ...
                nrows * ncols <= 1
                    || match (rstride, cstride) {
                        ...
                        (_, _) => {
                            ...
                            let (reduced_rstride, reduced_cstride) =
                                reduced_ratio(rstride, cstride);
                            nrows <= reduced_cstride || ncols <= reduced_rstride
                        }
                    }
            },
