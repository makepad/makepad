...
static MUSIC_FONT: OnceLock<MusicFont> = OnceLock::new();
...
/// The bundled SMuFL face and its metadata.
#[cfg(feature = "embed-bravura")]
pub fn bravura() -> EmbeddedFont {
    EmbeddedFont {
        name: "Bravura",
...
/// Install the bundled face as the fallback before the font is first resolved.
#[cfg(feature = "embed-bravura")]
pub fn ensure_default_font() {
    if MUSIC_FONT.get().is_none() && EMBEDDED.get().is_none() {
        set_embedded_music_font(bravura());
    }
}

/// Without the embedding feature, callers still have a portable no-op hook.
#[cfg(not(feature = "embed-bravura"))]
pub fn ensure_default_font() {}

pub fn music_font() -> &'static MusicFont {
    MUSIC_FONT.get_or_init(|| {
