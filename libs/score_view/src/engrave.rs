...
use crate::document::{
    pitch_to_midi, semantic_for_measure, semantic_for_note, DocumentError, DocumentOptions,
    SemanticElement, SemanticKind, DECORATION_TAG,
};
use crate::font::{music_font, Engraving, MusicFont};
use crate::spacing::{MeasurePlacement, PagePlacement, SystemPlacement};
use makepad_score::{
    model::{
        EventKind, KeySignature, Measure, Meter, Notehead, Pitch, Rational, Score,
        ScoreTime, StaffId, StaffKind, SyllabicRole, TimedEvent, VoiceId,
    },
    symbol::{
        Accidental, Clef, Direction, FlagDuration, NoteheadDuration, NoteheadShape,
        Placement, Symbol,
    },
};
use makepad_score_layout::LayoutStyle;
use makepad_score_render::{
    Beam, GlyphItem, Ink, InkRole, LineKind, LinearRgba, MusicFontRef, PageId, PaintItem,
    PaintKind, PaintList, Point, Primitive, Rect, RuleKind, SemanticId, SmuflGlyph,
    TextDirection, TextFontRef, TextRun,
};
...
pub const MARGIN_LEFT: f64 = 17.0;
pub const MARGIN_RIGHT: f64 = 14.0;
/// Shortest stem, in staff spaces, measured from the notehead centre.
const STEM_LENGTH: f64 = 3.5;
const BEAM_MIN_STEM: f64 = 3.0;
const DRUM_KEY_TEXT_SIZE: f64 = 1.4;
const DRUM_KEY_COLUMN_PAD: f64 = 1.5;
const DRUM_KEY_LEADER: f64 = 0.6;
const DRUM_KEY_COLLISION: f64 = 1.2;
...
pub(crate) fn score_staff_frames(score: &Score, top: f64) -> Vec<StaffFrame> {
    let Some(part) = score.parts.values().next() else {
        return staff_frames(top).into();
    };
    ...
    part.staves
        ...
        .enumerate()
        .map(|(index, staff_id)| {
            let fallback = if index == 0 { (Clef::G, 2) } else { (Clef::F, 4) };
            let (clef, line) = score
                .voices
                .values()
                .filter(|voice| voice.staff == *staff_id)
                .flat_map(|voice| &voice.events)
                .find_map(|event| match event.kind {
                    EventKind::Clef(change) => Some((change.clef, change.line)),
                    _ => None,
                })
                .unwrap_or(fallback);
            StaffFrame::for_clef(top + index as f64 * STAFF_GAP, clef, line)
        })
        .collect()
}

pub(crate) fn score_staff_span(score: &Score) -> f64 {
    let frames = score_staff_frames(score, 0.0);
    frames.last().map_or(STAFF_SPAN, |staff| staff.bottom())
}
...
impl StaffFrame {
    ...
    fn for_clef(top: f64, clef: Clef, line: u8) -> Self {
        let (middle_diatonic, key_shift) = match clef {
            Clef::F | Clef::F8va | Clef::F8vb | Clef::F15ma | Clef::F15mb => {
                (3 * 7 + 1, -2)
            }
            _ => (4 * 7 + 6, 0),
        };
        Self {
            top,
            clef: clef_name(clef),
            clef_line: top + f64::from(5_u8.saturating_sub(line.clamp(1, 5))),
            middle_diatonic,
            key_shift,
        }
    }
...
pub(crate) fn drum_key_width(score: &Score) -> f64 {
    drum_key_rows_width(&drum_key_rows(score))
}

fn drum_key_rows_width(rows: &[DrumKeyRow]) -> f64 {
    rows
        .iter()
        .map(|row| crate::title::text_width_sp(&row.label, DRUM_KEY_TEXT_SIZE))
        .reduce(f64::max)
        .map_or(0.0, |width| width + DRUM_KEY_COLUMN_PAD)
}

fn drum_key_rows(score: &Score) -> Vec<DrumKeyRow> {
    let Some(staff_id) = score.parts.values().next().and_then(|part| part.staves.first()) else {
        return Vec::new();
    };
    let Some(staff) = score.staves.get(staff_id) else {
        return Vec::new();
    };
    let StaffKind::Percussion(map) = &staff.kind else {
        return Vec::new();
    };
    let used: BTreeSet<u16> = score
        .voices
        .values()
        .filter(|voice| voice.staff == *staff_id)
        .flat_map(|voice| &voice.events)
        .flat_map(|event| match &event.kind {
            EventKind::Chord(notes) => notes.as_slice(),
            _ => &[],
        })
        .filter_map(|note| note.unpitched_sound)
        .collect();

    let mut sounds: Vec<(String, i32)> = map
        .entries
        .iter()
        .filter(|(sound_id, _)| used.contains(sound_id))
        .map(|(_, sound)| {
            let label = crate::build::DrumVoice::ALL
                .into_iter()
                .find(|voice| voice.gm_note() == sound.midi_note)
                .map(|voice| voice.short_label().to_string())
                .unwrap_or_else(|| sound.name.trim().chars().take(8).collect());
            (label, diatonic_index(sound.display))
        })
        .collect();
    sounds.sort_by(|left, right| right.1.cmp(&left.1).then_with(|| left.0.cmp(&right.0)));
    ...
    for (label, diatonic) in sounds {
        if let Some(group) = groups.last_mut() {
            let separation = f64::from(group.top_diatonic - diatonic) * 0.5;
            if separation < DRUM_KEY_COLLISION {
                group.labels.push(label);
                group.diatonic_sum += diatonic;
                continue;
            }
        }
        groups.push(Group {
            labels: vec![label],
            top_diatonic: diatonic,
            diatonic_sum: diatonic,
        });
    }
    groups
        .into_iter()
        .map(|group| DrumKeyRow {
            diatonic: f64::from(group.diatonic_sum) / group.labels.len() as f64,
...
pub(crate) struct HeadLayout {
    pub(crate) note: makepad_score::model::NoteId,
    midi: u8,
    diatonic: i32,
    y: f64,
...
impl PageBuilder<'_> {
    ...
    fn text(&mut self, text: impl Into<Arc<str>>, origin: Point, size: f64, z: i16) {
        ...
        self.push_text(text, origin, size, z);
    }

    /// Lyrics are score content, not furniture, so compact views which hide
    /// titles and measure labels must continue to show them.
    fn lyric_text(&mut self, text: impl Into<Arc<str>>, origin: Point, size: f64, z: i16) {
        self.push_text(text, origin, size, z);
    }

    /// Drum-key labels are score content too. Their anchor is the right edge
    /// and their y is the notehead centre, not the top of the text line box.
    fn drum_key_text(
        &mut self,
        ...
        let top = center_y - crate::title::line_box_sp(size) * 0.5;
        self.push_text_left(text, Point::new(right - width, top), width, size, z);
    }

    fn push_text(&mut self, text: impl Into<Arc<str>>, origin: Point, size: f64, z: i16) {
        let text = text.into();
        let width = crate::title::text_width_sp(&text, size).max(size * 0.5);
        self.push_text_left(
            text,
            Point::new(origin.x - width * 0.5, origin.y),
            width,
            size,
            z,
        );
...
pub fn make_page_with_options(
    ...
    if page_index == 0 {
        ...
        for line in block.lines() {
            builder.text(
                line.text.clone(),
                Point::new(options.page_size.x * 0.5, line.top),
                line.size,
                1,
            );
        }
    ...
    let drum_key_rows = if page_index == 0 && options.drum_key {
        drum_key_rows(score)
    } else {
        Vec::new()
    };
    let drum_key_width = drum_key_rows_width(&drum_key_rows);
    for (index, system) in page.systems.iter().enumerate() {
        ...
        let staves = score_staff_frames(score, system.top);
        let right = system
            .measures
            .last()
            .map(|last| last.right)
            .unwrap_or(system.right);
        let first_system = page_index == 0 && index == 0;
        let left = MARGIN_LEFT + if first_system { drum_key_width } else { 0.0 };
        draw_system_frame(&mut builder, &staves, index, left, right);
        if first_system {
            if let Some(staff) = staves.first().copied() {
                draw_drum_key(&mut builder, staff, left, &drum_key_rows);
            }
        }
        ...
        draw_system_lyrics(&mut builder, &staves, system, &measures);
    }

    builder.text(
        (page_index + 1).to_string(),
        Point::new(options.page_size.x * 0.5, options.page_size.y - 7.0),
        1.8,
        1,
    );
...
fn draw_system_frame(
    ...
    let Some(first) = staves.first() else { return };
    let last = staves.last().unwrap_or(first);
    ...
    // Multi-staff parts get a brace-substitute bracket; a drum part stays a
    // single, conventional five-line staff.
    if staves.len() > 1 {
        let id = builder.next_decor();
        builder.items.push(PaintItem::primitive(
            id,
    ...
    builder.rule(
        Rect::from_xywh(left, first.top, thin, last.bottom() - first.top),
...
fn draw_drum_key(
    ...
    let text_right = staff_left - DRUM_KEY_LEADER;
    let thickness = builder.engraving.staff_line_thickness;
    for row in rows {
        let y = staff.middle()
            - (row.diatonic - f64::from(staff.middle_diatonic)) * 0.5;
        builder.drum_key_text(row.label.as_str(), text_right, y, DRUM_KEY_TEXT_SIZE, 2);
        builder.rule(
            Rect::from_xywh(text_right, y - thickness * 0.5, DRUM_KEY_LEADER, thickness),
            RuleKind::Ledger,
            Ink::role(InkRole::Secondary),
            1,
        );
    }
}
...
fn draw_system_prefix(
    ...
    let steps = key_signature_steps(key.fifths);
    for staff in staves {
        builder.decor_glyph(staff.clef, Point::new(left + prefix.clef_x, staff.clef_line));
        if let Some(glyph) = &prefix.accidental {
            ...
            for (step, x) in steps.iter().zip(&prefix.accidental_x) {
                ...
                builder.decor_glyph(&glyph, Point::new(left + x, y));
            }
        }
        if let Some(meter) = &prefix.meter {
            for (digits, line) in [(&meter.numerator, 1.0), (&meter.denominator, 3.0)] {
                ...
                let mut x = left + meter.x + (meter.span - run) * 0.5;
                for name in digits {
                    let name = name.clone();
...
fn draw_measure(
    ...
    let Some(first_staff) = staves.first() else { return };
    let last_staff = staves.last().unwrap_or(first_staff);
    ...
    let bounds = Rect::from_xywh(
        ...
        first_staff.top - 3.0,
        x1 - x0,
        last_staff.bottom() - first_staff.top + 6.0,
    );
    builder.items.push(PaintItem::primitive(
        measure_semantic,
        -2,
    ...
    if first_in_system {
        builder.text(
            measure.label.clone(),
            Point::new(x0 + 0.9, first_staff.top - 1.4),
            1.5,
            1,
        );
    }
    ...
    let thin = builder.engraving.thin_barline_thickness;
    let height = last_staff.bottom() - first_staff.top;
    if last_of_score {
        let thick = builder.engraving.thick_barline_thickness;
        let separation = 0.4;
        builder.rule(
            Rect::from_xywh(x1 - thick - separation - thin, first_staff.top, thin, height),
            RuleKind::BarlineThin,
            Ink::role(InkRole::Primary),
            1,
        );
        builder.rule(
            Rect::from_xywh(x1 - thick, first_staff.top, thick, height),
        ...
        builder.rule(
            Rect::from_xywh(x1 - thin, first_staff.top, thin, height),
            RuleKind::BarlineThin,
            Ink::role(InkRole::Primary),
            1,
        );
...
/// Lyrics are laid out after the system's note columns so every syllable,
/// hyphen, and melisma uses the same solved x coordinate as its notehead.
fn draw_system_lyrics(
    builder: &mut PageBuilder<'_>,
    staves: &[StaffFrame],
    system: &SystemPlacement,
    measures: &[&Measure],
) {
    if builder.score.lyrics.is_empty() {
        return;
    }
    let head_width = builder.font.bbox("noteheadBlack").width();
    let mut anchors = Vec::new();
    for placement in &system.measures {
        let Some(&measure) = measures.get(placement.index) else { continue };
        let key = builder
            .score
            .maps
            .key_at(measure.start, None, None)
            ...
            .unwrap_or(KeySignature::C_MAJOR);
        let staff_columns =
            measure_staff_columns(builder.font, builder.score, measure, &key, staves);
        for (staff, voices) in staves.iter().zip(&staff_columns) {
            for columns in voices {
                for column in columns {
                    let x = placement.x_of(column.time);
                    for head in &column.heads {
                        let shift = if head.shifted {
                            if column.stem_up { head_width } else { -head_width }
                        } else {
                            0.0
                        };
    ...
    let mut visible = builder
        ...
        .filter_map(|lyric| {
            anchors
                .iter()
                .find(|(note, _)| *note == lyric.note)
                .map(|(_, anchor)| (lyric.clone(), *anchor))
        })
        .collect::<Vec<_>>();
    visible.sort_by(|(left, left_anchor), (right, right_anchor)| {
        left_anchor
            .staff_bottom
            .total_cmp(&right_anchor.staff_bottom)
            .then_with(|| left.verse.cmp(&right.verse))
            .then_with(|| left_anchor.x.total_cmp(&right_anchor.x))
    });

    const SIZE: f64 = 1.5;
    const BELOW_STAFF: f64 = 2.5;
    const VERSE_GAP: f64 = 1.8;
    for (index, (lyric, anchor)) in visible.iter().enumerate() {
        let y = anchor.staff_bottom
            + BELOW_STAFF
            + f64::from(lyric.verse.saturating_sub(1)) * VERSE_GAP;
        let label = match &lyric.elision {
            Some(elision) => format!("{}{}", lyric.text, elision),
            None => lyric.text.clone(),
        };
        let width = crate::title::text_width_sp(&label, SIZE).max(SIZE * 0.5);
        builder.lyric_text(label, Point::new(anchor.x, y), SIZE, 2);

        if matches!(lyric.role, SyllabicRole::Begin | SyllabicRole::Middle) {
            if let Some((next, next_anchor)) = visible.get(index + 1) {
                if next.verse == lyric.verse
                    && (next_anchor.staff_bottom - anchor.staff_bottom).abs() < 0.01
                    && matches!(next.role, SyllabicRole::Middle | SyllabicRole::End)
                {
                    builder.lyric_text(
                        "-",
                        Point::new((anchor.x + next_anchor.x) * 0.5, y),
                        SIZE,
                        2,
                    );
                }
        ...
        let Some(end_note) = lyric.melisma_to else { continue };
        let Some((_, end_anchor)) = anchors.iter().find(|(note, _)| *note == end_note) else {
            continue;
        };
        let start_x = anchor.x + width * 0.5 + 0.25;
        let end_x = end_anchor.x - 0.2;
        if end_x > start_x {
            let line_y = y + crate::title::line_box_sp(SIZE) * 0.75;
            let id = builder.next_decor();
            builder.items.push(PaintItem::primitive(
                id,
                1,
                Ink::role(InkRole::Secondary),
                Primitive::Line {
                    start: Point::new(start_x, line_y),
                    end: Point::new(end_x, line_y),
                    thickness: 0.08,
                    dash: None,
                    kind: LineKind::LyricExtender,
                },
            ));
        }
    }
...
/// One measure's columns, per staff and then per active
...
pub(crate) fn measure_staff_columns(
    ...
    let mut out = vec![Vec::new(); staves.len()];
    let measure_end = measure
        .start
        .checked_add(measure.extent)
        .unwrap_or(measure.start);
    for (slot, staff_frame) in staves.iter().copied().enumerate() {
        let Some(staff_id) = part.staves.get(slot).copied() else { continue };
...
pub mod tests {
    ...
    pub fn fixture(pitches: &[(Step, i8)], note_denominator: u64) -> Score {
    ...
    pub fn fixture_events(events: &[Placed]) -> Score {
    ...
    pub fn engrave(score: &Score) -> Drawn {
    ...
    fn a_lyric_is_text_below_the_staff_even_when_labels_are_hidden() {
        let note = crate::build::PitchedNote {
            onset_beats: 0.0,
            duration_beats: 1.0,
            midi: 60,
            velocity: 0.8,
        };
        let word = crate::build::LyricWord {
            onset_beats: 0.0,
            end_beats: 1.0,
            text: "sing".into(),
        };
        let score = crate::build::build_pitched_score_with_lyrics(
            &[note],
            &[word],
        ...
        spacing.rebuild(&score);
        let placement = spacing.pages()[0].clone();
        let staff_bottom = placement.systems[0].top + 4.0;
        let options = DocumentOptions::content(&score, true);
        let (page, _) = make_page_with_options(&score, &placement, 0, 1, options).unwrap();
        let lyric = page
            .items()
            .iter()
            .find(|item| {
                matches!(&item.kind, PaintKind::Text(run) if run.text.as_ref() == "sing")
            })
            .expect("lyric text is engraved");
        assert!(lyric.bounds.min.y >= staff_bottom + 2.0, "{:?}", lyric.bounds);
    }
