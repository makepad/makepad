...
use crate::engrave::{
    measure_staff_columns, Column, MARGIN_LEFT, MARGIN_RIGHT, STAFF_SPAN,
};
...
pub struct ScoreSpacing {
    ...
    /// Whether the first percussion system reserves a drum-key column.
    drum_key: bool,
    widths: LineWidths,
...
impl ScoreSpacing {
    pub fn new() -> Self {
        Self {
            ...
            page_width: PAGE_WIDTH_SP,
            staff_span: STAFF_SPAN,
    ...
    pub fn set_page_width(&mut self, page_width: f64) {
        self.page_width = page_width.max(MARGIN_LEFT + MARGIN_RIGHT + 1.0);
    }

    pub fn set_drum_key(&mut self, drum_key: bool) {
        self.drum_key = drum_key;
    }
    ...
    /// Page width that lets every measured bar sit on one system at its
    /// natural spring widths. Content-fit views use this as their widest
    /// useful reflow candidate; making the page wider would only add ragged
    /// paper after the final bar.
    pub(crate) fn natural_page_width(&self) -> f64 {
        let lead = self.style.distance.barline_to_note.0;
        let natural: f64 = self
            .sources
            .iter()
            .flat_map(|source| &source.columns)
            .map(|column| column.natural.0)
            .sum();
        (self.music_left_first - lead + natural + MARGIN_RIGHT)
            .max(MARGIN_LEFT + MARGIN_RIGHT + 1.0)
    }
    ...
    pub fn locate(&self, score: &Score, whole: f64) -> Option<CursorLocation> {
        let last_index = self.measure_ids.len().checked_sub(1)?;
        let index = self.measure_ids.iter().enumerate().position(|(index, id)| {
            score.measures.get(id).is_some_and(|measure| {
                ...
                whole >= start && (whole < end || (index == last_index && whole == end))
            })
        })?;
    ...
    pub fn rebuild(&mut self, score: &Score) {
        let font = music_font();
        self.staff_span = crate::engrave::score_staff_span(score);
    ...
    pub fn touch_measure(&mut self, score: &Score, index: usize) -> PagesDirty {
        ...
        let prefix_before = self.music_left_first;
        self.plan_prefix(score, font);
        ...
        if before != after || self.music_left_first != prefix_before {
            self.repaginate();
            return PagesDirty::All;
        }
    ...
    fn plan_prefix(&mut self, score: &Score, font: &'static MusicFont) {
        ...
        if self.drum_key {
            self.music_left_first += crate::engrave::drum_key_width(score);
        }
        let right = self.page_width - MARGIN_RIGHT;
    ...
    fn repaginate(&mut self) {
        let verticals: Vec<SystemVertical> = self
            ...
            .map(|system| {
                ...
                SystemVertical {
                    height: Sp(self.staff_span + above + below),
    ...
    fn place_page(&mut self, page_index: usize) {
        ...
        for system_index in fill.clone() {
            ...
            placed.systems.push(SystemPlacement {
                top,
                bottom: top + self.staff_span,
                music_left,
                right: self.page_width - MARGIN_RIGHT,
                show_meter,
                measures,
            });
        }
...
pub(crate) fn measure_ink(
    ...
    let frames = crate::engrave::score_staff_frames(score, 0.0);
    let staff_span = frames.last().map_or(STAFF_SPAN, |staff| staff.bottom());
    ...
    let lyric_verses = score
        .lyrics
        .iter()
        .filter(|lyric| {
            staves.iter().any(|voices| {
                voices.iter().any(|columns| {
                    columns
                        .iter()
                        .any(|column| column.heads.iter().any(|head| head.note == lyric.note))
                })
            })
        })
        .map(|lyric| lyric.verse.max(1))
        .max()
        .unwrap_or(0);
    let lyric_below = if lyric_verses == 0 {
        3.0
    } else {
        4.5 + f64::from(lyric_verses - 1) * 1.8
    };
    MeasureInk {
        columns: merged
            .into_iter()
        ...
        below: (bottom - staff_span + 1.0 + 2.0).max(lyric_below),
    }
}
...
mod tests {
    ...
    #[test]
    fn locate_maps_bar_starts_and_the_score_end_to_engraved_x() {
        let score = crate::document::demo_score(4).expect("the fixture is valid");
        let mut spacing = ScoreSpacing::new();
        spacing.rebuild(&score);

        let at_start = spacing.locate(&score, 0.0).expect("score start");
        let at_bar_two = spacing.locate(&score, 1.0).expect("second bar start");
        let at_end = spacing.locate(&score, 4.0).expect("score end");
        let placement = |index: usize| {
            spacing
                .pages
                .iter()
                .flat_map(|page| &page.systems)
                .flat_map(|system| &system.measures)
                .find(|measure| measure.index == index)
                .expect("measure placement")
        };
        let first = placement(0);
        let second = placement(1);
        let last = placement(3);

        assert_eq!(at_start.x_sp, first.x_at(0.0, 0.0, 1.0));
        assert_eq!(at_bar_two.x_sp, second.x_at(1.0, 1.0, 2.0));
        assert_eq!(at_end.x_sp, last.right);
        assert_eq!(at_end.page, spacing.page_of_measure(3).unwrap());
    }
