...
#[cfg(feature = "serde")]
use serde::de::{Deserialize, Deserializer, Error as DeserializeError, SeqAccess, Visitor};
#[cfg(feature = "serde")]
    ...
    A::Item: Clone,
{
    ...
    fn clone_from(&mut self, o: &Self) {
        ...
        if let Some(to_drop) = self
            .data
            .as_slice_mut()
            .get_mut((o.len as usize)..(self.len as usize))
        {
    ...
    <A as Array>::Item: borsh::BorshDeserialize,
{
    fn deserialize_reader<R: borsh::io::Read>(reader: &mut R) -> borsh::io::Result<Self> {
        ...
        for idx in 0..len {
            let value =
                <<A as Array>::Item as borsh::BorshDeserialize>::deserialize_reader(reader)?;
...
impl<A: Array> ArrayVec<A> {
    ...
    /// ```
    #[inline]
    pub fn try_insert(&mut self, index: usize, mut item: A::Item) -> Option<A::Item> {
        assert!(
    ...
    pub fn pop(&mut self) -> Option<A::Item> {
        if self.len > 0 {
            self.len -= 1;
            let out = core::mem::take(&mut self.data.as_slice_mut()[self.len as usize]);
            Some(out)
        } else {
            None
        }
...
impl<'p, A: Array, I: Iterator<Item = A::Item>> Iterator for ArrayVecSplice<'p, A, I> {
    type Item = A::Item;
...
impl<A: Array> Iterator for ArrayVecIterator<A> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let slice = &mut self.data.as_slice_mut()[self.base as usize..self.tail as usize];
    ...
    A::Item: Debug,
{
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        f.debug_tuple("ArrayVecIterator")
            .field(&self.as_slice())
            .finish()
    }
}
