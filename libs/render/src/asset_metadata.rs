...
/// Parsed once on the asset worker. The shape stays in its author's node-local
/// coordinates; placement composes instance * rest_transform before physics.
#[derive(Clone,Debug)]pub struct PreparedAssetCollision{pub node:usize,pub shape:AssetCollisionShape,pub rest_transform:makepad_draw::makepad_math::Mat4f,pub cooked:std::sync::Arc<makepad_game_sim::PreparedAssetCollider>}
pub fn prepare_asset_collisions(document:&GltfDocument)->Result<std::sync::Arc<Vec<PreparedAssetCollision>>,String>{
    ...
    for collision in shapes {
        ...
        if det.abs()<1e-10{return Err("singular authored collision owner transform".into());}
        if det<0. {for triangle in indices.chunks_exact_mut(3){triangle.swap(1,2);}}
        let cooked=makepad_game_sim::PreparedAssetCollider::cook(&positions,&indices)?;
        out.push(PreparedAssetCollision{node:collision.node,shape:collision.shape,rest_transform,cooked});
    }
...
#[test]fn cooked_collision_keeps_owner_transform_and_bounded_sphere_geometry(){
    ...
    assert_eq!(rows[0].cooked.center,makepad_draw::makepad_math::vec3f(3.,4.,5.));assert_eq!(rows[0].cooked.half,makepad_draw::makepad_math::vec3f(2.,1.,1.));assert_eq!(rows[0].cooked.data.triangles.len(),12);assert_eq!(rows[0].cooked.data.degenerate_count,0);
    assert_eq!(rows[1].cooked.data.triangles.len(),3968);assert!(rows[1].cooked.data.vertices.iter().all(|v|(v.x*v.x+v.y*v.y+v.z*v.z-4.).abs()<1e-4));assert_eq!(rows[1].cooked.data.degenerate_count,0);
}
