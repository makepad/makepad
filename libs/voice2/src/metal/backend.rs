...
fn log_mul_mat_requested() -> bool {
    crate::settings::LOG_METAL_MUL_MAT || log_metal_trace()
}
...
pub(crate) fn try_matmul_nt_f32_bytes(
    ...
    makepad_ggml::backend::metal::try_matmul_nt_f32_bytes(a, bt_bytes, m, k, n)
}
...
pub(crate) fn try_matmul_nt_f16_bytes(
    ...
    makepad_ggml::backend::metal::try_matmul_nt_f16_bytes(a, bt_f16_bytes, m, k, n)
}
...
pub(crate) fn try_matmul_nt_ggml_bytes(
    ...
    makepad_ggml::backend::metal::try_matmul_nt_ggml_bytes(a, bt_bytes, bt_ggml_type, m, k, n)
}
...
pub(crate) fn try_matmul_nt_ggml_bytes_add_bias(
    ...
    makepad_ggml::backend::metal::try_matmul_nt_ggml_bytes_add_bias(
        a,
        bt_bytes,
        bt_ggml_type,
        m,
...
pub(crate) fn try_flash_attn_f32_packed(
    ...
    makepad_ggml::backend::metal::try_flash_attn_f32_packed(q, k, v, n_q, n_kv, n_head, d, scale)
}
...
pub(crate) fn clear_decoder_kv_cache() {
    ...
    makepad_ggml::backend::metal::clear_decoder_kv_cache();
}
...
pub(crate) fn try_flash_attn_f32_self_kv_cache(
    ...
    makepad_ggml::backend::metal::try_flash_attn_f32_self_kv_cache(
...
pub(crate) fn try_flash_attn_f32_cross_kv_cache(
    ...
    makepad_ggml::backend::metal::try_flash_attn_f32_cross_kv_cache(
...
pub(crate) fn try_add_f32(
    ...
    makepad_ggml::backend::metal::try_add_f32(a, a_shape, b, b_shape)
}
...
pub(crate) fn try_mul_f32(
    ...
    makepad_ggml::backend::metal::try_mul_f32(a, a_shape, b, b_shape)
}
...
pub(crate) fn try_gelu_f32(a: &[f32], shape: &[usize]) -> Option<Vec<f32>> {
    ...
    makepad_ggml::backend::metal::try_gelu_f32(a, shape)
}
...
pub(crate) fn try_layer_norm_f32(x: &[f32], shape: &[usize], eps: f32) -> Option<Vec<f32>> {
    ...
    makepad_ggml::backend::metal::try_layer_norm_f32(x, shape, eps)
}
...
pub(crate) fn try_layer_norm_mul_add_f32(
    ...
    makepad_ggml::backend::metal::try_layer_norm_mul_add_f32(
        x,
        x_shape,
        mul,
        mul_shape,
...
pub(crate) fn try_im2col_1d_f32(
    ...
    makepad_ggml::backend::metal::try_im2col_1d_f32(input, ic, iw, kw, stride, pad)
}
...
mod imp {
    ...
    const _GGML_METALLIB_BYTES: &[u8] = include_bytes!(env!("MAKEPAD_VOICE2_GGML_METALLIB"));
    ...
    fn build_ggml_source() -> String {
        ...
        let mut src = read_text_with_fallback(
            &[
                "libs/voice2/src/ggml-metal/ggml-metal.metal",
                "src/ggml-metal/ggml-metal.metal",
                "libs/voice2/src/metal/ggml/ggml-metal.metal",
        ...
        let common_h = read_text_with_fallback(
            &[
                "libs/voice2/src/ggml-metal/ggml-common.h",
                "src/ggml-metal/ggml-common.h",
                "libs/voice2/src/metal/ggml/ggml-common.h",
        ...
        let impl_h = read_text_with_fallback(
            &[
                "libs/voice2/src/ggml-metal/ggml-metal-impl.h",
                "src/ggml-metal/ggml-metal-impl.h",
                "libs/voice2/src/metal/ggml/ggml-metal-impl.h",
    ...
    impl MetalContext {
        ...
        fn new() -> Result<Self, String> {
            ...
            let library = match Self::load_library_from_metallib(device.as_id()) {
                ...
                Err(err) => {
                    eprintln!(
                        "[voice2][metal] precompiled metallib load failed, compiling source: {}",
                        err
                    );
                    let source = build_ggml_source();
                    Self::compile_library(device.as_id(), &source)?
            ...
            if super::log_metal_trace() {
                eprintln!("[voice2][metal] backend initialized (ggml kernels)");
            }
        ...
        fn get_or_compile_cached_pipeline(
            ...
            if !self.pipeline_cache.contains_key(&cache_name) {
                if crate::settings::LOG_METAL_PIPELINES {
                    eprintln!("[voice2][metal] compile_pipeline base={}", base_name);
                }
        ...
        fn dispatch_mul_mv_ext(
            ...
            if super::log_metal_trace() && LOG_ONCE.set(()).is_ok() {
                eprintln!("[voice2][metal] mul_mat dispatch: mul_mv_ext");
            }
        ...
        fn dispatch_mul_mm(
            ...
            static LOG_ONCE: OnceLock<()> = OnceLock::new();
            if super::log_metal_trace() && LOG_ONCE.set(()).is_ok() {
                eprintln!("[voice2][metal] mul_mat dispatch: mul_mm");
            }
        ...
        fn dispatch_mul_mv(
            ...
            static LOG_ONCE: OnceLock<()> = OnceLock::new();
            if super::log_metal_trace() && LOG_ONCE.set(()).is_ok() {
                eprintln!("[voice2][metal] mul_mat dispatch: mul_mv");
            }
        ...
        fn matmul_nt_ggml_from_src1_buffer(
            ...
            let (kernel, compute_res) = if used_mul_mv_ext {
                ...
                match self.dispatch_mul_mm(
                    ...
                    Err(e) => ("mul_mv", {
                        eprintln!(
                                "[voice2][metal] mul_mm failed for type {:?}, falling back to mul_mv: {}",
                                src0, e
                            );
            ...
            if super::log_mul_mat_requested() {
                eprintln!(
                    "[voice2][metal] mul_mat kernel={} src0={} src1=f32 ne00={} ne01={} ne11={} ne12=1 nb01={} nb11={}",
                    kernel,
                    src0_type_name(src0),
                    ne00,
                    ne01,
        ...
        CONTEXT.with(|ctx| {
            ...
            if matches!(&*ctx, ContextState::Uninitialized) {
                *ctx = match MetalContext::new() {
                    ...
                    Err(err) => {
                        eprintln!("[voice2][metal] backend disabled: {}", err);
            ...
            match f(ctx) {
                Ok(v) => Some(v),
                Err(err) => {
                    eprintln!("[voice2][metal] compute failed: {}", err);
                    None
                }
            }
        })
