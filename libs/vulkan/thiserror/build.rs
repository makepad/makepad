...
fn main() {
    ...
    // Makepad: only a nightly or dev compiler turns on the unstable generic
    // member access API. Upstream also probes with RUSTC_BOOTSTRAP, but the
    // Makepad Builder sets RUSTC_BOOTSTRAP=1 only so the pinned stable
    // compiler accepts -Zthreads (the parallel frontend); with or without
    // it this crate compiles the same code.
