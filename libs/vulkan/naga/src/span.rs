...
impl<E> WithSpan<E> {
    ...
    fn format_with_path(&self, source: &str, path: &str) -> String
    where
        E: Error,
    {
        ...
        let source = replace_control_chars(source);

        let _ = writeln!(out, "error: {}", self.inner);
        for (span, desc) in self.spans() {
            if !span.is_defined() {
                continue;
            }
            let location = span.location(source.as_ref());
            let _ = writeln!(
                out,
                "  --> {path}:{}:{}: {}",
        ...
        let mut cause: &dyn Error = &self.inner;
        while let Some(next) = Error::source(cause) {
            let _ = writeln!(out, "note: {next}");
            cause = next;
        }
    ...
    where
        E: Error,
    {
        eprintln!("{}", self.format_with_path(source, path));
    }
    ...
    where
        E: Error,
    {
        self.format_with_path(source, path)
    }
}
