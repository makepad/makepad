...
mod imp {
    ...
    struct CudaDenseLinearBackend {
        ...
        blas_lt: makepad_cuda::cublasLtHandle_t,
    ...
    impl CudaDenseLinearBackend {
        fn load() -> Result<Self, String> {
            ...
            let blas_lt = match makepad_cuda::cublas_lt_create() {
                Ok(handle) => handle,
                Err(err) => {
                    let _ = makepad_cuda::cublas_destroy(blas);
                    let _ = makepad_cuda::destroy_stream(stream);
            ...
            F: FnOnce() -> Result<Vec<u8>, String>,
        {
            let half_type = match bt_ggml_type {
                GGML_TYPE_BF16 => makepad_cuda::CUDA_R_16BF,
                GGML_TYPE_F16 => makepad_cuda::CUDA_R_16F,
                ...
                GGML_TYPE_F8_E4M3 => makepad_cuda::CUDA_R_16BF,
                _ => {
                    return Err(format!(
            ...
            if let Some(words) = a_bf16_words {
                ...
                if half_type != makepad_cuda::CUDA_R_16BF {
                    return Err(
            ...
            let prof_on = crate::backend::prof::enabled();
            ...
            crate::backend::prof::record(
                crate::backend::prof::CAT_DENSE_WEIGHT_UPLOAD,
                t_weight,
                0,
            );
            ...
            crate::backend::prof::record(
                crate::backend::prof::CAT_DENSE_UPLOAD,
                t_upload,
                h2d_bytes as u64,
            );
            ...
            crate::backend::prof::record(crate::backend::prof::CAT_DENSE_GEMM, t_gemm, 0);
            ...
            crate::backend::prof::record(
                crate::backend::prof::CAT_DENSE_DOWNLOAD,
                t_download,
    ...
    impl Drop for CudaDenseLinearBackend {
        fn drop(&mut self) {
            let _ = makepad_cuda::cublas_lt_destroy(self.blas_lt);
    ...
    pub fn gpu_perf_stats(reset: bool) -> GpuPerfStats {
        ...
        let (mem_free_bytes, mem_total_bytes) = makepad_cuda::mem_get_info()
    ...
    pub struct GpuStepGraph {
        exec: makepad_cuda::CudaGraphExec,
    }
    ...
    pub fn gpu_graph_capture<T>(
        ...
        let graph = with_dense_linear_backend(|backend| {
            makepad_cuda::end_stream_capture(backend.stream)
    ...
    pub fn gpu_copy_into(src: &GpuTensor, dst: &GpuTensor) -> Result<(), String> {
        ...
        with_dense_linear_backend(|backend| {
            backend.prepare_device()?;
            makepad_cuda::check(unsafe {
                makepad_cuda::cudaMemcpyAsync(
                    dst.device_ptr()?.cast(),
    ...
    fn gpu_prof(stream: cudaStream_t, cat: usize, start: std::time::Instant, bytes: u64) {
        if crate::backend::prof::enabled() {
            let _ = makepad_cuda::synchronize_stream(stream);
            crate::backend::prof::record(cat, start, bytes);
        }
    }
    ...
    pub fn gpu_linear_nt_cached_bf16_mm(
        ...
        with_dense_linear_backend(|backend| {
            ...
            gpu_prof(
                backend.stream,
                crate::backend::prof::CAT_DENSE_TOTAL,
                prof_start,
                0,
            );
            Ok(output)
    ...
    pub fn gpu_linear_nt_cached_bf16_bias_epilogue(
        ...
        with_dense_linear_backend(|backend| {
            ...
            let a_desc = match makepad_cuda::cublas_lt_matrix_layout_create(
                makepad_cuda::CUDA_R_16BF,
                k as u64,
                n as u64,
                k as i64,
            ...
            let b_desc = match makepad_cuda::cublas_lt_matrix_layout_create(
                makepad_cuda::CUDA_R_16BF,
                k as u64,
                m as u64,
                k as i64,
                ...
                Ok(desc) => desc,
                Err(error) => {
                    let _ = makepad_cuda::cublas_lt_matrix_layout_destroy(a_desc);
                    let _ = makepad_cuda::cublas_lt_matmul_desc_destroy(operation);
                    return Err(format!("cuBLASLt B layout create failed: {error}"));
                }
            };
            let c_desc = match makepad_cuda::cublas_lt_matrix_layout_create(
                makepad_cuda::CUDA_R_16BF,
                n as u64,
                m as u64,
                n as i64,
                ...
                Ok(desc) => desc,
                Err(error) => {
                    let _ = makepad_cuda::cublas_lt_matrix_layout_destroy(b_desc);
                    let _ = makepad_cuda::cublas_lt_matrix_layout_destroy(a_desc);
                    let _ = makepad_cuda::cublas_lt_matmul_desc_destroy(operation);
                    return Err(format!("cuBLASLt C layout create failed: {error}"));
                }
            };
            let preference = match makepad_cuda::cublas_lt_matmul_preference_create() {
                Ok(desc) => desc,
                Err(error) => {
                    let _ = makepad_cuda::cublas_lt_matrix_layout_destroy(c_desc);
                    let _ = makepad_cuda::cublas_lt_matrix_layout_destroy(b_desc);
                    let _ = makepad_cuda::cublas_lt_matrix_layout_destroy(a_desc);
                    let _ = makepad_cuda::cublas_lt_matmul_desc_destroy(operation);
            ...
            let run = (|| -> Result<(), String> {
                ...
                let epilogue = makepad_cuda::CUBLASLT_EPILOGUE_BIAS;
                ...
                makepad_cuda::cublas_lt_matmul_desc_set_attribute(
                    operation,
                    makepad_cuda::CUBLASLT_MATMUL_DESC_EPILOGUE,
                    &epilogue,
                )
                .map_err(|error| format!("cuBLASLt epilogue attribute failed: {error}"))?;
                makepad_cuda::cublas_lt_matmul_desc_set_attribute(
                    operation,
                    makepad_cuda::CUBLASLT_MATMUL_DESC_BIAS_POINTER,
                    &bias_ptr,
                )
                ...
                for (attribute, pointer) in [
                    (
                        makepad_cuda::CUBLASLT_MATMUL_PREF_MIN_ALIGNMENT_A_BYTES,
                        weight.ptr.as_ptr().cast_const(),
                    ),
                    (
                        makepad_cuda::CUBLASLT_MATMUL_PREF_MIN_ALIGNMENT_B_BYTES,
                        input_bf16.ptr.as_ptr().cast_const(),
                    ),
                    (
                        makepad_cuda::CUBLASLT_MATMUL_PREF_MIN_ALIGNMENT_C_BYTES,
                        output_bf16.ptr.as_ptr().cast_const(),
                    ),
                    (
                        makepad_cuda::CUBLASLT_MATMUL_PREF_MIN_ALIGNMENT_D_BYTES,
                    ...
                    let alignment = makepad_cuda::cublas_lt_pointer_alignment(pointer);
                    makepad_cuda::cublas_lt_matmul_preference_set_attribute(
                        preference,
                        attribute,
                        &alignment,
            ...
            let _ = makepad_cuda::cublas_lt_matmul_preference_destroy(preference);
            let _ = makepad_cuda::cublas_lt_matrix_layout_destroy(c_desc);
            let _ = makepad_cuda::cublas_lt_matrix_layout_destroy(b_desc);
            let _ = makepad_cuda::cublas_lt_matrix_layout_destroy(a_desc);
            let _ = makepad_cuda::cublas_lt_matmul_desc_destroy(operation);
    ...
    fn gpu_linear_nt_impl(
        ...
        with_dense_linear_backend(|backend| {
            ...
            let native_type = match parts.first().map(|part| part.bt_ggml_type) {
                Some(GGML_TYPE_BF16) => makepad_cuda::CUDA_R_16BF,
                Some(GGML_TYPE_F16) => makepad_cuda::CUDA_R_16F,
                ...
                Some(ty) if gpu_quant_linear_type_supported(ty) => makepad_cuda::CUDA_R_16BF,
                other => {
            ...
            for part in parts {
                ...
                if f16acc {
                    ...
                    unsafe {
                        makepad_cuda::cublas_gemm_strided_batched_ex_raw(
                            backend.blas,
                            makepad_cuda::CUBLAS_OP_T,
                            makepad_cuda::CUBLAS_OP_N,
                            part.n as i32,
                            ...
                            0,
                            1,
                            makepad_cuda::CUBLAS_COMPUTE_16F,
                            makepad_cuda::CUBLAS_GEMM_DEFAULT,
                        )
                        .map_err(|err| {
                            format!(
    ...
    pub fn gpu_layer_norm_mul_add(
        ...
        with_dense_linear_backend(|backend| {
            ...
            gpu_prof(backend.stream, crate::backend::prof::CAT_LAYER_NORM, prof_start, 0);
            Ok(out)
        })
    }
    ...
    pub fn gpu_layer_norm_mul_add_grouped(
        ...
        with_dense_linear_backend(|backend| {
            ...
            gpu_prof(
                backend.stream,
                crate::backend::prof::CAT_LAYER_NORM,
                prof_start,
                0,
            );
            Ok(out)
    ...
    pub fn gpu_gated_residual_mod(
        ...
        with_dense_linear_backend(|backend| {
            ...
            gpu_check(status)?;
            gpu_prof(backend.stream, crate::backend::prof::CAT_ELEMENTWISE, prof_start, 0);
            Ok(out)
        })
    }
    ...
    pub fn gpu_rms_norm_mul(
        ...
        with_dense_linear_backend(|backend| {
            ...
            gpu_check(status)?;
            gpu_prof(backend.stream, crate::backend::prof::CAT_RMS_NORM, prof_start, 0);
            Ok(out)
        })
    }
    ...
    pub fn gpu_attention_packed_softcap(
        ...
        with_dense_linear_backend(|backend| {
            ...
            gpu_prof(backend.stream, crate::backend::prof::CAT_FLASH_ATTN, prof_start, 0);
            Ok(out)
        })
    }
    ...
    pub fn gpu_sparse_conv27(
        ...
        with_dense_linear_backend(|backend| {
            ...
            gpu_prof(backend.stream, crate::backend::prof::CAT_DENSE_GEMM, prof_start, 0);
            Ok(out)
        })
    }
    ...
    pub fn gpu_attention_packed_flash2_d64(
        ...
        with_dense_linear_backend(|backend| {
            ...
            gpu_prof(
                backend.stream,
                crate::backend::prof::CAT_FLASH_ATTN,
                prof_start,
                0,
            );
            Ok(out)
    ...
    pub fn gpu_runtime_trim() -> Result<(), String> {
        let result = DENSE_LINEAR_BACKEND.with(|slot| {
            ...
            backend.prepare_device()?;
            let sync = makepad_cuda::synchronize_stream(backend.stream)
    ...
    fn gpu_add_f16(a: &GpuTensor, b: &GpuTensor) -> Result<GpuTensor, String> {
        ...
        with_dense_linear_backend(|backend| {
            ...
            .map_err(|err| err.to_string())?;
            makepad_cuda::cudnn::add_same_f16(
    ...
    pub fn gpu_concat_rows_many(parts: &[&GpuTensor]) -> Result<GpuTensor, String> {
        ...
        with_dense_linear_backend(|backend| {
            ...
            gpu_prof(
                backend.stream,
                crate::backend::prof::CAT_ELEMENTWISE,
                prof_start,
                0,
            );
            Ok(out)
    ...
    fn gpu_transform_planar_nchw(
        ...
        if !makepad_cuda::cudnn::available() {
            return Err("cuDNN unavailable for planar↔NCHW transform".into());
        }
        let dtype = if x.half {
            makepad_cuda::cudnn::CUDNN_DATA_HALF
        } else {
            makepad_cuda::cudnn::CUDNN_DATA_FLOAT
        };
        with_dense_linear_backend(|backend| {
            backend.prepare_device()?;
            let out = if to_nchw {
            ...
            makepad_cuda::cudnn::transform_planar_nchw(
                batch as i32,
                channels as i32,
                height as i32,
                width as i32,
    ...
    pub fn gpu_nchw_group_norm(
        ...
        if !makepad_cuda::cudnn::group_norm_available() {
        ...
        with_dense_linear_backend(|backend| {
            ...
            makepad_cuda::cudnn::group_norm_nchw_f32(
                batch as i32,
                channels as i32,
                height as i32,
                width as i32,
            ...
            gpu_prof(
                backend.stream,
                crate::backend::prof::CAT_GROUP_NORM,
                prof_start,
                0,
            );
            Ok(out)
    ...
    pub fn gpu_nchw_to_tokens(
        ...
        with_dense_linear_backend(|backend| {
            ...
            makepad_cuda::cudnn::transform_nchw_nhwc(
                batch as i32,
                channels as i32,
                height as i32,
                width as i32,
    ...
    pub fn gpu_nhwc_add_channel(
        ...
        with_dense_linear_backend(|backend| {
            ...
            .map_err(|err| err.to_string())?;
            makepad_cuda::cudnn::add_bias_nhwc_f16(
    ...
    pub fn gpu_nchw_add_channel(
        ...
        with_dense_linear_backend(|backend| {
            ...
            .map_err(|err| err.to_string())?;
            if x.half {
                makepad_cuda::cudnn::add_bias_nchw_packed_f16(
                    bias_use.storage_ptr()?.cast(),
                    out.storage_ptr()?.cast(),
                    batch as i32,
                    channels as i32,
                ...
                makepad_cuda::cudnn::add_bias_nchw_f32(
        ...
        with_dense_linear_backend(|backend| {
            ...
            CUDNN_DESCS.with(|cell: &RefCell<CudnnDescMap>| {
                ...
                if !map.contains_key(&key) {
                    let desc = makepad_cuda::cudnn::prepare_nchw_contiguous(
                        batch as i32,
                        channels as i32,
                        out_channels as i32,
                        height as i32,
                    ...
                    PACKED_LOG.call_once(|| {
                        eprintln!(
                            ...
                            makepad_cuda::cudnn::algo_id(&desc),
                            makepad_cuda::cudnn::math_type(&desc)
                        );
                    });
                    map.insert(key, desc);
                }
                let desc = map.get(&key).expect("just inserted");
                let ws_bytes = makepad_cuda::cudnn::workspace_bytes(desc);
                ...
                makepad_cuda::cudnn::convolution_forward_f16(
                    desc,
                    x.storage_ptr()?.cast(),
                    weight_ptr,
                    out.storage_ptr()?.cast(),
                ...
                makepad_cuda::cudnn::add_bias_nchw_from_desc(
                    desc,
                    bias_ptr,
                    if x.half {
                        out.storage_ptr()?.cast()
                ...
                gpu_prof(backend.stream, crate::backend::prof::CAT_CONV2D, prof_start, 0);
                Ok(out)
            })
        })
    }
    ...
    pub fn gpu_conv2d_nhwc_ex(
        ...
        if !x.half || !makepad_cuda::cudnn::available() {
        ...
        with_dense_linear_backend(|backend| {
            ...
            let key = (
                ...
                stride_x as u32,
                0x10u32,
                makepad_cuda::cudnn::CUDNN_DATA_HALF as u32,
            );
            CUDNN_DESCS.with(|cell: &RefCell<CudnnDescMap>| {
                let mut map = cell.borrow_mut();
                if !map.contains_key(&key) {
                    let desc = makepad_cuda::cudnn::prepare_nhwc_ex(
                        batch as i32,
                        in_channels as i32,
                        out_channels as i32,
                        height as i32,
    ...
    pub fn gpu_conv2d_nchw_ex(
        ...
        if batch > 1 && gpu_conv_gemm_enabled() && makepad_cuda::cudnn::available() {
    ...
    type CudnnDescMap = HashMap<CudnnDescKey, makepad_cuda::cudnn::ConvDesc>;
    thread_local! {
    ...
    fn gpu_conv2d_nchw_cudnn(
        ...
        with_dense_linear_backend(|backend| {
            ...
            CUDNN_DESCS.with(|cell: &RefCell<CudnnDescMap>| {
                let mut map = cell.borrow_mut();
                if !map.contains_key(&key) {
                    let desc = makepad_cuda::cudnn::prepare_nchw_strided_ex(
                        batch as i32,
                        in_channels as i32,
                        out_channels as i32,
                        height as i32,
                ...
                if half {
                    ...
                    makepad_cuda::cudnn::add_bias_nchw_f16(
                        bias_buf.ptr.as_ptr(),
                        out.device_ptr_u16()?.cast(),
                        batch as i32,
                        out_channels as i32,
    ...
    pub fn gpu_realesrgan_conv3x3_f16(
        ...
        with_dense_linear_backend(|backend| {
            ...
            CUDNN_DESCS.with(|cell: &RefCell<CudnnDescMap>| {
                let mut map = cell.borrow_mut();
                if !map.contains_key(&key) {
                    let desc = makepad_cuda::cudnn::prepare_nchw_strided_f16(
                        1,
                        in_channels as i32,
                        out_channels as i32,
                        height as i32,
    ...
    pub fn gpu_realesrgan_conv3x3_f32(
        ...
        with_dense_linear_backend(|backend| {
            ...
            let key = (
                ...
                1u32,
                0x32u32,
                makepad_cuda::cudnn::CUDNN_DATA_FLOAT as u32,
            );
            CUDNN_DESCS.with(|cell: &RefCell<CudnnDescMap>| -> Result<(), String> {
                let mut map = cell.borrow_mut();
                if !map.contains_key(&key) {
                    let desc = makepad_cuda::cudnn::prepare_nchw_strided_f32_fma(
                        1,
                        in_channels as i32,
                        out_channels as i32,
                        height as i32,
    ...
    pub fn gpu_group_norm_nchw(
        ...
        let gn_result = with_dense_linear_backend(|backend| {
            ...
            let gn = makepad_cuda::cudnn::group_norm_nchw(
                batch as i32,
                channels as i32,
                height as i32,
                width as i32,
            ...
            if let Err(err) = gn {
                ...
                makepad_cuda::cudnn::group_norm_bn_nchw(
                    batch as i32,
                    channels as i32,
                    height as i32,
                    width as i32,
    ...
    pub fn gpu_group_norm_planar(
        ...
        with_dense_linear_backend(|backend| {
            ...
            gpu_prof(backend.stream, crate::backend::prof::CAT_GROUP_NORM, prof_start, 0);
            Ok(out)
        })
    }
    ...
    pub fn gpu_attention_planar_single(
        ...
        with_dense_linear_backend(|backend| {
            ...
            gpu_pool_release(scores);
            gpu_prof(backend.stream, crate::backend::prof::CAT_ATTN_SOFTMAX_WS, prof_start, 0);
            Ok(out)
        })
    }
