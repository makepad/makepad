...
pub fn shell_child_wait(child: ShellChild) -> Result<String, String> {
    let r = child.child.wait_with_output().map_err(|e| {
        format!(
            "Process {} in dir {:?} returned error {:?} ",
            child.cmd, child.cwd, e
        )
    })?;
    let stderr = std::str::from_utf8(&r.stderr).unwrap_or("could not decode utf8");
    let stdout = std::str::from_utf8(&r.stdout).unwrap_or("could not decode utf8");
...
pub fn shell_env_cap_split(
    ...
    if let Err(e) = r {
        return (
            "".to_string(),
            format!("Wait with output failed for process {}", e),
            false,
        );
    }
    let r = r.unwrap();
    let stderr = std::str::from_utf8(&r.stderr)
        .unwrap_or("could not decode utf8")
        .to_string();
    let stdout = std::str::from_utf8(&r.stdout)
        .unwrap_or("could not decode utf8")
        .to_string();
...
pub fn rmdir(path: &Path) -> Result<(), String> {
    match fs::remove_dir_all(path) {
        Err(e) => Err(format!("rmdir {:?} failed {:?}", path, e)),
        Ok(()) => Ok(()),
    }
}
...
pub fn rm(path: &Path) -> Result<(), String> {
    match fs::remove_file(path) {
        Err(e) => Err(format!("remove_file {:?} failed {:?}", path, e)),
        Ok(()) => Ok(()),
    }
}
...
pub fn cp(source_path: &Path, dest_path: &Path, exec: bool) -> Result<(), String> {
    let data =
        fs::read(source_path).map_err(|_e| format!("Cant open input file {:?}", source_path))?;
    mkdir(dest_path.parent().unwrap())?;
    let mut output =
        File::create(dest_path).map_err(|_e| format!("Cant open output file {:?}", dest_path))?;
    output
        .write(&data)
...
pub fn walk_all(
    source_dir: &Path,
    dest_dir: &Path,
    cb: &mut dyn Fn(&Path, &Path) -> Result<(), String>,
) -> Result<(), String> {
...
fn ls_recursive(dir: &Path, prefix: &Path, result: &mut Vec<PathBuf>) -> Result<(), String> {
    for entry in
        fs::read_dir(dir).map_err(|_e| format!("Unable to read source directory {:?}", dir))?
    {
        let entry = entry.map_err(|_e| format!("Unable to process directory entry"))?;
        let source_path = entry.path();
        if source_path.is_file() {
