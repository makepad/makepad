...
struct ProjectionBuffers {
    ...
    weight_name: String,
    weight_buf: makepad_ggml::backend::metal::MetalBuffer,
    scales_buf: makepad_ggml::backend::metal::MetalBuffer,
    biases_buf: makepad_ggml::backend::metal::MetalBuffer,
    args: MlxAffineQprojRowArgs,
    out_buf: makepad_ggml::backend::metal::MetalBuffer,
    out_len: usize,
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    while let Some(arg) = args_iter.next() {
        match arg.as_str() {
            ...
            "-h" | "--help" => {
                eprintln!(
                    "Usage: metal_rms_norm_qkv_row [model.safetensors] [--warmup N] [--iters N]"
                );
                return Ok(());
            }
    ...
    for oracle in [Q_PROJ_ORACLE, K_PROJ_ORACLE, V_PROJ_ORACLE] {
        let (weight_name, scales_name, biases_name) = projection_tensor_names(oracle.prefix);
        let weight_bytes = header.read_tensor_bytes(&weight_name)?;
        let scales_bytes = header.read_tensor_bytes(&scales_name)?;
        let biases_bytes = header.read_tensor_bytes(&biases_name)?;
        let weight_entry = header
            .tensor(&weight_name)
            .ok_or("missing projection weight entry")?;
        let scales_entry = header
            .tensor(&scales_name)
            .ok_or("missing projection scales entry")?;
        let out_len = usize::try_from(weight_entry.shape[0])?;
        ...
        projections.push(ProjectionBuffers {
            ...
            out_buf: runtime.create_buffer(out_len * 2, BufferStorageMode::Private)?,
            out_len,
        });
    }
    ...
    let projection_bindings = projections
        .iter()
        .map(|p| {
            (
                bytes_of(&p.args).to_vec(),
                [
                    MetalBufferBindingRef {
                        index: 1,
                        buffer: &h_buf,
                ...
                MetalSize {
                    width: 1,
                    height: (p.out_len as u64).div_ceil(8),
                    depth: 1,
                },
                MetalSize {
                    width: 32,
    ...
    for p in &projections {
        let out_bytes = runtime.read_buffer(&p.out_buf, p.out_len * 2)?;
        ...
        if out_bits[..16] != p.oracle.expected_first16_bits {
            return Err(format!(
                "{} first16 bits mismatch: got {:08X?} expected {:08X?}",
                p.oracle.prefix,
                &out_bits[..16],
                p.oracle.expected_first16_bits
            )
            .into());
        }
        ...
        println!("{}_weight_name={}", p.oracle.prefix, p.weight_name);
        println!("{}_out_len={}", p.oracle.prefix, p.out_len);
        println!("{}_full_row_fnv1a64=0x{hash:016X}", p.oracle.prefix);
        print!("{}_first16_f32_bits=", p.oracle.prefix);
