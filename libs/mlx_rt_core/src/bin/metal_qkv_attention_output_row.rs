...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    while let Some(arg) = args_iter.next() {
        match arg.as_str() {
            ...
            "-h" | "--help" => {
                eprintln!(
                    "Usage: metal_qkv_attention_output_row [model.safetensors] [--warmup N] [--iters N]"
                );
                return Ok(());
            }
    ...
    println!("v_weight_name={}", V_PATH_ORACLE.weight_name);
    println!("eps={EPS}");
    ...
    print!("attention_output_first16_f32_bits=");
    for (index, bits) in attn_out_bits.iter().take(16).enumerate() {
        if index != 0 {
            print!(",");
        }
        print!("0x{bits:08X}");
