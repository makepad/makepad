...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let post_feedforward_norm_len = usize::try_from(post_feedforward_norm_weight_entry.shape[0])?;
    ...
    let pre_feedforward_norm_weight_buf = runtime.create_buffer_with_bytes(
        &pre_feedforward_norm_weight_bytes,
        BufferStorageMode::Private,
    )?;
    let post_feedforward_norm_weight_buf = runtime.create_buffer_with_bytes(
        &post_feedforward_norm_weight_bytes,
        BufferStorageMode::Private,
    )?;
    ...
    let post_feedforward_norm_bindings = [
        ...
        MetalBufferBindingRef {
            index: 2,
            buffer: &post_feedforward_norm_weight_buf,
            offset_bytes: 0,
        },
        MetalBufferBindingRef {
            index: 3,
            buffer: &post_feedforward_norm_out_buf,
            offset_bytes: 0,
        },
    ];
    ...
    let run_once = || -> Result<(), Box<dyn Error>> {
        ...
        if post_feedforward_norm {
            ...
            runtime.dispatch_compute(
                &rms_pipeline,
                rms_args_bytes,
                &post_feedforward_norm_bindings,
                &[],
                rms_threadgroups,
                rms_threads_per_threadgroup,
            )?;
    ...
    let post_attention_norm_bits = decode_bits(
        runtime.read_buffer(&post_attention_norm_out_buf, post_attention_norm_len * 2)?,
    );
    let residual_bits =
        decode_bits(runtime.read_buffer(&residual_out_buf, post_attention_norm_len * 2)?);
    let pre_feedforward_norm_bits = decode_bits(
        runtime.read_buffer(&pre_feedforward_norm_out_buf, pre_feedforward_norm_len * 2)?,
    );
    let mlp_gate_bits = decode_bits(runtime.read_buffer(&mlp_gate_out_buf, mlp_gate_out_len * 2)?);
    let mlp_up_bits = decode_bits(runtime.read_buffer(&mlp_up_out_buf, mlp_up_out_len * 2)?);
    let geglu_bits = decode_bits(runtime.read_buffer(&geglu_out_buf, mlp_gate_out_len * 2)?);
    let mlp_down_bits = decode_bits(runtime.read_buffer(&mlp_down_out_buf, mlp_down_out_len * 2)?);
    ...
    let final_residual_bits = if final_residual {
        decode_bits(runtime.read_buffer(&final_residual_out_buf, post_feedforward_norm_len * 2)?)
    } else {
        Vec::new()
    };
    let (
    ...
    let final_residual_first16 = &final_residual_bits[..final_residual_bits.len().min(16)];
    ...
    if post_feedforward_norm {
        println!("post_feedforward_norm_weight_name={selected_post_feedforward_norm_weight_name}");
    }
