...
const POST_FEEDFORWARD_NORM_WEIGHT_NAME: &str =
...
const EXPECTED_PRE_FEEDFORWARD_DOWN_HASH: u64 = 0xEC78_BADF_1A26_0AA5;
...
const EXPECTED_POST_FEEDFORWARD_NORM_HASH: u64 = 0x8FF3_5580_0D94_5DD4;
const EXPECTED_POST_FEEDFORWARD_NORM_FIRST16_BITS: [u32; 16] = [
    0xC1CF_0000,
    0x41AA_0000,
    0xC12D_0000,
    0x4053_0000,
...
const EXPECTED_POST_FEEDFORWARD_RESIDUAL_HASH: u64 = 0x575B_ECCC_1974_6AD4;
const EXPECTED_POST_FEEDFORWARD_RESIDUAL_FIRST16_BITS: [u32; 16] = [
    0xC1D6_0000,
    0x419F_0000,
    0xC138_0000,
    0x4040_0000,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if moe_post_ffn_norm1 && final_residual {
        return Err("--moe-post-ffn-norm1 cannot be combined with --final-residual".into());
    }
    let debug_row_index = analyze_row.unwrap_or(1_836);
    let selected_post_feedforward_norm_weight_name = if moe_post_ffn_norm1 {
        POST_FEEDFORWARD_NORM1_WEIGHT_NAME
    } else {
        POST_FEEDFORWARD_NORM_WEIGHT_NAME
    };
    let expected_post_feedforward_norm_hash = if moe_post_ffn_norm1 {
        EXPECTED_MOE_POST_FEEDFORWARD_NORM1_HASH
    } else {
        EXPECTED_POST_FEEDFORWARD_NORM_HASH
    };
    let expected_post_feedforward_norm_first16 = if moe_post_ffn_norm1 {
        &EXPECTED_MOE_POST_FEEDFORWARD_NORM1_FIRST16_BITS
    } else {
        &EXPECTED_POST_FEEDFORWARD_NORM_FIRST16_BITS
    };
    ...
    let post_feedforward_norm_weight_bytes =
        header.read_tensor_bytes(selected_post_feedforward_norm_weight_name)?;
    ...
    let post_feedforward_norm_weight_entry = header
        .tensor(selected_post_feedforward_norm_weight_name)
        .ok_or("missing post-feedforward layernorm weight entry")?;
    ...
    let post_feedforward_norm_len =
        usize::try_from(post_feedforward_norm_weight_entry.shape[0])?;
    ...
    if post_feedforward_norm_len != mlp_down_out_len {
        return Err(format!(
            "invalid post-feedforward layernorm length: got {} expected {}",
            post_feedforward_norm_len, mlp_down_out_len
        )
        .into());
    }
    ...
    let post_feedforward_norm_weight_buf =
        runtime.create_buffer_with_bytes(&post_feedforward_norm_weight_bytes, BufferStorageMode::Private)?;
    ...
    let post_feedforward_norm_out_buf =
        runtime.create_buffer(post_feedforward_norm_len * 2, BufferStorageMode::Private)?;
    let final_residual_out_buf =
        runtime.create_buffer(post_feedforward_norm_len * 2, BufferStorageMode::Private)?;
    ...
    let mlp_down_bindings = [
        MetalBufferBindingRef { index: 1, buffer: &geglu_out_buf, offset_bytes: 0 },
    ...
    let post_feedforward_norm_bindings = [
        MetalBufferBindingRef { index: 1, buffer: &mlp_down_out_buf, offset_bytes: 0 },
        MetalBufferBindingRef { index: 2, buffer: &post_feedforward_norm_weight_buf, offset_bytes: 0 },
        MetalBufferBindingRef { index: 3, buffer: &post_feedforward_norm_out_buf, offset_bytes: 0 },
    ];
    let final_residual_bindings = [
        MetalBufferBindingRef { index: 1, buffer: &residual_out_buf, offset_bytes: 0 },
        MetalBufferBindingRef { index: 2, buffer: &post_feedforward_norm_out_buf, offset_bytes: 0 },
        MetalBufferBindingRef { index: 3, buffer: &final_residual_out_buf, offset_bytes: 0 },
    ];
    ...
    let run_once = || -> Result<(), Box<dyn Error>> {
        ...
        runtime.memory_barrier_buffers()?;
        runtime.dispatch_compute(&proj_pipeline, mlp_down_args_bytes, &mlp_down_bindings, &[], mlp_down_threadgroups, mlp_down_threads_per_threadgroup)?;
        if post_feedforward_norm {
            runtime.memory_barrier_buffers()?;
            runtime.dispatch_compute(&rms_pipeline, rms_args_bytes, &post_feedforward_norm_bindings, &[], rms_threadgroups, rms_threads_per_threadgroup)?;
            if final_residual {
                runtime.memory_barrier_buffers()?;
                runtime.dispatch_compute(&residual_pipeline, residual_args_bytes, &final_residual_bindings, &[], residual_threadgroups, residual_threads_per_threadgroup)?;
            }
        }
    ...
    let mlp_down_bits =
        decode_bits(runtime.read_buffer(&mlp_down_out_buf, mlp_down_out_len * 2)?);
    let post_feedforward_norm_bits = if post_feedforward_norm {
        decode_bits(runtime.read_buffer(
            &post_feedforward_norm_out_buf,
            post_feedforward_norm_len * 2,
        )?)
    } else {
        Vec::new()
    };
    let final_residual_bits = if final_residual {
        decode_bits(runtime.read_buffer(
            &final_residual_out_buf,
            post_feedforward_norm_len * 2,
        )?)
    } else {
        Vec::new()
    ...
    let mlp_down_hash = fnv1a64_u32_words(&mlp_down_bits);
    let post_feedforward_norm_hash = if post_feedforward_norm {
        fnv1a64_u32_words(&post_feedforward_norm_bits)
    } else {
        0
    };
    let final_residual_hash = if final_residual {
        fnv1a64_u32_words(&final_residual_bits)
    } else {
        0
    };
    ...
    let mlp_down_first16 = &mlp_down_bits[..mlp_down_bits.len().min(16)];
    let post_feedforward_norm_first16 =
        &post_feedforward_norm_bits[..post_feedforward_norm_bits.len().min(16)];
    let final_residual_first16 =
        &final_residual_bits[..final_residual_bits.len().min(16)];
    ...
    if dump_all_f32_bits {
        ...
        println!();
        print!("all_f32_bits=");
        let final_bits = if final_residual {
            &final_residual_bits
        } else if post_feedforward_norm {
            &post_feedforward_norm_bits
        } else {
            &mlp_down_bits
        };
        for (index, bits) in final_bits.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
    ...
    if post_feedforward_norm {
        println!(
            "post_feedforward_norm_weight_name={selected_post_feedforward_norm_weight_name}"
        );
    }
    println!("mlp_gate_weight_name={MLP_GATE_WEIGHT_NAME}");
    println!("mlp_up_weight_name={MLP_UP_WEIGHT_NAME}");
    println!("mlp_down_weight_name={MLP_DOWN_WEIGHT_NAME}");
    println!("eps={EPS}");
    ...
    println!("attention_pre_ffn_down_fnv1a64=0x{mlp_down_hash:016X}");
    if post_feedforward_norm {
        println!("attention_post_ffn_norm_fnv1a64=0x{post_feedforward_norm_hash:016X}");
    }
    if final_residual {
        println!("attention_post_ffn_residual_fnv1a64=0x{final_residual_hash:016X}");
    }
    ...
    println!();
    print!("attention_pre_ffn_down_first16_f32_bits=");
    for (index, bits) in mlp_down_first16.iter().enumerate() {
        if index != 0 {
            print!(",");
        }
        print!("0x{bits:08X}");
    ...
    println!();
    if post_feedforward_norm {
        print!("attention_post_ffn_norm_first16_f32_bits=");
        for (index, bits) in post_feedforward_norm_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
    ...
    if final_residual {
        print!("attention_post_ffn_residual_first16_f32_bits=");
        for (index, bits) in final_residual_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
