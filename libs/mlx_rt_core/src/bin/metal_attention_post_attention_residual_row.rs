...
use makepad_mlx_rt_core::{
    fnv1a64_u32_words, gemma4_qproj_case_input_bf16_words, MlxRouterTopKOutput,
    MlxSafetensorsHeader,
};
use std::env;
use std::error::Error;
...
const PRE_FEEDFORWARD_NORM1_WEIGHT_NAME: &str =
    "language_model.model.layers.0.pre_feedforward_layernorm.weight";
const POST_FEEDFORWARD_NORM1_WEIGHT_NAME: &str =
...
const EXPECTED_MOE_POST_FEEDFORWARD_NORM1_HASH: u64 = 0xA46D_73E3_A5AB_7FFA;
const EXPECTED_MOE_POST_FEEDFORWARD_NORM1_FIRST16_BITS: [u32; 16] = [
    0xBCA2_0000,
    0x3E07_0000,
    0xBF12_0000,
    0x3E8F_0000,
...
const ROUTER_PER_EXPERT_SCALE_NAME: &str = "language_model.model.layers.0.router.per_expert_scale";
...
const PRE_FEEDFORWARD_NORM2_WEIGHT_NAME: &str =
    "language_model.model.layers.0.pre_feedforward_layernorm_2.weight";
const POST_FEEDFORWARD_NORM2_WEIGHT_NAME: &str =
    "language_model.model.layers.0.post_feedforward_layernorm_2.weight";
const EXPERT_GATE_WEIGHT_NAME: &str =
    "language_model.model.layers.0.experts.switch_glu.gate_proj.weight";
const EXPERT_GATE_SCALES_NAME: &str =
    "language_model.model.layers.0.experts.switch_glu.gate_proj.scales";
const EXPERT_GATE_BIASES_NAME: &str =
    "language_model.model.layers.0.experts.switch_glu.gate_proj.biases";
const EXPERT_UP_WEIGHT_NAME: &str =
    "language_model.model.layers.0.experts.switch_glu.up_proj.weight";
const EXPERT_UP_SCALES_NAME: &str =
    "language_model.model.layers.0.experts.switch_glu.up_proj.scales";
const EXPERT_UP_BIASES_NAME: &str =
    "language_model.model.layers.0.experts.switch_glu.up_proj.biases";
const EXPERT_DOWN_WEIGHT_NAME: &str =
    "language_model.model.layers.0.experts.switch_glu.down_proj.weight";
const EXPERT_DOWN_SCALES_NAME: &str =
    "language_model.model.layers.0.experts.switch_glu.down_proj.scales";
const EXPERT_DOWN_BIASES_NAME: &str =
    "language_model.model.layers.0.experts.switch_glu.down_proj.biases";
const ROUTER_TOP_K: usize = 8;
const EXPECTED_ROUTER_SCALED_HASH: u64 = 0x8253_E14E_67A9_3FA6;
const EXPECTED_ROUTER_EXPERT_SCORES_HASH: u64 = 0x90EF_C2D7_85D0_3A74;
const EXPECTED_ROUTER_PROBS_HASH: u64 = 0xF774_0024_1F5D_476F;
const EXPECTED_ROUTER_TOPK_INDICES_HASH: u64 = 0x5308_58C0_415C_F0D8;
const EXPECTED_ROUTER_TOPK_WEIGHTS_HASH: u64 = 0x082A_2172_B067_6CC9;
...
const EXPECTED_ROUTER_TOPK_INDICES: [u32; ROUTER_TOP_K] = [51, 91, 3, 1, 127, 21, 5, 40];
...
const EXPECTED_MOE_EXPERT_GATE_HASH: u64 = 0xFABE_4C80_0013_0864;
const EXPECTED_MOE_EXPERT_UP_HASH: u64 = 0x5E63_11E7_B7AC_7D2D;
const EXPECTED_MOE_EXPERT_GEGLU_HASH: u64 = 0xC617_96CC_7790_7265;
const EXPECTED_MOE_EXPERT_DOWN_HASH: u64 = 0xEA84_5FF9_D8A1_59D3;
...
fn widened_bits_to_bf16_words(bits: &[u32]) -> Vec<u16> {
    bits.iter().map(|bits| (bits >> 16) as u16).collect()
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut moe_expert_gate = false;
    let mut moe_expert_up = false;
    let mut moe_expert_geglu = false;
    let mut moe_expert_down = false;
    let mut moe_experts = false;
    let mut moe_post_ffn_norm2 = false;
    let mut moe_merge = false;
    ...
    while let Some(arg) = args_iter.next() {
        match arg.as_str() {
            ...
            "--moe-expert-gate" => {
                moe_expert_gate = true;
            }
            "--moe-expert-up" => {
                moe_expert_up = true;
            }
            "--moe-expert-geglu" => {
                moe_expert_geglu = true;
            }
            "--moe-expert-down" => {
                moe_expert_down = true;
            }
            "--moe-experts" => {
                moe_experts = true;
            }
            "--moe-post-ffn-norm2" => {
                moe_post_ffn_norm2 = true;
            }
            "--moe-merge" => {
                moe_merge = true;
            }
            ...
            "-h" | "--help" => {
                eprintln!(
                    "Usage: metal_attention_post_attention_residual_row [model.safetensors] [--warmup N] [--iters N] [--dump-all-f32-bits] [--router] [--moe-expert-gate] [--moe-expert-up] [--moe-expert-geglu] [--moe-expert-down] [--moe-experts] [--moe-post-ffn-norm2] [--moe-merge] [--analyze-row-1836] [--analyze-row N]"
                );
                return Ok(());
            }
    ...
    let pre_feedforward_norm1_weight_bytes =
        header.read_tensor_bytes(PRE_FEEDFORWARD_NORM1_WEIGHT_NAME)?;
    let post_feedforward_norm1_weight_bytes =
        header.read_tensor_bytes(POST_FEEDFORWARD_NORM1_WEIGHT_NAME)?;
    ...
    let mlp_down_weight_bytes = header.read_tensor_bytes(MLP_DOWN_WEIGHT_NAME)?;
    ...
    let router_scale_words = header.read_bf16_tensor_words(ROUTER_SCALE_NAME)?;
    let router_per_expert_scale_words =
        header.read_bf16_tensor_words(ROUTER_PER_EXPERT_SCALE_NAME)?;
    let router_proj_weight_bytes = header.read_tensor_bytes(ROUTER_PROJ_WEIGHT_NAME)?;
    let router_proj_scales_bytes = header.read_tensor_bytes(ROUTER_PROJ_SCALES_NAME)?;
    let router_proj_biases_bytes = header.read_tensor_bytes(ROUTER_PROJ_BIASES_NAME)?;
    let pre_feedforward_norm2_weight_bytes =
        header.read_tensor_bytes(PRE_FEEDFORWARD_NORM2_WEIGHT_NAME)?;
    let post_feedforward_norm2_weight_bytes =
        header.read_tensor_bytes(POST_FEEDFORWARD_NORM2_WEIGHT_NAME)?;
    let expert_gate_weight_bytes = header.read_tensor_bytes(EXPERT_GATE_WEIGHT_NAME)?;
    let expert_gate_scales_bytes = header.read_tensor_bytes(EXPERT_GATE_SCALES_NAME)?;
    let expert_gate_biases_bytes = header.read_tensor_bytes(EXPERT_GATE_BIASES_NAME)?;
    let expert_up_weight_bytes = header.read_tensor_bytes(EXPERT_UP_WEIGHT_NAME)?;
    let expert_up_scales_bytes = header.read_tensor_bytes(EXPERT_UP_SCALES_NAME)?;
    let expert_up_biases_bytes = header.read_tensor_bytes(EXPERT_UP_BIASES_NAME)?;
    let expert_down_weight_bytes = header.read_tensor_bytes(EXPERT_DOWN_WEIGHT_NAME)?;
    ...
    let pre_feedforward_norm1_weight_entry = header
        .tensor(PRE_FEEDFORWARD_NORM1_WEIGHT_NAME)
        .ok_or("missing pre-feedforward norm1 weight entry")?;
    let post_feedforward_norm1_weight_entry = header
        .tensor(POST_FEEDFORWARD_NORM1_WEIGHT_NAME)
        .ok_or("missing post-feedforward norm1 weight entry")?;
    ...
    let mlp_down_weight_entry = header
        .tensor(MLP_DOWN_WEIGHT_NAME)
        .ok_or("missing mlp down_proj weight entry")?;
    let mlp_down_scales_entry = header
        .tensor(MLP_DOWN_SCALES_NAME)
        .ok_or("missing mlp down_proj scales entry")?;
    let router_proj_weight_entry = header
        .tensor(ROUTER_PROJ_WEIGHT_NAME)
        .ok_or("missing router proj weight entry")?;
    let router_proj_scales_entry = header
        .tensor(ROUTER_PROJ_SCALES_NAME)
        .ok_or("missing router proj scales entry")?;
    let pre_feedforward_norm2_weight_entry = header
        .tensor(PRE_FEEDFORWARD_NORM2_WEIGHT_NAME)
        .ok_or("missing pre-feedforward norm2 weight entry")?;
    let post_feedforward_norm2_weight_entry = header
        .tensor(POST_FEEDFORWARD_NORM2_WEIGHT_NAME)
        .ok_or("missing post-feedforward norm2 weight entry")?;
    let expert_gate_weight_entry = header
        .tensor(EXPERT_GATE_WEIGHT_NAME)
        .ok_or("missing expert gate weight entry")?;
    let expert_gate_scales_entry = header
        .tensor(EXPERT_GATE_SCALES_NAME)
        .ok_or("missing expert gate scales entry")?;
    let expert_gate_biases_entry = header
        .tensor(EXPERT_GATE_BIASES_NAME)
        .ok_or("missing expert gate biases entry")?;
    let expert_up_weight_entry = header
        .tensor(EXPERT_UP_WEIGHT_NAME)
        .ok_or("missing expert up weight entry")?;
    let expert_up_scales_entry = header
        .tensor(EXPERT_UP_SCALES_NAME)
        .ok_or("missing expert up scales entry")?;
    let expert_up_biases_entry = header
        .tensor(EXPERT_UP_BIASES_NAME)
        .ok_or("missing expert up biases entry")?;
    let expert_down_weight_entry = header
        .tensor(EXPERT_DOWN_WEIGHT_NAME)
        .ok_or("missing expert down weight entry")?;
    let expert_down_scales_entry = header
        .tensor(EXPERT_DOWN_SCALES_NAME)
        .ok_or("missing expert down scales entry")?;
    let expert_down_biases_entry = header
        .tensor(EXPERT_DOWN_BIASES_NAME)
        .ok_or("missing expert down biases entry")?;
    ...
    let expert_gate_out_len = usize::try_from(expert_gate_weight_entry.shape[1])?;
    let expert_up_out_len = usize::try_from(expert_up_weight_entry.shape[1])?;
    let expert_down_out_len = usize::try_from(expert_down_weight_entry.shape[1])?;
    ...
    if pre_feedforward_norm1_len != post_attention_norm_len {
        return Err(format!(
            "invalid pre-feedforward norm1 length: got {} expected {}",
            pre_feedforward_norm1_len, post_attention_norm_len
        )
        .into());
    }
    let mlp_gate_n_in = usize::try_from(mlp_gate_weight_entry.shape[1] * 8)?;
    if mlp_gate_n_in != pre_feedforward_norm1_len {
        return Err(format!(
            "invalid mlp gate_proj input size: got {} expected {}",
            mlp_gate_n_in, pre_feedforward_norm1_len
        )
        .into());
    }
    let mlp_up_n_in = usize::try_from(mlp_up_weight_entry.shape[1] * 8)?;
    if mlp_up_n_in != pre_feedforward_norm1_len {
        return Err(format!(
            "invalid mlp up_proj input size: got {} expected {}",
            mlp_up_n_in, pre_feedforward_norm1_len
        )
        .into());
    }
    ...
    if mlp_down_out_len != pre_feedforward_norm1_len {
        return Err(format!(
            "invalid mlp down_proj output size: got {} expected {}",
            mlp_down_out_len, pre_feedforward_norm1_len
        )
        .into());
    }
    if post_feedforward_norm1_len != mlp_down_out_len {
        return Err(format!(
    ...
    if router_scale_words.len() != post_attention_norm_len {
        return Err(format!(
    ...
    if router_out_len != router_per_expert_scale_words.len() {
        return Err(format!(
            "invalid router per-expert scale length: got {} expected {}",
            router_per_expert_scale_words.len(),
            router_out_len
        )
        .into());
    }
    if pre_feedforward_norm2_weight_entry.shape[0] as usize != post_attention_norm_len {
        return Err(format!(
            "invalid pre-feedforward norm2 length: got {} expected {}",
            pre_feedforward_norm2_weight_entry.shape[0], post_attention_norm_len
        )
        .into());
    }
    if post_feedforward_norm2_weight_entry.shape[0] as usize != expert_down_out_len {
        return Err(format!(
            "invalid post-feedforward norm2 length: got {} expected {}",
            post_feedforward_norm2_weight_entry.shape[0], expert_down_out_len
        )
        .into());
    }
    if expert_gate_weight_entry.shape.len() != 3
        || expert_gate_scales_entry.shape.len() != 3
        || expert_gate_biases_entry.shape.len() != 3
    {
        return Err("expert gate tensors must be rank-3".into());
    }
    if expert_gate_weight_entry.shape[0] as usize != router_out_len
        || expert_gate_scales_entry.shape[0] as usize != router_out_len
        || expert_gate_biases_entry.shape[0] as usize != router_out_len
    {
        return Err("expert gate tensor plane count must match num_experts".into());
    }
    if expert_gate_weight_entry.shape[1] != expert_gate_scales_entry.shape[1]
        || expert_gate_weight_entry.shape[1] != expert_gate_biases_entry.shape[1]
    {
        return Err("expert gate tensor row counts mismatch".into());
    }
    if expert_up_weight_entry.shape.len() != 3
        || expert_up_scales_entry.shape.len() != 3
        || expert_up_biases_entry.shape.len() != 3
    {
        return Err("expert up tensors must be rank-3".into());
    }
    if expert_up_weight_entry.shape[0] as usize != router_out_len
        || expert_up_scales_entry.shape[0] as usize != router_out_len
        || expert_up_biases_entry.shape[0] as usize != router_out_len
    {
        return Err("expert up tensor plane count must match num_experts".into());
    }
    if expert_up_weight_entry.shape[1] != expert_up_scales_entry.shape[1]
        || expert_up_weight_entry.shape[1] != expert_up_biases_entry.shape[1]
    {
        return Err("expert up tensor row counts mismatch".into());
    }
    if expert_gate_out_len != expert_up_out_len {
        return Err(format!(
            "expert gate/up output size mismatch: gate={} up={}",
            expert_gate_out_len, expert_up_out_len
        )
        .into());
    }
    if expert_down_weight_entry.shape.len() != 3
        || expert_down_scales_entry.shape.len() != 3
        || expert_down_biases_entry.shape.len() != 3
    {
        return Err("expert down tensors must be rank-3".into());
    }
    if expert_down_weight_entry.shape[0] as usize != router_out_len
        || expert_down_scales_entry.shape[0] as usize != router_out_len
        || expert_down_biases_entry.shape[0] as usize != router_out_len
    {
        return Err("expert down tensor plane count must match num_experts".into());
    }
    if expert_down_weight_entry.shape[1] != expert_down_scales_entry.shape[1]
        || expert_down_weight_entry.shape[1] != expert_down_biases_entry.shape[1]
    {
    ...
    let post_feedforward_norm1_weight_buf = runtime.create_buffer_with_bytes(
        &post_feedforward_norm1_weight_bytes,
        BufferStorageMode::Private,
    )?;
    ...
    let mlp_down_weight_buf =
        runtime.create_buffer_with_bytes(&mlp_down_weight_bytes, BufferStorageMode::Private)?;
    let mlp_down_scales_buf =
        runtime.create_buffer_with_bytes(&mlp_down_scales_bytes, BufferStorageMode::Private)?;
    let mlp_down_biases_buf =
        runtime.create_buffer_with_bytes(&mlp_down_biases_bytes, BufferStorageMode::Private)?;
    let router_proj_weight_buf =
        runtime.create_buffer_with_bytes(&router_proj_weight_bytes, BufferStorageMode::Private)?;
    let router_proj_scales_buf =
        runtime.create_buffer_with_bytes(&router_proj_scales_bytes, BufferStorageMode::Private)?;
    let router_proj_biases_buf =
        runtime.create_buffer_with_bytes(&router_proj_biases_bytes, BufferStorageMode::Private)?;
    let router_scale_weight_buf = runtime.create_buffer_with_bytes(
        &bytes_from_bf16_words(&router_scale_words),
        BufferStorageMode::Private,
    )?;
    let pre_feedforward_norm2_weight_buf = runtime.create_buffer_with_bytes(
        &pre_feedforward_norm2_weight_bytes,
        BufferStorageMode::Private,
    )?;
    let post_feedforward_norm2_weight_buf = runtime.create_buffer_with_bytes(
        &post_feedforward_norm2_weight_bytes,
        BufferStorageMode::Private,
    )?;
    let expert_gate_weight_buf =
        runtime.create_buffer_with_bytes(&expert_gate_weight_bytes, BufferStorageMode::Private)?;
    let expert_gate_scales_buf =
        runtime.create_buffer_with_bytes(&expert_gate_scales_bytes, BufferStorageMode::Private)?;
    let expert_gate_biases_buf =
        runtime.create_buffer_with_bytes(&expert_gate_biases_bytes, BufferStorageMode::Private)?;
    let expert_up_weight_buf =
        runtime.create_buffer_with_bytes(&expert_up_weight_bytes, BufferStorageMode::Private)?;
    let expert_up_scales_buf =
        runtime.create_buffer_with_bytes(&expert_up_scales_bytes, BufferStorageMode::Private)?;
    let expert_up_biases_buf =
        runtime.create_buffer_with_bytes(&expert_up_biases_bytes, BufferStorageMode::Private)?;
    let expert_down_weight_buf =
        runtime.create_buffer_with_bytes(&expert_down_weight_bytes, BufferStorageMode::Private)?;
    let expert_down_scales_buf =
        runtime.create_buffer_with_bytes(&expert_down_scales_bytes, BufferStorageMode::Private)?;
    let expert_down_biases_buf =
        runtime.create_buffer_with_bytes(&expert_down_biases_bytes, BufferStorageMode::Private)?;
    ...
    let expert_up_out_buf = runtime.create_buffer(
        ROUTER_TOP_K * expert_up_out_len * 2,
        BufferStorageMode::Private,
    )?;
    let expert_geglu_out_buf = runtime.create_buffer(
        ROUTER_TOP_K * expert_gate_out_len * 2,
        BufferStorageMode::Private,
    )?;
    let expert_down_out_buf = runtime.create_buffer(
        ROUTER_TOP_K * expert_down_out_len * 2,
        BufferStorageMode::Private,
    )?;
    let moe_top_k_indices_buf =
        runtime.create_buffer(ROUTER_TOP_K * size_of::<u32>(), BufferStorageMode::Shared)?;
    let moe_top_k_weights_buf =
        runtime.create_buffer(ROUTER_TOP_K * 2, BufferStorageMode::Shared)?;
    let moe_weighted_out_buf =
        runtime.create_buffer(expert_down_out_len * 2, BufferStorageMode::Private)?;
    let moe_post_ffn_norm2_out_buf =
        runtime.create_buffer(expert_down_out_len * 2, BufferStorageMode::Private)?;
    ...
    let weighted_rows_pipeline = runtime.get_or_compile_pipeline(&MetalPipelineDescriptor {
        cache_name: "kernel_mlx_weighted_sum_rows_bf16".to_string(),
        base_name: "kernel_mlx_weighted_sum_rows_bf16".to_string(),
        constants: Vec::new(),
        smem_bytes: 0,
        nr0: 0,
        nr1: 0,
    ...
    let router_scale_pipeline = runtime.get_or_compile_pipeline(&MetalPipelineDescriptor {
    ...
    let expert_geglu_pipeline = runtime.get_or_compile_pipeline(&MetalPipelineDescriptor {
    ...
    let router_proj_args = MlxAffineQprojRowArgs {
    ...
    let mlp_gate_args = MlxAffineQprojRowArgs {
        n_in: pre_feedforward_norm1_len as u32,
    ...
    let mlp_geglu_args = MlxGegluRowArgs {
        n: mlp_gate_out_len as u32,
    };
    let mlp_down_args = MlxAffineQprojRowArgs {
    ...
    let expert_gate_args = MlxAffineQprojRowArgs {
        n_in: post_attention_norm_len as u32,
    ...
    let expert_up_args = MlxAffineQprojRowArgs {
        n_in: post_attention_norm_len as u32,
    ...
    let expert_down_args = MlxAffineQprojRowArgs {
    ...
    let expert_gate_selected_args = MlxAffineSelectedExpertsQprojRowArgs {
        n_in: post_attention_norm_len as u32,
        weight_words_per_row: expert_gate_weight_entry.shape[2] as u32,
        qparams_per_row: expert_gate_scales_entry.shape[2] as u32,
        out_rows: expert_gate_out_len as u32,
    ...
    let expert_up_selected_args = MlxAffineSelectedExpertsQprojRowArgs {
        n_in: post_attention_norm_len as u32,
        weight_words_per_row: expert_up_weight_entry.shape[2] as u32,
        qparams_per_row: expert_up_scales_entry.shape[2] as u32,
        out_rows: expert_up_out_len as u32,
    ...
    let expert_down_selected_args = MlxAffineSelectedExpertsQprojRowArgs {
        n_in: expert_gate_out_len as u32,
        weight_words_per_row: expert_down_weight_entry.shape[2] as u32,
        qparams_per_row: expert_down_scales_entry.shape[2] as u32,
        out_rows: expert_down_out_len as u32,
    ...
    let router_root_size = bf16_round_to_f32((post_attention_norm_len as f32).powf(-0.5));
    let router_scale_args = MlxRouterScaleArgs {
        n: post_attention_norm_len as u32,
        eps: EPS,
        root_size: router_root_size,
    };
    let moe_weighted_rows_args = MlxWeightedRowsArgs {
        n: expert_down_out_len as u32,
        row_stride: expert_down_out_len as u32,
    ...
    let router_scale_bindings = [
        ...
        MetalBufferBindingRef {
            index: 2,
            buffer: &router_scale_weight_buf,
            offset_bytes: 0,
        },
        MetalBufferBindingRef {
            index: 3,
    ...
    let router_proj_bindings = [
        ...
        MetalBufferBindingRef {
            index: 2,
            buffer: &router_proj_weight_buf,
            offset_bytes: 0,
        },
        MetalBufferBindingRef {
            index: 3,
            buffer: &router_proj_scales_buf,
            offset_bytes: 0,
        },
        MetalBufferBindingRef {
            index: 4,
            buffer: &router_proj_biases_buf,
            offset_bytes: 0,
        },
        MetalBufferBindingRef {
            index: 5,
    ...
    let _expert_down_threadgroups = MetalSize {
        width: 1,
    ...
    let router_proj_args_bytes = bytes_of(&router_proj_args);
    let mlp_gate_args_bytes = bytes_of(&mlp_gate_args);
    let mlp_up_args_bytes = bytes_of(&mlp_up_args);
    let mlp_geglu_args_bytes = bytes_of(&mlp_geglu_args);
    let mlp_down_args_bytes = bytes_of(&mlp_down_args);
    ...
    let router_scale_args_bytes = bytes_of(&router_scale_args);
    let moe_weighted_rows_args_bytes = bytes_of(&moe_weighted_rows_args);
    let o_proj_row1836_args_bytes = bytes_of(&o_proj_row1836_args);
    let o_proj_reduce_args_bytes = bytes_of(&o_proj_reduce_args);
    let _expert_gate_args_bytes = bytes_of(&expert_gate_args);
    let _expert_up_args_bytes = bytes_of(&expert_up_args);
    let expert_geglu_args_bytes = bytes_of(&expert_geglu_args);
    let _expert_down_args_bytes = bytes_of(&expert_down_args);
    let expert_gate_selected_args_bytes = bytes_of(&expert_gate_selected_args);
    let expert_up_selected_args_bytes = bytes_of(&expert_up_selected_args);
    let expert_down_selected_args_bytes = bytes_of(&expert_down_selected_args);
    ...
    let compute_router_output =
        ...
        let router_scaled = if include_router_scaled {
            decode_bf16_buffer_bits(
        ...
        top_k_indices.truncate(ROUTER_TOP_K);
        ...
        for (slot, weight) in top_k_weights.iter_mut().enumerate() {
            ...
            let expert_scale =
                bf16_word_to_f32(router_per_expert_scale_words[top_k_indices[slot] as usize]);
    ...
    let write_moe_top_k_indices = |top_k_indices: &[u32]| -> Result<(), Box<dyn Error>> {
        if top_k_indices.len() != ROUTER_TOP_K {
            return Err(format!(
                "expected {} routed experts, got {}",
                ROUTER_TOP_K,
                top_k_indices.len()
            )
            .into());
        }
        let mut bytes = Vec::with_capacity(ROUTER_TOP_K * size_of::<u32>());
        for &index in top_k_indices {
            bytes.extend_from_slice(&index.to_le_bytes());
        }
        runtime.write_buffer(&moe_top_k_indices_buf, 0, &bytes)?;
        Ok(())
    };
    let dispatch_moe_expert_gate = |top_k_indices: &[u32]| -> Result<(), Box<dyn Error>> {
        write_moe_top_k_indices(top_k_indices)?;
        runtime.begin_command_batch()?;
        runtime.dispatch_compute(
            &rms_pipeline,
            rms_args_bytes,
        ...
        runtime.dispatch_compute(
            ...
            expert_gate_selected_args_bytes,
    ...
    let dispatch_moe_expert_gate_up = |top_k_indices: &[u32]| -> Result<(), Box<dyn Error>> {
        write_moe_top_k_indices(top_k_indices)?;
        runtime.begin_command_batch()?;
        runtime.dispatch_compute(
            &rms_pipeline,
    ...
    let dispatch_moe_expert_gate_up_geglu = |top_k_indices: &[u32]| -> Result<(), Box<dyn Error>> {
        write_moe_top_k_indices(top_k_indices)?;
        runtime.begin_command_batch()?;
        runtime.dispatch_compute(
            &rms_pipeline,
        ...
        runtime.memory_barrier_buffers()?;
        runtime.dispatch_compute(
            &expert_geglu_pipeline,
            expert_geglu_args_bytes,
            &expert_geglu_bindings,
            &[],
            expert_geglu_threadgroups,
            expert_geglu_threads_per_threadgroup,
        )?;
        runtime.end_command_batch()?;
        runtime.wait_idle()?;
        Ok(())
    };
    let dispatch_moe_expert_gate_up_geglu_down =
        |top_k_indices: &[u32]| -> Result<(), Box<dyn Error>> {
            write_moe_top_k_indices(top_k_indices)?;
            runtime.begin_command_batch()?;
            runtime.dispatch_compute(
                &rms_pipeline,
            ...
            runtime.dispatch_compute(
                &selected_expert_proj_pipeline,
                expert_down_selected_args_bytes,
    ...
    let dispatch_moe_expert_gate_up_geglu_down_post_ffn_norm2 =
        |top_k_indices: &[u32], top_k_weights_bits: &[u32]| -> Result<(), Box<dyn Error>> {
            write_moe_top_k_indices(top_k_indices)?;
            if top_k_weights_bits.len() != ROUTER_TOP_K {
                return Err(format!(
                    "moe top_k weight count mismatch: got {} expected {}",
                    top_k_weights_bits.len(),
                    ROUTER_TOP_K
                )
                .into());
            }
            let top_k_weight_words = widened_bits_to_bf16_words(top_k_weights_bits);
            let top_k_weight_bytes = bytes_from_bf16_words(&top_k_weight_words);
            runtime.write_buffer(&moe_top_k_weights_buf, 0, &top_k_weight_bytes)?;
            runtime.begin_command_batch()?;
            runtime.dispatch_compute(
                &rms_pipeline,
                rms_args_bytes,
            ...
            runtime.memory_barrier_buffers()?;
            runtime.dispatch_compute(
                &weighted_rows_pipeline,
                moe_weighted_rows_args_bytes,
                &moe_weighted_rows_bindings,
                &[],
                moe_weighted_rows_threadgroups,
                moe_weighted_rows_threads_per_threadgroup,
            )?;
            runtime.memory_barrier_buffers()?;
            runtime.dispatch_compute(
                &rms_pipeline,
    ...
    let dispatch_moe_full_merge =
        |top_k_indices: &[u32], top_k_weights_bits: &[u32]| -> Result<(), Box<dyn Error>> {
            ...
            runtime.dispatch_compute(
                &rms_pipeline,
                rms_args_bytes,
                &pre_feedforward_norm1_bindings,
                &[],
                rms_threadgroups,
                rms_threads_per_threadgroup,
            )?;
            ...
            runtime.dispatch_compute(
                &expert_geglu_pipeline,
                mlp_geglu_args_bytes,
                &mlp_geglu_bindings,
                &[],
                mlp_geglu_threadgroups,
                mlp_geglu_threads_per_threadgroup,
            )?;
            runtime.memory_barrier_buffers()?;
            runtime.dispatch_compute(
                &proj_pipeline,
            ...
            runtime.dispatch_compute(
                &rms_pipeline,
                rms_args_bytes,
                &post_feedforward_norm1_bindings,
                &[],
                rms_threadgroups,
                rms_threads_per_threadgroup,
            )?;
    ...
    for _ in 0..warmup_iters {
        ...
        if moe_expert_gate {
            ...
            dispatch_moe_expert_gate(&router_output.top_k_indices)?;
        } else if moe_expert_up {
            let router_output = compute_router_output(false, true)?;
            dispatch_moe_expert_gate_up(&router_output.top_k_indices)?;
        } else if moe_expert_geglu {
            let router_output = compute_router_output(false, true)?;
            dispatch_moe_expert_gate_up_geglu(&router_output.top_k_indices)?;
        } else if moe_expert_down {
            let router_output = compute_router_output(false, true)?;
            dispatch_moe_expert_gate_up_geglu_down(&router_output.top_k_indices)?;
        } else if moe_merge {
            let router_output = compute_router_output(false, true)?;
            ...
            let top_k_weights_bits = router_output
                ...
                .map(|value| value.to_bits())
                .collect::<Vec<_>>();
            dispatch_moe_expert_gate_up_geglu_down_post_ffn_norm2(
                &router_output.top_k_indices,
                &top_k_weights_bits,
            )?;
        } else if router {
    ...
    let q_norm_bits = decode_bf16_buffer_bits(&runtime.read_buffer(&q_norm_buf, q_out_len * 2)?);
    let q_rope_bits = decode_bf16_buffer_bits(&runtime.read_buffer(&q_rope_buf, q_out_len * 2)?);
    let k_norm_bits = decode_bf16_buffer_bits(&runtime.read_buffer(&k_norm_buf, k_out_len * 2)?);
    ...
    let router_output = if router {
        Some(compute_router_output(true, true)?)
    } else {
        None
    };
    if moe_expert_gate {
        dispatch_moe_expert_gate(
            &router_output
                .as_ref()
                .ok_or("moe expert gate path requires router output")?
                .top_k_indices,
        )?;
    } else if moe_expert_up {
        dispatch_moe_expert_gate_up(
            &router_output
                .as_ref()
                .ok_or("moe expert up path requires router output")?
                .top_k_indices,
        )?;
    } else if moe_expert_geglu {
        dispatch_moe_expert_gate_up_geglu(
            &router_output
                .as_ref()
                .ok_or("moe expert geglu path requires router output")?
                .top_k_indices,
        )?;
    } else if moe_expert_down {
        dispatch_moe_expert_gate_up_geglu_down(
            &router_output
                .as_ref()
                .ok_or("moe expert down path requires router output")?
                .top_k_indices,
        )?;
    } else if moe_merge {
    } else if moe_post_ffn_norm2 {
        ...
        dispatch_moe_expert_gate_up_geglu_down(
            &router_output
                .as_ref()
                .ok_or("moe expert path requires router output")?
                .top_k_indices,
        )?;
    }
    ...
    let top_k_indices = router_output
        .as_ref()
        .map(|output| output.top_k_indices.clone())
        .unwrap_or_default();
    let router_scaled_hash = if router_scaled_bits.is_empty() {
        0
    } else {
        fnv1a64_u32_words(&router_scaled_bits)
    };
    let expert_scores_hash = if expert_scores_bits.is_empty() {
        0
    } else {
        fnv1a64_u32_words(&expert_scores_bits)
    };
    let router_probs_hash = if router_probs_bits.is_empty() {
        0
    } else {
        fnv1a64_u32_words(&router_probs_bits)
    };
    let top_k_weights_hash = if top_k_weights_bits.is_empty() {
        0
    } else {
        fnv1a64_u32_words(&top_k_weights_bits)
    };
    let top_k_indices_hash = if top_k_indices.is_empty() {
        0
    } else {
        fnv1a64_u32_words(&top_k_indices)
    };
    let router_scaled_first16 = &router_scaled_bits[..router_scaled_bits.len().min(16)];
    let expert_scores_first16 = &expert_scores_bits[..expert_scores_bits.len().min(16)];
    let router_probs_first16 = &router_probs_bits[..router_probs_bits.len().min(16)];
    let top_k_weights_first8 = &top_k_weights_bits[..top_k_weights_bits.len().min(ROUTER_TOP_K)];
    let moe_gate_bits = if moe_expert_gate
        || moe_expert_up
        || moe_expert_geglu
        || moe_expert_down
        || moe_experts
    ...
    if moe_post_ffn_norm2 || moe_merge {
        dispatch_moe_expert_gate_up_geglu_down_post_ffn_norm2(
            &router_output
                .as_ref()
                .ok_or("moe post-ffn norm2 path requires router output")?
                .top_k_indices,
            &top_k_weights_bits,
        )?;
    }
    let moe_out_bits = if moe_experts {
        moe_weighted_expert_out_bits(&moe_down_bits, &top_k_weights_bits, expert_down_out_len)?
    } else if moe_post_ffn_norm2 || moe_merge {
        read_moe_weighted_out_bits()?
    } else {
        Vec::new()
    };
    let moe_post_ffn_norm2_bits = if moe_post_ffn_norm2 || moe_merge {
        read_moe_post_ffn_norm2_bits()?
    } else {
        Vec::new()
    };
    if moe_merge {
        dispatch_moe_full_merge(
            &router_output
                .as_ref()
                .ok_or("moe merge path requires router output")?
                .top_k_indices,
            &top_k_weights_bits,
        )?;
    }
    let moe_post_ffn_norm1_bits = if moe_merge {
        read_moe_post_ffn_norm1_bits()?
    } else {
        Vec::new()
    };
    let moe_merge_bits = if moe_merge {
        read_moe_merge_bits()?
    } else {
        Vec::new()
    };
    let moe_gate_hash = if moe_gate_bits.is_empty() {
        0
    } else {
        fnv1a64_u32_words(&moe_gate_bits)
    };
    let moe_up_hash = if moe_up_bits.is_empty() {
        0
    } else {
        fnv1a64_u32_words(&moe_up_bits)
    };
    let moe_geglu_hash = if moe_geglu_bits.is_empty() {
        0
    } else {
        fnv1a64_u32_words(&moe_geglu_bits)
    };
    let moe_down_hash = if moe_down_bits.is_empty() {
        0
    } else {
        fnv1a64_u32_words(&moe_down_bits)
    };
    let moe_out_hash = if moe_out_bits.is_empty() {
        0
    } else {
        fnv1a64_u32_words(&moe_out_bits)
    };
    let moe_post_ffn_norm2_hash = if moe_post_ffn_norm2_bits.is_empty() {
        0
    } else {
        fnv1a64_u32_words(&moe_post_ffn_norm2_bits)
    };
    let moe_post_ffn_norm1_hash = if moe_post_ffn_norm1_bits.is_empty() {
        0
    } else {
        fnv1a64_u32_words(&moe_post_ffn_norm1_bits)
    };
    let moe_merge_hash = if moe_merge_bits.is_empty() {
        0
    } else {
        fnv1a64_u32_words(&moe_merge_bits)
    };
    let moe_gate_first16 = &moe_gate_bits[..moe_gate_bits.len().min(16)];
    let moe_up_first16 = &moe_up_bits[..moe_up_bits.len().min(16)];
    let moe_geglu_first16 = &moe_geglu_bits[..moe_geglu_bits.len().min(16)];
    let moe_down_first16 = &moe_down_bits[..moe_down_bits.len().min(16)];
    let moe_out_first16 = &moe_out_bits[..moe_out_bits.len().min(16)];
    let moe_post_ffn_norm2_first16 =
        &moe_post_ffn_norm2_bits[..moe_post_ffn_norm2_bits.len().min(16)];
    let moe_post_ffn_norm1_first16 =
        &moe_post_ffn_norm1_bits[..moe_post_ffn_norm1_bits.len().min(16)];
    let moe_merge_first16 = &moe_merge_bits[..moe_merge_bits.len().min(16)];
    ...
    if dump_all_f32_bits && moe_expert_gate {
        print!("all_f32_bits=");
        for (index, bits) in moe_gate_bits.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!();
        io::stdout().flush()?;
    } else if dump_all_f32_bits && moe_expert_up {
        print!("all_f32_bits=");
        for (index, bits) in moe_up_bits.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!();
        io::stdout().flush()?;
    } else if dump_all_f32_bits && moe_expert_geglu {
        print!("all_f32_bits=");
        for (index, bits) in moe_geglu_bits.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!();
        io::stdout().flush()?;
    } else if dump_all_f32_bits && moe_expert_down {
        print!("all_f32_bits=");
        for (index, bits) in moe_down_bits.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!();
        io::stdout().flush()?;
    } else if dump_all_f32_bits && moe_experts {
        print!("all_f32_bits=");
        for (index, bits) in moe_out_bits.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!();
        io::stdout().flush()?;
    } else if dump_all_f32_bits && moe_merge {
        print!("all_f32_bits=");
        for (index, bits) in moe_merge_bits.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!();
        io::stdout().flush()?;
    } else if dump_all_f32_bits && moe_post_ffn_norm2 {
        print!("all_f32_bits=");
        for (index, bits) in moe_post_ffn_norm2_bits.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!();
        io::stdout().flush()?;
    } else if dump_all_f32_bits && router {
        print!("all_f32_bits=");
        for (index, bits) in top_k_weights_bits.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!();
        print!("all_u32_words=");
        for (index, word) in top_k_indices.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{word:08X}");
        ...
        println!();
        print!("all_f32_bits=");
        for (index, bits) in residual_bits.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
    ...
    if router
        && (router_scaled_hash != EXPECTED_ROUTER_SCALED_HASH
            || router_scaled_first16 != EXPECTED_ROUTER_SCALED_FIRST16_BITS)
    {
        return Err(format!(
            "router scaled mismatch: got hash 0x{router_scaled_hash:016X} first16 {:08X?} expected hash 0x{EXPECTED_ROUTER_SCALED_HASH:016X} expected first16 {:08X?}",
            router_scaled_first16,
            EXPECTED_ROUTER_SCALED_FIRST16_BITS
        )
        .into());
    }
    if router
        && (expert_scores_hash != EXPECTED_ROUTER_EXPERT_SCORES_HASH
            || expert_scores_first16 != EXPECTED_ROUTER_EXPERT_SCORES_FIRST16_BITS)
    {
        return Err(format!(
            "router expert_scores mismatch: got hash 0x{expert_scores_hash:016X} first16 {:08X?} expected hash 0x{EXPECTED_ROUTER_EXPERT_SCORES_HASH:016X} expected first16 {:08X?}",
            expert_scores_first16,
            EXPECTED_ROUTER_EXPERT_SCORES_FIRST16_BITS
        )
        .into());
    }
    if router
        && (router_probs_hash != EXPECTED_ROUTER_PROBS_HASH
            || router_probs_first16 != EXPECTED_ROUTER_PROBS_FIRST16_BITS)
    {
        return Err(format!(
            "router probs mismatch: got hash 0x{router_probs_hash:016X} first16 {:08X?} expected hash 0x{EXPECTED_ROUTER_PROBS_HASH:016X} expected first16 {:08X?}",
            router_probs_first16,
            EXPECTED_ROUTER_PROBS_FIRST16_BITS
        )
        .into());
    }
    if router && top_k_indices_hash != EXPECTED_ROUTER_TOPK_INDICES_HASH {
        return Err(format!(
            "router top_k indices hash mismatch: got 0x{top_k_indices_hash:016X} expected 0x{EXPECTED_ROUTER_TOPK_INDICES_HASH:016X}"
        )
        .into());
    }
    if router && top_k_indices.as_slice() != EXPECTED_ROUTER_TOPK_INDICES {
        return Err(format!(
            "router top_k indices mismatch: got {:?} expected {:?}",
            top_k_indices, EXPECTED_ROUTER_TOPK_INDICES
        )
        .into());
    }
    if router
        && (top_k_weights_hash != EXPECTED_ROUTER_TOPK_WEIGHTS_HASH
            || top_k_weights_first8 != EXPECTED_ROUTER_TOPK_WEIGHTS_FIRST8_BITS)
    {
        return Err(format!(
            "router top_k weights mismatch: got hash 0x{top_k_weights_hash:016X} first8 {:08X?} expected hash 0x{EXPECTED_ROUTER_TOPK_WEIGHTS_HASH:016X} expected first8 {:08X?}",
            top_k_weights_first8,
    ...
    {
        return Err(format!(
            "moe expert gate mismatch: got hash 0x{moe_gate_hash:016X} first16 {:08X?} expected hash 0x{EXPECTED_MOE_EXPERT_GATE_HASH:016X} expected first16 {:08X?}",
            moe_gate_first16,
            EXPECTED_MOE_EXPERT_GATE_FIRST16_BITS
        )
        .into());
    }
    if (moe_expert_up || moe_expert_geglu || moe_experts || moe_post_ffn_norm2 || moe_merge)
        && (moe_up_hash != EXPECTED_MOE_EXPERT_UP_HASH
            || moe_up_first16 != EXPECTED_MOE_EXPERT_UP_FIRST16_BITS)
    {
        return Err(format!(
            "moe expert up mismatch: got hash 0x{moe_up_hash:016X} first16 {:08X?} expected hash 0x{EXPECTED_MOE_EXPERT_UP_HASH:016X} expected first16 {:08X?}",
            moe_up_first16,
    ...
    if (moe_expert_geglu || moe_expert_down || moe_experts || moe_post_ffn_norm2 || moe_merge)
        && (moe_geglu_hash != EXPECTED_MOE_EXPERT_GEGLU_HASH
            || moe_geglu_first16 != EXPECTED_MOE_EXPERT_GEGLU_FIRST16_BITS)
    {
        return Err(format!(
            "moe expert geglu mismatch: got hash 0x{moe_geglu_hash:016X} first16 {:08X?} expected hash 0x{EXPECTED_MOE_EXPERT_GEGLU_HASH:016X} expected first16 {:08X?}",
            moe_geglu_first16,
            EXPECTED_MOE_EXPERT_GEGLU_FIRST16_BITS
        )
        .into());
    }
    if (moe_expert_down || moe_experts || moe_post_ffn_norm2 || moe_merge)
        && (moe_down_hash != EXPECTED_MOE_EXPERT_DOWN_HASH
            || moe_down_first16 != EXPECTED_MOE_EXPERT_DOWN_FIRST16_BITS)
    {
        return Err(format!(
            "moe expert down mismatch: got hash 0x{moe_down_hash:016X} first16 {:08X?} expected hash 0x{EXPECTED_MOE_EXPERT_DOWN_HASH:016X} expected first16 {:08X?}",
            moe_down_first16,
            EXPECTED_MOE_EXPERT_DOWN_FIRST16_BITS
        )
        .into());
    }
    if (moe_experts || moe_post_ffn_norm2 || moe_merge)
        && (moe_out_hash != EXPECTED_MOE_EXPERT_OUT_HASH
            || moe_out_first16 != EXPECTED_MOE_EXPERT_OUT_FIRST16_BITS)
    {
        return Err(format!(
            "moe expert out mismatch: got hash 0x{moe_out_hash:016X} first16 {:08X?} expected hash 0x{EXPECTED_MOE_EXPERT_OUT_HASH:016X} expected first16 {:08X?}",
            moe_out_first16,
    ...
    if (moe_post_ffn_norm2 || moe_merge)
        && (moe_post_ffn_norm2_hash != EXPECTED_MOE_POST_FFN_NORM2_HASH
            || moe_post_ffn_norm2_first16 != EXPECTED_MOE_POST_FFN_NORM2_FIRST16_BITS)
    {
        return Err(format!(
            "moe post_ffn_norm2 mismatch: got hash 0x{moe_post_ffn_norm2_hash:016X} first16 {:08X?} expected hash 0x{EXPECTED_MOE_POST_FFN_NORM2_HASH:016X} expected first16 {:08X?}",
            moe_post_ffn_norm2_first16,
            EXPECTED_MOE_POST_FFN_NORM2_FIRST16_BITS
        )
        .into());
    }
    if moe_merge
        && (moe_post_ffn_norm1_hash != EXPECTED_MOE_POST_FEEDFORWARD_NORM1_HASH
            || moe_post_ffn_norm1_first16 != EXPECTED_MOE_POST_FEEDFORWARD_NORM1_FIRST16_BITS)
    {
        return Err(format!(
            "moe post_ffn_norm1 mismatch: got hash 0x{moe_post_ffn_norm1_hash:016X} first16 {:08X?} expected hash 0x{EXPECTED_MOE_POST_FEEDFORWARD_NORM1_HASH:016X} expected first16 {:08X?}",
            moe_post_ffn_norm1_first16,
            EXPECTED_MOE_POST_FEEDFORWARD_NORM1_FIRST16_BITS
        )
        .into());
    }
    if moe_merge
        && (moe_merge_hash != EXPECTED_MOE_MERGE_HASH
            || moe_merge_first16 != EXPECTED_MOE_MERGE_FIRST16_BITS)
    {
        return Err(format!(
            "moe merge mismatch: got hash 0x{moe_merge_hash:016X} first16 {:08X?} expected hash 0x{EXPECTED_MOE_MERGE_HASH:016X} expected first16 {:08X?}",
            moe_merge_first16,
    ...
    print!("attention_post_attn_residual_first16_f32_bits=");
    for (index, bits) in residual_first16.iter().enumerate() {
        if index != 0 {
            print!(",");
        }
        print!("0x{bits:08X}");
    ...
    println!();
    if moe_expert_gate {
        println!("stage=attention_moe_expert_gate");
        println!("router_scaled_fnv1a64=0x{router_scaled_hash:016X}");
        println!("expert_scores_fnv1a64=0x{expert_scores_hash:016X}");
        println!("router_probs_fnv1a64=0x{router_probs_hash:016X}");
        println!("router_topk_indices_fnv1a64=0x{top_k_indices_hash:016X}");
        println!("router_topk_weights_fnv1a64=0x{top_k_weights_hash:016X}");
        println!("expert_gate_fnv1a64=0x{moe_gate_hash:016X}");
        print!("expert_scores_first16_f32_bits=");
        for (index, bits) in expert_scores_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!();
        print!("router_probs_first16_f32_bits=");
        for (index, bits) in router_probs_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!();
        print!("top_k_indices=");
        for (index, value) in top_k_indices.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("{value}");
        ...
        println!();
        print!("top_k_weights_first8_f32_bits=");
        for (index, bits) in top_k_weights_first8.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!();
        print!("expert_gate_first16_f32_bits=");
        for (index, bits) in moe_gate_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!("stage=attention_moe_expert_up");
        println!("router_scaled_fnv1a64=0x{router_scaled_hash:016X}");
        println!("expert_scores_fnv1a64=0x{expert_scores_hash:016X}");
        println!("router_probs_fnv1a64=0x{router_probs_hash:016X}");
        println!("router_topk_indices_fnv1a64=0x{top_k_indices_hash:016X}");
        println!("router_topk_weights_fnv1a64=0x{top_k_weights_hash:016X}");
        println!("expert_gate_fnv1a64=0x{moe_gate_hash:016X}");
        println!("expert_up_fnv1a64=0x{moe_up_hash:016X}");
        print!("expert_scores_first16_f32_bits=");
        for (index, bits) in expert_scores_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
        ...
        println!();
        print!("expert_up_first16_f32_bits=");
        for (index, bits) in moe_up_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!("stage=attention_moe_expert_geglu");
        println!("router_scaled_fnv1a64=0x{router_scaled_hash:016X}");
        println!("expert_scores_fnv1a64=0x{expert_scores_hash:016X}");
        println!("router_probs_fnv1a64=0x{router_probs_hash:016X}");
        println!("router_topk_indices_fnv1a64=0x{top_k_indices_hash:016X}");
        ...
        println!("expert_gate_fnv1a64=0x{moe_gate_hash:016X}");
        println!("expert_up_fnv1a64=0x{moe_up_hash:016X}");
        println!("expert_geglu_fnv1a64=0x{moe_geglu_hash:016X}");
        print!("expert_scores_first16_f32_bits=");
        for (index, bits) in expert_scores_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
        ...
        println!();
        print!("expert_geglu_first16_f32_bits=");
        for (index, bits) in moe_geglu_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!("stage=attention_moe_expert_down");
        println!("router_scaled_fnv1a64=0x{router_scaled_hash:016X}");
        println!("expert_scores_fnv1a64=0x{expert_scores_hash:016X}");
        println!("router_probs_fnv1a64=0x{router_probs_hash:016X}");
        println!("router_topk_indices_fnv1a64=0x{top_k_indices_hash:016X}");
        ...
        println!("expert_up_fnv1a64=0x{moe_up_hash:016X}");
        println!("expert_geglu_fnv1a64=0x{moe_geglu_hash:016X}");
        println!("expert_down_fnv1a64=0x{moe_down_hash:016X}");
        print!("expert_scores_first16_f32_bits=");
        for (index, bits) in expert_scores_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
        ...
        println!();
        print!("expert_down_first16_f32_bits=");
        for (index, bits) in moe_down_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!("stage=attention_moe_experts");
        println!("router_scaled_fnv1a64=0x{router_scaled_hash:016X}");
        println!("expert_scores_fnv1a64=0x{expert_scores_hash:016X}");
        println!("router_probs_fnv1a64=0x{router_probs_hash:016X}");
        println!("router_topk_indices_fnv1a64=0x{top_k_indices_hash:016X}");
        ...
        println!("expert_geglu_fnv1a64=0x{moe_geglu_hash:016X}");
        println!("expert_down_fnv1a64=0x{moe_down_hash:016X}");
        println!("expert_out_fnv1a64=0x{moe_out_hash:016X}");
        print!("expert_scores_first16_f32_bits=");
        for (index, bits) in expert_scores_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
        ...
        println!();
        print!("expert_out_first16_f32_bits=");
        for (index, bits) in moe_out_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!("stage=attention_moe_merge");
        println!("router_scaled_fnv1a64=0x{router_scaled_hash:016X}");
        println!("expert_scores_fnv1a64=0x{expert_scores_hash:016X}");
        println!("router_probs_fnv1a64=0x{router_probs_hash:016X}");
        println!("router_topk_indices_fnv1a64=0x{top_k_indices_hash:016X}");
        ...
        println!("expert_down_fnv1a64=0x{moe_down_hash:016X}");
        println!("expert_out_fnv1a64=0x{moe_out_hash:016X}");
        println!("post_feedforward_norm2_fnv1a64=0x{moe_post_ffn_norm2_hash:016X}");
        println!("dense_h1_fnv1a64=0x{moe_post_ffn_norm1_hash:016X}");
        println!("merged_h_fnv1a64=0x{moe_merge_hash:016X}");
        print!("expert_scores_first16_f32_bits=");
        for (index, bits) in expert_scores_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
        ...
        println!();
        print!("post_feedforward_norm2_first16_f32_bits=");
        for (index, bits) in moe_post_ffn_norm2_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!();
        print!("dense_h1_first16_f32_bits=");
        for (index, bits) in moe_post_ffn_norm1_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!();
        print!("merged_h_first16_f32_bits=");
        for (index, bits) in moe_merge_first16.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
        ...
        println!("stage=attention_moe_post_ffn_norm2");
        println!("router_scaled_fnv1a64=0x{router_scaled_hash:016X}");
        println!("expert_scores_fnv1a64=0x{expert_scores_hash:016X}");
        println!("router_probs_fnv1a64=0x{router_probs_hash:016X}");
        println!("router_topk_indices_fnv1a64=0x{top_k_indices_hash:016X}");
        ...
        println!("stage=attention_router");
        println!("router_scaled_fnv1a64=0x{router_scaled_hash:016X}");
        println!("expert_scores_fnv1a64=0x{expert_scores_hash:016X}");
        println!("router_probs_fnv1a64=0x{router_probs_hash:016X}");
        println!("router_topk_indices_fnv1a64=0x{top_k_indices_hash:016X}");
