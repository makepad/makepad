...
use {
    makepad_platform::makepad_micro_serde::*,
    makepad_platform::*,
    std::io,
    std::net::SocketAddr,
...
impl XrNetNode {
    ...
    pub fn new(_cx: &mut Cx) -> Self {
        let discovery_bind_addr =
            SocketAddr::new(IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)), DISCOVERY_PORT);
        let discovery_send_addr = SocketAddr::new(
        ...
        let discovery_write_thread = std::thread::spawn(move || {
            ...
            loop {
                if !tl_dw.load(Ordering::Relaxed) {
                    break;
                }
                    ...
                    match UdpSocket::bind("0.0.0.0:0") {
                        Ok(s) => {
                            if let Err(e) = s.set_broadcast(true) {
                                eprintln!(
                                    "DW: Failed to set broadcast: {:?}. Retrying socket creation.",
                                    e
                                );
                                socket = None;
                        ...
                        Err(e) => {
                            eprintln!(
                                "DW: Failed to bind discovery write socket: {:?}. Retrying in 1s.",
                                e
                            );
                            thread::sleep(Duration::from_secs(1));
                            continue;
        ...
        let discovery_read_thread = std::thread::spawn(move || {
            ...
            loop {
                if !tl_dr.load(Ordering::Relaxed) {
                    break;
                }
                ...
                if socket.is_none() {
                    match UdpSocket::bind(db_addr_dr) {
                        Ok(s) => {
                            ...
                            if let Err(e) = s.set_broadcast(true) {
                                eprintln!(
                                    "DR: Failed to set broadcast: {:?}. Retrying socket creation.",
                                    e
                                );
                                socket = None;
                                thread::sleep(Duration::from_secs(1));
                        ...
                        Err(e) => {
                            eprintln!(
                                "DR: Failed to bind discovery read socket: {:?}. Retrying in 1s.",
                                e
                            );
                            thread::sleep(Duration::from_secs(1));
                            continue;
                    ...
                    match s.recv_from(&mut read_buf) {
                        Ok((len, mut peer_addr)) => {
                            if len == std::mem::size_of::<u64>() {
                                let received_uid_bytes: [u8; 8] =
                                    read_buf[0..8].try_into().expect("Slice size known to be 8");
        ...
        let state_read_thread = std::thread::spawn(move || {
            ...
            loop {
                if !tl_sr.load(Ordering::Relaxed) {
                    break;
                }
                ...
                if socket.is_none() {
                    match UdpSocket::bind(sb_addr_sr) {
                        ...
                        Err(e) => {
                            eprintln!(
                                "SR: Failed to bind state read socket: {:?}. Retrying in 1s.",
                                e
                            );
                            thread::sleep(Duration::from_secs(1));
                            continue;
                        ...
                        Ok((len, addr)) => {
                            if let Ok(state) = XrState::deserialize_bin(&read_buf[0..len]) {
                                if let Some(p) = active_peers.iter_mut().find(|v| v.addr == addr) {
                                    ...
                                    is_sr
                                        .send(XrNetIncoming::Update {
                                            peer: XrNetPeer { addr },
                                            state,
                                        })
                                        .ok();
                                } else {
                                    active_peers.push(ActivePeer {
                                        addr,
                                        last_seen: Instant::now(),
                                    });
                                    is_sr
                                        .send(XrNetIncoming::Join {
                                            peer: XrNetPeer { addr },
                                            state,
                                        })
                                        .ok();
                ...
                match msg {
                    ...
                    XrNetOutgoing::State(state) => {
                        ...
                        if let Some(s) = &socket {
                            ...
                            for peer_target in &peers {
                                // Renamed to avoid conflict with XrNetPeer struct
                                if let Err(e) = s.send_to(&buf, peer_target.addr) {
                                    // Use peer_target.addr
