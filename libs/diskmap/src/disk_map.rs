...
script_mod! {
    ...
    mod.widgets.DiskMapBase = #(DiskMap::register_widget(vm))
    mod.widgets.DiskMap = set_type_default() do mod.widgets.DiskMapBase{
        width: Fill
        height: Fill
...
pub enum DiskMapAction {
    ...
    Selected(PathBuf),
    /// A primary double-click: the host may open the path. The map still
    /// zooms the camera onto it.
    Opened(PathBuf),
...
pub struct DiskMap {
    ...
    /// Host-supplied colours; Tokyo Night until a host pushes its own.
    #[rust]
    palette: MapPalette,
...
impl MapCamera {
    /// Shared camera for maps retaining their 2D allocation under extrusion.
    /// Angles are radians, elevation is in the same units as the map points.
    pub fn new(
        pivot: DVec2,
        yaw: f64,
        pitch: f64,
        ...
        let (yaw, pitch) = if projection == MapProjection::Flat {
            (0.0, 0.0)
        } else {
            (yaw, pitch.clamp(0.0, 70.0_f64.to_radians()))
        };
        Self {
            pivot,
            sin_yaw: yaw.sin(),
            ...
            sin_pitch: pitch.sin(),
            cos_pitch: pitch.cos(),
            persp: projection == MapProjection::Persp,
            eye: constrained_eye(pitch, max_surface),
        }
    }

    pub fn project(&self, point: DVec2, elevation: f64) -> DVec2 {
        self.project_view(self.view(point, elevation, dvec2(0.0, 0.0)))
    }

    /// Screen x/y numerators, near coordinate, and homogeneous denominator.
    /// This exposes the identical projection to retained GPU list matrices;
    /// callers choose their pass's depth band without dividing vertices early.
    pub fn homogeneous(&self, point: DVec2, elevation: f64) -> [f64; 4] {
        self.homogeneous_blended(point, elevation, f64::from(self.persp))
    }

    /// Interpolate reciprocal focal distance, preserving a linear homogeneous
    /// transform and its exact plane inverse throughout an Ortho/Persp tween.
    pub fn homogeneous_blended(&self, point: DVec2, elevation: f64, perspective: f64) -> [f64; 4] {
        let v = self.view(point, elevation, dvec2(0.0, 0.0));
        let d = point - self.pivot;
        let near =
            (d.x * self.sin_yaw + d.y * self.cos_yaw) * self.sin_pitch + elevation * self.cos_pitch;
        let denominator = 1.0 - perspective.clamp(0.0, 1.0) * near / self.eye;
        [
            self.pivot.x * denominator + v.view.x,
            self.pivot.y * denominator + v.view.y,
            near,
            denominator,
        ]
    }
    ...
    pub fn unproject_at(&self, s: DVec2, z: f64) -> DVec2 {
        self.unproject_at_blended(s, z, f64::from(self.persp))
    }

    /// Inverse at any known elevation of `homogeneous_blended`.
    pub fn unproject_at_blended(&self, s: DVec2, z: f64, perspective: f64) -> DVec2 {
        let sx = s.x - self.pivot.x;
        let sy = s.y - self.pivot.y;
        let (xr, yr);
        let perspective = perspective.clamp(0.0, 1.0);
        if perspective == 0.0 {
            // sy = yr·cosφ − z·sinφ.
            ...
            let eye = self.eye / perspective;
            let denom = eye * self.cos_pitch + sy * self.sin_pitch;
            yr = if denom.abs() < 1e-6 {
                0.0
            } else {
                (sy * eye - z * (sy * self.cos_pitch - eye * self.sin_pitch)) / denom
            };
            let sc = eye / (eye - yr * self.sin_pitch - z * self.cos_pitch);
            xr = sx / sc;
        }
...
impl ProjectedFace {
    fn of_rect(cam: &MapCamera, r: &MapRect, z: f64) -> Option<Self> {
        Self::from_corners(
            cam,
            [
                (dvec2(r.x, r.y), z),
    ...
    fn from_corners(cam: &MapCamera, corners: [(DVec2, f64); 4]) -> Option<Self> {
        let uvs = [
            dvec2(0.0, 0.0),
            dvec2(1.0, 0.0),
            dvec2(1.0, 1.0),
...
impl DiskMap {
    ...
    pub fn rescan(&mut self, cx: &mut Cx) {
        ...
        let _ = backend::backend().forget_scan_cache(&root);
    ...
    fn begin(&mut self, cx: &mut Cx, path: &Path, fresh: bool) {
        ...
        let keep_pick = if path == self.root {
            self.pick.take()
        } else {
            None
        };
        ...
        let scan = move || {
            ...
            let cached = if fresh {
                ...
                backend::backend().load_scan_cache(&root)
            };
            ...
            let ok = backend::backend().scan_stream(&root, &cancel, &sink, &scan_pool);
            let _ = sender.send(ScanMessage {
                generation,
                step: None,
                finished: Some(if ok {
    ...
    pub fn status(&self) -> String {
        ...
        let picked = match &self.pick {
            Some(pick) => format!(" · picked {}", backend::backend().display_name(&pick.path)),
            None => String::new(),
        };
        format!(
                    ...
                    Outcome::Failed => {
                        if self.tree.children.is_empty() {
                            self.error = Some(format!(
                                "Could not map {}",
                                backend::backend().display_name(&self.root)
                            ));
                        }
                    }
                }
    ...
    fn view_escaped_cull(&self) -> bool {
        ...
        match self.projection {
            ...
            _ => {
                // The draw path is screen = MapCamera(remap(ground)): undoing the
    ...
    fn cam_at(&self, body: Rect) -> MapCamera {
        ...
        let max_surface = self
            ...
            .chain(
                self.tween_leavers
                    .iter()
                    .map(|(_, _, depth)| self.elev_f(*depth)),
            )
            .fold(0.0f64, f64::max);
        MapCamera {
            pivot: dvec2(
    ...
    fn save_cache(&self, cx: &Cx) {
        if backend::backend().is_demo() {
            return;
        }
        let Some(bytes) = sizecache::encode(&self.root, &self.tree, self.scanned_at) else {
            return;
        };
        let root = self.root.clone();
        if backend::backend().is_instant() {
            let _ = backend::backend().store_scan_cache(&root, &bytes);
    ...
    fn relayout(&mut self, rect: Rect, now: f64) {
        ...
        let viewport = match self.projection {
            MapProjection::Flat => {
                ...
                let pad = dvec2(rect.size.x * MOTION_CULL_PAD, rect.size.y * MOTION_CULL_PAD);
            ...
            _ => {
                ...
                let half = ((max.x - min.x).powi(2) + (max.y - min.y).powi(2)).sqrt() * 0.5 + reach;
        ...
        match &self.filter {
            ...
            Some(query) => {
                    ...
                    || self.measure_query.as_ref() != Some(query)
                {
                    ...
                    let measured = treemap::measure(focused, query, query.name_hits(&focused.name));
    ...
    fn hit_cell(&self, pos: DVec2) -> Option<usize> {
        ...
        for &index in self.paint_order.iter().rev() {
            ...
            if z > 0.0 {
                for wall in wall_quads(&cam, &rect, z, rise.min(z))
                    .into_iter()
                    .flatten()
                {
    ...
    fn tile_colors(&self, cell: &Cell, palette: &MapPalette) -> (Vec4f, f32) {
        let bg = MapPalette::vec4(&palette.bg);
        if cell.kind == KIND_BUNDLE {
            ...
            return (blend(MapPalette::vec4(&palette.muted), bg, 0.45), 0.35);
        }
        ...
        if class == OTHER_CLASS && !cell.is_dir {
            ...
            hue = blend(hue, MapPalette::vec4(&palette.fg), 0.55);
        }
        if cell.is_group {
    ...
    fn draw_map(
        ...
        let border_ink = MapPalette::vec4(&palette.bg_dark);
        let accent = MapPalette::vec4(&palette.accent);
        let bright = MapPalette::vec4(&palette.fg_bright);
        let ink_dark = MapPalette::vec4(&palette.bg_dark);
        ...
        for &index in &order {
            ...
            } else if !cell.is_group && rect.size.x >= LABEL_MIN.x && rect.size.y >= LABEL_MIN.y {
    ...
    fn draw_labels(&mut self, cx: &mut Cx2d, labels: Vec<Label>, clip: Rect, palette: &MapPalette) {
        cx.push_clip_rect(clip);
        ...
        if labels.iter().any(|l| l.scrim) {
            ...
            let plate_ink = MapPalette::vec4(&palette.bg_dark);
    ...
    fn ensure_frozen_labels(&mut self, cx: &mut Cx2d, palette: &MapPalette, clip: Rect) {
        ...
        for cell in &self.cells {
            ...
            if cell.is_group && cell.header > 0.0 {
                ...
                placed_names.push((at_x, at_x + room, at_y));
                let width = text_width(&self.draw_text, cx, &line);
                let below = (rect.h + rect.y - at_y >= LABEL_TWO_LINE_H).then(|| {
                    fit_text(&self.draw_text, cx, &treemap::format_bytes(cell.size), room)
                });
                out.push(FrozenLabel {
                    at: dvec2(at_x, at_y),
                    line,
    ...
    fn draw_frozen_labels(&mut self, cx: &mut Cx2d, palette: &MapPalette, clip: Rect) {
    ...
    fn draw_crumbs(&mut self, cx: &mut Cx2d, strip: Rect, palette: &MapPalette) {
        self.crumbs.clear();
        self.draw_bg.color = MapPalette::vec4(&palette.bg_dark);
        self.draw_bg.draw_abs(cx, strip);

        let bright = MapPalette::vec4(&palette.fg_bright);
        let dim = MapPalette::vec4(&palette.fg_dim);
        self.rescan_hit = Rect::default();
        self.filter_hit = Rect::default();
        let limit = strip.pos.x + strip.size.x - 8.0;
        let mut x = strip.pos.x + 8.0;
        let names: Vec<String> = std::iter::once(backend::backend().display_name(&self.root))
        ...
        for (depth, name) in names.iter().enumerate() {
            ...
            if x + width > limit {
                ...
                self.draw_text
                    .draw_abs(cx, dvec2(x, strip.pos.y + 5.0), "…");
                break;
            }
            self.draw_bold.color = if depth == last { bright } else { dim };
            self.draw_bold
                .draw_abs(cx, dvec2(x, strip.pos.y + 4.0), &text);
    ...
    fn draw_footer(&mut self, cx: &mut Cx2d, strip: Rect, palette: &MapPalette) {
        self.draw_bg.color = MapPalette::vec4(&palette.bg_dark);
        self.draw_bg.draw_abs(cx, strip);
        ...
        let (head, path, ink) = match &self.pick {
            Some(pick) if pick.bundle > 0 => (
                ...
                format!("{} items each too small to draw on their own", pick.bundle),
                MapPalette::vec4(&palette.fg_dim),
            ),
            Some(pick) => (
                format!(
                    "{}   {:.1}% of {}{}",
                ...
                MapPalette::vec4(&palette.fg_bright),
            ),
            None => (
                String::new(),
                String::new(),
        ...
        self.draw_text.color = MapPalette::vec4(&palette.fg_dim);
    ...
    fn draw_tooltip(&mut self, cx: &mut Cx2d, body: Rect, palette: &MapPalette) {
        ...
        let width =
            text_width(&self.draw_bold, cx, &head).max(text_width(&self.draw_text, cx, &foot))
                + 14.0;
        ...
        self.draw_tile.new_draw_call(cx);
        self.draw_tile.color = MapPalette::vec4(&palette.bg_dark);
        self.draw_tile.edge = MapPalette::vec4(&palette.accent);
        self.draw_tile.cushion = 0.0;
        ...
        self.draw_bold.color = MapPalette::vec4(&palette.fg_bright);
    ...
    fn press(&mut self, cx: &mut Cx, at: DVec2, taps: u32, primary: bool) {
        if self.filter_hit.contains(at) {
            if primary {
                ...
                cx.widget_action(self.uid, DiskMapAction::FilterCleared);
            }
            return;
        }
        ...
        if !cell.is_bundle()
            && !backend::backend().is_demo()
            && !backend::backend().exists(&cell.path)
        {
            let path = cell.path.clone();
            self.forget(cx, &path);
            cx.widget_action(self.uid, DiskMapAction::Vanished(path));
            return;
        }
        self.pick = Some(pick_of(&cell));
        self.redraw(cx);
        ...
        if !primary {
            cx.widget_action(self.uid, DiskMapAction::Selected(cell.path));
            return;
        }
        if taps >= 2 {
            ...
            self.fit_rect(cx, rect);
            cx.widget_action(self.uid, DiskMapAction::Opened(cell.path.clone()));
        }
        cx.widget_action(self.uid, DiskMapAction::Selected(cell.path));
    }

    /// Drive the same scan [`DiskMap::set_root`] runs, for tests that cannot
    /// live-apply a widget.
    #[cfg(test)]
    pub fn scan_root_for_test(cx: &mut Cx, path: &Path) -> Node {
        ...
        assert!(ok, "scan of {} was cancelled", path.display());
    ...
    /// Push a host palette so the map follows the app's theme.
    pub fn set_palette(&mut self, cx: &mut Cx, palette: MapPalette) {
        self.apply_palette(palette);
        self.redraw(cx);
    }

    /// Replace the palette without scheduling a redraw — for a host that is
    /// already in `draw_walk`.
    pub fn apply_palette(&mut self, palette: MapPalette) {
        self.palette = palette;
    }
}

impl Widget for DiskMap {
    fn draw_walk(&mut self, cx: &mut Cx2d, _scope: &mut Scope, walk: Walk) -> DrawStep {
        let rect = cx.walk_turtle(walk);
        let palette = self.palette.clone();
        self.draw_bg.color = MapPalette::vec4(&palette.bg);
        ...
        let body = Rect {
            ...
            size: dvec2(rect.size.x - 2.0, rect.size.y - crumb_h - foot_h - 2.0),
        };
        let now = cx.seconds_since_app_start();
                ...
                if let Some(drag) = self.drag.take() {
                    if !drag.moved {
                        ...
                        if drag.secondary {
                            ...
                            cx.widget_action(self.uid, DiskMapAction::Context(drag.from));
                        }
                    } else {
...
impl DiskMapRef {
    ...
    /// Push a host palette so the map follows the app's theme.
    pub fn set_palette(&self, cx: &mut Cx, palette: MapPalette) {
        if let Some(mut inner) = self.borrow_mut() {
            inner.set_palette(cx, palette);
        }
    }

    /// Replace the palette without scheduling a redraw.
    pub fn apply_palette(&self, palette: MapPalette) {
        if let Some(mut inner) = self.borrow_mut() {
            inner.apply_palette(palette);
        }
    }
...
fn wall_quads(cam: &MapCamera, r: &MapRect, z: f64, band: f64) -> [Option<Wall>; 2] {
    ...
    for (a, b, n) in edges {
        ...
        if let Some(face) =
            ProjectedFace::from_corners(cam, [(a, z), (b, z), (b, z - band), (a, z - band)])
        {
            out[slot] = Some(Wall {
                face,
...
mod tests {
    ...
    fn the_camera_unprojects_its_own_ground() {
        for persp in [false, true] {
            for (yaw, pitch) in [(0.0, 0.0), (0.7, 0.66), (-2.1, 1.1), (3.0, 0.2)] {
                ...
                if !persp && pitch > 0.0 {
                    let flat =
                        cam.project_view(cam.view(dvec2(600.0, 500.0), 0.0, DVec2::default()));
                    let high =
                        cam.project_view(cam.view(dvec2(600.0, 500.0), 40.0, DVec2::default()));
    ...
    fn a_face_straddling_the_near_plane_is_clipped_before_projection() {
        ...
        let top =
            ProjectedFace::of_rect(&cam, &rect, 20.0).expect("part of the top face is visible");
