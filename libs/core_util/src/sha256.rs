//! Self-contained streaming SHA-256 (FIPS 180-4), the one implementation the
//! asset contract, the AI hub's download verification, the model document and
//! the flow engine share. Streaming, because model files are tens of GB and
//! blobs are hashed while they are written, never held in memory.
...
impl Sha256 {
    ...
    /// `finalize` under the name the AI hub's callers use.
...
mod hw {
    ...
    /// An architecture without a kernel has no caller outside the tests.
