...
impl ScriptThreads {
    ...
    /// Collect all stack values from all threads into the provided buffer for GC marking.
    /// Clears and reuses the buffer to avoid allocations.
    pub fn collect_stack_values_into(&self, values: &mut Vec<ScriptValue>) {
        values.clear();
        for thread in &self.threads {
            // Collect stack values
            values.extend_from_slice(&thread.stack);
            // Collect scopes (as objects)
            for scope in &thread.scopes {
                values.push((*scope).into());
            }
            // Collect mes (method call contexts)
            for me in &thread.mes {
                match me {
                    ScriptMe::Object(obj) => values.push((*obj).into()),
                    ScriptMe::Call { sself, args } => {
                        if let Some(s) = sself {
                            values.push(*s);
                        }
                        values.push((*args).into());
                    }
                    ScriptMe::Pod { pod, .. } => values.push((*pod).into()),
                    ScriptMe::Array(arr) => values.push((*arr).into()),
                }
            }
