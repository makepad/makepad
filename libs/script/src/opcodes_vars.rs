...
impl<'a> ScriptVm<'a> {
    ...
    pub(crate) fn handle_begin_proto(&mut self) {
        ...
        let me = self.bx.heap.new_with_proto_checked(proto, self.bx.threads.cur().trap.pass());
    ...
    pub(crate) fn handle_proto_inherit_read(&mut self) {
        ...
        let proto = if let ScriptMe::Object(object) = me {
            let object = *object;
            let value = self.bx.heap.proto_field_from_value(object, field, self.bx.threads.cur().trap.pass());
            if value.is_nil() || value.is_err() {
                self.bx.threads.cur().trap.err.take();
                if let Some(field_id) = field.as_id() {
                    self.bx.heap.proto_field_from_type_check(object, field_id, self.bx.threads.cur().trap.pass())
                } else {
                    NIL
                }
            } else {
    ...
    pub(crate) fn handle_proto_inherit_write(&mut self) {
        ...
        if let Some(me) = self.bx.threads.cur_ref().mes.last() {
            if let ScriptMe::Object(me_obj) = me {
                ...
                self.bx.heap.set_value(me_obj, field, object, self.bx.threads.cur().trap.pass());
            }
        }
    ...
    pub(crate) fn handle_scope_inherit_write(&mut self) {
        ...
        if let Some(id) = id.as_id() {
            self.bx.threads.cur().set_scope_value(&mut self.bx.heap, id, object);
        }
        self.bx.threads.cur().push_stack_unchecked(NIL);
        self.bx.threads.cur().trap.goto_next();
    }
    ...
    pub(crate) fn handle_field_inherit_read(&mut self) {
        ...
        let proto = if let Some(obj) = object.as_object() {
            let value = self.bx.heap.value(obj, field, self.bx.threads.cur().trap.pass());
            if value.is_nil() || value.is_err() {
                self.bx.threads.cur().trap.err.take();
                NIL
            } else {
    ...
    pub(crate) fn handle_field_inherit_write(&mut self) {
        ...
        if let Some(obj) = object.as_object() {
            ...
            self.bx.heap.set_value(obj, field, built_object, self.bx.threads.cur().trap.pass());
        }
        self.bx.threads.cur().push_stack_unchecked(NIL);
        self.bx.threads.cur().trap.goto_next();
    }
    ...
    pub(crate) fn handle_index_inherit_read(&mut self) {
        ...
        let proto = if let Some(obj) = object.as_object() {
            let value = self.bx.heap.value(obj, index, self.bx.threads.cur().trap.pass());
            if value.is_nil() || value.is_err() {
                self.bx.threads.cur().trap.err.take();
                NIL
            } else {
            ...
            let value = self.bx.heap.array_index(arr, idx, self.bx.threads.cur().trap.pass());
            if value.is_nil() || value.is_err() {
                self.bx.threads.cur().trap.err.take();
                NIL
            } else {
    ...
    pub(crate) fn handle_index_inherit_write(&mut self) {
        ...
        let object = self.bx.threads.cur().pop_stack_resolved(&self.bx.heap);
        if let Some(obj) = object.as_object() {
            self.bx.heap.set_value(obj, index, built_object, self.bx.threads.cur().trap.pass());
        } else if let Some(arr) = object.as_array() {
            let idx = index.as_index();
            self.bx.heap.set_array_index(arr, idx, built_object, self.bx.threads.cur().trap.pass());
        }
        self.bx.threads.cur().push_stack_unchecked(NIL);
        self.bx.threads.cur().trap.goto_next();
    }
    ...
    pub(crate) fn handle_end_proto(&mut self) {
        ...
        if let ScriptMe::Object(me) = me {
            self.bx.heap.finalize_maybe_pod_type(me, &self.bx.code.builtins.pod, self.bx.threads.cur().trap.pass());
        }
            ...
            if field.as_id() == Some(id!(*)) {
                ...
                for (k, v) in items {
                    if let Some(id) = k.as_id() {
                        self.bx.threads.cur().def_scope_value(&mut self.bx.heap, id, v);
                    }
                }
            }
            else {
                ...
                if !value.is_nil() {
                    if let Some(field) = field.as_id() {
                        self.bx.threads.cur().def_scope_value(&mut self.bx.heap, field, value);
                    }
                }
            }
        }
    ...
    pub(crate) fn handle_field(&mut self) {
        ...
        if let Some(obj) = object.as_object() {
            ...
            let value = self.bx.heap.pod_read_field(pod, field, &self.bx.code.builtins.pod, self.bx.threads.cur().trap.pass());
            self.bx.threads.cur().push_stack_unchecked(value);
        }
        else {
    ...
    pub(crate) fn handle_me_field(&mut self) {
        ...
        let value = match self.bx.threads.cur_ref().mes.last().unwrap() {
            ScriptMe::Array(_) => {
                script_err_not_allowed!(self.bx.threads.cur_ref().trap, "field access {:?} not allowed in array literal context", field)
            }
            ScriptMe::Call{args, ..} => {
                let args = *args;
                self.bx.heap.value(args, field, self.bx.threads.cur().trap.pass())
            }
            ScriptMe::Pod{pod, ..} => {
                let pod = *pod;
                self.bx.heap.pod_read_field(pod, field, &self.bx.code.builtins.pod, self.bx.threads.cur().trap.pass())
            }
            ScriptMe::Object(obj) => {
                let obj = *obj;
                self.bx.heap.value(obj, field, self.bx.threads.cur().trap.pass())
            }
        };
    ...
    pub(crate) fn handle_proto_field(&mut self) {
        ...
        let object = self.bx.threads.cur().pop_stack_resolved(&self.bx.heap);
        if let Some(obj) = object.as_object() {
            let value = self.bx.heap.proto_field_from_value(obj, field, self.bx.threads.cur().trap.pass());
            if value.is_nil() || value.is_err() {
                self.bx.threads.cur().trap.err.take();
                if let Some(field_id) = field.as_id() {
                    let value = self.bx.heap.proto_field_from_type_check(obj, field_id, self.bx.threads.cur().trap.pass());
                    self.bx.threads.cur().push_stack_unchecked(value);
                } else {
                    let value = script_err_not_found!(self.bx.threads.cur_ref().trap, "proto field lookup requires identifier, got {:?}", field.value_type());
                    self.bx.threads.cur().push_stack_unchecked(value);
                }
            } else {
                self.bx.threads.cur().push_stack_unchecked(value)
            ...
            let value = script_err_wrong_value!(self.bx.threads.cur_ref().trap, "proto_field {:?} target is not an object (got {:?})", field, object.value_type());
            self.bx.threads.cur().push_stack_unchecked(value);
        }
        self.bx.threads.cur().trap.goto_next();
    }
        ...
        match self.bx.threads.cur_ref().mes.last().unwrap() {
            ScriptMe::Object(obj) => {
                ...
                if let Some(source_obj) = source.as_object() {
                    self.bx.heap.merge_object(obj, source_obj, self.bx.threads.cur().trap.pass());
                }
                else if let Some(source_arr) = source.as_array() {
                    let len = self.bx.heap.array_len(source_arr);
                    for i in 0..len {
                        let v = self.bx.heap.array_index(source_arr, i, self.bx.threads.cur().trap.pass());
                        self.bx.heap.vec_push(obj, NIL, v, self.bx.threads.cur().trap.pass());
                    }
                }
            }
            ScriptMe::Array(arr) => {
                ...
                if let Some(source_arr) = source.as_array() {
                    self.bx.heap.merge_array(arr, source_arr, self.bx.threads.cur().trap.pass());
                }
                else if let Some(source_obj) = source.as_object() {
                    self.bx.heap.array_push_vec(arr, source_obj, self.bx.threads.cur().trap.pass());
                }
            }
            ScriptMe::Call{args, ..} => {
                let args = *args;
                if let Some(source_obj) = source.as_object() {
                    ...
                    for i in 0..len {
                        let kv = self.bx.heap.vec_key_value(source_obj, i, self.bx.threads.cur().trap.pass());
                        self.bx.heap.unnamed_fn_arg(args, kv.value, self.bx.threads.cur().trap.pass());
                    }
                }
                else if let Some(source_arr) = source.as_array() {
                    let len = self.bx.heap.array_len(source_arr);
                    for i in 0..len {
                        let v = self.bx.heap.array_index(source_arr, i, self.bx.threads.cur().trap.pass());
                        self.bx.heap.unnamed_fn_arg(args, v, self.bx.threads.cur().trap.pass());
                    }
                }
            }
            ScriptMe::Pod{..} => {
                script_err_not_impl!(self.bx.threads.cur_ref().trap, "splat operator (..) not supported for pod types");
            }
        }
        self.bx.threads.cur().trap.goto_next();
    }
    ...
    pub(crate) fn handle_array_index(&mut self) {
        ...
        if let Some(obj) = object.as_object() {
            ...
            let value = self.bx.heap.array_index(arr, index, self.bx.threads.cur().trap.pass());
            self.bx.threads.cur().push_stack_unchecked(value)
        }
        else if let Some(pod) = object.as_pod() {
            let index = index.as_index();
            let value = self.bx.heap.pod_array_index(pod, index, &self.bx.code.builtins.pod, self.bx.threads.cur().trap.pass());
            self.bx.threads.cur().push_stack_unchecked(value)
        }
        else {
            let value = script_err_wrong_value!(self.bx.threads.cur_ref().trap, "cannot index {:?} on {:?} (not an object/array/pod)", index, object.value_type());
            self.bx.threads.cur().push_stack_unchecked(value);
        }
        self.bx.threads.cur().trap.goto_next();
    }
        ...
        if let Some(loc) = self.bx.code.ip_to_loc(self.bx.threads.cur_ref().trap.ip) {
            if value != NIL {
                if let Some(err_ptr) = value.as_err() {
                    ...
                    self.bx.heap.to_debug_string(value, &mut recur, &mut out, true, 0);
