...
impl ScriptStringData{
    pub fn add_type_methods(native: &mut ScriptNative, heap: &mut ScriptHeap){
        ...
        native.add_type_method(heap, ScriptValueType::REDUX_STRING, id!(parse_json), &[], |vm, args|{
            ...
            let result = if let Some(r) = vm.bx.heap.string_mut_self_with(sself, |heap,s|{
                json_parser.read_json(s, heap)
            }){
                r
            }
            else{
                script_err_unexpected!(vm.bx.threads.cur_ref().trap, "parse_json called on non-string value")
            };
        ...
        native.add_type_method(heap, ScriptValueType::REDUX_STRING, id!(trim), script_args_def!(), |vm, args|{
            ...
            script_err_unexpected!(vm.bx.threads.cur_ref().trap, "trim called on non-string value")
        });
        ...
        native.add_type_method(heap, ScriptValueType::REDUX_STRING, id!(strip_prefix), script_args_def!(pat = NIL), |vm, args|{
            ...
            if let Some(Some(s)) = vm.bx.heap.string_mut_self_with(sself,|heap,sself|{
                heap.string_mut_self_with(pat,|heap,pat|{
                    heap.new_string_from_str(if let Some(s) = sself.strip_prefix(pat){s}else{sself})
                })
            }){
                return s.into()
            }
            script_err_unexpected!(vm.bx.threads.cur_ref().trap, "strip_prefix requires string arguments")
        });
        ...
        native.add_type_method(heap, ScriptValueType::REDUX_STRING, id!(strip_suffix), script_args_def!(pat = NIL), |vm, args|{
            ...
            if let Some(Some(s)) = vm.bx.heap.string_mut_self_with(sself,|heap,sself|{
                heap.string_mut_self_with(pat,|heap,pat|{
                    heap.new_string_from_str(if let Some(s) = sself.strip_suffix(pat){s}else{sself})
                })
            }){
                return s.into()
            }
            script_err_unexpected!(vm.bx.threads.cur_ref().trap, "strip_suffix requires string arguments")
        });
        ...
        native.add_type_method(heap, ScriptValueType::REDUX_STRING, id!(split), script_args_def!(pat = NIL), |vm, args|{
            ...
            script_err_unexpected!(vm.bx.threads.cur_ref().trap, "split requires string arguments for both self and pattern")
        });
    }
}