use super::{
    ...
    GemmaTextRuntimeSession, GemmaTextSamplingOptions, GemmaTextStepOutput, MlxTextSamplingRng,
    RopeSpec, TextLayerTensorNames,
};
...
fn greedy_generate_with_backend_config(
    ...
    let model =
        GemmaTextModel::load_with_backend_config(default_model_path(), backend_config).unwrap();
    let formatted_prompt_text = model
        .runtime
...
fn host_layer0_reports_against_exact_cached_stage_hashes() {
    ...
    .unwrap();
    let mut decode_q =
        rms_norm_rows_weighted_f32(&decode_q_raw, q_head_count, head_dim, &q_norm_weights).unwrap();
    ...
    .unwrap();
    let mut decode_k =
        rms_norm_rows_weighted_f32(&decode_k_raw, k_head_count, head_dim, &k_norm_weights).unwrap();
...
fn formatted_say_hi_layer0_last_prompt_token_stage_hashes_against_forced_exact() {
    ...
    let decode_hidden = runtime
        .weights
        .embed_token_bf16_words(last_token_id)
        .unwrap();
    let decode_input_norm = rms_norm_weighted_tensor(
        &runtime.weights,
        &decode_hidden,
    ...
    for (stage, host_hash, exact_hash) in stages {
        println!("{stage} host=0x{host_hash:016X} exact=0x{exact_hash:016X}");
    }
...
fn exact_cursor_matches_two_token_exact_path() {
    ...
    let mut cursor = runtime
        .start_generation_cursor(Arc::<[u32]>::from(exact.prompt_token_ids.clone()), Some(1))
        .unwrap();
...
fn poem_prompt_32_token_prefix_matches_local_mlx() {
    ...
    let expected_prompt_token_ids = [
        2, 105, 2364, 107, 6974, 496, 27355, 528, 7121, 236743, 236770, 236771, 4463, 1003, 44623,
        6571, 236764, 31770, 12207, 236764, 532, 6211, 657, 38735, 236761, 17608, 1546, 1757,
        63510, 236761, 106, 107, 105, 4368, 107,
    ];
    let expected_generated_token_ids = [
        4, 2, 2, 4, 2, 4, 4, 4, 2, 13, 36, 40, 20, 2, 4, 2, 13, 2, 9, 17, 16, 20, 20, 34, 2, 2, 11,
        2, 5, 2, 9, 2,
    ];
...
fn rotor_k_cache_reports_long_greedy_divergence_against_bf16_baseline() {
    ...
    println!("baseline_generated_ids={:?}", baseline.generated_token_ids);
...
fn rotor_planar3_k_cache_reports_long_greedy_divergence_against_bf16_baseline() {
    ...
    println!("baseline_generated_ids={:?}", baseline.generated_token_ids);
    println!(
        "rotor_planar3_generated_ids={:?}",
        rotor.generated_token_ids
    );
    println!(
            "matched_prefix_tokens={} mismatch={:?} baseline_len={} rotor_len={} baseline_stop={:?} rotor_stop={:?}",
            matched_prefix,
...
fn formatted_say_hi_exact_backend_hash_manifest() {
    let runtime =
        Arc::unwrap_or_clone(GemmaTextRuntimeSession::load(&default_model_path()).unwrap());
    let formatted_prompt = runtime.format_prompt_text("say hi", GemmaPromptFormat::Gemma4UserTurn);
    let prompt_token_ids = runtime.tokenize_prompt(&formatted_prompt).unwrap();
    let num_layers = runtime
        .weights
...
fn formatted_say_hi_exact_prefill_final_hidden() {
    ...
    let next_token = backend
        .greedy_token_from_hidden_words(&final_hidden_words)
        .unwrap();
    println!("prompt_ids={:?}", prompt_token_ids);
    println!(
        "final_hidden_fnv1a64=0x{:016X}",
...
fn formatted_say_hi_31b_exact_prefill_final_hidden() {
    let mut backend_config = GemmaExactMetalConfig::default();
    backend_config.backend_mode = GemmaExactMetalBackendMode::Force;
    let runtime = GemmaTextRuntimeSession::load_with_backend_config(
        &gemma_31b_q8_model_path(),
        backend_config,
    )
    .unwrap();
    let formatted_prompt = runtime.format_prompt_text("say hi", GemmaPromptFormat::Gemma4UserTurn);
...
fn formatted_say_hi_host_vs_exact_prefill_hash_manifest() {
    ...
    let host_hidden_bits = host_hidden_words
        ...
        .map(f32::to_bits)
        .collect::<Vec<_>>();
    let host_next = runtime
        .greedy_token_from_hidden(&host_hidden_words)
        .unwrap();
    ...
    let exact_hidden_bits = exact_hidden_words
        ...
        .map(f32::to_bits)
        .collect::<Vec<_>>();
    let exact_next = backend
        .greedy_token_from_hidden_words(&exact_hidden_words)
        .unwrap();
    ...
    println!(
        ...
        host_next.token_id, host_next.logit, exact_next.token_id, exact_next.logit
    );
