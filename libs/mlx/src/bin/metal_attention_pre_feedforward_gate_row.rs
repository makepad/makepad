...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    while let Some(arg) = args_iter.next() {
        match arg.as_str() {
            ...
            "-h" | "--help" => {
                eprintln!(
                    "Usage: metal_attention_pre_feedforward_gate_row [model.safetensors] [--warmup N] [--iters N] [--dump-all-f32-bits] [--analyze-row-1836] [--analyze-row N]"
                );
                return Ok(());
            }
    ...
    if dump_all_f32_bits {
        ...
        println!();
        print!("all_f32_bits=");
        for (index, bits) in mlp_gate_bits.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
