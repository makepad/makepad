...
const EXPECTED_PRE_FEEDFORWARD_NORM2_HASH: u64 = 0x8C56_E6A1_FF78_1B5F;
const EXPECTED_PRE_FEEDFORWARD_NORM2_FIRST16_BITS: [u32; 16] = [
    0xBE16_0000,
    0xBE6D_0000,
    0xBDE4_0000,
    0xBD5A_0000,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    while let Some(arg) = args_iter.next() {
        match arg.as_str() {
            ...
            "-h" | "--help" => {
                eprintln!(
                    "Usage: metal_attention_pre_feedforward_norm_row [model.safetensors] [--warmup N] [--iters N] [--dump-all-f32-bits] [--moe-branch] [--analyze-row-1836] [--analyze-row N]"
                );
                return Ok(());
            }
    ...
    let pre_feedforward_norm_weight_name = if moe_branch {
        PRE_FEEDFORWARD_NORM2_WEIGHT_NAME
    } else {
        PRE_FEEDFORWARD_NORM_WEIGHT_NAME
    };
    let expected_pre_feedforward_norm_hash = if moe_branch {
        EXPECTED_PRE_FEEDFORWARD_NORM2_HASH
    } else {
        EXPECTED_PRE_FEEDFORWARD_NORM_HASH
    };
    let expected_pre_feedforward_norm_first16_bits = if moe_branch {
        &EXPECTED_PRE_FEEDFORWARD_NORM2_FIRST16_BITS
    } else {
    ...
    if dump_all_f32_bits {
        ...
        println!();
        print!("all_f32_bits=");
        for (index, bits) in pre_feedforward_norm_bits.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
    ...
    println!("branch={}", if moe_branch { "moe" } else { "dense" });
    println!("eps={EPS}");
    ...
    print!("attention_pre_ffn_norm_first16_f32_bits=");
    for (index, bits) in pre_feedforward_norm_first16.iter().enumerate() {
        if index != 0 {
            print!(",");
        }
        print!("0x{bits:08X}");
