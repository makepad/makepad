...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    while let Some(arg) = args_iter.next() {
        match arg.as_str() {
            ...
            "-h" | "--help" => {
                eprintln!(
                    "Usage: metal_attention_oproj_row [model.safetensors] [--warmup N] [--iters N] [--dump-all-f32-bits] [--skip-oracle-checks] [--analyze-row-1836] [--analyze-row N]"
                );
                return Ok(());
            }
    ...
    if dump_all_f32_bits {
        ...
        println!();
        print!("all_f32_bits=");
        for (index, bits) in o_proj_bits.iter().enumerate() {
            if index != 0 {
                print!(",");
            }
            print!("0x{bits:08X}");
    ...
    print!("attention_oproj_first16_f32_bits=");
    for (index, bits) in o_proj_main_first16.iter().enumerate() {
        if index != 0 {
            print!(",");
        }
        print!("0x{bits:08X}");
