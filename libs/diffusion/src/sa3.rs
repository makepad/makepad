...
pub(crate) fn sa3_threads() -> usize {
...
pub(crate) fn par_rows<F: Fn(usize, &mut [f32]) + Sync>(out: &mut [f32], row_len: usize, f: &F) {
...
pub(crate) fn linear(a: &[f32], w: &[f32], bias: Option<&[f32]>, m: usize, k: usize, n: usize) -> Vec<f32> {
...
pub(crate) fn rms_norm_rows(x: &mut [f32], gamma: &[f32], width: usize, eps: f32) {
    let dim = gamma.len();
...
impl F16Weight {
    pub(crate) fn new(key: impl Into<String>, w: &[f32], n: usize, k: usize) -> Self {
        debug_assert_eq!(w.len(), n * k);
    ...
    pub(crate) fn part(&self) -> makepad_ggml::backend::cuda::GpuLinearPart<'_> {
...
pub(crate) fn dev_err(what: &str, err: String) -> DiffusionError {
