...
use makepad_diffusion::t5_encoder::{
    CompiledT5xxl, LazyT5xxl, LoadedT5xxlWeights, T5xxlExecutionMode,
};
use std::env;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let run = match mode {
        T5xxlExecutionMode::Lazy => {
            let lazy = LazyT5xxl::compile(&mut weights, &tokenized)?;
            lazy.execute(&weights, &tokenized.token_ids)?
        }
        T5xxlExecutionMode::Compiled => {
            let compiled = CompiledT5xxl::compile(&mut weights, &tokenized)?;
