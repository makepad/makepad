...
pub fn try_zeroed_slice_box<T: Zeroable>(length: usize) -> Result<Box<[T]>, ()> {
    ...
    if ptr.is_null() {
        ...
        let slice = unsafe { core::slice::from_raw_parts_mut(ptr as *mut T, length) };
...
pub fn try_cast_vec<A: NoUninit, B: AnyBitPattern>(
    ...
    if align_of::<A>() != align_of::<B>() {
        ...
        if (size_of::<B>() == 0 && input_capacity != 0)
            || (size_of::<B>() != 0
                && (input_size % size_of::<B>() != 0 || input_capacity % size_of::<B>() != 0))
        {
            // If the size in bytes of the underlying buffer does not match an exact
            // multiple of the size of B, we cannot cast between them.
            ...
            let length: usize = if size_of::<B>() != 0 {
                input_size / size_of::<B>()
            } else {
                0
            };
            let capacity: usize = if size_of::<B>() != 0 {
                input_capacity / size_of::<B>()
            } else {
                0
            };
...
pub fn pod_collect_to_vec<A: NoUninit, B: NoUninit + AnyBitPattern>(src: &[A]) -> Vec<B> {
    ...
    let dst_count = src_size / size_of::<B>() + if src_size % size_of::<B>() != 0 { 1 } else { 0 };
...
#[inline]
pub fn cast_rc<A: NoUninit + AnyBitPattern, B: NoUninit + AnyBitPattern>(input: Rc<A>) -> Rc<B> {
...
#[inline]
#[cfg(target_has_atomic = "ptr")]
pub fn cast_arc<A: NoUninit + AnyBitPattern, B: NoUninit + AnyBitPattern>(input: Arc<A>) -> Arc<B> {
...
impl<T: AnyBitPattern> sealed::FromBoxBytes for [T] {
    fn try_from_box_bytes(bytes: BoxBytes) -> Result<Box<Self>, (PodCastError, BoxBytes)> {
        ...
        if bytes.layout.align() != single_layout.align() {
            ...
            || (single_layout.size() != 0 && bytes.layout.size() % single_layout.size() != 0)
        {
            ...
            let ptr = core::ptr::slice_from_raw_parts_mut(ptr.as_ptr() as *mut T, length);
            // SAFETY: See BoxBytes type invariant.
            Ok(unsafe { Box::from_raw(ptr) })
        }
    }
