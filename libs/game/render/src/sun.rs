...
//! space (x east, y SOUTH, z up) while games are y-up. [`GameSun`] is that
...
pub fn resolve_sun(cfg: &makepad_game_sim::SunConfig) -> GameSun {
    let mut sun = match cfg.time_of_day {
        Some(hours) => GameSun::from_time_of_day(hours, cfg.latitude),
        None => GameSun::default(),
    };
...
mod tests {
    ...
    fn scene_sun_axis_mapping_round_trips() {
        ...
        let game = GameSun::from_scene_sun(&scene);
        // map z (up) -> game y (up)
    ...
    fn time_of_day_keeps_the_sun_above_the_horizon() {
        for hour in [6.0f32, 9.0, 12.0, 15.0, 18.0] {
            let sun = GameSun::from_time_of_day(hour, 52.0);
    ...
    fn noon_sun_is_higher_than_evening_sun() {
        let noon = GameSun::from_time_of_day(12.0, 52.0);
        let evening = GameSun::from_time_of_day(18.5, 52.0);
