...
impl LightBake {
    ...
    pub fn update(&mut self, world: &GameWorld, sun: &GameSun) -> bool {
    ...
    fn collect_occluders(&mut self, world: &GameWorld) {
        ...
        for e in world.entities.iter() {
            if e.sensor || !matches!(e.kind, BodyKind::Static | BodyKind::Kinematic) {
                continue;
            }
    ...
    fn bake_ao(&mut self, world: &GameWorld) {
        ...
        for e in world.entities.iter() {
            if e.kind != BodyKind::Static || e.sensor {
                continue;
            }
            statics += 1;
            let h = vec3f(
    ...
    fn bake_sun(&mut self, world: &GameWorld, sun: &GameSun) {
        let t0 = std::time::Instant::now();
        let terrain = world.terrain.as_ref();
...
mod tests {
    ...
    fn a_box_in_the_open_is_brighter_than_one_in_a_pit() {
        ...
        bake.update(&world, &GameSun::default());
