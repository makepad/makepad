//! `assets.query` — the Asset Store's read-only SQL surface over the live
//! catalog.
//!
//! This lives in the store host and opens the server's own `catalog.sqlite3`
//! read-only through makepad-sqlite's WAL-interop snapshot reads while the
//! store keeps serving. Clients reach it through `POST /v1/assets/query`;
//! the chat broker also uses this same implementation during its transition
//! out of the store.
//!
...
/// The hard server-side row ceiling for one query.
