...
pub fn sandbox_effect(name: &str) -> Option<SandboxEffect> {
    ...
    match canonicalize_tool_name(name).as_str() {
        ...
        | "world.get_plan" | "world.render" | "world.api" | "model.fetch" | "model.inspect" | "model.jobs" | "model.render" => Some(Read),
...
mod tests {
    ...
    fn every_definition_and_wire_spelling_has_an_effect() {
        ...
        assert_eq!(defs.len(), 31, "classify new Sandbox definitions explicitly");
        for def in defs {
