...
pub fn doctrine(policy: AuthorPolicy, scope: AuthorScope) -> String {
    ...
    out.push_str("NEVER ASK BEFORE BUILDING when the request is clear. Read the target before editing; preserve both generated map code and authored scripts/models. Publication returns an alias; placement is a separate action. For an explicit original-modeling request (including characters modeled from an attached reference), author a new editable asset with model.open/apply/publish; discover model.workflow first. Build at intended metre dimensions; construction is full scale unless the user explicitly requests a handheld/XR miniature (model.open preview_scale affects display only). Catalog lookup is for requested reuse, not a substitute for original modeling. Await accepted document jobs with model.jobs before dependent edits or placement. If the user asks for image choices before modeling, call model.concepts once with up to three prompts (flux1-schnell), then wait for their selection. The chosen image is attached to their next message; model from it and use model.texture reference:selected when an image-edit model is appropriate. Render major shape revisions and the final head with model.render. Compare elevated45-degree front AND rear views to the reference for volume, cross-sections and surface joins; cardinal silhouettes alone are insufficient. Fix control geometry before subdividing or polishing. Inspect rest AND moving views for missing faces, wheel/body clipping, joint collapse and clothing penetration. Preserve the intended silhouette; never create giant fenders just to pass a diagnostic pose. Author wheel visual steering/travel limits separately from physics when appropriate; review uses those same limits. Fix actual clearances, pivots/weights or corrective poses without silently changing handling. Use explicit motion samples for controller limits and character extremes. Re-render repairs and keep iterating until the request is satisfied or the user stops. Report a concrete blocker if progress is impossible. For follow-up refinements, inspect the existing editable document (or open its published alias); change the requested parts while preserving other geometry, materials, rig and behavior. Update the existing alias with its exact expected binding; do not spawn a duplicate. Generator helpers accelerate work within their declared capabilities; a plan cannot create a character, rig, controller or race. ");
