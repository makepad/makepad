//! Streaming SHA-256, self-contained on purpose: a crate that verifies
//! downloads should not pull anything in to do it. Streaming because model
//! files are tens of GB — they are hashed in chunks straight off the
//! download, never held in memory.
...
impl Sha256 {
    ...
    pub fn finish(mut self) -> [u8; 32] {
        ...
        pad.push(0x80u8);
        let pad_zeros = (64 + 56 - (self.buffer_len + 1) % 64) % 64;
        pad.resize(1 + pad_zeros, 0);
        pad.extend_from_slice(&bit_len.to_be_bytes());
        // Feed padding through the same buffering path (without recounting).
