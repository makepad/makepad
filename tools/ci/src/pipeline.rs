...
/// Cargo never removes what it built for a feature set or a dependency graph
/// that is gone: each such change leaves a whole second copy behind, for every
/// target the rows check. The worst a run can add is one more copy of what is
/// there, so when the build output has grown as large as the space the volume
/// has left, it goes, and this run builds from cold instead of failing every
/// row on a full disk.
fn make_room(run: &mut Run, target: &Path) -> Result<()> {
    if !target.is_dir() {
        return Ok(());
    }
    let available = makepad_widgets::makepad_platform::storage::volume_available_bytes(target)
        .map_err(|e| format!("free space of {}: {e}", target.display()))?;
    let size = tree_size(target, available);
    if size < available {
        return Ok(());
    }
    run.log(&format!(
        "build output {} is at least the {} GB left on its volume; removing it",
        target.display(),
        available / 1_000_000_000
    ));
    fs::remove_dir_all(target).map_err(|e| format!("removing {}: {e}", target.display()))
}

/// Bytes the files under `dir` hold, counted until they reach `limit`.
fn tree_size(dir: &Path, limit: u64) -> u64 {
    let mut total = 0u64;
...
fn sync(run: &mut Run, base: &Path, config: &Config, i: usize) -> Result<()> {
    ...
    if checkout.exists() {
        ...
        make_room(run, &checkout.join("target"))?;
    } else {
...
mod tests {
    use super::*;
    #[test]
    fn build_output_is_counted_through_nested_directories_up_to_the_limit() {
        let root = std::env::temp_dir().join(format!("ci-tree-size-{}-{}", std::process::id(), crate::report::stamp()));
        fs::create_dir_all(root.join("debug/deps")).unwrap();
        fs::write(root.join("debug/deps/a.rlib"), [0u8; 300]).unwrap();
        fs::write(root.join("debug/b.d"), [0u8; 200]).unwrap();
        fs::write(root.join("c"), [0u8; 100]).unwrap();
        assert_eq!(tree_size(&root, u64::MAX), 600);
        let capped = tree_size(&root, 250);
        assert!((250..600).contains(&capped), "{capped}");
