...
pub fn stage(d: &Dyn) -> Result<(), String> {
    ...
    let (copied, total) = sync_tree(repo, src, &files)?;
    ...
    let ma = cargo_metadata(src, &["--filter-platform", d.triple(), "--no-default-features"], &env)?;
    let android_ids = ma.closure(&roots)?;
    ...
    if src.is_dir() {
        ...
        while let Some(dir) = stack.pop() {
            for entry in fs::read_dir(&dir).map_err(|e| format!("{}: {e}", dir.display()))? {
                ...
                if ["vendor", ".cargo", "local", BOOTSTRAP].contains(&top) {
                    continue;
                }
