...
pub fn get_binary_name_from_args(args: &[String]) -> Result<&str, String> {
    if args.is_empty() {
    ...
    for i in 0..args.len() {
        if args[i] == "--bin" {
            if i + 1 < args.len() {
                ...
                return Err("Binary name not defined".into());
            }
        }
    }
    Err("Not found".into())
    ...
    for i in 0..args.len() {
        if args[i] == "-p" || args[i] == "--package" {
            if i + 1 < args.len() {
                ...
                return Err("Package name not defined".into());
            }
        }
    }
    Err("Crate name not found".into())
}
