...
fn print_usage() {
    eprintln!("Usage:");
    eprintln!("  Server: cargo makepad remote --server [--port PORT] [--all]");
    eprintln!("  Client: cargo makepad remote <ip:port> cargo [args...]");
    eprintln!(
        "          cargo makepad remote <ip:port> shell <command...>  (requires --all on server)"
    );
}

pub fn handle_remote(args: &[String]) -> Result<(), String> {
    if args.is_empty() {
        print_usage();
        return Err("remote mode requires arguments".to_string());
    }
