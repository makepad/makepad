...
//! subdirectory, so the packaged executable never replaces the plain one),
//! which makes it read its resources from inside the bundle rather than from
//! the source checkout:
//!
//! ```text
//! <Name>.app/Contents/
...
/// compile time (by `app_main!`, in the app crate), so in the normal one the
/// packaged executable would replace the plain `target/<profile>/<bin>`,
/// which then looks for its resources beside itself.
