...
mod native {
    ...
    fn read_utf8(first: u8, extra: usize, rx: &mpsc::Receiver<u8>) -> InputEvent {
        ...
        match String::from_utf8(bytes) {
            Ok(s) => s
                .chars()
                .next()
                .map_or(InputEvent::Unknown, InputEvent::Char),
    ...
    fn parse_csi(rx: &mpsc::Receiver<u8>) -> InputEvent {
        ...
        loop {
            match rx.recv_timeout(Duration::from_millis(50)) {
                ...
                Ok(b) if (0x20..=0x2f).contains(&b) => loop {
                    match rx.recv_timeout(Duration::from_millis(50)) {
    ...
    impl App {
        ...
        fn init(&mut self) -> io::Result<()> {
            ...
            tlog!(
                "init: rows={} cols={} content_row={} scroll_bottom={}",
                self.rows,
                self.cols,
                self.content_row,
                self.scroll_bottom
            ...
            tlog!(
                "init: needed_below={} available_below={}",
                needed_below,
                available_below
            );
        ...
        fn draw_pinned(&self, buf: &mut Vec<u8>) {
            ...
            if self.input.is_empty() && !self.working {
                write!(buf, "{BOLD}\u{203a}{NO_DIM} {DIM}Enter a prompt...{RST}").unwrap();
            } else {
            ...
            write!(buf, "  fake-model \u{00b7} 100% left \u{00b7} {}", self.cwd).unwrap();
        }
    ...
    impl TermGuard {
        ...
        fn cleanup(&self) -> io::Result<()> {
            ...
            write!(stdout, "\x1b[<1u\x1b[?2004l\x1b[?1004l\x1b[0m\x1b[?25h")?;
            stdout.flush()?;
            Ok(())
        }
    }
    ...
    fn run() -> io::Result<()> {
        ...
        tlog!(
            "startup: terminal_size={}x{} stdin_fd={}",
            rows,
            cols,
            stdin_fd
        );
        ...
        tlog!(
            "startup: DSR cursor_pos=row:{} col:{}",
            start_row,
            _start_col
        );
        ...
        tlog!(
            "startup: scroll_bottom={} content_row={} PINNED_ROWS={}",
            app.scroll_bottom,
            app.content_row,
            PINNED_ROWS
        );
        app.init()?;
        tlog!(
            "startup: init done, content_row after init={}",
            app.content_row
        );
