...
impl Setup {
    ...
    fn setup_tools(&mut self) -> Result<(), String> {
        let ready = self.ready();
        if ready.compiler() && !self.compiler_retry {
            if cfg!(windows) {
    ...
    fn install_compiler(&mut self, release: &Release) -> Result<bool, String> {
        ...
        if !self.compiler_consent(ready, release)? {
    ...
    fn compiler_consent(&self, ready: Ready, release: &Release) -> Result<bool, String> {
...
impl Setup {
    ...
    fn build_release(&mut self, release: Release) -> Result<(), String> {
        ...
        if self.merge_marker(&release.id).is_file() {
...
/// Update, disk and the scope command.
impl Setup {
    ...
    fn clear_build(&mut self) -> Result<(), String> {
        ...
        // The target folder is named explicitly: cargo clean never guesses.
        command.current_dir(&environment.cwd).envs(&environment.vars).arg("clean").arg("--target-dir").arg(&environment.build);
