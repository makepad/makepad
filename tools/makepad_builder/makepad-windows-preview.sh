...
setup_rust() {
    ...
    bar 'Build tools' 0 1480 MB 3 'Microsoft C++ Build Tools and Windows SDK'
    spin 'Unpacking the build tools' 0.8
    bar Rust 0 66.4 MB 1.5 static.rust-lang.org
    spin 'Checking the compiler' 0.4
    rust=local accepted=yes
    message="${ok}✓$r0 Build tools and Rust $rust_pin installed in this folder. Nothing else on your PC changed."
}
...
setup_cuda() {
    ...
    cuda=installing; draw
    bar CUDA 0 2940 MB 3 developer.download.nvidia.com
    spin 'Checking CUDA' 0.4
    cuda=ready
    [ "$(get amp)" != ready ] || put amp compile
    message="${ok}✓$r0 CUDA $cuda_pin installed. Makepad Amp recompiles with its AI features on its next run."
}
