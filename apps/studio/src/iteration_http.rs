        ...
        let outcome: Result<(u16, String), (u16, String)> = (|| {
            match (request.method.as_str(), parts.as_slice()) {
                ...
                ("GET", [_, _, _, "calls"]) => {
                    ...
                    let calls = code_trace_read(&capability.control, CODE_TRACE_PAGE).map_err(|error| (500, error))?;
                    Ok((200, json::obj(vec![("schema_version", Value::Int(1)), ("flow_id", s(resolved)), ("calls", Value::Arr(calls))]).to_json()))
                },
...
fn flow_http_manifest(flow: &str) -> Result<String, (u16, String)> {
    let tools = crate::iteration_tools::tool_defs()
        ...
        .chain(crate::atlas::registry::lane_tool_defs(false))
    ...
    Ok(json::obj(vec![("schema_version", Value::Int(1)), ("flow_id", s(flow)), ("tools", Value::Arr(tools)),
        ...
        ("code", s(crate::atlas::registry::LANE_GUIDANCE)),
        ("calls", s("GET calls lists this lane's recent StudioCall trace records (tool, args, result_ref, outcome, revision, duration, truncated). GET result/<id> fetches a deferred code_arch_diff reply once it is published.")),
