//! The shared workspace surface: the Structured Dock, the Architecture Dock
//! and the Disk Dock, one drawn at a time, with the async pump that reaches
//! every resident of all three. The surface changes which Dock shows; it never owns
...
pub struct StudioSurface {
    ...
    #[live]
    dock: WidgetRef,
    /// The Architecture presentation's own Dock: the plan and its report.
    #[live]
    architecture_dock: WidgetRef,
    ...
    #[rust]
    architecture_draw_list: Option<DrawList2d>,
    #[rust]
...
impl WidgetNode for StudioSurface {
    ...
    fn redraw(&mut self, cx: &mut Cx) {
        self.area.redraw(cx);
        self.dock.redraw(cx);
        self.architecture_dock.redraw(cx);
        self.disk_dock.redraw(cx);
    }
    fn children(&self, visit: &mut dyn FnMut(LiveId, WidgetRef)) {
        visit(id!(dock), self.dock.clone());
        visit(id!(architecture_dock), self.architecture_dock.clone());
    ...
    fn find_widgets_from_point(&self, cx: &Cx, p: DVec2, found: &mut dyn FnMut(&WidgetRef)) {
        match self.workspace.mode {
            ...
            Mode::Architecture => self.architecture_dock.find_widgets_from_point(cx, p, found),
...
impl StudioSurface {
    ...
    fn cancel_resident_gestures(&self, cx: &mut Cx) {
        for dock in [&self.dock, &self.architecture_dock, &self.disk_dock] {
            let bodies = dock
                .borrow_mut::<Dock>()
    ...
    pub(crate) fn pump_async(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        let key = (self.workspace.mode, Self::dock_identity(&self.dock), Self::dock_identity(&self.architecture_dock), Self::dock_identity(&self.disk_dock), external);
        if self.pump_key.as_ref() != Some(&key) || self.pump.is_none() {
            let mut pump = ResidentPump::new(self.external_uids());
            match self.workspace.mode {
                Mode::Architecture => {
                    pump.add_dock(&self.architecture_dock);
                ...
                Mode::Disk => {
                    ...
                    pump.add_hidden_dock(&self.dock);
                    pump.add_hidden_dock(&self.architecture_dock);
                }
    ...
    pub fn architecture_view(&self, cx: &mut Cx) -> WidgetRef {
        self.architecture_dock
            .as_dock()
            .item_or_create(cx, id!(plan_tab), id!(ArchitectureTab))
            .unwrap_or_default()
    }
    /// Materialize the plan view and its side report so toolbar/status binds.
    pub fn ensure_architecture_tabs(&self, cx: &mut Cx) {
        let _ = self
            .architecture_dock
            .as_dock()
            .item_or_create(cx, id!(plan_tab), id!(ArchitectureTab));
        let _ = self
            .architecture_dock
            .as_dock()
            .item_or_create(cx, id!(plan_report_tab), id!(PlanReportTab));
    }
...
/// The Architecture presentation: the plan and its report Dock.
...
impl Presenter for ArchitecturePresenter<'_> {
    ...
    fn draw(&mut self, cx: &mut Cx2d, scope: &mut Scope, view: Rect) {
        let mut list = self.0.architecture_draw_list.take().unwrap_or_else(|| DrawList2d::new(cx));
        list.begin_always(cx);
        cx.begin_turtle(
            Walk {
                abs_pos: Some(view.pos),
        ...
        self.0.architecture_dock.draw_walk_all(cx, scope, Walk::fill());
        cx.end_turtle();
        list.end(cx);
        list.set_view_transform_self_only(cx, &Mat4f::identity());
        self.0.architecture_draw_list = Some(list);
    }
    fn input(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        if !surface_pump::is_input(event) {
            self.0.pump_async(cx, event, scope);
            if let Some(mut dock) = self.0.architecture_dock.borrow_mut::<Dock>() {
                dock.handle_event_with_bodies(cx, event, scope, BodyDispatch::Skip);
            }
        } else {
            self.0.architecture_dock.handle_event(cx, event, scope);
        }
    }
    fn deactivate(&mut self, cx: &mut Cx) {
        cx.hide_text_ime();
