...
const HELP: &str = "director-flow [--request-id <id>] <tool-or-alias> '<JSON object>'\ndirector-flow --result <id>\ndirector-flow --tools | --url | --help\n\nAliases: inspect, todos, requirement, prepared, build, freeze, feedback, git_inspect, diff, test, agent.\nRun director-flow --tools for current argument schemas. Run director-flow --url for the current lane's local callback URL. Keep that capability private.\n\n--request-id <id> fixes the request identity so an interrupted call can be recovered with --result <id> (an identical repeat reuses the retained reply; changed arguments are refused). For agent start the request id is also the durable launch identity: a repeated or recovered id reports the existing child instead of starting another.\n\nManaged screen sessions resolve their current owner from MAKEPAD_AGENTS_SESSION and MAKEPAD_AGENTS_STATE_DIR on every call, including after a split. Mirrored displays do not change ownership. Other terminals use their Director launch environment. Other flows and global tools are refused.\n\nagent '{\"action\":\"start\",\"provider\":\"codex\",\"title\":\"...\",\"task\":\"...\",\"repo\":\"/abs/worktree\"}' delegates visible work to a child lane in its own repository screen terminal (provider is claude, codex or grok; omit repo for read-only shared source). agent '{\"action\":\"list\"}', status/result '{\"child\":\"<agent>\"}', message '{\"to\":\"<agent>\",\"kind\":\"note\",\"text\":\"...\"}' (kind is task, note or result) and inbox '{\"ack\":<id>}' continue that conversation.\n\nRead apps/director/AGENTS.md. Inspect first; report compact todo deltas with expected revision v. Record requirements as they arrive. Prepared is an agent report; build admission is not a passing build. You may code, check and build while this workflow's app is open; once the replacement is ready, gracefully restart it with the same workspace and state without separate confirmation. Never repeat an uncertain mutation blindly.\n\nReplies wait up to 30 seconds. Requests and immutable receipts are retained across UI restarts; interrupted requests are not replayed. Control history is bounded to 4096 receipts and 32 MiB of replies; full spools refuse new requests until reviewed cleanup.\n\nExamples:\n  director-flow todos '{\"v\":0,\"u\":[[\"fix-size\",\"w\",\"Fix window size\"]]}'\n  director-flow agent '{\"action\":\"start\",\"provider\":\"grok\",\"title\":\"Run the widget tests\",\"task\":\"Run cargo test -p makepad-widgets and report failures\"}'";
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            return Ok(json::obj(vec![("request_id", json::s(id)), ("flow_id", json::s(flow)), ("status", json::s("uncertain")),
                ("error", json::s("Timed out after 30 seconds. The request may still complete after Director reconnects. Keep this request ID and read it with director-flow --result <id> before repeating a mutation."))]));
        }
