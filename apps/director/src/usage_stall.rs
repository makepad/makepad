...
impl UsageStalls {
    ...
    pub fn ingest_terminal(
        ...
        if lane.is_empty() || lane.len() > 96 || lane.chars().any(char::is_control) {
            return false;
        }
        if !self.lanes.contains_key(lane) && self.lanes.len() >= MAX_LANES {
            return false;
        }
        ...
        if now < seen.observed_at || (generation == seen.generation && sequence <= seen.sequence) {
            return false;
        }
        ...
        if provider != seen.provider || generation != seen.generation {
            ...
            for (index, line) in lines.iter().enumerate() {
                ...
                *count += 1;
                ...
                if !fresh {
                    ...
                    if evidence == Some(Evidence::Working) && matches!(last_evidence, Some(Some(_)))
                    {
                        last_evidence = None;
                    }
                    continue;
                ...
                if let Some(Evidence::Limit) = evidence {
                    let reset_text = reset_text(line).or_else(|| {
                        lines
                            .get(index + 1)
                            .filter(|next| is_reset_line(next))
                            .and_then(|next| reset_text(next))
                    });
                    last_evidence = Some(Some(LimitReport {
                        lane: lane.to_owned(),
                        provider,
                        observed_at: now,
                        excerpt: bounded_prefix(line, 360),
                        reset_text,
                    }));
                } else if evidence == Some(Evidence::Working) {
                    last_evidence = Some(None);
                }
            }
            ...
            if let Some(report) = last_evidence {
                seen.report = report;
            } else if signal != ActivitySignal::None {
                seen.report = None;
            }
            seen.sequence = sequence;
            seen.observed_at = now;
    ...
    pub fn report(&self, lane: &str) -> Option<&LimitReport> {
        self.lanes.get(lane)?.report.as_ref()
    }
    ...
    pub fn reports(&self, provider: UsageProvider) -> Vec<&LimitReport> {
        self.lanes
            ...
            .filter_map(|lane| lane.report.as_ref())
            .collect()
    }
...
fn classify_line(
    ...
    if context.fenced || text.is_empty() {
        return None;
    }
    ...
    let tool = match provider {
        ...
        // Grok's transcript markers have not been observed; none is guessed.
        UsageProvider::Grok => false,
    };
    if tool {
        context.user = false;
        context.tool = true;
    ...
    if provider == UsageProvider::Claude
        && text.starts_with('⎿')
        && !context.tool
        && hard_limit(provider, &lower)
        && reset_text(body).is_some()
    {
        context.user = false;
        return Some(Evidence::Limit);
    }
    if context.user || context.tool || (child_output && !text.starts_with('⎿')) {
        return None;
    }
    ...
    if ((provider == UsageProvider::Codex && codex_marker)
        || (provider == UsageProvider::Claude && spinner))
...
fn hard_limit(provider: UsageProvider, line: &str) -> bool {
    ...
    match provider {
        ...
        .iter()
        .any(|prefix| status_prefix(line, prefix)),
        UsageProvider::Claude => [
        ...
        .iter()
        .any(|prefix| status_prefix(line, prefix)),
        // No Grok limit has been observed in a live terminal, nor the work
        // output that would clear one. A report that could never recover is
        // worse than none: Grok lanes raise no limit report yet.
        UsageProvider::Grok => false,
    }
}
...
fn status_prefix(line: &str, prefix: &str) -> bool {
    ...
    rest.is_empty()
        || rest.starts_with(['.', '!', '·', '—', '–', ':'])
        || ["for ", "resets ", "reset at ", "try again "]
            .iter()
            .any(|suffix| rest.starts_with(suffix))
}
fn is_reset_line(line: &str) -> bool {
    let lower = line.trim().to_ascii_lowercase();
    lower.starts_with("resets ")
        || lower.starts_with("reset at ")
        || lower.starts_with("try again at ")
}
fn reset_text(line: &str) -> Option<String> {
    ...
    .min()
    .map(|start| bounded_prefix(&line[start..], 180))
}
...
fn normalize_tail(text: &str) -> Vec<String> {
    ...
    let mut lines: Vec<_> = clean
        .lines()
        .rev()
        .take(MAX_LINES)
        .map(|line| bounded_prefix(line.trim_end(), MAX_LINE_BYTES))
        .collect();
    lines.reverse();
    lines
}
