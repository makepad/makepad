...
impl AccountUsageHistory {
    pub fn latest(&self) -> Option<&AccountUsageSample> {
        self.samples.last()
    }
    pub fn json(&self) -> Value {
        makepad_strict_json::obj(vec![
            ...
            (
                ...
                Value::Bool(self.last_check_succeeded),
            ),
            (
                "samples",
                Value::Arr(self.samples.iter().map(AccountUsageSample::json).collect()),
            ),
        ])
    }
}
fn usage_history_window_json(window: &UsageWindow) -> Value {
    makepad_strict_json::obj(vec![
        ...
        (
            "used_percent",
            window.used_percent.map(Value::F64).unwrap_or(Value::Null),
        ),
        (
            "remaining_percent",
            window
...
impl UsageHistoryStore {
        ...
        if let Some(path) = &store.path {
            match usage_history_read(path) {
                Ok(Some(bytes)) => match usage_history_decode(&bytes) {
                    ...
                    Err(error) => {
                        store.error = Some(format!("Saved usage history was not loaded: {error}"));
    ...
    fn observe(&mut self, incoming: &ProviderUsage, checked_at: u64) {
        ...
        if !self
            .accounts
            .iter()
            .any(|account| account.provider == incoming.provider && account.email == email)
        {
            let count = self
                .accounts
                .iter()
            ...
            if count >= USAGE_HISTORY_ACCOUNTS {
                if let Some(index) = self
                    .accounts
                    .iter()
                    .rposition(|account| account.provider == incoming.provider)
                {
        ...
        let account = self
            .accounts
            .iter_mut()
            .find(|account| account.provider == incoming.provider && account.email == email)
            .unwrap();
        account.last_seen_at = checked_at;
        account.last_checked_at = checked_at;
        account.last_check_succeeded = success;
        if success {
            let sample = AccountUsageSample {
                observed_at: incoming.observed_at,
                source: truncate(&incoming.source, 160),
                plan: incoming.plan.as_ref().map(|plan| truncate(plan, 80)),
                session,
                week,
            };
            if account
                .samples
                .last()
                .is_some_and(|last| last.session == sample.session && last.week == sample.week)
            {
                *account.samples.last_mut().unwrap() = sample;
            } else {
                if account.samples.len() >= USAGE_HISTORY_SAMPLES {
                    account.samples.remove(0);
                }
        ...
        if let Some(path) = &self.path {
            let value = makepad_strict_json::obj(vec![
                ("version", Value::Int(1)),
                (
                    "accounts",
                    Value::Arr(
                        self.accounts
                            .iter()
                            .map(AccountUsageHistory::json)
                            .collect(),
                    ),
                ),
            ]);
            self.error = usage_history_write(path, value.to_json().as_bytes())
                .err()
                .map(|error| format!("Usage history could not be saved: {error}"));
        }
    }
}
fn usage_history_text(value: &Value, key: &str, max: usize) -> Result<String, String> {
    let text = value
        ...
        .ok_or_else(|| format!("Invalid usage history {key}"))?;
    if text.len() > max || text.chars().any(char::is_control) {
        return Err(format!("Invalid usage history {key}"));
    }
    Ok(text.into())
}
...
fn usage_history_window(value: &Value, scope: &'static str) -> Result<Option<UsageWindow>, String> {
    ...
    let reset_at = match value.get("reset_at") {
        None | Some(Value::Null) => None,
        Some(_) => Some(usage_history_time(value, "reset_at")?),
    };
    let reset_text = match value.get("reset_text") {
        None | Some(Value::Null) => None,
        Some(_) => Some(usage_history_text(value, "reset_text", 512)?),
    };
    Ok(Some(UsageWindow {
        name: usage_history_text(value, "name", 240)?,
        scope: Some(scope),
        used_percent: percent("used_percent")?,
        remaining_percent: percent("remaining_percent")?,
        reset_at,
        reset_text,
    }))
}
fn usage_history_decode(bytes: &[u8]) -> Result<Vec<AccountUsageHistory>, String> {
    ...
    if value.get("version").and_then(Value::as_u64) != Some(1) {
        return Err("Unsupported usage history version".into());
    }
    let accounts = value
        .get("accounts")
        .and_then(Value::as_arr)
        .filter(|accounts| accounts.len() <= USAGE_HISTORY_ACCOUNTS * 2)
        .ok_or("Invalid usage account history count")?;
    let mut result = Vec::<AccountUsageHistory>::new();
    for value in accounts {
        let provider = match value.get("provider").and_then(Value::as_str) {
            Some("claude") => UsageProvider::Claude,
            Some("codex") => UsageProvider::Codex,
            Some("grok") => UsageProvider::Grok,
            _ => return Err("Invalid history provider".into()),
        };
        let email = account_email(value.get("email").and_then(Value::as_str))
            .ok_or("Invalid history email")?;
        if result
            .iter()
            ...
            || result
                ...
                .count()
                >= USAGE_HISTORY_ACCOUNTS
        {
            return Err("Duplicate account or provider history exceeds its bound".into());
        }
        let rows = value
            .get("samples")
            .and_then(Value::as_arr)
            .filter(|samples| samples.len() <= USAGE_HISTORY_SAMPLES)
            .ok_or("Invalid account sample count")?;
        let mut samples = Vec::new();
        for row in rows {
            let session =
                usage_history_window(row.get("session").unwrap_or(&Value::Null), "session")?;
            let week = usage_history_window(row.get("week").unwrap_or(&Value::Null), "week")?;
            if session.is_none() && week.is_none() {
                return Err("History sample has no session or week observation".into());
            }
            if provider != UsageProvider::Claude && session.is_some() {
                return Err(format!(
                    "{} history records only its weekly limit",
                    provider.label()
                ));
            }
            samples.push(AccountUsageSample {
                observed_at: usage_history_time(row, "observed_at")?,
                source: usage_history_text(row, "source", 160)?,
                plan: match row.get("plan") {
                    None | Some(Value::Null) => None,
                    Some(_) => Some(usage_history_text(row, "plan", 80)?),
                },
                session,
                week,
            });
        ...
        result.push(AccountUsageHistory {
            provider,
            email,
            first_seen_at: usage_history_time(value, "first_seen_at")?,
            last_seen_at: usage_history_time(value, "last_seen_at")?,
            last_checked_at: usage_history_time(value, "last_checked_at")?,
            last_check_succeeded: value
                .get("last_check_succeeded")
                .and_then(Value::as_bool)
                .ok_or("Invalid history check status")?,
            samples,
        });
    }
    Ok(result)
...
fn usage_history_read(path: &std::path::Path) -> Result<Option<Vec<u8>>, String> {
    ...
    file.take(USAGE_HISTORY_BYTES as u64 + 1)
        .read_to_end(&mut bytes)
        .map_err(|error| error.to_string())?;
    if bytes.len() > USAGE_HISTORY_BYTES {
        return Err("Usage history exceeds 4 MiB".into());
    }
    Ok(Some(bytes))
}
fn usage_history_regular(metadata: &std::fs::Metadata) -> Result<(), String> {
    if !metadata.is_file() || metadata.len() > USAGE_HISTORY_BYTES as u64 {
        return Err("Usage history is not a bounded regular file".into());
    }
    #[cfg(unix)]
    {
        ...
        if metadata.uid() != unsafe { geteuid() } || metadata.mode() & 0o077 != 0 {
            return Err("Usage history must be user-owned with mode 0600".into());
        }
    }
    Ok(())
}
