...
impl App {
    ...
    fn workspace_mode(&self, cx: &mut Cx) -> Mode {
        self.ui
            .widget(cx, ids!(workspace))
            .borrow::<StudioSurface>()
            .map(|s| s.mode())
    ...
    /// Ordinary open/select actions need a presentation that shows the Dock;
    /// Architecture and Disk never draw it, so they switch to Structured first.
    fn ensure_visible_host(&mut self, cx: &mut Cx) {
        if matches!(self.workspace_mode(cx), Mode::Architecture | Mode::Disk) {
    ...
    /// Drop a worker watch and registry entry when no tab still wants the
    /// path, unless the buffer is dirty or conflicted.
