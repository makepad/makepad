//! reminders — the standalone window.
...
impl App {
    fn ai_summary(&self, cx: &mut Cx) -> String {
        self.ui
            .widget(cx, ids!(reminders))
            .borrow::<RemindersView>()
            .map(|view| view.ai_summary())
            .unwrap_or_else(|| "Reminders are loading".to_string())
    }
    ...
    fn ai_answer(&self, cx: &mut Cx, call: &makepad_ai_services::wire::ServiceCall) -> makepad_ai_services::wire::ToolResult {
        self.ui
            ...
            .unwrap_or_else(|| makepad_ai_services::wire::ToolResult::unavailable(&call.call_id, "reminders are gone"))
    }
        ...
        for ev in events {
            match ev {
                PortEvent::Registered(endpoint) => {
                    log!("reminders: AI service registered as {}", endpoint.as_str());
...
impl MatchEvent for App {
    fn handle_startup(&mut self, cx: &mut Cx) {
        ...
        if let Some(mut view) = self.ui.widget(cx, ids!(reminders)).borrow_mut::<RemindersView>() {
            view.set_storage(cx.storage("reminders"));
        }
    }
...
fn is_close_requested(json: &str) -> bool {
    json.contains("\"wm\"") && json.contains("CloseRequested")
}
