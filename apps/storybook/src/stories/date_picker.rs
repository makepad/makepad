...
script_mod! {
    ...
    mod.stories.DatePickerOverview = StoryPage{
        StoryNote{text: "Two widgets over one date. A field you type into, and the same field with a calendar under it — which, asked for a `range`, is the two-ended form for a start and an end. All of them read a date against `format`, put it back in that shape when the field is left, and keep what will not parse instead of clearing it."}
...
fn date_actions(cx: &mut Cx, root: &WidgetRef, actions: &Actions) {
    ...
    if let Some((start, end)) = root.date_picker(cx, ids!(range)).range_changed(actions) {
