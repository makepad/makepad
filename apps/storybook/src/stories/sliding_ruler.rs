...
script_mod! {
    ...
    mod.stories.SlidingRulerOverview = StoryPage{
        ...
        StoryHeading{text: "Filling the room"}
        StoryNote{text: "A picker's length is normally a property of the control rather than of the room it is given: a fixed count is what lets the middle row mean something. `fill_travel` is the one property that suspends that, and only along the travel. The walk is honoured as written — `width: Fill` on the first ruler below — and the number of graduations is however many that width holds at `row_height`. A scale usually wants to span the panel it measures, and a `visible_items` picked to approximate one container is a number that is wrong at every other window size."}
        StoryNote{text: "Narrow the window and the first ruler regrades itself; the second is in a 280-pixel box and counts that instead. Every ruler elsewhere on this page keeps its declared length, because leaving `fill_travel` alone changes nothing whatsoever. The fade at the ends is counted in rows too, so it is counted against the derived number and still runs the ruler's whole length — left on the declared 41 while the width grew to 121 rows it would be spent a third of the way out and leave the other two thirds flat. `Fit` is not made to grow either: it asks what the natural size is, and a drum's natural size is still its declared rows, so a filled ruler with nothing to fill falls back to `visible_items`."}
        StoryRow{
            width: Fill
            flow: Down
            spacing: 8.0
...
pub const STORIES: &[Story] = &[Story {
    ...
    controls: &[
        ...
        Control { label: "Filled ruler takes its room", target: "filled", kind: ControlKind::Bool { prop: "fill_travel", default: true } },
