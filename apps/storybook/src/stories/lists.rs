...
script_mod! {
    ...
    mod.stories.ListsOverview = StoryPage{
        ...
        StoryNote{text: "Take hold of a gripper and move. The whole row comes up and follows the pointer, the rows it reaches over make way, and the gap it would land in stands open with a line across it. Escape abandons the gesture and everything drops back, the wheel is ignored while it lasts so the rows cannot slide out from under the pointer, and holding at the top or bottom edge crawls the list, so a long one can be reordered end to end in one gesture."}
...
`drag_anywhere: true` widens the grip to the row itself: its background, its words, the space between its cells. That press is offered to the rows **first** and taken up only if nothing on the row held on to the finger, so a button, a slider, a chip and a picker keep their presses and a label does not. It costs the row's own drag — a list that carries rows this way is scrolled by the wheel, the bar and the edge crawl, not by dragging a row — so it is off by default, and it suits a list of panels rather than a list of controls.

While the gesture lasts the list carries the whole row: it is drawn under the pointer and over its neighbours, keeping the point of it the press took hold of, every row between where it lay and where it would land shifts by its height so that gap stands open, and an indicator line marks the gap. Escape abandons it and the rows drop back, the wheel is swallowed so they cannot slide out from under the pointer, and a pointer held at the top or bottom edge crawls the list, so a long list can be reordered end to end in one gesture. `drag_state()` answers with `(from, slot)` while you draw, for a host that wants to tint the row being carried.

The carry is ink and not layout. It is applied when the list has finished drawing, by moving the ink each row already put down and lifting the carried row out of the paint order, so the list itself never learns of it and the pass after the gesture starts from an untouched list. Nothing is allocated per frame and no row is built twice.
