...
//! timed, the three things that separate it from a slider, and the player a
//! bar like it drives.
...
script_mod! {
    ...
    mod.stories.PlaybackBarOverview = StoryPage{
        ...
        StoryHeading{text: "The player it drives"}
        StoryNote{text: "A Video plays a network stream, a file or a bundled resource, and draws a strip of controls of its own over the picture on hover. A host that draws its own transport hands the bar's seek to seek_to, which takes milliseconds, and sets the bar's position from current_position_ms. This one points at a placeholder address and draws no thumbnail, so it stays idle until play is pressed and then shows its error state."}
        StoryRow{
            Video{
