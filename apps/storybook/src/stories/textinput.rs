...
/// How many times the first text input changed.
static COUNTER: AtomicUsize = AtomicUsize::new(0);

fn overview_actions(cx: &mut Cx, root: &WidgetRef, actions: &Actions) {
    if let Some(txt) = root.text_input(cx, ids!(simpletextinput)).changed(actions) {
        let n = COUNTER.fetch_add(1, Ordering::Relaxed);
        log!("TEXTBOX CHANGED {}", n);
        root.label(cx, ids!(simpletextinput_outputbox))
            .set_text(cx, &format!("{} {}", n + 1, txt));
    }

    if let Some(is_multiline) = root.check_box(cx, ids!(multiline_toggle)).changed(actions) {
        root.text_input(cx, ids!(multiline_toggleable))
            .set_is_multiline(cx, is_multiline);
    }
}

pub const STORIES: &[Story] = &[Story {
    key: "inputs/textinput/overview",
    category: "Inputs",
    component: "TextInput",
    also: &[],
    name: "Overview",
