...
script_mod! {
    ...
    mod.stories.ItemGridOverview = StoryPage{
        ...
        StoryNote{text: "The face must not answer the press. The grid tests the pointer against the items it put on screen, so a face that handles its own presses — a button, an interactive row — takes the press and nothing is picked. The face the grid ships is a row with `interactive: false` for exactly that reason. It must draw something of its own as well: where an item landed is read back from the face's area, so a face that painted nothing has no rect and that one item quietly stops answering the pointer."}
