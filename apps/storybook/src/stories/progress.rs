...
script_mod! {
    ...
    mod.stories.ProgressOverview = StoryPage{
        StoryNote{text: "Every shape of progress. The first bar advances a tenth per click; the rest are set values, an unknown one, the intents, labels, stacked segments, then rings, an arc, activity rings, gauges and the navigation line. A live reading that falls as well as rises is a level meter, on the next page."}
...
pub const STORIES: &[Story] = &[
    Story {
        ...
        added: "2026-09-05",
        tags: &["controls", "new"],
        doc: "# Progress\n\nEvery widget that answers \"how far along is it\", and the two that answer \"where does it stand\".\n\n## Which one to use\n\n| Widget | Reach for it when |\n|---|---|\n| `ProgressBar`, `ProgressRing`, `ProgressArc` | a task is under way and the question is how far along it is |\n| `ActivityRings` | several goals fill at once |\n| `Gauge`, `GaugeLinear` | a reading sits on a scale with safe, warning and critical zones, and the needle is the news |\n| `LevelMeter` | a live reading rises and falls and its peaks matter: load, throughput, latency, headroom. It has its own page, Level meter |\n| `NavigationProgress` | a page is loading and the line only has to say so |\n\nWork with no known end is an indeterminate bar or ring where there is room for one, and a spinner, on its own page, where there is not.\n\n## The contract\n\nThe bars and rings share one contract: `value` is a fraction 0..1 that eases into place over `theme.motion_medium_1`, only ever forward (a lower value snaps, a negative one is indeterminate), and `Completed` is raised when the value reaches the end. The gauge is the one that eases both ways, toward a `value` between its own ends.\n\n## The widgets\n\n- `ProgressBarFlat` is the default; `ProgressBar` adds the inset bevel, `ProgressBarGradientX/Y` shade the fill. `show_percent` or `text` puts a label beside the bar; `segments` stacks sections; `draw_bg.gap` and `draw_bg.stop_indicator` are the track details.\n- `ProgressRingFlat`/`ProgressRing` fill clockwise from the top with the label in the middle; `sections` cuts the ring. `ProgressArc` is half a ring with the label inside. `ActivityRings` nests one ring per entry of `values`.\n- `Gauge` is a read-only dial with safe, warning and critical zones and a needle; `GaugeLinear` lays the same zones along a bar.\n- `NavigationProgress` is the hairline at the top of a page: `start`, `increment`, `complete`, `reset`.",
        subject: "bar",
        feature: None,
        controls: &[
