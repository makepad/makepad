...
/// `--mute` is the explicit switch; `MAKEPAD_VIDEO_MUTE=1` is the same thing for a
...
pub fn wants_mute(args: &[String], env: impl Fn(&str) -> Option<String>) -> bool {
    ...
    args.iter().any(|arg| arg == "--mute") || truthy("MAKEPAD_VIDEO_MUTE") || truthy("MAKEPAD_HIDE_WINDOWS")
}
...
mod tests {
    ...
    fn a_plain_run_makes_sound() {
        ...
        assert!(!wants_mute(&args(&["clip.mp4"]), |k| (k == "MAKEPAD_VIDEO_MUTE")
            .then(|| "0".to_string())));
    }
