//! The palette bridge. `makepad_wm_theme::apply` retints the *stock* widgets from the
//! window manager's theme.splash; this module carries the same palette into
//! `mod.sheets`, which is where sheets' own splash reads its colours from.
//!
//! Standalone (no `MAKEPAD_WM_THEME_SPLASH`) every key falls back to Tokyo Night, so
//! the app is dark whether or not the WM is running.

use makepad_widgets::*;

/// key, fallback — the whole surface sheets paints with.
const KEYS: &[(&str, &str)] = &[
    ("background", "#1a1b26"),
...
fn hex_of(key: &str) -> String {
    let fallback = KEYS
        ...
        .map(|(_, f)| *f)
        .unwrap_or("#ff00ff");
    match makepad_wm_theme::current() {
...
pub fn install(vm: &mut ScriptVm) {
    ...
    let script_mod_id = ScriptMod {
        ...
        file: "sheets_theme.splash".to_string(),
        line: 0,
        column: 0,
        code,
        values: vec![],
