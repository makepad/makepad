...
impl Pty {
    /// Spawn the login shell in a new PTY. With `command`, the shell runs
    /// that one job instead of an interactive session (`shell -l -c cmd`,
    /// `cmd.exe /c cmd`) and the PTY ends when it exits — mpterm's
    /// `--preview` pager.
    pub fn spawn(
        cols: u16,
        rows: u16,
    ...
    fn spawn_windows(
        ...
        let command_line = windows_shell_command(&shell, command);
    ...
    pub fn take_reader(&mut self) -> PtyReader {
        ...
        #[cfg(windows)]
        {
            ...
            PtyReader {
                read_rx: std::mem::replace(&mut self.read_rx, dead_rx),
            }
        }
        #[cfg(not(any(target_os = "macos", target_os = "linux", windows)))]
        {
    ...
    /// True once the shell process has exited (poll; non-blocking).
    pub fn child_exited(&mut self) -> bool {
        ...
        {
            unsafe {
                let handle = raw_handle(self.process_handle);
                if handle.is_invalid() {
                    return true;
                }
                // WAIT_OBJECT_0 == 0 means signaled (process ended).
                WaitForSingleObject(handle, 0).0 == 0
            }
        }
        #[cfg(not(any(target_os = "macos", target_os = "linux", windows)))]
        {
...
/// Blocking read half for a dedicated reader thread (see `Pty::take_reader`).
...
impl PtyReader {
    /// Block until output arrives. `None` = EOF/closed (shell exited).
...
#[cfg(windows)]
fn windows_shell_command(shell: &str, command: Option<&str>) -> String {
    match command {
        Some(command) => format!("{} /c {}", shell, command),
