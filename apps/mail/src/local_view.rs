...
/// A moment in local time (UTC where the host has no zone), with its local day number.
struct Civil {
    year: i32,
    month: usize,
    day: u32,
    ...
    days: i64,
    /// False when the host had no local zone and this is UTC.
    zoned: bool,
}

fn civil(secs: i64) -> Civil {
    let t = makepad_civil_time::local_or_utc(secs);
    Civil { year: t.year, month: t.month as usize, day: t.day, hour: t.hour as i64, minute: t.minute as i64, days: t.day_number() as i64, zoned: t.zoned }
}
...
fn reader_date(secs: i64) -> String {
    let date = civil(secs);
    let zone = if date.zoned { "" } else { " UTC" };
