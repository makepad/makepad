...
//!
//! A viewer started as `mpimage --preview <path>` is not thrown away when
...
//! * `PreviewUnload` — drop what is loaded (here: the decoded texture and
//!   the folder's sibling list) and idle as a blank themed panel at
//!   near-zero cost until the next `PreviewFile`. **An unload is not a
...
mod tests {
    ...
    fn a_warm_viewer_unloads_and_reloads() {
        let mut state = PreviewState::new(true, Some(Path::new("/a/one.png")));
        assert!(state.is_loaded());
        assert_eq!(state.title("mpimage"), "Preview \u{2014} one.png");
        ...
        assert_eq!(state.title("mpimage"), "Preview");
        ...
        assert_eq!(
            ...
            PreviewAction::Show(PathBuf::from("/a/two.png"))
        );
        assert!(state.is_loaded());
        assert_eq!(state.title("mpimage"), "Preview \u{2014} two.png");
    }
    ...
    fn a_retarget_replaces_without_an_unload_in_between() {
        ...
        assert_eq!(state.showing(), Some(Path::new("/a/three.png")));
    }
    ...
    fn the_wire_reaches_the_state_machine() {
        ...
        assert_eq!(
            state.on_wm_event(&event),
            PreviewAction::Show(PathBuf::from("/a/b.png"))
        );
    ...
    fn a_standalone_run_keeps_the_apps_own_title() {
        let state = PreviewState::new(false, Some(Path::new("/a/one.png")));
        assert_eq!(state.title("mpimage"), "mpimage \u{2014} one.png");
        assert_eq!(PreviewState::new(false, None).title("mpimage"), "mpimage");
    }
}
