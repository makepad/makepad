//! mpimage: a dumb picture viewer. `mpimage <path>` opens fit-to-window,
//! centered on a dark mp_theme background. `mpimage --preview <path>` sizes
//! the window as a small popup instead.

pub use makepad_widgets;
use makepad_widgets::*;
use mpimage::preview::{PreviewAction, PreviewState};
use mpimage::widget::{MpImageAction, MpImageView};
    ...
    fn handle_startup(&mut self, cx: &mut Cx) {
        ...
        for arg in std::env::args().skip(1) {
            if arg == "--preview" {
                ...
                // --remote, --stdin-loop, or any other flag every mp* app
        ...
        // A theme.splash background (mp_theme::apply already retinted
        // theme.color_bg_app from it, if MPWM_THEME_SPLASH is set); patch the
        // pass clear color explicitly too so the dark letterbox is right even
        // if the stock theme default ever drifts.
        if let Some(bg_hex) = mp_theme::current().map(|p| p.hex("background", "#1a1b26")) {
...
impl App {
    ...
    fn set_title(&mut self, cx: &mut Cx) {
        let title = self.preview.title("mpimage");
