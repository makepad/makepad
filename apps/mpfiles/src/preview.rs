//! Opening and previewing files, through `mp_wm_api`.
//!
//! Which app answers is never decided here, and there is deliberately no
//! association table in this crate: `mp_wm_api::viewer_for` is the one the
//! compositor and the browser share (pictures → mpimage, video → mpvideo,
//! csv/tsv → mpsheets, pdf → mppdf, html → mpbrowser, everything else →
//! mpterm's `--preview` pager). A file type that opens in the wrong app is
//! fixed there, never here.
//!
//! Hosted as an mpwm tile, an app never spawns anything: it asks, and the
...
//! preview, which mpfiles spawns itself so that Space and Escape can take the
//! popup away again; `mp_wm_api::preview`'s child is detached and could not be
//! dismissed.
//!
...
pub fn open_file_with(cx: &Cx, path: &Path, app: &str) -> String {
    ...
    if mp_wm_api::hosted(cx) {
        let request = mp_wm_api::WmRequest::Open {
...
mod tests {
    use super::*;
    use mp_wm_api::WmRequest;

    #[test]
    fn the_shared_table_picks_the_viewer() {
        // mpfiles keeps no association table of its own: every kind it shows
        // is routed by mp_wm_api, and the kinds it thumbnails are exactly the
        // ones the picture and video viewers claim.
        for ext in crate::model::IMAGE_EXTS {
            assert_eq!(viewer_for(&PathBuf::from(format!("/a/x.{ext}"))), "mpimage");
        }
        for ext in crate::model::PLAYABLE_VIDEO_EXTS {
            assert_eq!(viewer_for(&PathBuf::from(format!("/a/x.{ext}"))), "mpvideo");
        }
        // Text and code fall to the terminal's pager, not to nothing.
        assert_eq!(viewer_for(Path::new("/a/m.rs")), "mpterm");
        assert_eq!(viewer_for(Path::new("/a/n.txt")), "mpterm");
        // Every type this browser names in its Kind column has an owner, and
        // none of them is decided in this crate.
        assert_eq!(viewer_for(Path::new("/a/d.pdf")), "mppdf");
        assert_eq!(viewer_for(Path::new("/a/t.csv")), "mpsheets");
        assert_eq!(viewer_for(Path::new("/a/p.html")), "mpbrowser");
    }
