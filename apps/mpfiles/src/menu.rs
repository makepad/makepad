...
pub fn open_with_apps(path: &Path, available: &dyn Fn(&str) -> bool) -> Vec<(String, String)> {
    ...
    let primary = mp_wm_api::viewer_for(path);
    if available(primary) {
        out.push((primary.to_string(), format!("Open with {primary}")));
    }
    if primary != "mpterm" && available("mpterm") {
        out.push(("mpterm".to_string(), "Open in the terminal pager".to_string()));
    }
...
mod tests {
    ...
    fn open_with_offers_only_apps_that_exist() {
        ...
        assert_eq!(offered[0].0, "mpimage");
        assert_eq!(offered[1].0, "mpterm");
