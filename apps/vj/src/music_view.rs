...
script_mod! {
    ...
    // ---- the lyrics reader -------------------------------------------------
    mod.widgets.VjLyricReaderBase = #(VjLyricReader::register_widget(vm))
    mod.widgets.VjLyricReader = set_type_default() do mod.widgets.VjLyricReaderBase{
        width: Fill
        height: Fill
        draw_bg +: {
            color: #x0c1116
        ...
        // the same accent the waveform's playhead uses.
        draw_row +: {
...
impl WidgetNode for VjLyricReader {
    fn widget_uid(&self) -> WidgetUid {
        self.uid
    }
    fn walk(&mut self, _cx: &mut Cx) -> Walk {
