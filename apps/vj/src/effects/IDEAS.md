...
| A document's frame depends on the document shown before it | open (pre-existing) | Found by the capture instrument: loading a doc does not clear the post chain's textures, so feedback/bloom carry the previous effect's pixels into the first frames. Harmless live (the VJ crossfades), but it means capture sweeps must be compared in the same order. Fix = clear the stage textures in `PostChain::configure` |
