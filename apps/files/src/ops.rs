...
impl Progress {
    fn new(id: u64, kind: OpKind, total: u64, updates: Arc<Mutex<VecDeque<OpUpdate>>>, notify: Arc<dyn Fn() + Send + Sync>) -> Self {
        Progress {
            ...
            last_emit: Cell::new(Cx::monotonic_now()),
...
mod tests {
    ...
    fn fresh_dir(tag: &str) -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("files-ops-test-{tag}-{}-{n}", std::process::id()));
    ...
    fn wait_for_done(ops: &Ops, id: u64, timeout: Duration) -> OpUpdate {
        ...
        loop {
            ...
            if Cx::monotonic_now() - start > timeout.as_secs_f64() {
