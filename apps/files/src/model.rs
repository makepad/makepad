...
pub use makepad_diskmap::{
    display_name, extension_of, is_playable_video, is_thumbnailable, kind_for, skip_for_scan,
    FileKind,
};

#[cfg(test)]
pub use makepad_diskmap::{
    home_scan_exclusion, scan_exclusions, IMAGE_EXTS, PLAYABLE_VIDEO_EXTS,
};
...
pub fn set_scan_all(on: bool) {
    *scan_all_flag().lock().unwrap_or_else(|e| e.into_inner()) = on;
    makepad_diskmap::set_scan_all(on);
...
fn scan_all_flag() -> &'static std::sync::Mutex<bool> {
    ...
    FLAG.get_or_init(|| {
        let on = std::env::var_os("MAKEPAD_FILES_SCAN_ALL").is_some_and(|v| v != "0")
            || pref_get("scan_all").as_deref() == Some("1");
        makepad_diskmap::set_scan_all(on);
        std::sync::Mutex::new(on)
    })
}
