//! mpwm theming lives in SPLASH. A theme is a `theme.splash` file defining
//! `mod.mpwm_theme = { ... }`, evaluated into the script VM at startup and
//! on switch; the chrome styles itself from `mod.mpwm_theme.*` and the
//! terminal palette handed to child mpterms comes from the same object.
...
//!
//! Installed layout: `~/.config/mpwm/themes/<name>/theme.splash`
//!                   `~/.config/mpwm/themes/<name>/backgrounds/*`
...
/// the theme: mpwm evaluates it, the chrome reads `mod.mpwm_theme.*`, and
/// the `term` block is the palette handed to child terminals.
pub fn splash_source(theme: &ImportedTheme) -> String {
    let mut s = String::new();
    let _ = writeln!(
        s,
        "// mpwm theme \"{}\" — imported from the omarchy theme of the same",
        theme.name
    );
    let _ = writeln!(
        s,
    ...
    let _ = writeln!(s, "mod.mpwm_theme = {{");
    {
...
pub fn themes_dir() -> PathBuf {
    ...
    PathBuf::from(home).join(".config/mpwm/themes")
}
...
impl TermPalette {
    /// The MPTERM_COLORS env value handed to child terminals.
...
pub const BUNDLED_TOKYO_NIGHT_SPLASH: &str = r##"// mpwm theme "tokyo-night" — imported from the omarchy theme of the same
...
/// `mod.mpwm_theme.shell = { ... }`, resolved from the theme's own palette
...
pub fn shell_splash_block(source: &str) -> String {
    ...
    let mut s = String::new();
    let _ = writeln!(s, "mod.mpwm_theme.shell = {{");
...
"##;
    ...
    fn scan_borders_reads_the_emitted_shape() {
        ...
        let solid = scan_borders("mod.mpwm_theme = {\n    active_border: #7aa2f7\n}\n");
    ...
    const SHELL_SRC: &str = r##"mod.mpwm_theme = {
    accent: #7aa2f7
    background: #1a1b26
    foreground: #a9b1d6
    active_border: #7aa2f7
...
"##;
    ...
    fn the_shell_block_resolves_the_tpl_contract() {
        ...
        assert!(block.starts_with("mod.mpwm_theme.shell = {"));
        ...
        assert!(theme_defines_shell("mod.mpwm_theme = {\n    shell: {}\n}\n"));
