...
//! `MPWM_FILES_REAL` knob that flips the Files app back to the real disk).
...
pub fn demo_home_path() -> Option<PathBuf> {
    ...
    Some(PathBuf::from(home).join(".cache/mpwm/demo-home"))
}
...
fn generate(root: &PathBuf) -> std::io::Result<()> {
    ...
    let files: &[(String, String)] = &[
        ...
        (".generated".into(), "mpwm demo home v1\n".into()),
    ];
...
mod tests {
    ...
    fn generates_once_and_reuses() {
        let tmp = std::env::temp_dir().join(format!("mpwm-demo-home-test-{}", std::process::id()));
        ...
        let tmp2 = std::env::temp_dir().join(format!("mpwm-demo-home-test2-{}", std::process::id()));
