//! mpvideo: a dumb video player. `mpvideo <path>` opens one clip
//! fit-to-window on a dark mp_theme background and plays it.
//! `mpvideo --preview <path>` sizes the window as a small popup instead —
//! that is mpfiles' Quick Look.

pub use makepad_widgets;
use makepad_widgets::*;
use mpvideo::preview::{PreviewAction, PreviewState};
use mpvideo::theme::Palette;
use mpvideo::widget::{format_time, MpVideoAction, MpVideoView};
...
impl MatchEvent for App {
    fn handle_startup(&mut self, cx: &mut Cx) {
        ...
        // Silent start for test/agent runs — see `mpvideo::wants_mute`.
        let muted = mpvideo::wants_mute(&argv, |key| std::env::var(key).ok());
        ...
        for arg in argv.iter().cloned() {
            if arg == "--preview" {
                ...
                // flag every mp* app already understands elsewhere are not
        ...
        cx.audio_output(0, move |info, output| {
            output.zero();
            mpvideo::player::mix_into(output, info.sample_rate);
        });
...
impl App {
    ...
    fn set_title(&mut self, cx: &mut Cx) {
        let title = self.preview.title("mpvideo");
