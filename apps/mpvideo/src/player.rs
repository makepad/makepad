...
impl VideoPlayer {
    pub fn new(path: &str) -> Result<Self, String> {
        ...
        log!(
            "mpvideo: {} {}x{} {}/{} fps codec {:?} audio {} ({} Hz, {} ch)",
            path,
            info.width,
            info.height,
            info.fps_num,
    ...
    loop {
        ...
        if seek >= 0 {
            ...
            match decoder.seek(seek) {
                Ok(()) => {
                    ...
                    loop {
                        ...
                        match decoder.next_frame() {
                            ...
                            Ok(None) => break,
                            Err(e) => {
                                log!("mpvideo: seek decode error: {}", e);
                                break;
                            }
                        }
                    }
                ...
                Err(e) => log!("mpvideo: decoder seek failed: {}", e),
            }
            continue;
        }
        if info.has_audio && !audio_eos {
            while video_audio().lock().unwrap().buffered_secs() < AUDIO_AHEAD_SECS {
                match decoder.next_audio() {
                    ...
                    Err(e) => {
                        log!("mpvideo: audio decode error: {}", e);
                        audio_eos = true;
                        break;
                    }
                }
                ...
                if info.has_audio && !audio_eos {
                    loop {
                        ...
                        match decoder.next_audio() {
                            ...
                            Ok(None) => break,
                            Err(e) => {
                                log!("mpvideo: audio tail decode error: {}", e);
                                break;
                            }
                        }
                    }
