//! The mpvideo view: one clip, letterboxed to the window, with a slim
...
//! out as children, for the same reason mpimage positions its `Image`
...
impl MpVideoView {
    ...
    pub fn open(&mut self, cx: &mut Cx, path: &Path) {
        ...
        match VideoPlayer::new(&path.to_string_lossy()) {
            ...
            Err(error) => {
                log!("mpvideo: cannot open {}: {}", path.display(), error);
