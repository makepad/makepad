...
//! for a chosen city, as a plain full-window Makepad app, part of the
...
impl App {
    fn ai_summary(&self, cx: &mut Cx) -> String {
        self.ui.widget(cx, ids!(weather)).borrow::<WeatherView>().map(|view| view.ai_summary()).unwrap_or_default()
    }

    fn ai_answer(&self, cx: &mut Cx, call: &makepad_ai_services::wire::ServiceCall) -> makepad_ai_services::wire::ToolResult {
        self.ui
            .widget(cx, ids!(weather))
            .borrow::<WeatherView>()
        ...
        for ev in events {
            match ev {
                PortEvent::Registered(endpoint) => {
                    log!("weather: AI service registered as {}", endpoint.as_str());
                ...
                // Nothing here runs long enough to cancel, and the forecast
                // has no chat of its own to step aside.
...
impl MatchEvent for App {
    fn handle_startup(&mut self, cx: &mut Cx) {
        ...
        if args.iter().any(|a| a == "--tile") {
            ...
            self.ui.widget(cx, ids!(weather)).handle_event(cx, &Event::Custom(HostedViewMode::Tile.to_json()), &mut Scope::empty());
        }
        // `--sky=<class>` (clear, partly, overcast, rain, snow, fog,
        // thunder) with `--night` forces the sky art, so every condition
        // can be checked against the text without waiting for weather.
        if let Some(kind) = args.iter().find_map(|a| a.strip_prefix("--sky=")) {
            let night = args.iter().any(|a| a == "--night");
            let (kind, cover, precip) = match kind {
                "clear" => (SkyKind::Clear, 0.05, 0.0),
                "partly" => (SkyKind::Partly, 0.5, 0.0),
                "overcast" => (SkyKind::Overcast, 0.95, 0.0),
                "rain" => (SkyKind::Rain, 0.9, 0.6),
                "snow" => (SkyKind::Snow, 0.9, 0.5),
                "fog" => (SkyKind::Fog, 0.8, 0.0),
                "thunder" => (SkyKind::Thunder, 0.95, 0.8),
                _ => (SkyKind::Unknown, 0.6, 0.0),
            };
            if let Some(mut view) = self.ui.widget(cx, ids!(weather)).borrow_mut::<WeatherView>() {
                view.set_sky_override(cx, Some(SkyInputs { kind, phase: if night { 0.0 } else { 1.0 }, cover, precip }));
            }
        }
        self.ai_port = AiServicePort::open(cx, ai::manifest());
        makepad_wm_api::set_title(cx, "Weather");
        if let Some(mut view) = self.ui.widget(cx, ids!(weather)).borrow_mut::<WeatherView>() {
            view.set_storage(cx.storage("weather"));
        }
    }
