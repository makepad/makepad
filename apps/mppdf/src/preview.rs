...
//! apps/mpfiles/src/preview.rs.)
//!
//! A viewer started as `mppdf --preview <path>` is not thrown away when
//! the panel closes: mpwm keeps the process warm and points it at the next
...
//! the app as `Event::Custom(json)`; `mp_wm_api::WmEvent::parse` turns that
...
//! * `PreviewUnload` — drop what is loaded (here: the parsed document, the
//!   page cache and the thumbnail strip) and idle as a blank themed panel
//!   at near-zero cost until the next `PreviewFile`. **An unload is not a
//!   quit**: only `CloseRequested` / `Kill` from the WM, or the viewer's
//!   own Escape/Q, end the process.
//!
...
pub fn hide_panel(cx: &Cx, preview: bool) -> bool {
    preview && mp_wm_api::send(cx, &WmRequest::PreviewClose)
}
...
mod tests {
    ...
    fn a_warm_viewer_unloads_and_reloads() {
        let mut state = PreviewState::new(true, Some(Path::new("/a/one.pdf")));
        assert!(state.is_loaded());
        assert_eq!(state.title("mppdf"), "Preview \u{2014} one.pdf");
        ...
        assert_eq!(state.title("mppdf"), "Preview");
        ...
        assert_eq!(
            ...
            PreviewAction::Show(PathBuf::from("/a/two.pdf"))
        );
        assert!(state.is_loaded());
        assert_eq!(state.title("mppdf"), "Preview \u{2014} two.pdf");
    }
    ...
    fn a_retarget_replaces_without_an_unload_in_between() {
        ...
        assert_eq!(state.showing(), Some(Path::new("/a/three.pdf")));
    }
    ...
    fn the_wire_reaches_the_state_machine() {
        ...
        assert_eq!(
            state.on_wm_event(&event),
            PreviewAction::Show(PathBuf::from("/a/b.pdf"))
        );
        // Somebody else's Custom JSON is not ours to act on.
        assert!(WmEvent::parse(r#"{"mpwm_fullscreen":true}"#).is_none());
    ...
    fn a_standalone_run_keeps_the_apps_own_title() {
        let state = PreviewState::new(false, Some(Path::new("/a/one.pdf")));
        assert_eq!(state.title("mppdf"), "mppdf \u{2014} one.pdf");
        assert_eq!(PreviewState::new(false, None).title("mppdf"), "mppdf");
    }
}
