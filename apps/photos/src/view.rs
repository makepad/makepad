...
mod tests {
    ...
    fn items() -> Vec<(ItemId, String, String)> {
        vec![
            (1, "2024-01-02: A robot learns to love".into(), "https://example.com/1".into()),
            (2, "2024-01-03: Physics of a cat".into(), "https://example.com/2".into()),
            (3, "2024-01-04: The Robot uprising, again".into(), "https://example.com/3".into()),
        ]
    }
    ...
    fn every_query_word_must_appear_case_insensitively() {
        ...
        // The link counts too.
        assert_eq!(search_items(&items(), "example.com/2")[0].0, 2);
    }
