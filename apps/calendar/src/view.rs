...
impl CalendarView {
    ...
    fn apply_layout(&mut self, cx: &mut Cx, next: LayoutDecision, size: Vec2d) {
        ...
        // The wide sidebar lists the calendars itself: a Calendars sheet
        // carried over from the phone or landscape layout would float a
        // second copy of the list over it, anchored where its button was.
        if next.sidebar_width() > 0.0 && self.current_overlay(cx) == OverlayKind::Calendars {
