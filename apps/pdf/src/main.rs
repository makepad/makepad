...
impl App {
    ...
    fn set_title(&mut self, cx: &mut Cx) {
        let title = self.preview.title("pdf");
...
fn status_line(status: &PdfStatus) -> String {
    ...
    if status.page_count == 0 {
        return if status.name == "no document" {
            "No document \u{2014} pdf <file.pdf>".to_string()
        } else {
