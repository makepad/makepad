...
//! When wm is nested inside another window manager, that manager commonly
...
///
/// Linux needs the Ctrl+Alt spelling too when wm is nested inside a real
/// desktop compositor: Hyprland consumes its SUPER bindings before the key
/// event can reach wm. Keeping Logo first preserves direct/session use,
