...
impl App {
    ...
    fn start_recording(&mut self, cx: &mut Cx) {
        ...
        let texture_id = match view.cached_capture_source() {
            Ok(makepad_widgets::makepad_platform::CaptureSource::CachedView { draw_pass_id }) => {
                let Some(color_texture) = cx.passes[draw_pass_id].color_textures.first() else {
            ...
            Ok(makepad_widgets::makepad_platform::CaptureSource::Framebuffer) | Err(_) => {
                self.set_status(cx, "Record surface does not expose a cached texture yet; try again");
                self.ui.redraw(cx);
                return;
            }
        };
