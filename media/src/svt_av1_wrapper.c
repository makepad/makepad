...
int mp_svt_av1_encoder_send_i420(void* encoder,
    ...
    const uint32_t y_bytes = y_stride * height;
    const uint32_t u_bytes = u_stride * chroma_h;
    const uint32_t v_bytes = v_stride * chroma_h;
    in.n_filled_len = y_bytes + u_bytes + v_bytes;
    in.pts = pts;
...
int mp_svt_av1_encoder_get_packet_copy(void* encoder, int pic_send_done, MpSvtAv1Packet* out_packet) {
    ...
    out_packet->data = out->p_buffer;
    out_packet->out_buffer = out;
...
void mp_svt_av1_packet_free(MpSvtAv1Packet* packet) {
    ...
    if (packet->out_buffer) {
        EbBufferHeaderType* out = (EbBufferHeaderType*)packet->out_buffer;
