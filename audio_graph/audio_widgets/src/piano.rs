...
pub struct Piano {
    ...
    #[rust]
    white_keys: ComponentMap<PianoKeyId, PianoKey>,
    #[rust]
    black_keys: ComponentMap<PianoKeyId, PianoKey>,
}

impl LiveHook for Piano {
    fn after_apply(&mut self, cx: &mut Cx, apply: &mut Apply, index: usize, nodes: &[LiveNode]) {
        for piano_key in self
            .white_keys
            .values_mut()
            .chain(self.black_keys.values_mut())
        {
...
impl Widget for Piano {
    ...
    fn draw_walk(&mut self, cx: &mut Cx2d, _scope: &mut Scope, walk: Walk) -> DrawStep {
        ...
        for i in midi_a0..midi_c8 {
            ...
            let key = self
                .white_keys
                .get_or_insert(cx, key_id, |cx| PianoKey::new_from_ptr(cx, piano_key));
            key.draw_abs(
                cx,
                depth,
                0.0,
        ...
        cx.turtle_mut()
            .set_used(white_size.x * (midi_c8 - midi_a0) as f64, white_size.y);
...
impl PianoRef {
    pub fn notes_played(&self, actions: &Actions) -> Vec<PianoNote> {
        ...
        for action in actions {
            match action
                .as_widget_action()
                .widget_uid_eq(self.widget_uid())
                .cast()
            {
                PianoAction::Note(note) => notes.push(note),
                PianoAction::None => (),
            }
        }
        notes
