...
struct Instrument {
    #[rust]
    step_order: Vec<LiveId>,
    #[rust]
    steps: ComponentMap<LiveId, AudioComponentRef>,
...
impl AudioGraphNode for Node {
    ...
    fn render_to_audio_buffer(
        ...
        for i in (0..num_steps).rev() {
            if i == 0 {
                // first one uses our main output buffer
                let step = &mut steps[0];
                if i == num_steps - 1 {
                    // last one uses external inputs
                    step.graph_node
                        .render_to_audio_buffer(info, outputs, inputs, display);
                } else {
                    step.graph_node.render_to_audio_buffer(
                        info,
                        outputs,
                        &[&step.input_buffer],
                        display,
                ...
                if i == num_steps - 1 {
                    // last one uses external inputs
                    step1[0].graph_node.render_to_audio_buffer(
                        info,
                        &mut [output_buffer],
                        inputs,
                        display,
