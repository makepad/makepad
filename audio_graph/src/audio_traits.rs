use {crate::makepad_platform::*, std::collections::BTreeMap};
...
impl<'a> DisplayAudioGraph<'a> {
    ...
    pub fn send_buffer(&self, active: bool, voice: usize, buffer: AudioBuffer) {
        self.to_ui
            .send(ToUIDisplayMsg::DisplayAudio {
                active,
                voice,
                buffer,
            })
...
pub trait AudioResultApi<'a> {
    fn not_found() -> AudioResult<'a> {
        AudioResult::Ok(())
    }
    fn found(arg: &'a mut Box<dyn AudioComponent>) -> AudioResult<'a> {
        AudioResult::Err(arg)
    }
