...
function requestHandler(req, res) {
    ...
    // rip out the path
    let m = filename.match(/^\/makepad\/workshops\/[a-zA-Z0-9\_]+\/(.*)$/);
    if(m){
        filename = '/'+m[1]
    }
    else{
        let m = filename.match(/^\/makepad\/target\/(.*)$/);
        if(m){
            // check where this thing exists
            filename = '/target/'+m[1]
            if(!Fs.existsSync(httproot + filename)){
                filename = "/../../target/"+m[1]
            }
        }
    }
