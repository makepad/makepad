            ...
            fn pixel() -> vec4 {
                ...
                return vec4(
                    mix(fract.xyz, mix(
                        cam.xyz,
                        mix(
                            mix(fract.xyz, cam.xyz, center_blend),
                            mix(
                                fract.xyz * cam.xyz,
                                mix(
                                    mix(fract.xyz, cam.xyz, smoothstep(0.5, 1.0, max(max(cam.x, cam.y), cam.z))),
                                    mix(
                                        mix(fract.xyz, cam.xyz, smoothstep(0.5, 1.0, max(max(fract.x, fract.y), fract.z))),
                                        mix(
                                            mix(fract.xyz, df_iq_pal5(cam.y), smoothstep(0.5, 1.0, max(max(fract.x, fract.y), fract.z))),
                                            mix(
                                                fract.xyz,
                                                df_iq_pal5(sample2d(texcam, kaleido(geom.xy - vec2(0.5, 0.5), 3.14 / 8., 0., 0.)).y),
                                                smoothstep(0.5, 1.0, max(max(fract.x, fract.y), fract.z))
                                            ),
                                            clamp(blend_mode - 6.0, 0., 1.)
                                        ),
                                        clamp(blend_mode - 5.0, 0., 1.)
                                    ),
                                    clamp(blend_mode - 4.0, 0., 1.)
                                ),
                                clamp(blend_mode - 3.0, 0., 1.)
                            ),
                            clamp(blend_mode - 2.0, 0., 1.)
                        ),
                        clamp(blend_mode - 1.0, 0., 1.)
                    ), 
                    clamp(blend_mode, 0., 1.)
                ),
                1.0
            );
        }})
...
fn do_gamepad_interaction(&mut self, cx: &mut Cx) {
    ...
    if !preset_handled {
        if self.gamepad.buttons & GamepadButtonA > 0 {
            if state.palette > 0.0 {state.palette -= 0.02;}
            else {state.palette = 0.0;}
        }
        if self.gamepad.buttons & GamepadButtonB > 0 {
