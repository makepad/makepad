...
impl App {
    ...
    fn do_gamepad_interaction(&mut self, _cx: &mut Cx) {
        ...
        state.loc.rotate += 0.03 * self.gamepad.left_trigger as f64;
        state.loc.rotate -= 0.03 * self.gamepad.right_trigger as f64;
        
        let mut zoom = 0.0;
        if self.gamepad.left_thumb.y > 0.4{
            zoom = 0.03 * (self.gamepad.left_thumb.y-0.4);//self.gamepad.right_trigger;
        }
        ...
        if self.gamepad.left_thumb.x > 0.4{
            state.color_offset += 0.03 * (self.gamepad.left_thumb.x-0.4);//self.gamepad.right_trigger;
        }
        if self.gamepad.left_thumb.x < -0.4{
            state.color_offset -= 0.03 * (-self.gamepad.left_thumb.x-0.4);//self.gamepad.right_trigger;
        }
        ...
        // preset mgmt
        if self.gamepad.buttons & GamepadButtonStart>0 {
            let id = if self.gamepad.buttons_down_edge & GamepadButtonDpadUp > 0 {0}
            else if self.gamepad.buttons_down_edge & GamepadButtonDpadDown > 0 {1}
            else if self.gamepad.buttons_down_edge & GamepadButtonDpadLeft > 0 {2}
            else if self.gamepad.buttons_down_edge & GamepadButtonDpadRight > 0 {3}
            else if self.gamepad.buttons_down_edge & GamepadButtonLeftShoulder > 0 {4}
            else if self.gamepad.buttons_down_edge & GamepadButtonRightShoulder > 0 {5}
            else if self.gamepad.buttons_down_edge & GamepadButtonA > 0 {6}
            else if self.gamepad.buttons_down_edge & GamepadButtonB > 0 {7}
            else if self.gamepad.buttons_down_edge & GamepadButtonX > 0 {8}
            else if self.gamepad.buttons_down_edge & GamepadButtonY > 0 {9} else {0};
        }
        ...
        if self.gamepad.buttons & GamepadButtonLeftShoulder > 0 {
            if state.palette > 0.0 {state.palette -= 0.02;}
            else {state.palette = 0.0;}
        }
        if self.gamepad.buttons & GamepadButtonRightShoulder > 0 {
            if state.palette < 7.0 {state.palette += 0.02;}
