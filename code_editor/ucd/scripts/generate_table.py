...
def group_code_points(code_points):
    ...
    for code_point in sorted(code_points):
        if code_point == last + 1:
            last = code_point
            continue
        code_points.append(range(first, last + 1))
        first = code_point
        last = code_point
    return code_points
...
def load_property_data(filename):
    ...
    points for which the property with that name has the value "True".
    """
    property = {}
...
def print_binary_property_table(name, property):
    ...
    The property is specified as the list of code points for which the
    ...
    entries = []
    for value in property:
        entries.extend([(code_points[0], code_points[-1])
                       for code_points in property[value]])
    entries.sort()
...
def main():
    ...
    if args.name == "Extended_Pictographic":
        print_binary_property_table(args.name, load_property_data(
            args.ucd_dir + "/emoji/emoji-data.txt"))
